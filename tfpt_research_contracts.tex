...
theorems (harmonicity alone is insufficient, exact counterexample); on the Fock side the NS deck
...
\emph{position} (which half-period) remains the carrier input. The full \texttt{QGEO.REALIZE.01} stays \mO;
...
(WP1--WP5d, both WP5d stages, $+$ the WP5e $\alpha$, $\beta$ and $\gamma$ stages) --- the narrative arc from the $\mu_4$ clock to the $(E_8)_1$ boundary shadow --- is given as
...
(\veri{v507\_seam\_bit\_origin.py}, \texttt{SEAM.BIT.ORIGIN.01}).
...
automatically --- a relocation of the same order-$4$ carrier input, not a new derivation.
...
ingredients of complete rationality on the lattice), and WP5e-$\alpha$/$\beta$/$\gamma$ have anchored \emph{both}
faces of the twistor uplift and closed its exchange sub-branch (the CFT-side prefactor $+$ level
pinning; the equivariant twistor skeleton with the index bridge and the rigid $32\,T_3$ residual;
the exchange no-go with certificate),
leaving the global BCOV quantisation (WP5e proper) as the remaining open piece --- narrowed to the
twisted contact terms ($\delta_1$) and the full-tensor ledger ($\delta_2$).
\item \textbf{WP5 (the summit) --- subdivided; WP5a--WP5d (both WP5d stages) plus the WP5e $\alpha$, $\beta$ and $\gamma$ stages are landed.} The twistorial
...
of which WP5b--WP5d and the WP5e $\alpha$, $\beta$ and $\gamma$ stages are executed below.
...
the sphere-axion pairing check, a rigid \emph{negative} result with certificate; the global BCOV
quantisation stays \mO.}
...
$\Rightarrow$ one level (\vref{v493} periods). (v)~\emph{The honest refutation}: the
...
\emph{closed} by the $\gamma$ stage; the named remaining targets, with preregistered
success/kill criteria: \textbf{($\delta_1$)} the twisted BCOV one-loop \emph{contact terms} on
$\mathbb C^2/\Z_4$ (orbifold elliptic-genus / twisted-character arithmetic), which \emph{must}
carry the $32\,T_3$ residual --- a \emph{binary} test: success $=$ the one-loop coefficient comes
out exactly $(9,-30,-15,0,32)$, computed, not fitted; kill $=$ any other exact value;
\textbf{($\delta_2$)} the full-tensor ledger with non-Cartan external legs (the $\gamma$-stage
collapse is a Cartan-restricted statement; the $V^{(j,j')}$ with $j'\neq-j$ are nonzero as
tensors) --- the named escape route if $\delta_1$ kills; \textbf{($\varepsilon_1$)} the $O(-2)$
bulk-axion construction ($\tilde\lambda=6$); \textbf{($\varepsilon_2$)} the
Costello--Paquette--Sharma level-from-flux mechanism ($k=1$ from flux quantisation on the
lockstep spheres). Success (unchanged): partition function $=E_4/\eta^8$ \emph{including} the
...
nor the exchange no-go of v508 trigger it (all computable dials say $k=1$), but the global
quantisation is untested.
\end{itemize}
Honest status line: WP5a--WP5d (both WP5d stages) and the WP5e $\alpha$, $\beta$ and $\gamma$ stages are
...
$72=2\tilde\lambda^2_{e_8}$); \emph{WP5a--WP5d plus WP5e-$\alpha$/$\beta$/$\gamma$ are landed ---
all three KLM ingredients of complete rationality carry lattice witnesses, both faces of the
inflow are anchored, and the exchange sub-branch is closed with a certificate}
...
the exchange sub-branch is closed by v508, and the named remaining targets are the twisted BCOV
contact terms ($\delta_1$, binary test) and the full-tensor ledger ($\delta_2$)
--- \texttt{SEAM.EQUIV.01} stays \mO\
and nothing in WP5a--WP5e-$\gamma$ moves it.
\end{itemize}
...
skeleton), and the exchange no-go by WP5e-$\gamma$
...
to the twisted contact terms, $\delta_1$ binary), leaving the global BCOV quantisation
(WP5e proper) as the single remaining open piece.
...
$\alpha$, $\beta$ and $\gamma$ stages are
...
closes the exchange sub-branch with a certificate; \emph{WP5e proper
...
with the three sphere classes filling the three twisted axion slots), and the WP5e-$\gamma$
...
that converts the pairing question into the binary $\delta_1$ contact-term test) --- exactly the kind of sharp,
...
certificate; the twisted contact terms $\delta_1$ and the full-tensor ledger $\delta_2$ stay \mO);
