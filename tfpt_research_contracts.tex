...
datum $\tau=i$ alone. The full \texttt{QGEO.REALIZE.01} stays \mO;
...
square-modulus datum $\tau=i$ alone.
...
\texttt{SEAM.EQUIV.01}, closed modulo cited theorems (\vref{v367}--\vref{v379}) with the cited
continuum scaling-limit existence (\vref{v336}) as its \mO\ leg --- is in red, the frozen 4D fork and
...
% =====================================================================
\section*{Research Contract --- \texttt{CELEST.SEAM.01} (the celestial-holographic route)}
...
(WP1--WP5d, both WP5d stages, $+$ the WP5e $\alpha$, $\beta$, $\gamma$, $\delta_2$ and $\varepsilon_2$ stages) --- the narrative arc from the $\mu_4$ clock to the $(E_8)_1$ boundary shadow --- is given as
a dedicated section in \texttt{tfpt\_3\_e8\_audit\_bootstrap} (``The celestial route: from the $\mu_4$
clock to the $(E_8)_1$ boundary shadow''); this contract remains the full technical reference (typing
fence, kill tests, work-package statements).
...
ingredients of complete rationality on the lattice), and WP5e-$\alpha$/$\beta$/$\gamma$/$\delta_2$/$\varepsilon_2$ have anchored \emph{both}
faces of the twistor uplift, closed its exchange sub-branch, executed the full-tensor ledger and
executed the level-from-flux dial
...
``one level'' a theorem of clock invariance, with the sector-counter dial pinning $k=1$),
...
physical justification of the cubic GS term, the $O(-2)$ bulk-axion
construction ($\varepsilon_1$) and the $\mathbb{PT}/\Z_4$ back-reaction.
\item \textbf{WP5 (the summit) --- subdivided; WP5a--WP5d (both WP5d stages) plus the WP5e $\alpha$, $\beta$, $\gamma$, $\delta_2$ and $\varepsilon_2$ stages are landed.} The twistorial
...
type-I B-model back-reaction on $\mathbb{PT}/\Z_4$ stays \mO.
...
$1920=|W(D_5)|=8\cdot240$. Controls: the Killing control replicates the $\gamma$-stage channels
...
$\ge4$ ledger, the \emph{physical} justification of the cubic GS term ($c_d=1920$ under the
$e_8$-Killing propagator; ch$_2$-naturalness for cubic vertices \emph{undecided} --- no canonical
geometric datum exists for a cubic vertex), and the burden on $\delta_1$ is \emph{sharpened}: it
must supply either the $\psi=64$ slice or the selection-rule relaxation.
...
\emph{closed} by the $\gamma$ stage, the full-tensor ledger ($\delta_2$) and the level-from-flux
dial ($\varepsilon_2$) are \emph{executed}, and $\delta_1$ stands \emph{undecided} at exploration
...
term)} the physical justification of the $d$-symbol vertex with $c_d=1920=|W(D_5)|$ (a
ch$_2$-type naturalness datum for \emph{cubic} vertices does not yet exist);
\textbf{($\varepsilon_1$)} the $O(-2)$
bulk-axion construction ($\tilde\lambda=6$); plus the type-I B-model \emph{back-reaction} on
$\mathbb{PT}/\Z_4$ (the $A_3$ analogue of CPS eq.~3.5), which would upgrade the $\varepsilon_2$
CPS dictionary from \mC\ to derived. Success (unchanged): partition function $=E_4/\eta^8$
\emph{including} the
...
v511 trigger it (all computable
dials say $k=1$, and ``one level'' is now a theorem), but the global
quantisation is untested.
\end{itemize}
Honest status line: WP5a--WP5d (both WP5d stages) and the WP5e $\alpha$, $\beta$, $\gamma$,
$\delta_2$ and $\varepsilon_2$ stages are
...
$1920=|W(D_5)|$; the WP5e-$\varepsilon_2$ CPS dictionary on $\mathbb{PT}/\Z_4$);
\emph{WP5a--WP5d plus WP5e-$\alpha$/$\beta$/$\gamma$/$\delta_2$/$\varepsilon_2$ are landed ---
...
confirmed full-tensorially with one cubic door open, and ``one level'' is
now a theorem of clock invariance with the sector-counter dial pinning $k=1$}
...
level-from-flux dial executed by v509, $\delta_1$ \emph{undecided} at exploration level (method
...
($c_d=1920=|W(D_5)|$), the $\varepsilon_1$
bulk-axion construction and the $\mathbb{PT}/\Z_4$ back-reaction
--- \texttt{SEAM.EQUIV.01} stays \mO\
and nothing in WP5a--WP5e-$\varepsilon_2$ moves it.
\end{itemize}
...
$\psi(A_{\mathrm{fix}})=64$ and the relaxed solution $c_d=1920=|W(D_5)|$), and the
...
plus the sector-counter dial $\#\mathrm{prim}((E_8)_k)=1\Leftrightarrow k=1$), leaving the global
...
\emph{exceeded}: WP1--WP4, all five WP5 constructive milestones (both WP5d stages) and the WP5e
$\alpha$, $\beta$, $\gamma$, $\delta_2$ and $\varepsilon_2$ stages are
...
theorem and pins its value by the sector counter; \emph{WP5e proper
...
falsifier; the sector counter $\#\mathrm{prim}((E_8)_k)=1\Leftrightarrow k=1$) --- exactly the kind of sharp,
...
$\mathbb{PT}/\Z_4$ typed \mC, the type-I B-model back-reaction stays \mO);
