...
the carrier input. The full \texttt{QGEO.REALIZE.01} stays \mO;
...
mark-$D_4$'' --- with the clock's \emph{order} fermionically forced ($U^2=(-1)^F$ exact).
...
(WP1--WP5d, both WP5d stages, $+$ the WP5e $\alpha$ and $\beta$ stages) --- the narrative arc from the $\mu_4$ clock to the $(E_8)_1$ boundary shadow --- is given as
...
ingredients of complete rationality on the lattice), and WP5e-$\alpha$/$\beta$ have anchored \emph{both}
faces of the twistor uplift (the CFT-side prefactor $+$ level pinning; the equivariant twistor
skeleton with the index bridge and the rigid $32\,T_3$ residual),
leaving the global BCOV quantisation (WP5e proper) as the remaining open piece.
\item \textbf{WP5 (the summit) --- subdivided; WP5a--WP5d (both WP5d stages) plus the WP5e $\alpha$ and $\beta$ stages are landed.} The twistorial
...
of which WP5b--WP5d and the WP5e $\alpha$ and $\beta$ stages are executed below.
...
twistor space, including the honest $a_0$ refutation; the global BCOV quantisation stays \mO.}
...
kill does \emph{not} fire on the equivariant skeleton. \textbf{WP5e proper \mO\ (the global BCOV
quantisation):} the BCOV/Kodaira--Spencer theory on $\mathbb{PT}/\Z_4$ itself (quantisation,
non-fixed-point contributions, the twisted closed-string measure), the $O(-2)$ bulk-axion
construction, the Costello--Paquette--Sharma level-from-flux mechanism, and the pairing of the
rigid $32\,T_3$ twisted residual with the three sphere axions --- the named remaining targets.
Success: partition function $=E_4/\eta^8$ \emph{including} the $q^{-1/3}$ prefactor \emph{derived
from the twistor side}; kill: an anomaly mismatch at the Costello--Li grade --- neither the
CFT-side dials of v502 nor the equivariant skeleton of v505 trigger it (all computable dials say
$k=1$), but the global quantisation is untested.
\end{itemize}
Honest status line: WP5a--WP5d (both WP5d stages) and the WP5e $\alpha$ and $\beta$ stages are
...
$\Omega$-contraction); \emph{WP5a--WP5d plus WP5e-$\alpha$/$\beta$ are landed ---
all three KLM ingredients of complete rationality carry lattice witnesses, and both faces of the
inflow are anchored}
...
and the rigid $32\,T_3$ twisted residual $+$ the sphere-axion pairing as the named remaining targets
--- \texttt{SEAM.EQUIV.01} stays \mO\
and nothing in WP5a--WP5e-$\beta$ moves it.
\end{itemize}
...
vacuum-energy bookkeeping, $k=1$ forced three ways), and the equivariant twistor skeleton by
...
skeleton), leaving the global BCOV quantisation
...
$\alpha$ and $\beta$ stages are
executed with sharp structural results (verdicts B, B, B, B(ii); WP5a--WP5d passed;
WP5e-$\alpha$ pins the CFT side, WP5e-$\beta$ the equivariant twistor skeleton; \emph{WP5e proper
...
retired), and the WP5e-$\beta$ equivariant ledger (the index bridge $f(m)=\mathrm{ch}_2(T_m)$
exact with the $-78$ defect; the rigid $32\,T_3$ twisted residual; the honest $a_0$ refutation
with the three sphere classes filling the three twisted axion slots) --- exactly the kind of sharp,
