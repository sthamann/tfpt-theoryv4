...
\begin{winbox}[Progress: the \emph{selector side} of U4 is closed on the explicit point \mE]
...
\begin{winbox}[Breakthrough: the harmonic metric is \emph{finite linear algebra}, not a PDE \mE]
...
\begin{winbox}[The final leg-assignment test (honest result) \mE]
...
\begin{winbox}[The torsion normal form and the $\delta=\tfrac12$ theorem \mE\ (\veri{v114\_torsion\_delta.py})]
...
\begin{winbox}[The anchor pins the residue: an exact normal form for the $(U_{\mathrm{wall}})$ bundle \mE\ (\veri{v115\_anchor\_residue.py})]
...
\emph{R4$'$ residue \mC\ (final form):} derive the three pairings --- everything else in the chain
...
\begin{winbox}[Progress: an explicit valid flat bundle is realised (RH solve) \mE]
...
gravity's $1/G$ (\vref{v68}) --- the two $[A]$ anchors collapse to one.}
\end{keybox}
...
\begin{winbox}[The simpler resolution: ``full QG'' for physics is the gap-decoupled admissible sector \mE]
...
\begin{winbox}[The glue Q-system: the index-$4$ object of the $G_{\mathrm{net}}$ statement exists explicitly \mE/\mC\ (\veri{v125\_glue\_qsystem.py})]
