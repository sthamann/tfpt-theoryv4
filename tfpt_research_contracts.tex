...
into a $13$-fold exact equivalence web --- the remaining input is \emph{one discrete symmetry-lift
...
(defect/twist insertions, the interacting $\mathcal A_{\mathrm{hol}}$) --- the bit remains genuine
discrete input.
...
$\iff\tau=i$; and since \vref{v521} free OS positivity is documented as the \emph{eighth}
side-blind test --- the bit is not derivable from free RP/$\Theta$ existence and remains
...
TP-1..TP-4, forcing the declared \vref{v516} completion reading; the residual \mO\ is the
constructive BCOV derivation of the $w_m$ normalisation itself (throughout,
...
tester-dead). \mO: the constructive BCOV derivation of the $w_m$ normalisation itself.
...
by \vref{v518} (kill under the derived measure), and the measure question has since been
decided at probe level by \vref{v520} for the declared reading; what stays \mO\ is the
constructive $w_m$ derivation.
...
(the Quillen structure of BCOV $F_1$). The residual \mO\ is the constructive BCOV derivation of
the $w_m$ normalisation itself --- the success branch (``the BCOV derivation lands on the
declared completion reading, carrying the forced $4{:}3$ leading ratio'') has fired at probe
level, the kill branch (``it lands on the derived chiral measure'') is dead;
\textbf{(the cubic GS
...
and the declared-vs-derived measure question has since been decided at probe level by
\vref{v520} for the declared reading (typed premises TP-1..TP-4); the residual \mO\ is the
constructive $w_m$ derivation;
...
integral), and the named
remaining target is the constructive BCOV derivation of the $w_m$ normalisation itself
...
constructive sources, the declared reading wins under TP-1..TP-4), leaving the global
BCOV quantisation (WP5e proper, narrowed to the constructive $w_m$ derivation) as the single
...
measure, and the measure decision (\vref{v520}) selects the declared reading at probe level;
\emph{WP5e proper
--- the global BCOV quantisation, narrowed to the constructive $w_m$ derivation ---
...
by v520 for the declared reading, sharpening this kill; the residual \mO\ is the constructive
$w_m$ derivation);
...
gauge datum typed as the GSO $\Z_2$ and contract precision (iii) added; no marker move).
