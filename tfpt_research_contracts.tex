...
back-reacted $\Omega_N$, twisted KS measure, $a_0$ uplift, the $\delta_1$ decision
(\vref{v505}/\vref{v509}/\vref{v511}/\allowbreak\vref{v513}/\vref{v514}/\vref{v515}/\allowbreak\vref{v516}/\vref{v517}/\vref{v518}) &
...
kill \emph{fired on the derived measure} (\vref{v518}) --- the declared/derived measure
tension is the named \mO \\
...
\vref{v517}),
leaving the global BCOV quantisation (WP5e proper) as the remaining open piece --- narrowed to
the BCOV \emph{measure question}: the $\delta_1$ chain itself has been decided by \vref{v518}
(kill under the derived measure, all three testers fail), so what remains is which of the two
exact measures --- the declared \vref{v516} completion reading or the derived \vref{v518}
chiral measure --- the BCOV integral actually produces (throughout,
...
\emph{decided} by \vref{v518} --- kill under the derived measure, the declared/derived
measure tension the named \mO).
...
fails all three testers --- both are exact; the sharp open question is which of the two is the
...
result is hidden behind the other. \mO: the BCOV-integral derivation on $\mathbb{PT}/\Z_4$
that decides between the two measures.
...
(\vref{v515}/\vref{v516}/\vref{v517}), and the $\delta_1$ chain has since been \emph{decided}
by \vref{v518} (kill under the derived measure); what stays \mO\ is the BCOV measure question
(the declared \vref{v516} reading vs the derived \vref{v518} measure).
...
\textbf{(the measure question --- the $\delta_1$ successor)} the $\delta_1$ $\tau$-integral has
...
are exact, and the stated tension is the sharp remaining question: \emph{derive} from the BCOV
integral on $\mathbb{PT}/\Z_4$ which of the two measures is the true one --- success $=$ the
BCOV derivation lands on the declared completion reading (carrying the forced $4{:}3$ leading
ratio); kill $=$ it lands on the derived chiral measure (then the twisted-contact route of the
$\gamma$-stage fence is dead and the $32\,T_3$ cancellation must come from elsewhere);
\textbf{(the cubic GS
...
integral --- the $\delta_{1b}/\delta_{1c}/\delta_{1d}$ chain has since been executed and
decided by \vref{v518} (the \emph{derived} chiral measure fails all three testers), so the
open face is now the declared-vs-derived measure question, \mO;
...
v515/v516/v517, nor the $\delta_1$ decision of v518 trigger it (all computable
...
on the unique quartic-free weighting, the $a_0$ uplift couples to the centre count, and the
$\delta_1$ kill is a statement about the derived measure, not about the algebra),
but the global
...
chain decided by v518, and the named
remaining target is the BCOV measure question: the declared completion reading (delivered by
\vref{v516}, supplying the $\psi=64$ slice) versus the derived chiral measure (\vref{v518},
failing all three testers) --- which of the two exact measures the BCOV integral on
$\mathbb{PT}/\Z_4$ actually produces (the cubic-GS-term question stays dissolved: \vref{v518}
does not revive the $d$-channel)
--- \texttt{SEAM.EQUIV.01} stays \mO\
and nothing in WP5a--WP5e-M3 or the $\delta_1$ decision moves it.
\end{itemize}
...
stated), leaving the global
BCOV quantisation (WP5e proper, narrowed to the measure question: declared \vref{v516} reading
vs derived \vref{v518} measure) as the single remaining open piece.
...
measure;
\emph{WP5e proper
--- the global BCOV quantisation, narrowed to the declared-vs-derived measure question ---
...
as the mandatory fence, \vref{v518}) --- exactly the kind of sharp,
...
completion reading stays \mC-declared --- the $\delta_1$ chain since decided by v518, the
declared/derived measure question the named \mO);
...
question --- the stated v516 tension --- is the named \mO);
...
kill: the quotient degenerates (zero or indefinite completion).
