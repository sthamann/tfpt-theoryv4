...
counting level (the coefficientwise $\chi_w$ limit below), leaving the operator statements to WP5b--e.
\item \textbf{WP5 (the summit) --- subdivided; the first milestone is landed.} The twistorial
...
\item \textbf{WP5b \mO\ (the singular vector as an operator):} realise the level-2 singular vector of
weight $2\theta$ as an \emph{operator} in the quarter-moded current Fock of the $\chi_w$ limit.
Success: an explicit operator whose kernel/image reproduces the $27000$ count level by level; kill:
a proof that no such operator exists in the limit variables.
\item \textbf{WP5c \mO\ (the GNS limit state):} construct the limit state of the $\chi_w$ family
($w\to\infty$) and its GNS representation. Success: the kernel of the limit state \emph{contains} the
null ideal, so the quotient is automatic in the GNS space; kill: the limit state separates the ideal.
\item \textbf{WP5d \mO\ (the local net):} local normality and Haag duality of the quotient net on the
seam collar via the KLM route (\vref{v336}/\vref{v449}). Success: a local net whose vacuum character is
$E_4/\eta^8$; kill: a duality or normality obstruction.
\item \textbf{WP5e \mO\ (the twistor uplift):} the BCOV/Costello--Li construction of the holomorphic
boundary theory from the twistor side. Success: partition function $=E_4/\eta^8$ \emph{including} the
$q^{-1/3}$ prefactor; kill: an anomaly mismatch at the Costello--Li grade.
\end{itemize}
Honest status line: WP5a is counting-level \mE\ arithmetic plus \mC\ named identifications (``radial
decoupling $=$ collar thickness $\to0$''; the Kac singular vector via its counting consequence);
WP5b--e are the open \emph{operator} statements --- \texttt{SEAM.EQUIV.01} stays \mO\ and nothing in
WP5a moves it.
\end{itemize}
...
$\chi_w$ limit with threshold $w=n{+}1$ and the derived ideal), leaving the operator statements to
WP5b--e.

\par\smallskip\noindent\textbf{Honest success estimate.} The probability of \emph{full} success
(WP5b--e: a second continuum route to \texttt{SEAM.EQUIV.01}) remains low --- it is constructive
holography at the Costello--Li grade. The \emph{partial}-findings expectation is now \emph{realised}:
WP1--WP4 and the first WP5 milestone (WP5a) are executed with sharp structural results (verdicts B, B,
B, B(ii); WP5a milestone passed) --- the WP1 critical correction (clock $\neq$ deck; clock$^2=$ deck),
...
structure), and the WP5a null-ideal derivation (stabilisation threshold $w=n{+}1$; $27000=(h^\vee)^3$
derived; quotient $=$ sector sum $=4124$; $SO(16)_1$ contrast) --- exactly the kind of sharp,
falsifiable refinement the contract format is for. Status: \texttt{CELEST.SEAM.01} \mO\ (research
contract, WP5b--e open); \texttt{CELEST.WP1.01} \mE/\mC\ (executed); \texttt{CELEST.WP2.01} \mE/\mC\
(executed); \texttt{CELEST.WP3.01} \mE/\mC\ (executed); \texttt{CELEST.WP4.01} \mE/\mC\ (executed);
\texttt{CELEST.WP5A.01} \mE/\mC\ (executed); \texttt{SEAM.EQUIV.01} untouched, stays \mO.
