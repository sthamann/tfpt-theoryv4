...
\item \textbf{WP3 (alignment test only) \mO:} the Green--Schwarz axion coefficient of the celestial
$E_8$ theory, $\lambda_{\mathfrak e_8}/(8\pi\sqrt3)$ (Costello's normalisation), against the TFPT seam
constant $\cthree=1/(8\pi)$ --- an \emph{alignment} check between two fixed normalisations, never a
derivation of $\cthree$ from celestial data.
\item \textbf{WP4 (the type mismatch, head-on) \mO:} the $(E_8)_1$ \emph{character} as a conformal block
of the celestial $E_8[\mathbb C^2]$ S-algebra --- WP1 shows the four glue sectors sum to the $(E_8)_1$
character at the counting level, but the S-algebra is graded by polynomial degree ($60/64$ towers), not by
conformal weight; WP4 must either produce the character from the S-algebra trace or localise the
obstruction.
\item \textbf{WP5 (the summit) \mO:} the twistorial construction as a \emph{second continuum route} for
\texttt{SEAM.EQUIV.01} --- a Costello--Li-grade construction of the holomorphic boundary theory from the
twistor uplift; hard, and explicitly typed as a research programme.
\end{itemize}
...
\textbf{(K3)} if WP3 finds no exact alignment between the GS coefficient and the $\cthree$ normalisation,
the $\cthree$-connection of the celestial route dies (the orbifold arithmetic of WP1 survives on its own).
\textbf{(K4)} if WP4 is negative, the contract degrades to ``$E_8$ is an admissible gauge algebra for
celestial SDYM'' --- already known from the literature, i.e.\ zero TFPT content.

\par\smallskip\noindent\textbf{Honest success estimate.} The probability of \emph{full} success (WP5: a
second continuum route to \texttt{SEAM.EQUIV.01}) is low --- it is constructive holography at the
Costello--Li grade. The probability of \emph{partial} findings is high: WP3 is a finite computation on
fixed data, and WP1/WP2 already returned nontrivial structural matches (verdict B each) with sharp
refinements --- the WP1 critical correction (clock $\neq$ deck; clock$^2=$ deck) and the WP2 sharpenings
(the clock \emph{is} the Picard--Lefschetz/Coxeter monodromy of the seam deformation; the surviving
direction is the $\chi_1$-Fourier diagonal, $a_0$ pure scale with the $\tau{=}i$ shape frozen) --- exactly
the kind of sharp, falsifiable refinement the contract format is for. Status: \texttt{CELEST.SEAM.01}
\mO\ (research contract); \texttt{CELEST.WP1.01} \mE/\mC\ (executed); \texttt{CELEST.WP2.01} \mE/\mC\
(executed); \texttt{SEAM.EQUIV.01} untouched, stays \mO.
