...
Rigidity $+$ Pl\"ucker, U2/v49/v71), $U_{\mathrm{point}}$ collapses to the \emph{one} dimensionful unit
$v_{\mathrm{geo}}$ shared with $1/G$ (v75/v68) --- what remains is a scale anchor, not a flavor gap (the
...
unique 2D bulk then \emph{follow} (v89/v92/\texttt{GATE.METRIC.05--07}); and the carrier-side QBL input
...
certified Calder\'on datum --- closing $G_{\mathrm{net}}$ therefore also closes the carrier choice \tL\
...
since v118--v122 the dictionary itself is exact} --- hexagon $=$ sign-twisted family spectrum, lepton
...
\begin{winbox}[The torsion normal form and the $\delta=\tfrac12$ theorem \tI/\tN\ (\veri{v114\_torsion\_delta.py})]
The v41 ``genericity is future work'' item is now executed --- and yields a theorem. \emph{(i) Flatness
$=$ $\mu_4$ torsion \tI:} for \emph{any} $M$ (symbolic), $\prod_k U^kMU^{-k}=(MU)^4$ --- the $\Z_4$-family
...
\tI\ (both directions):} on the \emph{involutive} branch ($T^2=\mathbf1$) $T$ is a Hermitian reflection
...
value that v20 used by hand is a \emph{$\mu_4$-torsion identity}, not an observation. \emph{(iii)
Reflection lemma \tI:} the $D_4$ reflection forces $\operatorname{diag}M=(r,z,\bar z)$ with $r$ real and
$\operatorname{Re}z=-r/2$ --- the whole $D_4$ diagonal freedom is \emph{one} real parameter, and
$\delta=\tfrac12\iff r=0$. \emph{(iv) Branch census \tN\ (seeded):} the full $D_4$-fixed flat cusp locus
...
--- and the explicit v33/v40 point sits exactly in its $d_1=0$ slice, the \emph{same} $\delta$ value.
\emph{Residue \tP\ (recorded):} whether the anchor splitting $\mathcal O(-2)\oplus\mathcal O(-1)^2$
(Mehta--Seshadri side) forces the $d_1=0$ slice --- or selects the involutive branch --- is the remaining
question; $\delta=\tfrac12$ itself is no longer the open item.
\end{winbox}

\begin{winbox}[The anchor pins the residue: an exact normal form for the $(U_{\mathrm{wall}})$ bundle \tI/\tN\ (\veri{v115\_anchor\_residue.py})]
The v114 branch question is answered --- and the v33 \emph{numerical} Riemann--Hilbert point acquires an
\emph{exact closed form}. \emph{(i) $\mu_4$-average lemma \tI:} $\sum_kU^kXU^{-k}=4\operatorname{diag}X$
...
(v33's diagonal was forced, not chosen). \emph{(ii) The $(8,0,5)/144$ lemma \tI:} the anchor diagonal
...
denominates $c_u/c_d=55/117=5\cdot11/(9\cdot13)$. \emph{(iii) Exact normal form \tI:}
...
solution \tN:} its big-circle monodromy is trivial to $10^{-10}$, $\prod M_k=\mathbf1$, and the
unitarised holonomy has $|\operatorname{diag}\widetilde M_0|=(0,\tfrac12,\tfrac12)$,
$\operatorname{tr}(\widetilde M_0U)=1$ --- the v33/v40 point is gauge-equivalent to $A_0^\star$ (the
$a_{12},a_{23}$ phases are diagonal gauge). \emph{(v) The anchor forces the $d_1=0$ slice \tN:} a
...
with v114, the whole harmonic diagonal is \textbf{anchor $+$ torsion forced}; $\delta=\tfrac12$ needs no
selection. \emph{Residue \tP:} $a_{13}=0$ and the gauge-orbit uniqueness are numerical (one component
across all seeds); promoting them to \tI/\tL\ via Gr\"obner on the RH constraints is the remaining
$U_{\mathrm{point}}$ step.
\end{winbox}

\begin{keybox}[The resonance theorem: $M_\infty=\mathbf1\iff a_{13}=0$ --- one gauge orbit \tI\ (\veri{v116\_resonance\_uniqueness.py})]
...
$B_m=\sum_k i^{km}\,U^kA_0U^{-k}$, the \emph{twisted} $\mu_4$ averages collapse \tI:
\[
...
\emph{Uniqueness corollary \tI:} $a_{13}=0$ $+$ the $(8,0,5)/144$ moduli $+$ phases $=$ diagonal gauge
...
unique}. \emph{Sharpness \tN:} $\|M_\infty-\mathbf1\|\sim8.5\,|a_{13}|$ ($0.17$ at $a_{13}=0.02$) vs
$10^{-10}$ at $A_0^\star$. \emph{Honest scope \tP:} the equivariant ansatz is the established $D_4$
selector input (v19/v30); the \emph{residue} side is now \tI, while the holonomy values (the harmonic
diagonal $(0,\tfrac12,\tfrac12)$) remain \tN\ --- transcendental monodromy of an exact system.
\end{keybox}

\begin{keybox}[The monodromy is the Weyl group of $A_3$ --- the holonomy is exact \tI/\tN\ (\veri{v117\_monodromy\_weyl\_a3.py})]
The last \tN\ item of the chain --- the holonomy \emph{values} --- closes: the unitarised monodromy of
...
v114/v115 located the physical point). \emph{The group} \tI: exact enumeration of
...
\emph{Identification \tN:} the ODE monodromy of $A_0^\star$ equals the exact $\widetilde M_0$ to
$3.5\times10^{-10}$, and the invariant form $H$ is $U$-invariant to $10^{-11}$. \emph{Status:} holonomy
values \tI; only the analytic identification stays \tN. The $\delta$ thread
(v20 $\to$ v40/v41 $\to$ v114 $\to$ v115 $\to$ v117) is closed end to end.
\end{keybox}

\begin{keybox}[The hexagon is the sign-twisted family spectrum --- first exact H2 piece \tI\ (\veri{v118\_hexagon\_family\_dictionary.py})]
The missing ``$C_6\leftrightarrow\PP\setminus\mu_4$ dictionary'' (H2) acquires its first exact entry.
\emph{Sign-twist lemma \tI:} $-\widetilde M_0$ has order $6$ and
\[
...
sheet twist ($\Z_6=\Z_2\times\Z_3$ realised as $(-1)\times\langle\widetilde M_0\rangle$); the v20
denominators $\tfrac54-\cos(r\pi/3)$ are exactly the eigen-denominators $|1-\zeta_6^r/2|^2$ of the two
family resolvents $(\mathbf1\mp\tfrac12\widetilde M_0)^{-1}$. \emph{Cyclotomic determinant \tI:}
...
by the matrix itself. \emph{Sheet-extended group \tI\ (audit):} $\langle U,\widetilde M_0,-\mathbf1\rangle$
has order $48=\Omega_{\mathrm{adm}}=\Nfam\dim S^+$ --- the seed's quartic coefficient. \emph{Residue \tP:} the
dictionary's \emph{values} are exact; its \emph{address table} (which hexagon site each fermion
occupies; the quark composition) remains v20's established input / open --- nothing re-fished.
\end{keybox}

\begin{keybox}[The address table: the lepton words are the compiler atoms \tI/\tP\ (\veri{v120\_address\_table.py})]
The address table acquires its structure --- on word-length integers \emph{frozen} in v18/v20, long
before today's readings (the red-team firewall). \emph{(i) The lepton words are the atoms \tI:}
\[
...
\tI:} $(r,w)=L\ \mathrm{divmod}\ p_2$ with $p_2=6=|R^+(A_3)|$ itself an atom: $e\to(2,1)$,
$\mu\to(5,0)$, $\tau\to(3,0)$; $\sum r=10=p_3=\mathcal A_\Lambda$, $\sum L=16=\dim S^+$. \emph{(iii) Sheet parity
\tI:} by the v118 dictionary, $e$ sits on the \emph{untwisted} sheet ($\omega\in\operatorname{spec}
\widetilde M_0$), $\mu,\tau$ on the \emph{twisted} sheet ($-\omega,-1$); the anchor lepton $\tau$
occupies the unique \emph{real} twisted eigenvalue $-1$ --- exactly the site where v20's product rule
replaces the resolvent. \emph{(iv) Quark sum rules \tI\ (frozen v18 words $7,7,5,3,2,0$):} up-type sum
$=10=p_3$; down-type sum $=14=p_1{+}p_3$; \textbf{all quarks $=24=|W(A_3)|$} (the monodromy group
order, v117); \textbf{all nine charged fermions $=40=p_1p_3=a^TRa$} (v119); quark $\sum r=2p_2$,
$\sum w=|\Z_2|$; the top sits at the vacuum site $(0,0)$ --- zero word, no suppression, the
established ladder anchor. \emph{Residue \tP:} five independent closures ($16,10,14,24,40$) constrain
the address map; the per-fermion \emph{assignment} remains the v18/v20 input --- deriving it is the
remaining H2 content.
\end{keybox}

\begin{keybox}[The address table is pinned: $R$ is the unique hexagon matrix with atom margins \tI\ (\veri{v121\_address\_pinning.py})]
The per-fermion assignment closes at the \emph{information} level. The established identity
$L=R+2U$ (v95) says the word-length table \emph{is} $L[\text{sector}][\text{generation}]$ --- rows
(up; down; lepton) $=$ $((7,3,0);(7,5,2);(8,5,3))$, the v18/v20 words verbatim --- so the addresses are
the entries of the \emph{residue operator} $R$ plus one hexagon turn for the first generation, and
$R$'s first column is the \textbf{anchor} $a=(1,1,2)$. The margins are atoms \tI: row sums
...
\tP:} the open H2 item shrinks from ``derive $9$ per-fermion integers'' to ``derive the six atom
margins $+$ $\det=\operatorname{rank}$''. \emph{Firewall:} $R$, $L$ and all word lengths were frozen in
v4/v18/v20/v71 long before this census --- no degrees of freedom spent.
\smallskip\\
...
--- pin $R$ \emph{uniquely} among hexagon matrices \tI: the $(S1)$ census has $4\times4\times4=64$
...
status is revoked. \emph{Bonus lemma \tI:} on the $(S1)+(S2)$ locus $\det(M+2U)=20=\det L$ holds for
\emph{all} $12$ candidates --- the fourth quartet determinant is redundant, the compiler's familiar
overdetermination. \emph{Residue \tP:} H2 reduces to the establishment of the selectors themselves
...
column problems \tI. \emph{Honesty:} $K$ is \emph{not} pinned directly ($d^\top K=(1,\tfrac12,1)$); $\det
...
$\sigma\to n$ is a genuine mechanism beyond the monodromy group. \emph{R4$'$ restated \tP:} derive the
...
\emph{R4$'$ residue \tP\ (final form):} derive the three pairings --- everything else in the chain
anchor $\to(d,n)\to R$ is exact.
\end{keybox}
...
independent appearance \tI: with $h=\mathbf1+a=(2,2,3)$ in the canonical anchor ordering,
...
natural guess, and we do \emph{not} fish for $55/117$. \emph{Resolved since (v118): the correct
...
v20), every cross-sector $c$-ratio is closed (Pl\"ucker), and each \emph{sector product} is a clean
$\phiz$-power (Grand Mass Volume, v46: $\det M_{\rm sector}\!\sim\!(\phiz)^{6,9,10}$; the lepton product is
...
gravity's $1/G$ (v68) --- the two $[A]$ anchors collapse to one.}
\end{keybox}
...
\begin{winbox}[The glue Q-system: the index-$4$ object of the $G_{\mathrm{net}}$ statement exists explicitly \tI/\tP\ (\veri{v125\_glue\_qsystem.py})]
...
exactly \tI: associativity, unit law, the Frobenius identity
...
\emph{Locality $=$ isotropy \tI:} on the Lagrangian glue $\langle(1,1)\rangle$ the discriminant form
gives $q(k(1,1))=k^2\in\Z$ for every $k$ --- the braiding obstruction vanishes (v92 in Q-system
language); the two Lagrangian glues (the sheet pair) are the only maximal isotropic choices.
\emph{Halfway step \tI:} $\{0,2\}<\Z_4$ closes and gives the unique intermediate Q-system
$\mathbb C[\Z_2]$ $=$ the $SO(16)_1$ tower step, with index factorisation $4=2\times2$. \emph{R2 sharpened
\tP\ (recorded, not claimed):} what remains is the operator-algebraic \emph{identification} ``the
seam--Calder\'on inclusion of the $16$-Majorana net (v113) realises \emph{this} Q-system'' --- matching
two constructed objects, not a search; the three loads (metric $+$ carrier $+$ QBL, v123) hang on this
one identification.
\smallskip\\
\noindent\textbf{The hull is the graded module of the Q-system (\veri{v128\_graded\_hull.py}).} The
Q-system is not merely an abstract companion of $E_8$ --- $E_8$ \emph{realises} it \tI: the $240$
roots, built as explicit norm-$2$ vectors in $D_5\oplus A_3+$glue coordinates, decompose as
$240=52+64+60+64$ over the glue $\Z_4$ (the v89 branching as vectors), and for \emph{all} $6720$ root
...
measure, no longer a diffuse ``full QG missing''. \tP/\tA
