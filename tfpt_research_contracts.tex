...
into a $12$-fold exact equivalence web --- the remaining input is \emph{one discrete symmetry-lift
bit}, its negation concretely measurable (odd-doublet split $(2/\pi)\ln\cot(\delta/2)\neq0$), and
the search space for a future \mE\ closure is narrowed to the RP positivity of the odd sector.
...
discrete symmetry-lift bit ($V_4\to D_4$: flag transitivity $\iff\tau=i$).
...
$\varepsilon_2$ and $\varepsilon_1$ stages $+$ the first back-reaction milestone M1) --- the narrative arc from the $\mu_4$ clock to the $(E_8)_1$ boundary shadow --- is given as
...
back-reacted $\Omega_N$
(\vref{v505}/\vref{v509}/\vref{v511}/\vref{v513}/\vref{v514}/\vref{v515}) &
the preregistered level kill (``inflow demands $k\neq1$''); the $\delta_1$/M2--M3 kills (the M1
kill was evaluated by \vref{v515} and did not fire) \\
...
back-reaction step), and the first quantised back-reaction milestone M1 has since been executed
(the closed-form $\Omega_N$ with $(2\pi i)^2$-integral lockstep periods and forced source charge
$4=|\mu_4|$, \vref{v515}),
leaving the global BCOV quantisation (WP5e proper) as the remaining open piece --- narrowed to the
$\delta_1$ $\tau$-integral (twisted contact terms, \emph{undecided} at exploration level), the
physical justification of the cubic GS term and the remaining $\mathbb{PT}/\Z_4$ back-reaction
...
$\varepsilon_1$ slot itself is executed; of the quantised milestones M1--M3 preregistered in
\vref{v514}, M1 is executed via \vref{v515}, M2--M3 stay preregistered \mO).
\item \textbf{WP5 (the summit) --- subdivided; WP5a--WP5d (both WP5d stages) plus the WP5e $\alpha$, $\beta$, $\gamma$, $\delta_2$, $\varepsilon_2$ and $\varepsilon_1$ stages and the back-reaction milestone M1 are landed.} The twistorial
...
type-I B-model back-reaction on $\mathbb{PT}/\Z_4$ stays \mO\ (its first milestone M1 has since
been executed, \vref{v515}; M2--M3 remain).
...
\textbf{The $\delta_1$ stage --- executed at exploration level, \emph{undecided}: a method
boundary proven at the $\Z_2$ anchor.} The twisted BCOV contact-term ledger on $\mathbb C^2/\Z_4$
(the binary test of the $\gamma$ stage) has been run as a preregistered exploration probe
(exploration-level, v-numbered on demand); its verdict is \emph{undecided}, and its four exact
sharpenings now fence the target: (i)~the strict holomorphic $q^0$ reading is \emph{refuted at
the $\Z_2$/Eguchi--Hanson anchor} (the published EH celestial chiral algebra has a nonzero
one-loop deformation, so a reading that returns an identically empty twisted one-loop term at
$\Z_2$ is a method boundary, \emph{not} a kill of the contract); (ii)~the forced leading
$(T_5,T_3)$ ratio is $4{:}3$ (sector $2$ : sectors $1{+}3$) --- every successful measure must
carry it; (iii)~the $SL(2,\Z)$ orbit decomposition of the $16$ sector pairs is $15=12+3$: the
contact term is the \emph{modular completion} of the Atiyah--Bott data (a Harvey--Moore-type
$\tau$-integral), not a free number; (iv)~massive coset levels $n\ge2$ are mandatory. The
remaining $\delta_1$ target is therefore the $\tau$-integral itself, evaluated against the
$\delta_2$-sharpened criterion ($\psi=64$ slice or selection-rule relaxation).
...
($32\,T_3$) --- preregistered as M1--M3 below; M1 has since been executed (\vref{v515}).
...
$\delta_1$ stands \emph{undecided} at exploration
level; the named remaining targets, with preregistered success/kill criteria:
\textbf{($\delta_1$, sharpened)} the Harvey--Moore $\tau$-integral behind the twisted contact
terms --- success $=$ the one-loop coefficient comes out exactly $(9,-30,-15,0,32)$, computed,
not fitted (equivalently: it supplies the $\psi=64$ slice or the selection-rule relaxation),
carrying the forced $4{:}3$ leading ratio; kill $=$ any other exact value; \textbf{(the cubic GS
term)} the physical justification of the $d$-symbol vertex with $c_d=32\times60$ (the honest
target after \vref{v513}: derive the factor $32=\Phi_{T_3}(A_{\mathrm{fix}})$ physically --- the
$1920=|W(D_5)|$ fingerprint is look-elsewhere-loaded and convention-contingent, not a target; a
ch$_2$-type naturalness datum for \emph{cubic} vertices does not yet exist);
\textbf{(the back-reaction milestones, preregistered as M1--M3 in \vref{v514}; M1 executed)} the
type-I B-model
...
\textbf{M2} the twisted KS measure (quantise the $O(-2)$ tower on $\mathbb{PT}/\Z_4$ with the
$\varepsilon_1$ mode ledger and compute the one-loop box coefficient per sector; success $=$ the
bulk sector reproduces $\tilde\lambda^2=36$ \emph{and} the twisted sectors pair with the three
sphere axions cancelling the rigid $32\,T_3$ residual; kill $=$ a leftover independent
$T_3$-type quartic in any sector); \textbf{M3} the $a_0$ uplift (lift the $(4,\pm4)$ GH
multipole to the Beltrami differential and compute the induced K\"ahler-potential correction;
success $=$ a log-type correction with coefficient tied to the source charge $4=|\mu_4|$; kill
$=$ decoupling from the centre count). Success (unchanged): partition function $=E_4/\eta^8$
\emph{including} the
...
v511, nor the axion-slot construction of v514, nor the M1 back-reaction of v515 trigger it (all
computable
dials say $k=1$, ``one level'' is now a theorem, the bulk axion survives the projection, and the
back-reacted $\Omega_N$ carries the forced integral lockstep flux),
but the global
quantisation is untested.
\end{itemize}
Honest status line: WP5a--WP5d (both WP5d stages), the WP5e $\alpha$, $\beta$, $\gamma$,
$\delta_2$, $\varepsilon_2$ and $\varepsilon_1$ stages and the M1 back-reaction milestone are
...
kernel patching, Hitchin small resolution and CPS brane dictionary, \vref{v515});
\emph{WP5a--WP5d plus WP5e-$\alpha$/$\beta$/$\gamma$/$\delta_2$/$\varepsilon_2$/$\varepsilon_1$ plus M1 are landed ---
...
slot is a construction with $\tilde\lambda=6$ triply pinned, and the back-reacted $\Omega_N$ is
...
level-from-flux dial executed by v509, the axion slot executed by v514, the M1 back-reaction
milestone executed by v515, $\delta_1$
\emph{undecided} at exploration level (method
boundary proven at the $\Z_2$ anchor), and the named
remaining targets are the $\delta_1$ Harvey--Moore $\tau$-integral (sharpened to the $\psi=64$
slice or the selection-rule relaxation), the physical justification of the cubic GS term
($c_d=32\times60$; the $1920=|W(D_5)|$ reading look-elsewhere-loaded and convention-contingent,
\vref{v513}) and the remaining $\mathbb{PT}/\Z_4$ back-reaction milestones M2--M3 (preregistered
in \vref{v514}; M1 executed via \vref{v515})
--- \texttt{SEAM.EQUIV.01} stays \mO\
and nothing in WP5a--WP5e-$\varepsilon_1$/M1 moves it.
\end{itemize}
...
$\tilde\lambda=6$ triply pinned, the first GH/$A_3$ back-reaction step), and the back-reacted
$\Omega_N$ by the M1 milestone (\vref{v515}: closed form, $(2\pi i)^2$-integral lockstep
periods, forced source charge $4=|\mu_4|$), leaving the global
BCOV quantisation (WP5e proper) as the single remaining open piece.
...
$\alpha$, $\beta$, $\gamma$, $\delta_2$, $\varepsilon_2$ and $\varepsilon_1$ stages and the M1
back-reaction milestone are
...
slot with $\tilde\lambda=6$ triply pinned, and M1 delivers the closed-form back-reacted
$\Omega_N$ with forced charge $4=|\mu_4|$; \emph{WP5e proper
--- the global BCOV quantisation ---
...
from geometry) and the M1 back-reacted $\Omega_N$ (the residue form derived with cover factor
...
$4=|\mu_4|$; the honest integrality-is-not-the-discriminator fence) --- exactly the kind of sharp,
...
with the weaker certificate $\psi=64$; the arity $\ge4$ ledger, the cubic GS term and $\delta_1$
stay \mO);
...
$\mathbb{PT}/\Z_4$ typed \mC, of the type-I B-model back-reaction milestones M1 is since
executed via v515, M2--M3 stay \mO);
...
back-reaction step; the quantised BCOV coefficient and the twisted channels stay \mO);
...
the discriminator; M2--M3 and the quantised BCOV coefficient stay \mO);
...
\vref{v492}--\vref{v515}); $\rho$, the order-$4$ clock $\operatorname{diag}(i,1)$; $\Theta$, the
anti-linear real structure induced by the seam reflection (the deck/covering involution, whose
free-ness is topology, \vref{v510}); and $\mu_{\mathrm{BCOV+SDYM}}$, the interacting functional
(the global BCOV$+$SDYM quantisation on $\mathbb{PT}/\Gamma$).
...
\texttt{WOIT.OS.TWISTOR.01} \mO\ (research contract).
