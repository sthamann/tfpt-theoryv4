...
Bittleston--Homans--Sharma on ALE spaces; Costello's twistorial SDYM) produces a coherent twelve-step
story --- executed as work packages WP1--WP5d (both WP5d stages) plus the WP5e $\alpha$, $\beta$ and
$\gamma$ stages of the research contract
...
(exact counterexample), and the fermionic $\Z_4$ forces the \emph{order}, not the alignment
...
(\veri{v507\_seam\_bit\_origin.py}, \texttt{SEAM.BIT.ORIGIN.01}; no marker moves).
...
$(0,360,814,1362)$, one scale $\Rightarrow$ one level). And an honest \emph{refutation}: the
...
verified --- the exchange sub-branch is closed by Step 12, so the named remaining targets are the
twisted BCOV contact terms (the $\delta_1$ binary test: the one-loop coefficient exactly
$(9,-30,-15,0,32)$) and the full-tensor ledger ($\delta_2$; the Step-12 collapse is a
Cartan-restricted statement), with preregistered success/kill criteria in the contract --- together with the
...
constructed, the index chain measured, the KLM triple witnessed, and both faces of the inflow
anchored --- while the global quantisation stays open. \texttt{SEAM.EQUIV.01}
