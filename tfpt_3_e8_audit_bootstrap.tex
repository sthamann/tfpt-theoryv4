...
milestones M1--M3 and the measure decision (Step 21) of the research contract
\texttt{CELEST.SEAM.01}, plus the RP-blindness decision on the alignment bit (Step 22) and the
$\alpha$ and $\beta_1$ stages of the OS twistor bridge
\texttt{WOIT.OS.TWISTOR.01} (Steps 20 and 23), whose
...
(the Step-19 kernel convention), TP-4 (the Quillen structure of BCOV $F_1$). What stays \mO:
the constructive BCOV derivation of the $w_m$ normalisation itself. No marker moves.
...
(defect/twist insertions, the interacting $\mathcal A_{\mathrm{hol}}$) --- the alignment bit
...
moduli integral), so the named remaining target narrows to the \emph{constructive BCOV
derivation of the $w_m$ normalisation itself}; the cubic $d$-channel stays not needed
...
kill under the derived measure --- and the measure question decided at probe level for the
declared reading (Step 21) --- while the
global quantisation (now narrowed to the constructive $w_m$ derivation) stays open.
\texttt{SEAM.EQUIV.01}
