...
Bittleston--Homans--Sharma on ALE spaces; Costello's twistorial SDYM) produces a coherent ten-step
story --- executed as work packages WP1--WP5d (both WP5d stages) plus the WP5e $\alpha$ stage of the
research contract
...
moves, \texttt{QGEO.REALIZE.01} stays \mC/\mO).
...
The single open step is the BCOV/Costello--Li twistor uplift itself: the boundary
partition function $E_4/\eta^8$ \emph{derived from the twistor side}, including the $q^{-1/3}$
prefactor whose CFT side Step 9 has pinned (WP5e proper), with preregistered success/kill
criteria in the contract --- together with the continuum uplift of the Step-10 lattice witnesses
(Xu's theorem for the abstract orbifold statement; the concrete seam quotient net and the
interacting condensed $(E_8)_1$ net are Costello--Li territory). The route does \emph{not} close
\texttt{SEAM.EQUIV.01}; it gives the keystone a
second, quantitative approach --- the same two-step shape as MMST (limit $+$ maximal ideal) with the
ideal quantitatively fixed, the deleting operator explicit, the limit state constructed, the index
chain measured, and the KLM triple witnessed --- while the twistor uplift stays open.
\texttt{SEAM.EQUIV.01}
