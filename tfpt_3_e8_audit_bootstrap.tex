...
Bittleston--Homans--Sharma on ALE spaces; Costello's twistorial SDYM) produces a coherent twenty-step
...
$\gamma$, $\delta_1$, $\delta_2$, $\varepsilon_2$ and $\varepsilon_1$ stages and the three back-reaction
milestones M1--M3 of the research contract
\texttt{CELEST.SEAM.01}, plus the $\alpha$ stage of the OS twistor bridge
\texttt{WOIT.OS.TWISTOR.01} (Step 20), whose
...
\emph{declared}, supported by the $\delta_1$ modular-completion finding, \emph{not} derived from
the BCOV integral --- the $\delta_1$ chain has since been decided in Step 19 (\vref{v518}: the
\emph{derived} chiral measure fails all three testers), so the open face is the
declared-vs-derived measure question, \mO.
...
exact; the sharp open question is which of the two is the true BCOV measure. Neither result is
...
off}), and the $\delta_1$ chain is \emph{decided} by Step 19 (kill under the derived measure),
so the named remaining target narrows to the \emph{measure question}: the declared completion
reading (Step 17: loop $=$ unphased sector trace with the same zero-mode normalisation,
delivering the $\psi=64$ slice) and the derived chiral measure (Step 19: solved from blockwise
covariance, failing all three testers) are \emph{both exact and disagree} --- the global BCOV
quantisation must decide which of the two it produces; the cubic $d$-channel stays not needed
...
dissolved (Step 19 does not revive it) --- together with the
...
$a_0$ uplift coupled to the centre count on four scales, and the $\delta_1$ chain decided ---
kill under the derived measure, with the declared/derived tension stated --- while the
...
contract slots. \emph{The interacting statement stays \mO} --- kill test 1 is discharged only at
