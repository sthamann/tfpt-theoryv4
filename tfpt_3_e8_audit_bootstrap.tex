...
% ---- central marker legend in the page footer (markers documented on every page) ----
\usepackage{fancyhdr}
...
\input{tfpt_docset}
\noindent\textbf{You are reading document \#4.}

\tableofcontents
\clearpage
\part{The $E_8$ secondary-branching atlas}
\begin{keybox}[Purpose and discipline]
...
structurally strong. All arithmetic identities below are exact \tI; the \emph{physical readings} are
audit-level \tA\ unless tied to a derivation. (All 77 numeric claims were machine-checked with exact
...
\begin{winbox}[The anchor difference ladder: the binary carrier spine \tI]
...
\begin{keybox}[Pascal closure: $\gcar=5$ is the unique solution (kills alternatives) \tI]
...
certifies code degrees $\le2$; Paper~1). Neighbour worlds on the ladder: $K{=}1\to g{=}3$ (a
...
--- two independent selections agree on the same world. The hypothesis itself stays \tP.
\end{keybox}
...
of the whole fermion spectrum. \tI
\end{winbox}
...
decuple denominator $\AL=10$. \tI

\begin{winbox}[Anchor-plane Plücker coordinates: scattered hits become oriented areas \tI]
...
the family roots. All entries are exact \tI; this is an audit layer, not a closure (the physical quark
amplitudes stay \tP). \veri{v37\_plucker\_anchor.py}
\end{winbox}

\begin{winbox}[The numerology null test: the look-elsewhere burden quantified \tN]
...
``certainty''. Complementary to the frozen registry: the freeze (v84) prevents \emph{future}
...
% =====================================================================
Each maximal subalgebra of $E_8$ mirrors a TFPT module. The dimension identities are exact \tI; the
readings are audit-level \tA. The seven \emph{slices} of $\dim E_8=248$ are shown below: the same total
...
\begin{theorem}[$E_6\times A_2$ flavor-residue shadow \tA]
...
\begin{theorem}[$E_7\times A_1$ scalaron shadow \tA]
...
\begin{proposition}[$A_4\times A_4$ action-ladder and $F_4\times G_2$ occupancy \tA]
...
\begin{audbox}[Flavor-matrix and prime-table fingerprints (audit-level, machine-verified)]
...
% =====================================================================
\begin{audbox}[Weyl completion index]
...
$D_5\times A_3$ stabiliser. \tI\ (arithmetic) / \tA\ (reading)
\end{audbox}

\begin{audbox}[$E_8$ theta series as a higher-shell raster]
...
the theta series is the right place to type them --- as an audit raster, not a claim. \tA
\end{audbox}
...
\begin{winbox}[(i) Full-length Frobenius norm is three $E_6$ blocks]
\[
  \|L\|_F^2=234=\Nfam\,\|R\|_F^2=3\dim E_6,\qquad
...
residue $E_6$ norm $+$ the $A_3$-winding over $\mu_4$ $+$ the family-winding square. \tI
\end{winbox}

\begin{winbox}[(ii) $A_3$-winding determinant lemma]
The rank-one winding $L=R+6\,\mathbf 1 e_1^\top$ and the matrix-determinant lemma with
$e_1^\top R^{-1}\mathbf 1=\tfrac14$ give
\[
  \det L=\det R\Bigl(1+\tfrac{|R^+(A_3)|}{|\mu_4|}\Bigr)=8\Bigl(1+\tfrac64\Bigr)=8\cdot\tfrac52=20=2\AL .
\]
So $h(D_5)=8\xrightarrow{\,A_3^+/\mu_4\,}20=|R(A_4)|$: the winding injects the carrier factor $5$ into
the cokernel ($\operatorname{coker}R\simeq\Z_8$, $\operatorname{coker}L\simeq\Z_{20}$). \tI
\end{winbox}

\begin{winbox}[(iii) The scalaron $7$ is the $E_8$ exponent variance per decuple]
Centring the exponents at $h/2=15$,
\[
  \sum_m(m-15)^2=560,\quad \operatorname{Var}=\tfrac{560}{8}=70=7\cdot\AL=(\text{scalaron }7)\cdot(\text{decuple }10).
\]
...
Also $\sum_{\text{pairs}}m(30-m)=620=\tfrac{h\dim E_8}{12}=2\cdot10\cdot31$. \tI
\end{winbox}

\begin{winbox}[(iv) Exponents $+$ degrees give the $120{+}128$ split]
The invariant degrees $d_i=m_i+1=\{2,8,12,14,18,20,24,30\}$ obey
\[
  \sum_i m_i=120,\qquad \sum_i d_i=128,\qquad 248=120+128,\qquad \prod_i d_i=|W(E_8)| ,
\]
matching the $D_8$ branching $248=120+128$; the degrees are a TFPT dictionary
($2,8,12,14,18,20,24,30$ $=$ sheet, $h(D_5)$, $|R(A_3)|$, $\dim G_2$, $p_4$, $\det L$, $\dim A_4$, $h(E_8)$). \tI
\end{winbox}

\begin{winbox}[(v) Root shell $=$ local $E_7\times A_1$]
...
and an active coupling shell $56{+}56$. \tI
\end{winbox}
...
This is a \emph{program}, not a proof of new physics: all readings are \tA. The two genuinely
load-bearing, derived results remain the $\mu_4$ glue ($E_8=(D_5\oplus A_3)+\mu_4$, \tL\ \veri{v1\_e8\_glue.py}) and the
spectral selector of the flavor matrix ($\det R=h(D_5)$, $\operatorname{PrinMin}_2=(2,3,5)$, \tI\ \veri{v4\_flavor\_matrix.py}).
\end{keybox}
...
matrix'', but ``prove this one geometric realisation''. (Status: since v118--v122 the dictionary
...
\begin{keybox}[The connection in one sentence]
...
\noindent Three structural facts jump out (all \tI\ arithmetic):
...
length ($27$ nodes, $26$ steps) carries the $E_6$ cube and the $F_4$ fundamental. \tI
\end{winbox}
...
$n{=}27\ldots30$ the compiler atoms. (Mark the tail as the \emph{compiler tail}, not orbits.) \tI
\end{keybox}
...
\midrule
$\Omega_b$ & $\phiz(1-2\cthree)=0.04894$ & $(1-\tfrac1{4\pi})\phiz=0.04894$ & \textbf{identical} ($2\cthree=\tfrac1{4\pi}$) \tI \\
$V_{us}/V_{ud}$ & $\sqrt{\phiz}=0.2306$ & $\lambda_C=\sqrt{\phiz(1-\phiz)}=0.2244$ & refined (the $(1-\phiz)$ factor) \tI \\
$r$ (tensor) & $(\phiz)^2=0.00283$ & $12/N_\star^2=0.00397$ & two branches, same order \tP \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
\midrule
near $n{=}0$ & $\Omega_b=\phiz(1{-}2\cthree)$, $V_{us}/V_{ud}=\sqrt{\phiz}$, $r=(\phiz)^2$ & \textbf{covered} --- dimensionless, derived from $\{\cthree,\phiz\}$; see the $\phiz$-table above. $\Omega_b$ is \emph{identical}. & \tN \\
EW, $n{=}12$ & $v_H=\zeta_{\rm EW}M_{\mathrm{Pl}}\varphi_{12}$, $M_W{=}\tfrac12 g_2 v_H$, $M_Z$ & \textbf{scale covered, absolute scheme-level} --- the EW scale enters the current note via the action-grammar exponent and the two-loop flow; $M_W,M_Z$ are dimensionful and remain RG/scheme outputs (one $\zeta_{\rm EW}$), \emph{not} new compiler powers. & \tP \\
hadronic, $n{=}15,17$ & $\mathbf{m_p}{=}\zeta_p M_{\mathrm{Pl}}\varphi_{15}$, $m_b{=}\zeta_b M_{\mathrm{Pl}}\varphi_{15}$, $m_u{=}\zeta_u M_{\mathrm{Pl}}\varphi_{17}$ & \textbf{ratios closed, absolute scale an anchor.} The quark mass \emph{ratios} sit on the $\phiz$-word-length ladder and are closed (integer Pl\"ucker readouts, v49/v71); the \emph{absolute} values need the per-sector $\Lambda$/$U_f^\star$ normalisation, an anchor (\tA). The \textbf{proton mass} is QCD-confinement, $m_p\sim\Lambda_{\rm QCD}$ by dimensional transmutation --- a \emph{cross-sector} ratio, listed as a genuine frontier item ($m_p/m_e$, tfpt\_4). & \tA \\
cosmo, $n{=}25$ & $T_{\gamma0}{=}\zeta_\gamma M_{\mathrm{Pl}}\varphi_{25}$, $T_\nu{=}(\tfrac4{11})^{1/3}T_{\gamma0}$, $\rho_\Lambda$ & \textbf{partly.} $\rho_\Lambda$/$\Lambda$ is \emph{typed} by the action grammar and the seam ($\Lambda$-typing, tfpt\_2); $T_\nu/T_{\gamma0}$ is standard QFT (not cascade). The \textbf{absolute CMB temperature} $T_{\gamma0}$ is a downstream readout of the reheating pipeline (Paper~6/7), conditional --- not re-derived as a compiler number. & \tP \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
as a compiler power --- it stays a frontier item (\tA, tfpt\_4); the quark mass \emph{ratios} are closed
(integer Pl\"ucker, v49/v71) and only the \emph{absolute} quark scale $m_b,m_u$ needs the per-sector
$\Lambda$/$U_f^\star$ normalisation, an anchor (\tA). So: yes, the \emph{spine} is fully audited and the
...
\item The flavor selector $\det R=h(D_5)=8$ is the \emph{terminal orbit} of the $E_8$ cascade; the
      compiler is the cascade's IR endpoint. \tI
\item The atlas branching blocks ($52,56,60$) are the cascade's \emph{UV head} ($n=0,2,4$); the secondary
      atlas and the orbit cascade are the same spine read top-down vs.\ as subalgebras. \tI
...
The \emph{dimension/rung coincidences} are exact arithmetic \tI\ --- they show the old and new
...
\begin{keybox}[The question, and the honest answer]
...
\Rightarrow h(E_8)=30=2\cdot3\cdot5\Rightarrow$ largest prime $=5=\gcar$. \tI
\end{keybox}
...
\]
--- $\gcar=5$ is recovered from the closure, not just posited. \tI
\end{keybox}
...
$\gcar=5$ is forced \emph{three} ways: rank-fill, Coxeter-match, and integer-glue/norm closure. \tL\ \veri{v6\_bootstrap.py}
\end{keybox}

\begin{keybox}[Bootstrap classification: $D_5\oplus A_3$ is the \emph{unique} familyful $E_8$ glue \tL]
...
\begin{winbox}[Net: two axioms $\to$ one primitive $+$ one fixed point]
\[
  \{\cthree,\gcar\}\ \longrightarrow\
...
\]
The discrete data $\{2,3,4,5,6,8,30\}$ are \emph{all} self-consistent outputs of the $E_8$ closure
($\gcar=5$, $h(D_5)=8$, $h(A_3)=4=|\mu_4|$, $h(E_8)=30=2{\cdot}3{\cdot}5$, $|R^+(A_3)|=6$). The only
irreducible input on the \emph{dimensionless} axis is $\pi$. \tI\ (discrete loop) / \tA\ ($\pi$ is primitive)
\smallskip\\
...
($8=2|\mu_4|=\operatorname{rank}E_8=h(D_5)=\varphi(30)=\det R$). \tI\ \veri{v54\_seam\_horizon\_keystones.py}

\begin{keybox}[A cyclic element \emph{inside} the hull: the order-$30$ Coxeter rotation \tL\ (\veri{v55\_coxeter\_cycle.py})]
...
\begin{keybox}[The compiler atoms \emph{are} the $E_8$ Casimir degrees \tL\,(\veri{v66\_e8\_casimir\_degrees.py})]
...
commutator $G_2$, \S Paper~2), $20{=}\det L$, $24{=}|W(A_3)|{=}4!$, $30{=}h(E_8)$ (and the fittable
...
\textbf{Local foundation:} $\cthree,\gcar$ are the two operational inputs (Papers 1--2).
...
continuous origin is $\pi$ from the Möbius boundary geometry.} (Not ``zero axioms''.) \tI/\tA
\end{keybox}
...
% =====================================================================
\begin{itemize}[leftmargin=1.4em]
\item \textbf{What is genuinely shown \tI:} $\gcar=5$ is an \emph{overdetermined} fixed point of the
  ...
  is unique, not one of many. \textbf{This uniqueness is now machine-formalised \tF\ in Lean 4} (Theorem A,
  ...
  (\veri{v108\_pascal\_ladder.py}) is thereby \tF.
...
1a & absolute quark $c$-scale (ratios closed) & \tA & quark \emph{ratios} are integer Pl\"ucker readouts on the derived selector stratum (v49/v69/v71); only the \emph{absolute} $U_f^\star$ normalisation remains --- an anchor \\
1b & $\theta_{12}$ solar angle & \tN & \textbf{texture closed (below):} explicit $\mu\tau$-symmetric texture verified; only $\varepsilon{=}\tfrac34\phiz{\approx}\cthree$ ($0.23\%$) stays conditional \\
\midrule
\multicolumn{4}{@{}l}{\textit{2. Scheme layer --- standard RG, no new input}}\\
2a & $m_W,m_Z,m_H,\sin^2\theta_W,\alpha_s$ & \tP & RG threshold matching $R_{\mathrm{SM}}$; $m_H$ on the closed UV quartic $1/16\pi^2$ \\
2b & hadron pole spectrum (singlet) & \tP & singlet algebra $\mathcal A_{\mathrm{sing}}$ explicit; dynamics already closed (gap $6\log\tfrac32$) \\
\midrule
...
4 & P1 ($\cthree$), P2 ($\gcar{=}5$) & \tA & more Lean; P2 algebra already machine-verified; remain inputs \\
\midrule
...
\begin{winbox}[Item 1b closed: the explicit solar-angle Majorana texture \tN]
...
\begin{equation}
  ...
  \theta_{23}=45^\circ,\quad \theta_{13}=0\ }\quad\tN
\end{equation}
...
\midrule
$A_4{\times}A_4=SU(5)^2$ & $\sin^2\theta_W|_{\mathrm{GUT}}=3/8$ sits in the $SU(5)$ hypercharge embedding; the $12$ coset states are the proton-decay $X,Y$ bosons & \tI\ (known) \\
$F_4{\times}G_2$ / Koide & Koide $Q=2/3=|\Z_2|/\Nfam$ (sheet over families); $J_3(\mathbb O)$ has $\dim 27{=}26{+}1$, $3{\times}3$ octonionic ``generations'' on the diagonal & \tP\ structural \\
$D_8=SO(16)$ & $128=(16,4)\oplus(\overline{16},\overline4)$ --- \emph{same} spinor as $D_5{\times}A_3$; $\nu_R$ (seesaw) lives in the $16$; \textbf{no new dark-matter state} (only the 4th $\mu_4$ direction) & \tI \\
...
\begin{winbox}[The genuine content: $A_8$ and $D_5{\times}A_3$ agree on the family \tI]
...
Each $SU(5)$ pair $(\mathbf{10}+\bar{\mathbf5})$ is anomaly-free, so the embedding is clean. \tP
\end{gapbox}
...
% =====================================================================
The audit/bridge identities of this document (marked \tI, \tL) are re-derived from $\{\cthree,\gcar\}$ and
...
\end{itemize}
The $E_8$-slice/atlas readings are explicitly \tA\ (audit-level), not part of the pass/fail suite. Run
