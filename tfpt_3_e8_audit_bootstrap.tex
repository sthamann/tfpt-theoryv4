...
The celestial reading of this sphere is now pinned at the exact-arithmetic level: on the $A_3$ ALE orbifold
$\mathbb C^2/\Z_4$ the deck group induces only the sheet flip $z\mapsto-z$, while the order-$4$ clock is the
K\"ahler $U(2)$ phase $\operatorname{diag}(i,1)$ whose spin lift squares to the deck ($\Z_8$ bridge,
$8=2|\mu_4|$ --- the $\cthree=1/(8\pi)$ winding integer), and the $E_8$ $\mu_4$-glue is the flat $\Z_4$
monodromy of the glue-equivariant celestial chiral algebra there --- WP1 of the research contract
\texttt{CELEST.SEAM.01} (\texttt{tfpt\_research\_contracts}), \mE\ arithmetic with the celestial-dual
construction itself left \mO\ \veri{v492\_celestial\_z4\_orbifold.py}. The deformation theory of that
orbifold (WP2) is executed as well: the clock-invariant family $XY{=}Z^4{+}a_0$ carries \emph{no} shape
modulus --- the binary-quartic invariants are $I{=}12a_0$, $J{=}0$ identically, so $j{=}1728$ and the
$\tau{=}i$ pillowcase is frozen for every $a_0$ ($a_0$ is a pure scale) --- and the clock \emph{is} the
Picard--Lefschetz/Coxeter monodromy of the family (eigenvalues $\{i,-1,-i\}$ $=$ the $A_3$ exponents,
$h(A_3){=}4{=}|\mu_4|$), verdict B with the deformed-algebra transfer grade-conserving
\veri{v493\_celestial\_wp2\_clock\_deformation.py}. WP3 and WP4 of the contract are executed as well,
conservatively typed: the Green--Schwarz coefficient of celestial $E_8$ is exactly
$\tilde\lambda=6$ with $(\kappa/\cthree)^2=12=|\mu_4|\Nfam$, but the alignment format holds for
\emph{all eight} algebras on Costello's list ($8/8$) --- compatibility, not $\mathfrak e_8$-selective
evidence \veri{v495\_celestial\_wp3\_gs\_alignment.py}; and the $(E_8)_1$ character is \emph{not} a
conformal block of the celestial S-algebra in its jet grading (null ideal $27000=(h^\vee)^3$ localised),
surviving only as a boundary/limit shadow at the current stratum --- the \texttt{SEAM.EQUIV.01}
scaling-limit shape \veri{v496\_celestial\_wp4\_character\_shadow.py}. The first WP5 milestone (WP5a)
makes that shadow a \emph{precise coefficientwise limit} (stabilisation threshold $w=n{+}1$ in an exact
$\chi_w$ family containing the jet grading at $w{=}2$) and \emph{derives} the null ideal from root data
($\operatorname{Sym}^2(248)=27000{+}3875{+}1$, $27000=(h^\vee)^3$; quotient $31124{-}27000=4124=$ the
independent $\mu_4$ theta-split sector sum), with an $SO(16)_1$ negative control --- the operator
statements stay \mO\ (WP5b--e) \veri{v497\_celestial\_wp5a\_null\_ideal.py}.
