...
grown into an eight-step programme of its own (the research contract \texttt{CELEST.SEAM.01}); it is
presented as a dedicated section below (Section~\ref{sec:celestial}).
...
% =====================================================================
\section{The celestial route: from the $\mu_4$ clock to the $(E_8)_1$ boundary shadow}\label{sec:celestial}
% =====================================================================
...
Bittleston--Homans--Sharma on ALE spaces; Costello's twistorial SDYM) produces a coherent fourteen-step
story --- executed as work packages WP1--WP5d (both WP5d stages) plus the WP5e $\alpha$, $\beta$,
$\gamma$, $\delta_2$ and $\varepsilon_2$ stages of the research contract
...
\emph{programme} \mO, not a claim, and nothing in it moves \texttt{SEAM.EQUIV.01}.
...
Step 14 and the level-from-flux dial is executed
by Step 13, so the named remaining targets are the
...
($c_d=1920=|W(D_5)|$), the $\varepsilon_1$ $O(-2)$ bulk-axion construction, and the type-I
B-model back-reaction on $\mathbb{PT}/\Z_4$ (which would upgrade the Step-13 CPS dictionary from
\mC\ to derived), with preregistered success/kill criteria in the contract --- together with the
...
Costello--Li territory). The route does \emph{not} close \texttt{SEAM.EQUIV.01}; it gives the
keystone a second, quantitative approach --- the same two-step shape as MMST (limit $+$ maximal
...
anchored, the collapse confirmed full-tensorially with one cubic door open, and one level a
theorem with its value pinned by the sector counter --- while the global
quantisation stays open. \texttt{SEAM.EQUIV.01}
...
\item \textbf{The current reduction state (2026-07-21; text consolidation, markers unchanged):} the
  compiler's dimensionless inputs reduce to \emph{four marks} (derived from P1-side topology,
  \vref{v216}), \emph{one square-modulus datum} $\tau=i$ (equivalently: the clock / the CM-fixed
  half-period / the non-split extension class on the free seam circle;
  \vref{v499}/\vref{v506}/\vref{v507}/\vref{v510}), and $\pi$ --- with the \mC\ identifications R1/R2
