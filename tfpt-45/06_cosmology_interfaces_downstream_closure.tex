...
\smallskip
\textbf{\stag{I} (Lean interfaced).}\ Paper-1's input enters
as \texttt{BoundaryPolarization}; no \stag{I} obligation is
discharged here.

\smallskip
\textbf{\stag{M} (manuscript theorems in this paper's body).}\
The body contains manuscript-level theorems on the
determinant-line axion interface, leptogenesis interface,
FRW reduction, and boundary-winding control. These are
\stag{M}, not \stag{L}. In particular, the
boundary-winding-control theorem
($N_{\mathrm{fam}} = 3n$, $N_{\mathrm{DW}} = n$,
$\sum L_{f,j} = 40n$) is a manuscript theorem and depends on
Paper 2's \stag{C} downstream branch ledger; it is not the
\stag{L} carrier core.

\smallskip
\textbf{\stag{C} (conditional readouts) and \stag{P}
(programmatic).}\ The numerical cosmological readouts ---
seam-transfer outputs, determinant-line phase, axion
oscillation data, reheating temperature, leptogenesis input
block, CMB Stage-1 spectra --- are \stag{C}; Stage 2 (sky
realisation) is \stag{P} and must not be read as a
theorem-level claim.
\end{tfptstatusbox}
...
All legacy \stag{A} theorem labels in the body have been
renamed to \stag{M} (manuscript theorem) to match the binding
taxonomy of this series. The body's theorems on the
determinant-line axion interface, the canonical leptogenesis
interface, the FRW reduction, and boundary-winding control
are \stag{M}. The numerical cosmological readouts threaded
through these theorems are \stag{C}, conditional on Paper 2's
\stag{C} downstream branch ledger. Nothing in this paper is
\stag{L}.
\end{tfptwarningbox}

\tfptDraftXrefNotice

\input{source_extracts/06_cosmology_source.tex}
...
\clearpage
\input{source_extracts/bibliography_cosmology.tex}
