...
\begin{tikzpicture}[
    >=Latex,
    chain/.style={draw=black!55, rounded corners, align=center, text width=2.45cm, minimum height=1.0cm},
    optchain/.style={draw=purple!70!black, dashed, rounded corners, align=center, text width=3.0cm, minimum height=0.95cm},
    eq/.style={font=\Large},
    note/.style={font=\scriptsize, align=center}
]
\node[chain, fill=green!7, draw=green!50!black] (a) at (0,0) {$10\,\bone$\\\scriptsize abelian index};
\node[eq] at (2.0,0) {$=$};
\node[chain, fill=orange!9, draw=orange!75!black] (b) at (4.0,0) {$\Oadm\gamma+1$\\\scriptsize occupancy $+$ Higgs};
\node[eq] at (6.0,0) {$=$};
\node[chain, fill=gray!8, draw=black!65] (c) at (8.0,0) {$41$\\\scriptsize integrity marker};
\node[optchain] (d) at (4.0,-1.8) {$48\,I_1^{\mathrm{EW}}=41$ on the electroweak normalization\\$32\,k_{\mathrm{EW}}=41$ only on the conditional E$_8$ reading};
\node[note] at (4.0,-3.0) {optional extension, not part of the hard chain};
\end{tikzpicture}
...
32\,k_{\mathrm{EW}}=41
\]
belongs only to the conditional E$_8$ reading and requires the conjectural block-index relation.
\end{remark}
...
\end{equation}
where $C_n$ are the Catalan numbers. In the present paper the safe interpretation of this identity is noncrossing combinatorics on the seed side; it is an exact algebraic strengthening of the one-seed compression, not yet a theorem about planar Feynman graphs, a specific large-$N$ / 't Hooft resummation, or a holographic flavor limit.
\end{remark}
...
=\phiz.
\]
This is best read as an exact seed-splitting identity inside the benchmark bridge, not as an independent dynamical conservation law.
...
\[
\alpha^3-2\cthree^3\alpha^2-8\cthree^6\,\bone(N_{\mathrm{fam}},N_\Phi)\ln\!\bigl(\phiztwo(\alpha)^{-1}\bigr)=0,
\]
with
\[
\phiztwo(\alpha)=\phibase+\deltatop e^{-2\alpha}+\delta_2 e^{-4\alpha},
\qquad
\deltatop=\Oadm\cthree^4,
\qquad
\delta_2=\frac54\,\deltatop^2,
...
\[
F:=\mathcal H^1_{\mathrm{APS}}(X_f)\cong H^1(X_f,\mathbb C).
\]
We use the distinguished cycle basis $(C_1,C_2,C_T)$ only as a concrete carrier-side basis of this family space, not as a replacement for its APS/Hodge realization.
\end{definition}
In this interpretation $F$ is neither an arbitrary flavor vector space nor a loose bookkeeping trick; it is a cycle space attached to the seam data. The actual bridge logic is shown in \Cref{fig:family-triangle}: the local patch-deficit count fixes $N_{\mathrm{fam}}=3$, the APS/local-system step promotes that count to a family space, and the occupancy reinterpretation then feeds back into the defect channel.
...
\node[closure, text width=2.1cm, minimum height=0.9cm] at (2.0,-2.2) {APS / local-system closure\\$F\cong\mathbb C^3$};

\node[closure] (occ) at (5.85,0.55) {$\Omega_{\mathrm{adm}}$\\$=3\times16=48$};
\node[bridge] (dtop) at (5.85,-1.15) {$\deltatop=\Omega_{\mathrm{adm}}\cthree^4$\\imported bridge corollary};
...
\end{tikzpicture}
\caption{Carrier-to-family closure bridge. The local patch-deficit count gives the hard multiplicity $N_{\mathrm{fam}}=3$, the APS/local-system step turns that multiplicity into a genuine family space, and the resulting occupancy $\Omega_{\mathrm{adm}}=48$ feeds back into the defect reading $\deltatop=\Omega_{\mathrm{adm}}\cthree^4$.}
...
\begin{theorem}[Imported orbifold--APS family master theorem]
\label{thm:orbifold-aps-family-master}
Let $X_f$ be the spin-resolved orbifold seam section with four $\mathbb Z_4$ corners, and let $L_F$ be the associated flat family bundle. Then the local corner count, the orbifold topology, and the APS family index compress to the single statement
\begin{equation}
\label{eq:orbifold-aps-family-master}
\Ind^{\mathrm{orb,APS}}(D_{\mathrm{rel}}^+\otimes L_F)
:=
\int_{X_f}\hat A(TX_f)\,\mathrm{ch}(L_F)
-\frac{\eta_\Sigma+h_\Sigma}{2}
+\sum_{p\in\mathrm{corners}}\delta_p
=3,
\end{equation}
with
\[
\delta_p=\frac34
\qquad
\text{for each of the four }\mathbb Z_4\text{ corners.}
\]
Equivalently,
\[
...
\]
and the local patch-deficit count is
\begin{equation}
...
\begin{remark}[Status of the imported family theorem]
In the present main-text form, this statement is imported rather than fully rederived. The proof obligation is the precise definition of $X_f$, the orbifold APS index formula being used, the derivation of the four corner terms $\delta_p=\frac34$, the evaluation of the bulk and boundary correction terms, and the computation of $\rank H_1^{\mathrm{free}}(X_f,\mathbb Z)=3$.
\end{remark}
...
\]
Hence the topological surcharge enters the present paper as an imported occupancy corollary with a bridge reading:
\[
\deltatop
=
...
The family subsection is therefore no longer split between a local count, a global APS realization, and a later occupancy reinterpretation. Those three statements are compressed into one imported theorem with one bridge corollary, while the holonomy proposition below records the resulting unitary geometry on $F$.

This master theorem is the family statement referenced later by the admissibility theorems: the closure layer does not rederive $N_{\mathrm{fam}}=3$, but imports it as the already globalized orbifold--APS result of the present section.
...
\]
In the present manuscript this imported occupancy corollary is used with a bridge reading: it is the boundary-to-bulk step that later feeds both the electromagnetic and geometric benchmark channels. The coefficient $5/4$ is therefore not an extra numerical motif; it is the minimal-carrier compression factor $B\gamma$ rewritten in defect language. At the same time, the equality $\deltatop=\Oadm\cthree^4$ is not presented as an independent dynamical transgression theorem in the ordinary field-theory sense.

\subsection{From bridge to geometric completion}

\begin{principle}[Closure split]
Once the carrier packet and family closure architecture are fixed, the focused main-text closure problem splits into a geometric program $\Ageom$ and a transport program $\Atrans$. Record algebra and prediction semantics are now tied directly to the admissible gap in the main theorem stack, while E$_8$ scale grammar and horizon extrapolations remain conditional layers.
\end{principle}
...
\Ageom
:=
\langle E_3\oplus E_2,\ D_{\mathrm{rel}},\ \mathcal B_{\mathrm{rel}},\ \wspin\rangle,
\qquad
\Atrans
:=
\langle U_6,\ T,\ D_y,\ \mathcal Y_y^{(\varepsilon)},\ \mathcal C_{\Seam}\rangle.
\]
This split keeps the carrier theorem, the geometric reconstruction problem, and the record-level extensions from being flattened into one undifferentiated claim. In particular, the metric field is no longer treated here as an independent entry of $\Ageom$; it is reconstructed from the retained relative operator branch introduced next.
...
\[
\mathbb A_{\Seam}:=(D_{\mathrm{rel}},A_{\Sigma}^{\mathrm{geo}}),
\qquad
A_{\Sigma}^{\mathrm{geo}}:=\mathcal B_{\mathrm{rel}}\oplus\nabla_{\chigeo}.
\]
Here $A_{\Sigma}^{\mathrm{geo}}$ is the geometric-branch reorganization of the primitive seed package $A_{\Sigma}^{\mathrm{seed}}$ introduced earlier in \Cref{eq:tfpt-master}.
\begin{theorem}[Relative metric reconstruction from $D_{\mathrm{rel}}$ on the admissible branch]
\label{thm:relative-geometric-reconstruction}
Assume the admissible center satisfies
\[
Z(\mathcal A_{\mathrm{adm}})\cong C^\infty(M),
\]
and the principal symbol of the retained relative operator obeys
\[
\sigma(D_{\mathrm{rel}})(x,\xi)^2
=
g^{\mu\nu}(x)\xi_\mu\xi_\nu\,\mathbf 1.
\]
Then the principal symbol of $D_{\mathrm{rel}}$ reconstructs the metric $g$, the spin structure, and the Levi--Civita connection on the retained branch. Inner fluctuations of the same relative spectral datum generate the gauge sector. Thus $g$ is read from $D_{\mathrm{rel}}$ rather than inserted as an independent member of $\Ageom$.
\end{theorem}

\begin{proof}[Proof sketch]
On the admissible branch the commutative center identifies the manifold algebra, while the principal symbol determines the quadratic form on cotangent vectors. Standard Dirac-type reconstruction then recovers the spin bundle and Levi--Civita connection from the same symbol data. The relative formulation changes the reference bookkeeping, not the reconstruction mechanism, so the metric information is still carried by $D_{\mathrm{rel}}$ itself.
\end{proof}

\begin{remark}[Scope of the present reconstruction theorem]
In v3.1 the theorem above reconstructs the metric, spin structure, and Levi--Civita connection on the admissible branch. The topology of $X$ and the seam $\Seam$ still remain primitive data in the present main-text form. Full operator reconstruction in the stronger sense outlined later in Section~13.3 is therefore not yet claimed here.
\end{remark}
...
\qquad
D_\sigma:=e^{-\sigma/2}D_{\mathrm{rel}}e^{-\sigma/2}.
\]
The relative spectral action is then organized as
\[
S_{\mathrm{rel}}[\sigma,D]
:=
\tr f\!\left(\frac{D_\sigma+\mathcal B_{\mathrm{rel}}}{\Lambda}\right)
...
\begin{remark}[Full operator-reconstruction route]
The stronger closure target is that geometry is nothing but the reconstruction of
$D_{\mathrm{rel}}$ on the admissible branch. In that sharper form one would write
\[
X=\operatorname{Spec} Z(\mathcal A_{\mathrm{adm}}),
\qquad
\Seam=\operatorname{Fix}(\inv),
\]
and reconstruct the metric from the relative Connes distance
\[
d_{\mathrm{rel}}(x,y)
:=
\sup\Bigl\{
|a(x)-a(y)|:
a\in Z(\mathcal A_{\mathrm{adm}}),\;
\|[\Padm D_{\mathrm{rel}}\Padm,a]\|\le 1
\Bigr\}.
\]
The same route would read the local Weyl factor from the symbol norm, so that
$\chigeo(x)=\Lambda e^{\sigma(x)}$ becomes a reconstructed spectral scale rather than
an auxiliary field. The present theorem records the symbol-level reconstruction
already used in the manuscript; this stronger metric-space formulation is the
next natural theorem path.
\end{remark}
...
\begin{conjecture}[Center-neutral Wilson area law on the admissible branch]
If, in addition, the admissible center-neutral Wilson functional obeys strong-coupling dominance on the retained branch, then the relative Wilson loop satisfies the area law
\[
...
\sigma=\cthree^2\chi_{\QCD}^2,
\]
and the physical Hilbert space contains no color nonsinglet asymptotic states.
\end{conjecture}
...
We write $\mathcal S_{\mathrm{int}}$ for the intrinsic closure surface generated before comparison, containing in particular the internal compiler masses $\hat m_f$, the transport outputs, and the retained geometric parameters. The observation surface is
\[
\mathcal S_{\mathrm{obs}}:=\mathcal R_{\SM}(\mathcal S_{\mathrm{int}}),
\]
that is, the scheme-specified comparison image after matching, thresholds, and observable conventions are imposed. The first surface is theorem- and branch-facing; the second is the benchmark-facing interface used by the ledgers below.
\end{definition}
...
$\phi_{\mathrm{EM}}(\alpha)=\phibase+\deltatop e^{-2\alpha}+\delta_2 e^{-4\alpha}$\\
...
\textbf{Comparison map $\mathcal R_{\mathrm{SM}}$}\\
matching scale, thresholds, scheme map\\
CODATA / PDG / NuFIT / Planck snapshot
};
...
comparison-facing rows under $\mathcal R_{\mathrm{SM}}$
};

\node[anchorrow, below=of benchmarks] (bosons) {%
\textbf{Gauge / Higgs masses}\\
exact: $m_\gamma=0$, $m_g=0$\\
branch/anchor surface: $m_W$, $m_Z$, $m_H$\\
status remains row-dependent
};
...
\end{scope}
\end{tikzpicture}
\caption{Carrier-first compiler from primitive seam data to explicit particle-content and mass surfaces. Gray nodes denote the primitive datum, blue nodes are hard carrier/kernel statements, orange nodes are closure statements built from the admissibility complex and the declared closure inputs, green nodes denote the benchmark bridge and its evaluated rows, and gray/red nodes are comparison or anchor-dependent output surfaces. The figure intentionally separates exact structural outputs from candidate and anchor-governed mass rows.}
\label{fig:compiler-to-masses}
\end{figure}

The diagram in \Cref{fig:compiler-to-masses} is epistemically layered: it visualizes one compiler flow, not one uniform status class. Exact massless gauge directions, candidate charged-lepton rows, anchor-governed heavy rows, oscillation/seesaw neutrino rows, and hadronic cross-checks are shown together only to display the architecture, not to erase their different proof status.
...
\begin{tikzpicture}[
    ...
    candidate/.style={draw=gray!70!black, fill=gray!12, rounded corners, text width=2.45cm, minimum height=1.0cm, align=left, inner sep=4pt},
...
\node[hdr] at (6.3,0) {Benchmark bridge};
\node[hdr] at (9.45,0) {Conditional};

\node[rowhdr] at (-2.55,-1.2) {structural\\outputs};
\node[rowhdr] at (-2.55,-2.6) {derived\\quantities};
\node[rowhdr] at (-2.55,-4.0) {candidate /\\branch rows};
\node[rowhdr] at (-2.55,-5.4) {anchor /\\interface rows};

\node[hard] at (0,-1.2) {$E_3\oplus E_2$\\$G_{\SM}^{\mathrm{glob}}$, $S^+$};
\node[closure] at (3.15,-1.2) {$\Padm$\\$\Hhad$, $\bar\theta=0$};
\node[bridge] at (6.3,-1.2) {$\mathcal R_{\SM}$\\declared scheme map};
...
\node[cond] at (9.45,-2.6) {$f_a$, $m_a$, $\nu_a$\\downstream scale rows};

\node[hard] at (0,-4.0) {---};
\node[candidate] at (3.15,-4.0) {charged-lepton candidates\\minimal neutrino branch};
\node[bridge] at (6.3,-4.0) {$\delta_{\mathrm{CKM}}^{(2)}$\\oscillation / fit-facing rows};
\node[cond] at (9.45,-4.0) {$\eta_B$\\optional branch output};

\node[hard] at (0,-5.4) {---};
\node[closure] at (3.15,-5.4) {hadronic cross-checks\\transport-side residuals};
\node[bridge] at (6.3,-5.4) {$m_W,m_Z,m_H$\\$\hat s_Z^2,\alpha_s$};
\node[cond] at (9.45,-5.4) {haloscope window\\horizon / UV tails};
\end{tikzpicture}
\caption{Epistemic output matrix. Columns encode the manuscript's truth/status classes; rows encode output mode. The point is to stop derived, candidate, anchor-dependent, and interface rows from looking like one homogeneous notion of prediction.}
