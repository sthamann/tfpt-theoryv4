...
\date{Version 2.4 --- 22 January 2026}
...
(and admitting a parameter-free next-order refinement to $\sim-0.007$ ppm; see the sensitivity discussion).
...
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt]
    \item Fine-structure constant: $\alpha^{-1} = 137.0360\ldots$ at $-0.037$ ppm (closed form consequence of defect sector reweighting on the orientable double cover; a parameter-free next term reduces this to $\sim-0.007$ ppm)
    ...
    \item Two-loop RG fingerprints: $\alpha_3(\mu) \approx \phiz, \cthree$ at sub-percent (checked by \texttt{two\_loop\_rg\_fingerprints} in \texttt{tfpt-suite/})
\end{itemize}
...
\begin{remark}
\textbf{Parameter-free next term (diagnostic refinement):}
If one allows two-defect occupancy in the defect-sector sum (no new parameter; next weight fixed as $\deltatop^2$),
the response becomes
\(\phiz(\alpha)=\phitree+\deltatop e^{-2\alpha}+\deltatop^2 e^{-4\alpha}+\cdots\),
which shifts the self-consistent solution to
\(\alpha^{-1}\approx 137.03599819\) (about $-0.007$ ppm vs.\ CODATA).
This refinement is tracked by the \texttt{alpha\_precision\_audit} verification module.
\end{remark}
...
Two-defect refinement ($+\deltatop^2 e^{-4\alpha}$) & 137.03599819 & $-0.007$ ppm & parameter-free next term (diagnostic)\\
...
\quad Global consistency (suite) & $\chi^2$ scorecard & p-values OK & \textcolor{green!60!black}{\textbf{PASS}}\\
\quad Flavor (suite) & CKM/PMNS $\chi^2$ & p-values OK & \textcolor{green!60!black}{\textbf{PASS}}\\
\quad Axion DM (relic) & $\Omega_a h^2 \approx 0.123$ & $+2.2\%$ & \textcolor{blue}{\textbf{Derived (post-inflation; $C_{\mathrm{str}}=7/3$)}}\\
\quad Dark energy & $\rho_\Lambda$ (torsion condensate) & 0.09 dex & \textcolor{blue}{\textbf{Derived (discrete scan; suite)}}\\
...
\begin{itemize}[leftmargin=1.5em, topsep=2pt, itemsep=1pt]
    ...
    \item Verification suite modules include: \texttt{effective\_action\_r2}, \texttt{bounce\_perturbations} (see \texttt{tfpt-suite/} and the generated \texttt{tfpt-suite/tfpt-test-results.pdf})
\end{itemize}
