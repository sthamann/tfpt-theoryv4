...
$m_2/m_3$ & candidate \mO: $\sqrt{|J_{\mathrm{PMNS}}|}$ (\vref{v468}; the older heuristic $\sim\pi\phiz$ is $-2.4\sigma$, superseded as comparator) & $0.17220$ (NuFIT $0.17264(229)$) \\
...
seed closure. The old ratio heuristic
$m_2/m_3\sim\pi\phiz$ (motivated by the carrier complement $1-\gamma=1/6$) is now $-2.44\sigma$ against
NuFIT~6.0 and is \emph{superseded as the comparator} by the parameter-free candidate relation
$\Delta m^2_{21}/\Delta m^2_{31}=|J_{\mathrm{PMNS}}|$ (\mO, \vref{v468}; below). The heavy scale $M_R\sim10^{15}$ GeV links the seesaw bridge to the
...
$\delta_{\mathrm{CP}}=240^\circ$ is the $\mu_6$/triality phase --- now a Galois-\emph{forced} relation to the
\emph{leading} ($\pi/3$) component of the quark phase, $\delta_{\mathrm{PMNS}}=\delta_{\mathrm{CKM}}^{\mathrm{lead}}+\pi$
(one hexagonal unit $\rho=\zeta_6$; the lock is to the structural $\pi/3$, \emph{not} the full measured
$\gamma=\delta_{\mathrm{CKM}}^{\mathrm{lead}}+3\lambda_C^2\approx68.7^\circ$ that carries the quark transport
correction; \vref{v231}/\vref{v233}/\vref{v320}), not from $M_\nu$, and a pre-registered kill test at
DUNE/Hyper-K (\mX),
...
\par\smallskip\noindent\textbf{PMNS closed modulo the $\tau{=}\omega$ keystone (\veri{v406\_pmns\_phase\_origin.py}).}
The CP-phase \emph{origin} this leaves open is supplied by the dual keystone \texttt{SEAM.EQUIV.02} (\vref{v405}):
$\delta_{\mathrm{PMNS}}=4\pi/3$ is the hexagonal CM phase at the family CM point $\tau{=}\omega$ (where the
$(E_8)_1$ character $\chi_{E_8}$ vanishes), so the phase is a keystone consequence, not just an assembled
$\mu_6$ input. With the angles fixed (\vref{v9}/\vref{v268}), the derived $J_{\mathrm{PMNS}}=-0.0297$
(\vref{v270}) and the absolute scale reduced to one seesaw ratio $=v_{\mathrm{geo}}$-class unit (\vref{v272}),
the PMNS dynamics carry \emph{zero free flavor dials beyond $v_{\mathrm{geo}}$}; the residual is the seesaw
operator realisation ($M_R$, \vref{v184}) and the $\Sigma m_\nu$ ordering kill test --- under the carrier
normalisation $y_\nu{=}y_t$ that residual is a \emph{one}-parameter, PS-window-bracketed candidate
(\vref{v481}), no longer a two-dial trade-off. \mE/\mC
...
PMNS items.
\par\smallskip\noindent\textbf{The honest pressure point: $\theta_{13}$ at $+2.0\sigma$ (\veri{v328\_theta13\_pressure.py}).}
This reactor channel is TFPT's most-tensioned core prediction, and it is named, not hidden: $\sin^2\theta_{13}=\phiz
e^{-5/6}=0.023108$ sits $+2.0\sigma$ \emph{above} the NuFIT~6.0 normal-ordering fit ($0.02195\pm0.00058$). \mC\ it is
not relieved cheaply: the back-solved seed $\phiz(\theta_{13})=e^{5/6}\sin^2\theta_{13}^{\mathrm{exp}}=0.0505$ is
${\sim}5\%$ below the axiom, while PMNS $\theta_{13}$ RG running ($<1\%$, NO) and the \emph{exact} exponent $5/6$
leave no freeze-respecting sub-leading correction --- so the prediction stays frozen (\texttt{REG.FREEZE.01}).
\mX\ pre-registered kill: a stable $\sin^2\theta_{13}$ robustly away from $0.023108$ at $>3\sigma$ (JUNO / a future
global fit) flags the carrier-trace / seed-grammar reading. It is exactly the single ${\sim}2\sigma$ outlier of the
seed-hyperplane cross-validation (\vref{v306}) --- one honest pressure point among the seed observables, not a
scatter of unrelated failures; across the \emph{mixing} channels, however, it is not alone --- $\theta_{23}$ and
$|V_{cb}|$ sit at the same ${\sim}1.8\sigma$, and the three form one named candidate pattern (\vref{v467}, below).
\par\smallskip\noindent\textbf{The sub-leading budget, fenced (\veri{v338\_theta13\_budget.py}).} Quantified against
\emph{both} NuFIT~6.0 normal-ordering variants, the tension is data-systematics dependent: $+2.0\sigma$ vs the
no-SK central ($0.02195$) but $+1.7\sigma$ vs the with-SK best fit ($0.02215\pm0.00057$) --- a ${\sim}0.3\sigma$
spread, so not a clean $2\sigma$. \mE\ crucially $0.023108$ lies \emph{inside} the with-SK $3\sigma$ band (upper
bound $0.02388$): the frozen value is an upper-edge pressure point, \emph{not} a falsification. Closing the
${\sim}4\%$ gap to the central would need a ${\sim}3\%$ sub-leading channel (beyond the $<1\%$ RG and the exact
$5/6$), larger than natural --- so it stays a genuine, mild tension. \mX\ JUNO's sub-1\% precision
($\sigma\!\sim\!1.5{\times}10^{-4}$) turns today's $1.7$--$2.0\sigma$ into a ${\sim}6\sigma$ kill-or-confirm.
\par\smallskip\noindent\textbf{Post-hoc candidate \mO: the third-gen pattern.}
\veri{v467\_thirdgen\_mixing\_pattern.py}\
The suite's three ${\sim}2\sigma$ mixing tensions --- $\sin^2\theta_{13}$ ($+2.0\sigma$), $\sin^2\theta_{23}$
($+1.76\sigma$) and $|V_{cb}|$ ($+1.79\sigma$) --- are exactly the three \emph{third-generation} channels, and all
three sit low by one common relative amount: $-5.36\%\pm1.67\%$ ($3.2\sigma$ non-zero, $-0.03\sigma$ from $-\phiz$).
In the existing vocabulary the pattern is parameter-free: the seed enters third-generation mixing through the
\emph{ladder base} $\lamC^2=\phiz(1-\phiz)$ (the Yukawa mass-ladder base $\lambda_Y$) instead of bare $\phiz$, and
the $2$--$3$ texture carries the same $-\phiz/2$ shift as $\theta_{12}$: $\sin^2\theta_{13}=\lamC^2 e^{-5/6}=0.021880$
($-0.12\sigma$), $\sin^2\theta_{23}=(1-\phiz)/2\Leftrightarrow\cos2\theta_{23}=\phiz$ ($+0.20\sigma$; this
\emph{candidate} would select the lower octant --- the frozen record does not), and $|V_{cb}|=\lamC^2/(1+\lamC)=0.041119$
($+0.01\sigma$); the joint $\chi^2$ over the four third-generation observables drops $10.4\to0.11$ with \emph{zero}
new parameters, while $|V_{ub}|=\lamC^3/3$ (no bare $\phiz$) and the $1$--$2$ channels stay untouched and correct
(dressing $\lamC$ itself would fail at $-24\sigma$ --- the scope is checked, not chosen). \mC\ honestly: the pattern
is \emph{post-hoc} (5 of 16 equally-simple dressings fix the two PMNS tensions; $(1-\phiz)$, $(1-\cthree)$ and the
resolvent $1/(1+\phiz)$ are degenerate at current precision), the suppression size $\phiz=5.3\%$ sits inside the
seam-gapped correction band $(2/3)^6=8.8\%$ (\vref{v393}) but \emph{no seam-transport derivation exists} --- so the
frozen record (\texttt{REG.FREEZE.01}) is unchanged and the row is a named \mO\ candidate
(\texttt{FLAV.THIRDGEN.PATTERN.01}). \mX\ pre-registered, symmetric: $\sin^2\theta_{13}$ stable at $0.0231$ kills the
pattern; stable at ${\sim}0.0219$ with sub-1\% precision kills the record's bare-$\phiz$ reading; an exclusive-$|V_{cb}|$
resolution at ${\sim}0.0395$ kills both --- JUNO-era reactors and Belle~II decide (Daya~Bay alone already prefers the
pattern form, $\Delta\chi^2=4.3$).
\par\smallskip\noindent\textbf{The full PMNS matrix and the CP strength (\veri{v270\_pmns\_jarlskog\_assembly.py}).}
The four channels --- $\sin^2\theta_{12}=\tfrac13-\tfrac{\phiz}2$ (\vref{v9}/\vref{v263}),
$\sin^2\theta_{23}=\tfrac12$ (\vref{v9}/\vref{v263}), $\sin^2\theta_{13}=\phiz e^{-5/6}$ (\vref{v16}/\vref{v268})
and $\delta=4\pi/3$ (\vref{v231}/\vref{v233}) --- assemble into \emph{one} exactly-unitary complex matrix
$U=R_{23}\,U_{13}(\delta)\,R_{12}$. \mE\ the assembly is unitary; \mC\ the leptonic Jarlskog
$J_{\mathrm{PMNS}}=\Im(U_{e1}U_{\mu2}U_{e2}^{*}U_{\mu1}^{*})$, i.e.\
$s_{12}c_{12}s_{23}c_{23}s_{13}c_{13}^2\sin\delta=-0.02965$ (maximal amplitude $J_{\max}=0.03424$), is then a
\emph{derived} CP \emph{strength} --- the lepton-sector analogue of the CKM $J$ (\vref{v88}). \mC\ data: $J_{\max}$ vs NuFIT\,5.2 $0.0332$ ($\sim3\%$),
$J_{\mathrm{PMNS}}$ vs the best fit $\approx-0.026$ ($\delta_{\mathrm{data}}\approx232^\circ$, TFPT $240^\circ$
within $\sim1\sigma$), with the negative CP-violating sign reproduced. \mO\ this closes the CP-\emph{strength}
readout but not the phase's operator origin ($\delta$ stays the $\mu_6$/triality phase, an assembled input,
not an $M_\nu/D_F$ output) nor the absolute scale.
\par\smallskip\noindent\textbf{Post-hoc candidate \mO: splitting ratio $=$ Jarlskog.}
\veri{v468\_dm2\_ratio\_jarlskog.py}\
The dimensionless splitting ratio $\Delta m^2_{21}/\Delta m^2_{31}$ was previously \emph{unpredicted} (the table
heuristic $m_2/m_3\sim\pi\phiz=0.16704$ is now $-2.44\sigma$ against NuFIT~6.0 and is superseded as the comparator).
The frozen matrix above has exactly one canonical derived CP invariant, and numerically
$|J_{\mathrm{PMNS}}|=0.029653$ meets the measured ratio $0.029805\pm0.000792$ at $-0.19\sigma$ (equivalently
$m_2/m_3=\sqrt{|J|}=0.17220$ vs $0.17264(229)$) --- the only tested closed form inside the $1\sigma$ band, with
\emph{zero} dials (angles and phase are the frozen ones). \mC\ consistency: with $r=|J|$ the normal-ordering floor
$\Sigma m_\nu=m_3(1+\sqrt{|J|})=0.0588$ eV reproduces the documented $0.0586$ eV (\vref{v272}); the absolute scale
stays the one seesaw ratio --- no absolute number is added. \mO\ honestly: post-hoc, and \emph{no mechanism} --- the
$\mu\tau$ texture gives $\theta_{13}=0$ at leading order and cannot carry this relation, so the row stays a named
\mO\ candidate (\texttt{FLAV.DM2RATIO.01}) until an $M_\nu$-level derivation exists. \mX\ JUNO shrinks the
$r$-window ${\sim}4\times$ (a stable offset $>3\sigma$ kills it); adopting the \vref{v467} pattern would shift
$|J|\to0.028849$ ($-1.21\sigma$ today), so the two candidates \emph{cross-discriminate} at JUNO precision.
\par\smallskip\noindent\textbf{The absolute $\nu$-mass scale is one seesaw ratio (\veri{v272\_nu\_mass\_scale.py}).}
The one PMNS residual left is the overall scale, and it is typed honestly --- not fabricated. The type-I seesaw
makes the absolute scale a \emph{single} ratio, $m_3=(y_\nu v_{\mathrm{EW}})^2/M_R$ (\mE), exactly the same kind of
one irreducible input as $v_{\mathrm{geo}}$ (the No-Unit theorem, \vref{v153}); the dimensionless angles and ordering
are pinned, and the ratio $m_2/m_3$ now carries the \mO\ candidate comparator $\sqrt{|J|}$ (\vref{v468}). \mC\ the observed $m_3=\sqrt{\Delta m^2_{\mathrm{atm}}}
=0.050$ eV together with the TFPT PS$\to$GUT scale window $M_R\in[M_{\mathrm{PS}}{=}4.2{\times}10^{13},
M_{\mathrm{GUT}}{=}2.4{\times}10^{15}]$ GeV (\vref{v249}) requires a third-generation Dirac Yukawa
$y_\nu\in[0.26,2.0]$ --- $O(1)$, no tuning, so the observed scale is consistent with the TFPT seesaw scale.
\mO\ the $y_\nu{=}1$ value $M_R=v_{\mathrm{EW}}^2/m_3=6.1{\times}10^{14}$ GeV is \emph{not} a clean $\Mpl$ power
($\log_{c_3}(M_R/\Mpl)=2.57$), so the absolute $\Sigma m_\nu$ stays one open UV input (\vref{v184}). \mX\ the
normal-ordering floor $\Sigma m_\nu=0.0586$ eV is the near-term cosmological kill test (\texttt{FLAV.NUSCALE.01}).
\par\smallskip\noindent\textbf{The carrier normalisation: one parameter
(\veri{v481\_seesaw\_carrier\_ladder.py}).} The $y_\nu{=}1$ probe above is \emph{not} the carrier normalisation:
the carrier structure itself (one $SO(10)$ $\mathbf{16}$ per family, the PS algebra \vref{v245}/\vref{v248})
with the \emph{minimal} Yukawa sector ($\mathbf{10}_H$ / PS $(1,2,2)$) forces $y_\nu=y_t$ at the matching scale
--- a \emph{named} premise (conjectured, ledger-typed), not a proof. With that normalisation and explicit $1$-loop RG
(gauge/$y_t$/$\lambda$ up; the ADKLR Weinberg-operator running down) the $(y_\nu,M_R)$ trade-off collapses to
$M_R$ \emph{alone}: the observed $m_3{=}0.0503$ eV demands $M_R=9.3{\times}10^{13}$ GeV --- \emph{inside} the
compiler's own PS window $[4.2{\times}10^{13},2.4{\times}10^{15}]$ (\vref{v249}), at
$\log_{c_3}(\Mpl/M_R)=3.15$ (versus the structureless $2.58$ of the $y_\nu{=}1$ probe). \mX\ honesty gate: the
integer rung $M_R=c_3^3\Mpl=1.53{\times}10^{14}$ GeV predicts $m_3=0.030$ eV --- $40\%$ low at $1$-loop
($\kappa$ run-down factor $0.794$) --- so the ladder pin is \emph{declined} per the anti-numerology rule
(\vref{v354}/\vref{v355}). The named decision computation is meanwhile \emph{executed} in bracketed,
assumption-robust form (\veri{v482\_seesaw\_rung\_decision.py}): the rung needs a rescue factor
$\times1.670$ in $m_3$, while $m_t\pm3$ GeV ($>3\sigma$), $\alpha_s$ at $>3\sigma$, a $\pm10\%$
$\kappa$-run band and a PS-leg beta envelope $\times[0.5,1.5]$ move it by at most $\times1.165$
combined --- the \emph{unstructured} rung is excluded (the $1$-loop miss is not an RG artifact); the
only escape is a third-generation Majorana structure factor $r\approx1.67$, whose $0.18\%$ proximity to
$\gcar/\Nfam=5/3$ is recorded post-hoc and \emph{declined} (no forcing mechanism --- the $t_0{\sim}30$
discipline of \vref{v478}). What survives regardless: over the PS window the carrier-normalised seesaw spans
$m_3\in[0.002,0.115]$ eV, bracketing the observation, and DESI cuts the window from below --- a
\emph{one}-parameter, window-bracketed \mO\ candidate (\texttt{FLAV.NUSCALE.02}, genre
\vref{v467}/\vref{v468}); the frozen record and the \texttt{FLAV.NUSCALE.01} kill test are untouched.
\par\smallskip\noindent\textbf{The $3/5$ Clebsch door, enumerated and closed
(\veri{v488\_majorana\_clebsch\_door.py}).} The one named escape is now \emph{decided}. The mechanism search
enumerates the minimal Pati--Salam/$SO(10)$ Majorana channels: since the $\overline{\mathbf{126}}$ is not in the
$E_8$ hull $\{1,10,16,45\}$ (\vref{v247}), $M_R$ can only come from the $d{=}5$ operator
$(\mathbf{16}_F\overline{\mathbf{16}}_H)^2/\Lambda$, whose insertion channels are the singlet and the
$\mathbf{45}$ ($\mathbf{16}\times\overline{\mathbf{16}}=\mathbf{1}+\mathbf{45}+\mathbf{210}$, the $\mathbf{210}$
absent from the hull). \mE\ Every available $\nu^c$ channel weight --- singlet $1$, $T_{3R}^2=\tfrac14$,
$(B{-}L)^2=1$, $T_{3R}(B{-}L)=-\tfrac12$, the $SU(4)$-adjoint weight $\tfrac38$ --- is a $\{2,3\}$-smooth
rational, and products of $\{2,3\}$-smooth rationals can never produce $\tfrac53$: a factor $5$ cannot appear.
The \emph{unique} natural $5$ of the embedding, $k_Y=\operatorname{Tr}_{16}Y^2/\operatorname{Tr}_{16}T_{3L}^2
=\tfrac53$ (\vref{v470}), is a full-multiplet trace whose carrier direction has $Y(\nu^c)=0$ \emph{exactly} ---
the only $5$-carrying group number is structurally decoupled from the Majorana operator. \mE\ And Clebsch factors
are generation-blind (family-space scalars on identical $\mathbf{16}$'s), so the required
$\operatorname{diag}(1,1,\tfrac35)$ texture cannot arise from group theory by construction. \mO\ The frozen
address tables contain exact $\tfrac35$'s only in \emph{wrong} slots ($K$ lepton row $(5,3,2)$: gen2/gen1; $R$
and $L$ lepton rows: gen3/gen2 --- never gen3 against a degenerate base), typed and declined
(\vref{v354}/\vref{v355}). Net: the rung${}+{}5/3$ rescue is \emph{dead} as a mechanism candidate --- a clean
negative --- and the one-parameter window candidate above stands as the honest endpoint
(\texttt{FLAV.NUSCALE.04}).
...
5 & Hodge gravity, metrology & $\xi=c_3/\phiz$, $v_{\mathrm{phys}}$; local field equation parameter-free (\vref{v358}/\vref{v359}); perturbative graviton unitarity established (\vref{v370}/\vref{v380}); ambient measure a \mC\ redundancy (\vref{v369}) \mE/\mC \\
...
\subsection*{(2) Gravity --- the local field equation is parameter-free; the $R^2$ inflation layer up to one truncation \mE/\mC}
The metric-sector \emph{field equation} itself is parameter-free: fixed-\emph{volume} entanglement
...
expansion). The \emph{perturbative} 4D readout built on this sector now closes \emph{to all orders} as a typed
...
stay \mO\ (the seam being \emph{closed modulo a cited theorem}, \vref{v336}), and the ambient metric/projective QG
measure, now a \mC\ redundancy (\vref{v369}; G6 projective limit, \texttt{tfpt\_research\_contracts}).
So: \emph{admissible gapped transfer sector solved under RP/gap hypotheses; the seam closed modulo a cited theorem;
...
$\mathrm{AQFT}_{\mathrm{bdry}}$ is the holomorphic $c=8$ $(E_8)_1$ boundary net (now \emph{closed modulo a cited
theorem} via the lattice/$S_3$ stack and \texttt{FORM.SEAM.MMST.01}, \mC); and $\mathrm{QG}_{\mathrm{amb}}$ is the
...
a controlled truncation. What remains are the cited continuum-existence step (the seam closed modulo a cited
theorem; the ambient measure a \mC\ redundancy, \vref{v369}) and the two declared inputs P1/P2 --- a
...
The CMB $\mu$-distortion would decide the slow-vs-fast branch \emph{within} the band: the
Chluba-window readout gives $\mu=1.6\times10^{-8}$ at the reheating point $51.4$ versus
$2.0\times10^{-8}$ at the $A_s$-profiled $56.1$ (band $1.5$--$2.3\times10^{-8}$), a $3\sigma$
separation at $\sigma(\mu)=10^{-9}$ (Voyage-2050 class; the PIXIE baseline $9\times10^{-8}$ does
not reach the band) \mC\ \veri{v494\_cosmo\_running\_mu\_record.py}.
\end{keybox}

% =====================================================================
\section{The ambient closure: one spectral-action measure, IR-complete by decoupling}
% =====================================================================
The two formerly-hardest items --- the \emph{ambient} (unprojected) QFT measure beyond the selector
...
\emph{boundary relative object}; the \emph{real $4$D interacting} ambient measure $\mathrm{QG}_{\mathrm{amb}}$ is
now a \mC\ redundancy (\vref{v369}, conditional on \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann), with only the
cited continuum-existence theorems staying \mO\ (\vref{v336}).
...
conditional reduction:} the \emph{full metric-sector quantum-gravity measure is not constructed here}, but it is
now a \mC\ \emph{redundancy} (\vref{v369}, conditional on \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann), a
certification object rather than missing dynamics, as carried in the frontier document
(\texttt{tfpt\_4\_frontier}), which is the single status source for QG. The admissible-sector measure
(gap $6\log\tfrac32$, OS) is genuinely closed; perturbative graviton unitarity is established (the Stelle ghost a
truncation artefact, \vref{v304}/\vref{v370}/\vref{v380}).\end{gapbox}

\begin{keybox}[The $R+R^2$ action-layer equation --- the full Einstein equation is separately parameter-free \mE/\mC]
This box gives the \emph{action-layer} ($f(R)=R+R^2$) field equation; independently of it, the \emph{full}
covariant Einstein equation $G_{ab}{+}\Lambda g_{ab}=\cthree^{-1}T_{ab}$ is parameter-free (fixed-volume
entanglement equilibrium, both coefficients fixed, \vref{v358}/\vref{v359}). The low-curvature regime is
fully closed at the level of the covariant Einstein-side equation. With
\[
...
regime; the field equation itself is parameter-free (\vref{v358}/\vref{v359}), so the deepest open item \mO\
is the \emph{measure} --- the \emph{full} metric-sector path-integral measure beyond FRW
...
$\theta_{12}^{\rm nonlin}$ are \emph{scheme diagnostics} --- the seam-corrected and full non-linear
\emph{observation} maps of the same texture (different truncation/linearisation), \emph{not} alternative
predictions and \emph{not} rescue branches. The three are one \emph{renormalisation orbit} (source seed
$\to$ seam map $\to$ non-linear observation map): higher precision tests \emph{which point of the orbit}
the data prefer, not which of three competing predictions to keep. This matters for JUNO, which will
resolve this band sharply, so TFPT commits to one number in advance --- consistent with current global
fits, not a post-hoc choice.
...
becomes unconditional only once the seam-misalignment lemma is proved. \emph{Legacy note:} earlier
versions of the theory carried $\theta_{12}$ as \emph{open}; in this compiler version it is \emph{conditional}
...
\]
so $E=\mathcal O(-2)\oplus\mathcal O(-1)^2$ and $a=(1,1,2)$ --- machine-checked with its negative
controls (positivity/sum/part-count/integrality each load-bearing; $\gcar=e_2=5$ a corollary) in
\veri{v491\_p2\_partition\_corollary.py}, sharpening \vref{v53}; the typing behind (a) --- integrality,
positivity $=h^0{=}0$, sum $=-\deg E$ --- is itself derived as Deligne $+$ Birkhoff--Grothendieck $+$
Mehta--Seshadri stability in \veri{v499\_p2\_weights\_deligne\_bg.py} (\texttt{P2.TYPING.01}).
...
Starobinsky $M$ / $A_s$ & \mE/\mC & $M{=}\cthree^{7/2}\Mbar{=}3.06{\times}10^{13}$ GeV exact \mE; $A_s$ a \mC\ band over $N_\star\in[50,60]$ \\
...
\begin{gapbox}[Deliberately not forced --- typed transfer targets, never compiler powers]
$\eta_B$, $m_p/m_e$, exact Koide (only near), and the dark-matter \emph{relic scale} ($f_a,m_a$; the axion
candidate itself is fixed) are deliberately \emph{not} pressed into the compiler: they are typed \mC\ transfer
targets, now runnable solvers with kill tests (\vref{v371}--\vref{v375}). Full quantum gravity beyond the $R^2$
scalaron is likewise sharpened --- perturbative graviton unitarity is established (the Stelle ghost a truncation
artefact, \vref{v304}/\vref{v370}/\vref{v380}) and the ambient measure is a \mC\ redundancy (\vref{v369}).
\end{gapbox}
...
$\Lambda$ ambiguity is \emph{typed} away to a single branch label. The typed transfer targets and the QG items
($\eta_B$, $m_p/m_e$, exact Koide, dark matter, full quantum gravity --- now runnable \mC\ solvers and, for QG,
perturbative unitarity $+$ a \mC\ ambient redundancy) are in \texttt{frontier\_items}.
