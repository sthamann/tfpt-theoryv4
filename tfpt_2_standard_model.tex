...
\newtcolorbox{keybox}[1][]{enhanced,breakable,colback=tfblue!5!white,colframe=tfblue,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{winbox}[1][]{enhanced,breakable,colback=tfgreen!6!white,colframe=tfgreen!75!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{gapbox}[1][]{enhanced,breakable,colback=tfred!5!white,colframe=tfred!80!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{auditbox}[1][]{enhanced,breakable,colback=tfgold!8!white,colframe=tfgold!80!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{audbox}[1][]{enhanced,breakable,colback=tfgold!8!white,colframe=tfgold!80!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{resbox}[1][]{enhanced,breakable,colback=tfgreen!5!white,colframe=tfgreen!55!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
\newtcolorbox{roadbox}[1][]{enhanced,breakable,colback=tfgreen!5!white,colframe=tfgreen!55!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
...
\input{tfpt_docset}
\noindent\textbf{You are reading document \#3.}
...
Pl\"ucker readouts, v49/v71), and only the \emph{absolute} quark amplitude scale is a finite
$(U_{\mathrm{wall}})$ anchor.
\end{keybox}

\noindent\textbf{Status markers (refined):} \tI\ exact identity, \tL\ Lie/lattice theorem, \tF\ formalised,
\tN\ numerical fixed point, \tP\ physical/conditional (named hypotheses), \tA\ axiom/open.
...
h(E_8)$). \tI\ \veri{v4\_flavor\_matrix.py}
\end{winbox}
...
\begin{keybox}[The winding-deformation theorem: $R\to L$ in one parameter \tI]
...
$|\mu_4|=4\to\mathcal A_L=10$. \tI
\end{itemize}
\end{winbox}

\begin{auditbox}[Audit-level only: flavor linear-algebra fingerprints (not load-bearing)]
The following are \emph{audit fingerprints} (exact linear algebra of $R,L$), kept here, not in any
derivation, under the no-free-pattern rule. \tI
\begin{itemize}\setlength\itemsep{2pt}
...
\begin{keybox}[Where the lepton rationals come from: $\delta=\tfrac12$ resolvent $+$ product rule \tI]
...
the glue index (the same pair whose \emph{sum} is the $E_8$ root norm $2$, Paper~1), matching the
...
constants stop being separate inputs \tI: $\operatorname{spec}(\widetilde M_0)\cup\operatorname{spec}(-\widetilde M_0)=\mu_6$
...
resolvent $D_y^{-1}$ at the \emph{same} pole $\delta_{\mathrm{ph}}$ as the leptons. For every fermion the
...
carrier-rational rungs --- quarks \emph{and} leptons in \emph{one} family. \tI\ (leptons exact) / \tP\
(quark rationals from the resolvent)
\end{winbox}

\begin{keybox}[Quark \emph{ratio} closure: the invariant readouts of the character sector \tI/\tN]
...
is \emph{generated} as the $L^1$ norm of the anchor-plane minors of $K$ (v42), and by Readout Rigidity (v49,
with the selector stratum now \emph{derived} --- $\det R=8$ lattice and $\operatorname{Spec}(Q_+)=\{1,2,3\}$
from $D_4$, v69/v71) the \emph{ratio} $c_u/c_d=\tfrac{55}{117}$ is \emph{constant} on that stratum, i.e.\ an
...
\begin{keybox}[Anchor-plane Plücker localisation of the ``$11$'' (and $26$) \tI]
...
\textbf{What this does and does not buy.} The Plücker values are exact identities \tI, and the ``$11$'' now
...
\begin{keybox}[Readout Rigidity --- the quark \emph{ratios} are closed combinatorially \tI\ (residual: absolute scale \tA)]
...
\]
is now \emph{fully derived}: $\det R=8=\operatorname{rank}E_8$ and the SNF are the lattice invariant (v70,
$\det Q=3=\Nfam$), and $\operatorname{Spec}(Q_+)=\{1,2,3\}$ is the $D_4$-equivariant result (v69). By
...
\begin{equation}
  \boxed{\ K=R+Q\,\Sigma,\qquad L=R+Q\,(I+\Sigma)\ }\qquad\tI\ \veri{v10\_projection\_involution.py} ,
\end{equation}
...
\begin{winbox}[The Sheet Diamond: all four operators are points of \emph{one} surface \tI\ (\veri{v94\_sheet\_diamond.py})]
...
determinant factorises as $(3x{+}2)(3x{+}5)$ --- the master cover of the operator-pencil sections below. The
...
\[
  \boxed{\ Q=U+V,\qquad R=C{-}U,\quad L=C{+}U,\quad K=C{-}V,\quad F=C{+}V\ }\qquad\tI ,
\]
...
(cf.\ $Q_+=3\,\mathrm{cusp}+1$ with $\{1,2,3\}$, v69/v72). The center is tightly typed:
...
and all integers are exact \tI; the ``$G_2$ center'' reading ($14=\dim G_2$, also the constant term of
...
$\propto$ Nariai), $n$ the \emph{torsion} normal (first-generation channel). \tI; bridge reading \tP.
\smallskip\\
...
root count ($40{+}12$, \veri{v128\_graded\_hull.py}). All \tI; the ``2d surface with atom walls'' reading \tP.
\end{winbox}

\begin{keybox}[The $Q_+$ grading is cohomology: one spectrum in four readouts \tI\ (\veri{v137\_qplus\_cohomology.py})]
...
$\operatorname{Spec}(Q_+)$ ($Q_+=3\,\mathrm{cusp}+1$, v69/v72), and the cusp weights $\{0,\tfrac13,
\tfrac23\}$ underneath are $\operatorname{spec}(A_0^\star)$ (v115):
\[
  \boxed{\ \text{MS weights}\ =\ \text{cusp classes}\ =\ H^1\text{ characters}\ =\
  \operatorname{Spec}(Q_+)-1\ \text{over } \Nfam\ }
\]
--- one spectrum in four readouts; and $(1,2,3)$ are the \emph{exponents of $A_3$} ($W(A_3)=S_4$, v117;
$h(A_3)=4=|\mu_4|$): the $Q_+$ generation grading \emph{is} the $A_3$ exponent grading, realised on the
seam curve's cohomology. \emph{Typing:} all spectrum statements \tI; what remains of \texttt{GATE.QGEO}
...
the $H^1$ set, by the same sign twist that builds the v118 hexagon. Both sides are multiplicity-free, so
...
character algebra, rigidity and grading transport \tI; the residue of \texttt{GATE.QGEO} is the \emph{one}
discrete choice of which sign-twisted action is the geometric boundary deck \tP.
\end{keybox}

\begin{winbox}[Spectra and the determinant ladder $\to|W(D_5)|$ \tI]
...
\begin{gathered}
  ...
  \boxed{\ \det Q\cdot\det K\cdot\det R\cdot\det L=1920=|W(D_5)|\ }\qquad\tI ,
\end{gathered}
\end{equation}
...
\begin{winbox}[Mass and transport are one pencil: $\det(K+xQ)$ carries the scalaron sequence \tI]
...
$\dim G_2=14$ (audit-level \tA). \veri{v37\_plucker\_anchor.py}
\end{winbox}

\begin{keybox}[Operator-pencil geometry: the anchor plane as a small area in operator space \tI/\tL\ (\veri{v80\_operator\_pencil\_geometry.py})]
...
\[
  \boxed{\ \det B_{K+xQ}=9x^2+21x+10=(\Nfam x+|\Z_2|)(\Nfam x+\gcar)=(3x+2)(3x+5)\ }\qquad\tI ,
\]
...
\smallskip\\
\noindent\emph{Audit layer (exact values \tI, decorative reading \tP).} $D(x)=\det(K{+}xQ)$ and its derivatives
...
(\tP), since the coefficients are themselves atoms.
\end{keybox}

\begin{keybox}[The Koide point is one branch of a double cover \tI/\tL\ (\veri{v81\_singular\_pencil\_matrices.py})]
...
are exact; their per-entry atlas reading stays audit-level \tP.) \emph{Gravitational twin:} the same split
...
\begin{keybox}[The Koide branch is a \emph{forced} attractor, and the clean split is non-generic \tI/\tP\ (\veri{v82\_koide\_attractor\_splitting.py})]
...
all follow. That single identification stays \tP; the rest is \tI. \textbf{(B) The clean rational split is
...
identification of the three placements is not claimed and stays \tP.
\end{keybox}

\begin{keybox}[Branch kernels select the sectors; one deck involution \tI/\tL\ (\veri{v96\_branch\_kernel\_selection.py}, \veri{v97\_sheet\_conjugation\_bridge.py})]
...
\tL:} rank one forces $P(x_0)\,w\perp\operatorname{span}\{\mathbf1,a\}$, hence proportional to the
...
with $\Sigma=T_AG$. \emph{Honest scope:} the sheet question therefore carries \textbf{no separate \tP\
...
\begin{winbox}[Grand Mass Volume: the absolute mass-scaling normalisation \tI]
...
\tI\ determinant identity (each sector $=$ its $K$-row sum); the only genuine scale input is the overall
...
\begin{keybox}[Theorem Q --- the $\Sigma$-split as a parabolic projection (sharper uniqueness) \tI/\tP]
...
\tP$\to$\tL.} The four punctures $\mu_4=\{1,i,-1,-i\}$ are the vertices of a square with symmetry
...
$\Sigma$-split are now \emph{derived} from the $D_4$-equivariant geometry \tL; the residual is only that the
cusp weights $\{0,\tfrac13,\tfrac23\}$ are the established monodromy exponents (v61) and the explicit integer
...
from the geometry, not posited (\tI\ chain; the coker$=$deck-group identification is the standard non-abelian
Hodge reading \tL).
\end{keybox}

\begin{gapbox}[Status: $Q$-geometry derived (spectra, $\Sigma$-split \emph{and} integer lift) \tL]
What is \emph{proved} (\tI): the algebraic identities $K=R+Q\Sigma$, $L=R+Q(I+\Sigma)$, the spectra, the
...
(coker$\,Q=\Z/\Nfam=$ triality deck group). So the $Q,\Sigma$ algebra is now an \tI/\tL\ layer whose spectra,
...
\begin{keybox}[Gate 1 (closed): the EM budget $M{=}41$ is the $U(1)$ index, not a fit \tI]
...
\begin{keybox}[Gate 2 (forced): $K$ and $Q$ are not read off the masses \tI]
...
\begin{keybox}[Gate 3 (sharpened): the geometric origin of $Q$ reduces to two named pieces \tP]
...
pivot $N_\star$ stays a \emph{reheating input} (\tP), not a compiler output.
\end{keybox}

\begin{keybox}[Gate 4 (new): the solar angle from a shared dual anchor \tI/\tP]
...
\[
  \boxed{\ a^\top R^{-1}=a^\top L^{-1}=(-\tfrac12,-\tfrac12,1)\ }\qquad\tI .
\]
...
(\tP). \veri{v16\_solar\_dual\_anchor.py}\,\veri{v21\_solar\_product\_quark.py}
\end{keybox}

\begin{keybox}[Inverse Anchor Normalisation Theorem \tI/\tL\ (\veri{v79\_review\_identities.py})]
...
pins the self-duality to precisely the residue/mass/transport triple, not the pure $\Sigma$-difference $Q$. \tI/\tL
\end{keybox}
...
$\delta_{\mathrm{CKM}}$ & $\tfrac{\pi}{3}+3\lamC^2$ (leading, \tP) & $1.198$ rad & $\sim1.14$ \\
...
vs $65.7^\circ$) are expected and the CP sector is typed \tN/\tP, \emph{not} \tI\ --- only $\beta$ happens to be
near-exact. (To upgrade to \tI\ the higher-order $\delta$ correction $\Delta_\triangle$ must be given
explicitly.)
\end{winbox}
...
$\sin^2\theta_{12}^{\rm seed}=\tfrac13-\tfrac{\phiz}{2}=0.306747$ ($\varepsilon=q(A_3)\phiz=\tfrac34\phiz$,
conditional on the seam-misalignment lemma); the seam-leading $0.306808$ and the full non-linear $0.307020$
are \emph{derived variants} of the same texture, \emph{not} alternative predictions (named box below). The
...
inflation/reheating scale (Paper 6).

% =====================================================================
\section{How each of the seven papers simplifies}
% =====================================================================
\begin{center}\small
\renewcommand{\arraystretch}{1.25}
\begin{tabularx}{\linewidth}{@{}p{0.7cm}p{3.5cm}Y@{}}
\toprule
\textbf{P} & \textbf{before} & \textbf{in the compiler picture} \\
\midrule
1 & boundary $\to c_3=\tfrac1{8\pi}$ & axiom (P1), unchanged; hardenable via Calder\'on/Lean \tA \\
2 & carrier, $16$, $\Nfam{=}3$, $\Oadm{=}48$, $b_1$ & all \emph{one} Pascal row $1,5,10$ on $\gcar{=}5$; $16=\dim S^+$ is the $D_5$ half-spinor \tI \\
3 & EM fixed point $+$ flavor transport theorem & $\ainv$ as a unique root; flavor $=$ \emph{one} residue matrix $R$ ($\det 8$, minors $2{,}3{,}5$); masses $=$ master formula \tI/\tP \\
4 & admissibility, strong CP & selector $+$ $\theta_{\mathrm{eff}}{=}0$, conditional on QFT axioms \tP \\
5 & Hodge gravity, metrology & $\xi=c_3/\phiz$, $v_{\mathrm{phys}}$; gravity closure conditional \tP \\
6 & cosmology interfaces & \emph{one} scale engine $\ainv$: $v_{\mathrm{EW}}{\sim}e^{-\ainv/5}$, $\Lambda{\sim}e^{-2\ainv}$, $A_s{=}\tfrac{N_\star^2}{24\pi^2}c_3^7$ \tI/\tP \\
7 & CMB pipeline & programmatic; neutrino-sum lemma (conjecture) \tA \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey

\begin{winbox}[What the simplification means net]
The discrete numbers of Papers 2 and 3 ($16,40,41,48,240,248$ and the flavor matrix) are \emph{not}
...
\midrule
\textcolor{tfgreen!60!black}{\textbf{Closed \tI}} & gauge group $3{+}2$, $\Nfam{=}3$, hypercharge; $\alpha^{-1}{=}137.0359992$ (value \tN, ledger \texttt{EM.FP.01}); seed $\phiz$; CKM magnitudes $s_{12},s_{23},s_{13}$; $\sin^2\theta_{13}$; \emph{all} charged-lepton masses (in $\phiz$); word-lengths $L$; \textbf{charged-lepton $\Lambda$ exact}; $v_{\mathrm{geo}}/\Mpl$; geometric quartic $\lambda_\Phi{=}\tfrac1{16\pi^2}$ \\
\textcolor{tfblue!70!black}{\textbf{Reduced \tA}} & the quark $\Lambda$ (resolvent outputs in the $O(1)$ band): the \emph{ratios} $c_u/c_d=\tfrac{55}{117}$ etc.\ are \emph{closed} (integer Plücker, v49/v71); only the \emph{absolute} digits reduce to the $(U_{\mathrm{wall}})$ stable point --- an anchor \\
\textcolor{tfgold!80!black}{\textbf{Scheme layer \tP}} & dimensionful EW/QCD masses $m_W,m_Z,\mathbf{m_H}$, $\hat s_Z^2(M_Z)$, $\alpha_s(M_Z)$, absolute quark GeV masses --- all via the standard RG/threshold projection $\mathcal R_{\mathrm{SM}}$; $m_H$ rests on the \emph{closed} UV quartic $\tfrac1{16\pi^2}$ (near-criticality) \\
\textcolor{tfgold!75!black}{\textbf{Conditional \tN/\tP}} & the CKM phase $\delta_{\mathrm{CKM}}=\tfrac\pi3+3\lamC^2$ (leading-order readout, \tP\ --- consistent with the mixing table above); the solar angle $\sin^2\theta_{12}=\tfrac13-\tfrac{\phiz}{2}$ (cond.\ derived, pending the seam-misalignment lemma); the quark $c$-\emph{ratios} are closed (v49/v71), only the absolute $c$-normalisation is the $U_f^\star$ anchor \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey

\begin{keybox}[The disciplined main statement]
...
anchor --- the \emph{ratios} are closed, v49/v71), the
...
\begin{keybox}[Standard-Model Projection Functor $\mathcal R_{\mathrm{SM}}$ (Theorem D) \tP]
...
\]
\textbf{Source layer (closed, \tI):} $\hat y_f=\pi c_f(\phiz)^{k_f}$, $\hat m_f=\tfrac{\vgeo}{\sqrt2}\hat y_f$.
\textbf{Projection (standard QFT, \tP):} the SM RGEs run $g_i,y_f,\lambda$ to $M_Z$ and threshold matching
...
\textbf{Point:} TFPT derives the UV source data and the discrete skeleton (\tI); $\mathcal R_{\mathrm{SM}}$
derives the measurable poles/running by \emph{known} machinery (\tP). The scheme layer is therefore
deterministic but \emph{not} a new compiler claim --- declared as a functor, it is physics, not integer magic.
\end{keybox}

% =====================================================================
\section{Complete solutions of the five open questions}
% =====================================================================
For each question I give the \emph{most complete honest} solution: the proof chain isolating \emph{one}
sharply named remainder --- and, where possible, a \emph{reduction} that makes the problem smaller than
previously thought.

\subsection*{(1) Flavor residual --- ratios closed combinatorially; only the absolute scale is the wall-selection $(U_{\mathrm{wall}})$ \tI/\tA}
...
$D_4$-equivariant geometry and the lattice (v69/v70), and the ratios are rigid on it (v49/v71). The
...
\subsection*{(2) Gravity --- exact in the $R^2$ regime, one named truncation remainder \tP}
...
\item \textbf{Spectral action:} the Seeley--DeWitt coefficients give Einstein--Hilbert
      ...
      $M^2/\Mbar^2=6(4\pi)^2/f_0$ is fixed by the cutoff moment ($\cthree^7$). \tI\
...
\item \textbf{Hodge/BRST closure:} the geometric BRST/Hodge differential quotients diffeomorphisms $+$
      ...
      complex on the \emph{compactified} normal slice $S^2$ (APS boundary conditions, Paper 5) is
      Fredholm --- again standard index theory. \tI\ (given Fredholm)
\item \textbf{FRW reduction:} minisuperspace symmetry reduction onto the scale factor $+$ scalaron;
      yields the Starobinsky potential. Standard. \tI
...
\begin{winbox}[Sharpening (2): the truncation is explicitly bounded]
...
predictions
\[
  n_s=1-\frac{2}{N_\star}=0.964,\qquad r=\frac{12}{N_\star^2}=0.004
\]
hit Planck. The status is thus not ``conditional on an unproved Hodge theorem'' but ``exact up to an
$O(1/N_\star^2)$ truncation''. \textbf{Remaining path:} the Fredholm property of the seam elliptic
complex (APS, standard index theory). \tI\ in the regime.
\end{winbox}

\subsection*{(3) Strong CP --- decoupled from the mass-gap problem \tI/\tP}
...
\begin{tikzpicture}[font=\footnotesize,
  ...
  \node[box,draw=tfgold!80!black,fill=tfgold!12,very thick] (cp) at (7.6,0) {$\boxed{\theta_{\mathrm{eff}}=0}$\\\tI\ (given RP)};
  \draw[ar] (i)--(rp); \draw[ar] (ii)--(rp); \draw[ar] (iii)--(rp); \draw[ar] (rp)--(cp);
\end{tikzpicture}
\end{center}

\noindent The three facts are algebraic and proved on $\operatorname{Ran}(P_{\mathrm{adm}})$ (Paper 4,
``admissibility as a three-sector motif''). \textbf{Separately}, Paper 4 builds the \emph{full} OS
...
\begin{winbox}[Sharpening (3)]
Strong CP is \textbf{decoupled} from the mass-gap problem: $\theta_{\mathrm{eff}}=0$ is \tI\ given
reflection positivity (three structural facts). Only the \emph{full} QFT dynamics rests on mass-gap
stability --- the remaining constructive \tP. \textbf{Hardening path:} prove the parent-domination
lemma rigorously; all other OS axioms then follow.
\end{winbox}

\subsection*{(4) Full quantum dynamics --- solvable on the physical sector \tI/\tP}
Here is the complete solution. The input previously listed as ``open'' --- the parent-domination /
mass-gap lemma from (3) --- is \emph{not} analytically open: it is \emph{computed} by the
\textbf{explicit discrete spectrum} of the transport operator.
...
\item \emph{Explicit gap.} The finite cyclic transfer operator is gapped with $\Delta=6\log\tfrac32$ ---
      this is the ``parent gapped quasilocal'' object of parent domination, written out. \tI
\item \emph{Thermodynamic limit.} A \emph{uniform} gap of a finite-range transfer operator gives, by the
      standard cluster expansion, exponential clustering and stability in the limit. \tI\ (standard)
\item \emph{OS package.} With admissible reflection positivity (Paper 4 theorem) $+$ gap $+$ clustering
      all Osterwalder--Schrader axioms hold $\Rightarrow$ OS reconstruction: Hilbert space, Hamiltonian
      and a \emph{measure} on the admissible sector. \tI
...
\begin{winbox}[Sharpening (4): what is and is not solved \tI/\tP]
...
(4) is \tI\ on the admissible sector and the only standard step is gap-$\to$-limit stability (cluster
...
\subsection*{(5) Axioms P1/P2 --- P2 machine-verified, P1 as an interface \tI/\tA}
...
\begin{winbox}[Sharpening (5): build result this round (reproducible)]
\textbf{Provenance.} Canonical shipped folder \texttt{lean4-carrier-rigidity/} (source repo: \texttt{experiments/lean4-carrier-rigidity/}); toolchain
\texttt{leanprover/lean4:v4.29.1}; one command runs the hard gate:
\[
  \texttt{bash scripts/audit.sh}
\]
...
\texttt{sorry}-laundering. So ``\textbf{F}'' is reproducible, not a textual claim.
\end{winbox}
...
index-theoretic premises directly (substantially more work, beyond the current Lean scope). \tA
...
\midrule
1 & \tI\ (closed) & unitarity of $\nabla_F^\star$ (Paper-3 structure) \\
2 & \tI\ in the $R^2$ regime & controlled $R^{n\ge3}$ truncation $+$ Fredholm \\
3 & \tI\ (strong CP, given RP) & full dynamics: admissible gapped sector via (4) under RP/gap; ambient QG open \\
4 & \tI\ adm.\ sector & admissible-sector measure closed (gap $6\log\tfrac32$, OS) \emph{under RP/gap hyp.}; ambient metric measure open ($G_{\mathrm{metric}}$) \\
5 & \tI\ (P2 algebra, Lean) & P1 analytics as an interface; index premises \\
\bottomrule
\end{tabularx}
\markerkey
...
\begin{winbox}[What improves about the predictions]
\textbf{(i) From theorems to formulas:} quantities that arose in the papers through multi-stage theorems
($\alpha^{-1}$, flavor transport, masses) are now \emph{one-line} closed expressions.
\textbf{(ii) Fewer inputs:} the whole mass spectrum hangs on \emph{one} seed $\phiz$ and \emph{one}
matrix --- the $\Lambda$ table (formerly 9 numbers) becomes the $\phiz$-ladder.
\textbf{(iii) New sharp tests:} residue invariants ($\det 8$, minors $2{,}3{,}5$), the explicit gap
$6\log\tfrac32$, $n_s{=}0.964$, $r{=}0.004$ --- all fit-free and independently checkable.
\end{winbox}
...
\emph{ratios} are closed too (integer Plücker readouts on the derived selector stratum, v49/v71) --- only the
...
Paper 3's transport-kernel theorem derives the down branch $\{1,2,5\}$ (the spectral-selector branch,
$\det R=h(D_5)$, principal minors $(2,3,5)$) from the uniquely-fixed family holonomy; only the
parabolic$\leftrightarrow$transport equivalence remains open. Status flags: \tI\ exact identity, \tL\
lattice/Lie theorem, \tN\ numerical, \tP\ conditional/project, \tA\ open.
\end{abstract}
...
$\mu_4=\{1,i,-1,-i\}$ (Paper 2). The flavor bundle is a rank-$\Nfam=3$ parabolic bundle $E_\bullet$ on
...
\begin{equation}
  \boxed{\ \alpha=\Bigl(0,\tfrac13,\tfrac23\Bigr)\ \text{at each of the four punctures}\ }\qquad\tI ,
\end{equation}
...
\begin{equation}
  ...
  \quad\Longrightarrow\quad \boxed{\ \deg E=-|\mu_4|=-4\ }\qquad\tI .
\end{equation}
The balanced (stable) underlying splitting is therefore $E\cong\mathcal O(-2)\oplus\mathcal O(-1)\oplus\mathcal O(-1)$. \tI
\end{proposition}
...
\begin{equation}
  ...
  w_{f,j}\in\{0,\tfrac13,\tfrac23\},\ s_{f,j}\in\{0,1\}\ }\qquad\tI ,
\end{equation}
...
is the non-primitive $\mathbb Z_{30}$ load $h-r$ of the main note. \tI
...
$\{i,-1,-i\}$: the $A_3$ Coxeter element. \tI\ This independently reproduces the corner-rotation
$=$ Coxeter identity and confirms the family connection exists.

\begin{auditbox}[Rigidity: honest count]
...
branch, i.e.\ Paper 2's classification, which the explicit solution above is consistent with. We do not
claim Katz rigidity for the bare $\mathbb Z_4$ system.
\end{auditbox}

\begin{auditbox}[Why the monodromy alone does not give the integers]
...
\begin{roadbox}[Derivation record for $L_{f,j}$ (how the word-lengths are obtained)]
...
\begin{equation}
  \boxed{\ \mu_j=d_j+\textstyle\sum_{k=1}^4\alpha(v_j,M_k)=d_j+4\,\alpha_j,\qquad \alpha_j=\alpha(v_j,C)\ }\qquad\tI .
\end{equation}
\end{proposition}

\begin{auditbox}[What the explicit computation shows: the local model is insufficient]
...
parabolic stability fixing the saturation), confirming the $\tP$ classification: it is an
...
\begin{equation}
  \boxed{\ \sum_{k=0}^{3}R^kA_0R^{-k}=4\operatorname{diag}(A_0)\ }\qquad\tI ,
\end{equation}
...
direction) selects
\begin{equation}
  \boxed{\ \text{exponents at }\infty=\{1,1,2\}=(a_u,a_d,a_e)=-\deg\bigl(\mathcal O(-2)\oplus\mathcal O(-1)^2\bigr)\ }\qquad\tI .
\end{equation}
...
\begin{equation}
  \boxed{\ (a_u,a_d,a_e)=(1,1,2)=\bigl(\operatorname{round}(2\,\mathrm{cusp}_f)\bigr)_f\ }\qquad\tI/\tP ,
\end{equation}
...
\begin{auditbox}[The single remaining piece --- part (ii)]
...
\end{tabular}
\end{center}
\textbf{Provable structure \tI:} (a) windings are first-generation only, $n=(1,0,0)$ per sector,
...
transport gaps); it is physically forced but stated here as a hypothesis. \tP
\item (H3) is \emph{derived}: the anchor multiset $\{1,1,2\}$ is the Schur--Horn$+$stability splitting
type (Section above), and the per-sector assignment $(1,1,2)$ is the $D_5$ cusp ordering. \tI/\tP
...
$\{1,3,4\}$. Paper 3's transport-kernel theorem \emph{derives} the former (down $=\{1,2,5\}$) from the
...
the parabolic$\leftrightarrow$transport equivalence remains. \tI/\tP
\end{itemize}
...
\begin{equation}
  ...
  \textstyle\prod_i a_i=2=|\mathbb Z_2|_{\text{sheet}}\ }\ \tI .
\end{equation}
...
\begin{equation}
  \boxed{\ \textstyle\sum_{f,j}L_{f,j}=\|a\|_1+2\Nfam|R^+(A_3)|=4+2\cdot3\cdot6=40=\underbrace{4}_{\text{anchor}}+\underbrace{18}_{\text{winding}}+\underbrace{18}_{\text{non-anchor residue}}\ }\qquad\tI .
\end{equation}
...
$(G_1,G_2,G_3)=(\|a\|_1+\Nfam|R^+(A_3)|,\,|R(A_3)|+N_\Phi,\,\gcar)=(4{+}18,\,12{+}1,\,5)=(22,13,5)$. \tI\
Finally the scalaron exponent is the parabolic anti-degree plus the family rank,
\begin{equation}
  \boxed{\ \frac{M_{\mathrm{scal}}^2}{\Mbar^2}=\cthree^{\,-\deg E+\operatorname{rk}E}=\cthree^{\,4+3}=\cthree^7\ }\qquad\tI\ \veri{v7\_gravity\_cosmo.py} ,
\end{equation}
...
the two overflows of $6$ being exactly $|R(A_3)|=12$. \tI
...
\begin{equation}
  \boxed{\ (p_1,p_2,p_3,p_4,p_5)=(4,6,10,18,34)=\bigl(|\mu_4|,\,|R^+(A_3)|,\,\AL,\,\Nfam|R^+(A_3)|,\,\text{raw }\mathbb Z_6\text{ lift}\bigr)\ }\qquad\tI ,
\end{equation}
...
\begin{equation}
  \boxed{\ e_1=4=|\mu_4|,\quad e_2=5=\gcar,\quad e_3=2=|\mathbb Z_2|,\qquad p_0^2+e_1^2=3^2+4^2=5^2=e_2^2\ }\qquad\tI ,
\end{equation}
...
readings $7=p_0+p_1=\Nfam+|\mu_4|=e_2+e_3=\gcar+|\mathbb Z_2|=h(D_5)-1=\dim A_3-h(D_5)=\Oadm-10b_1$. \tI
...
\begin{equation}
  ...
  \boxed{\ \operatorname{tr}R=9=\Nfam^2,\ \det R=8=h(D_5),\ \chi_R(t)=t^3-\Nfam^2t^2+\AL\,t-h(D_5)\ }\ \tI ,
\end{equation}
\begin{equation}
  \boxed{\ \text{principal $2$-minors}(R)=(2,3,5)=(\text{sheet},\,\Nfam,\,\gcar),\quad
  \text{their }\textstyle\sum=10=\AL,\ \prod=30=h_{E_8}\ }\qquad\tI ,
\end{equation}
so the three TFPT atoms $2,3,5$ are literally the principal minors of the flavor residue matrix, and
$\|R\|_F^2=78=\dim E_6$ (an $E_6$ residue-shadow fingerprint \tA), $\operatorname{SNF}(R)=\operatorname{diag}(1,1,8)$.
...
$a^{\!\top}La-a^{\!\top}Ra=64-40=24=|W(A_3)|$ is the $A_3$ Weyl group. \tI
\begin{auditbox}[(H2), now a small algebraic selector --- and an honest negative result]
...
\smallskip
\noindent\textbf{But H2 is not a free input: Paper 3 derives it.} The transport-kernel theorem of
Paper 3 (``canonical transport kernel and single-winding closure'') fixes the residue packets from the
...
the transport-holonomy word-lengths of Paper 3. H2's \emph{answer} is fixed; only this parabolic
$\leftrightarrow$ transport bridge is open. \tI\ (transport derivation) / \tP\ (parabolic equivalence)
\end{auditbox}
...
data, with the spectral selector as their common invariant. \tI\ (residue $+$ winding identification)
/ \tP\ (stable $=$ minimal).
\end{proposition}
...
$U(\Nfam)$, generated by the cyclic $U_6$ shift and the cusp data --- Paper 3, metric family connection).
...
\begin{auditbox}[Flavor block: reduced to one structural input $(U)$]
...
a structural property asserted by Paper 3 (the family connection is metric, holonomy in $U(\Nfam)$), not
a new conjecture. The flavor block is therefore \emph{reduced} to (U) --- closed given (U), open without it. \tI\
(given (U)).
\end{auditbox}
...
geometry equation on $\mathbb P^1\setminus\mu_4$ --- a finite, well-posed step, not a search. \tP
\end{theorem}
...
\midrule
weights $\alpha=(0,\tfrac13,\tfrac23)$ from cusps $=2y_\pm$ data & \tI & Mehta--Seshadri \\
$\pardeg=0\Rightarrow\deg E=-|\mu_4|=-4$ & \tI & exact \\
balanced bundle $\mathcal O(-2)\oplus\mathcal O(-1)^2$ & \tI & stable splitting \\
$L=6n+r$, $n=$ degree, $r=6w+3s\in\mathbb Z_6$ & \tI & winding/residue split \\
$\sum L=22+18=40=|R(D_5)|$, $\sum n=\Nfam=3$ & \tI & budget bookkeeping \\
rigid $SU(3)$ system exists; rotation $=A_3$ Coxeter & \tI & Riemann--Hilbert (numeric) \\
bare $\mathbb Z_4$ moduli $\dim=2$ (not Katz-rigid) & \tI & rigidity via full $D_4$ (Paper 2) \\
single-fibre reduction $\mu_j=d_j+4\alpha_j$ & \tI & $R$-eigenline weights puncture-independent \\
single-fibre weights degenerate $\Rightarrow$ global computation & \tI & residues are global parabolic degrees \\
$\sum_k R^kA_0R^{-k}=4\operatorname{diag}(A_0)$ (exact collapse) & \tI & exponents at $\infty$ $=4\operatorname{diag}$ \\
\textbf{anchor multiset} $\{1,1,2\}$ from Schur--Horn $+$ stability & \tI & $=$ splitting type, derived not free \\
per-sector assignment $(a_u,a_d,a_e){=}(1,1,2)$ & \tI/\tP & max cusp $\to\mathcal O(-2)$ (stability) \\
residue $(c,s)$ decomposition $r=2c+3s$; windings g1-only; 2 sheet-odd/sector & \tI & exact $\mathbb Z_3{\times}\mathbb Z_2$ bookkeeping \\
anchor power compiler: $p_n(a)=2{+}2^n$, $e_2(a){=}5{=}\gcar$, $3^2{+}4^2{=}5^2$ & \tI & all budgets are $p_n$ products \\
glue norms $q(A_3){=}p_2/2p_1$, $q(D_5){=}p_3/2p_1$; $\AL{=}p_3$ & \tI & from the anchor alone \\
budget master table: $40,41,48,60,120,240,248,128$ from $d_p{=}4$ & \tI & all $=d_p\times$ Coxeter datum \\
scalaron $7=-\deg E+\operatorname{rk}E=\Nfam{+}|\mu_4|=\gcar{+}|\mathbb Z_2|$ & \tI & flavor-bundle $\to$ inflation link \\
\textbf{flavor transport theorem}: (H1,H2,H3)$\Rightarrow$ full matrix & \tI & combinatorial core proved by enumeration \\
\quad H1 non-degeneracy $\Rightarrow$ distinct-distance triple $\{0,1,3\}$ & \tP & genericity/spread \\
\quad H3 anchor $(1,1,2)$ $+$ gen-1 winding $+$ monotone & \tI/\tP & derived (Schur--Horn $+$ cusp) \\
\quad H2 reduced to down branch $\{1,2,5\}$ vs $\{1,3,4\}$ & \tP & single binary cusp choice \\
\textbf{spectral selector}: $\det R{=}h(D_5)$, prin.\ minors $(2,3,5)$ & \tI & selects correct branch; sibling fails all \\
$\|R\|_F^2{=}78{=}\dim E_6$, $\chi_R{=}t^3{-}\Nfam^2t^2{+}\AL t{-}h(D_5)$ & \tI/\tA & $E_6$ residue shadow (fingerprint) \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
\begin{keybox}[Scope and honesty]
...
assumption), so the parent-domination input left open in Paper~4 is discharged on the transport sector.
\end{proof}

\noindent\textbf{Consequence.} Together with admissible reflection positivity (Paper 4) this completes the
...
(4) is thus closed on the physical sector \tI, and strong CP (3) needs only RP. The gap is a compiler
...
``admissible-sector measure'' (\emph{closed here}, \tI) and ``ambient projection'' (the selector, \tI),
with no hidden cluster-expansion assumption. \tI
...
\label{lem:aps}
Compactify the oriented two-dimensional normal slice $N_\Sigma$ to $S^2=D_N\cup_{S^1}D_S$ (Paper 5).
...
equals the compact bosonic/fermionic index already computed in Paper 5 ($2$ and $3$ respectively).
\end{proof}
...
property, so the $R^2$/scalaron sector is well-defined \tI\ (the only residual is the low-curvature
truncation of \S4).
...
\begin{winbox}[The scalaron mass is the seam power --- Starobinsky's one parameter removed \tI]
...
\end{tabular}
\end{center}
Planck favours $n_s\approx0.965$ ($N_\star\approx57$); $A_s\approx2.1\times10^{-9}$ ($N_\star\approx56$).
The current tensor bound $r<0.036$ (BICEP/Keck BK18) sits far above all rows; CMB-S4
($\sigma_r\!\le\!5\times10^{-4}$, first light $\sim$2033) is the decisive test. \tN
\end{keybox}
...
\[
  \boxed{\ \text{admissible transfer-sector measure: \emph{closed} \tN}\ }
\]
\[
  \boxed{\ \text{physical projection via }P_{\mathrm{adm}}:\ \text{\emph{claimed} (Papers 2--4) \tP}\ }
\]
\[
  \boxed{\ \text{metric-coupled ambient RP: \emph{conditionally reduced} by Prop.~\ref{prop:metricrp} (gap-dominated bound) \tP}\ }
\]
...
built here} and is carried as \emph{open} \tA\ in the frontier document (\texttt{tfpt\_4\_frontier}),
...
\begin{keybox}[The closed $R+R^2$ covariant field equation \tI/\tP]
...
\]
inside the Planck window. \textbf{Typing:} the $R+R^2$ covariant equation is \emph{closed} \tI/\tP\ in its
regime; the \emph{full} metric-sector path-integral measure (beyond FRW) is the deepest open item \tA\
...
graph ($H$ over the $5$ vertices, $\Lambda$ over the $10$ edges) --- not separate modules. \tI/\tP
\end{winbox}
...
\label{prop:lambda}
\begin{enumerate}[leftmargin=1.6em]
\item \emph{Seam determinant} (Paper 6): $\dfrac{\Lambda_{\mathrm{IR}}}{\Mpl^4}=-\log\det\nolimits_{\mathrm{adm}}(1-U_\Sigma)$,
...
which branch (rec vs car) and pure-vs-total action enters the determinant. \tP
\end{proposition}
...
coefficient $\varepsilon=\tfrac34\phiz\approx\cthree$, a $0.23\%$ conditional input).
...
Therefore $\theta_{12}$ is \emph{conditionally derived}: a numerically reproduced fixed value \tN\ that
...
on the seam-misalignment lemma --- older ``open'' statements are historical status, not current. \tN/\tP
...
already-fixed connection --- no new physics. \tI\ (structure) / \tP\ (digits)
...
not the resolvent. The quark mass \emph{ratios} are closed independently (integer Pl\"ucker, v49/v71); only
the \emph{absolute} per-sector scale $y_{\rm up},y_{\rm down}$ remains --- a named, bounded normalisation
(an anchor), not new physics. \tA
\end{winbox}
...
\begin{winbox}[H2 closed by two counting facts \tI]
...
number $h(D_5)=8$ selects $R$ uniquely. Thus $L=6n+r$ is fully determined --- no search, two identities. \tI
\end{winbox}

\begin{keybox}[The $U_f^\star$ holonomy reduction: the exact quark $c$'s reduce to $(U)$ \tI/\tA]
...
\end{enumerate}
\noindent\textbf{Update --- the ratios no longer reduce to this gate.} Since v49/v69/v70/v71 the geometric
...
\begin{keybox}[The $(U_{\mathrm{wall}})$ gate, explicitly: one $D_4$-fixed polystable wall point \tP]
...
(v69/v70) and fix the \emph{ratios} $c_u/c_d$ and the H2 transport equivalence by rigidity (v49/v71);
...
readouts on a now-derived selector stratum (v49/v69/v71). Only the \emph{absolute} $c_q$ scale reduces to
...
amplitudes --- hence all nine $c$'s --- follow. \tI\ (reduction) / \tA\ (the $(U)$ selection)
\end{keybox}
...
\midrule
Dynamics / clustering (Lemma~\ref{lem:gap}) & \tI & explicit gap $6\log\tfrac32$, Dobrushin unique state \\
Gravity Fredholm (Lemma~\ref{lem:aps}) & \tI & APS on $S^2$; index $(2,3)$ \\
$R^{n\ge3}$ truncation (Lemma~\ref{lem:trunc}) & \tI & $O(1/N^2)$; $n_s{=}0.964$, $r{=}0.004$ \\
Starobinsky $M$ / $A_s$ & \tI & $M{=}\cthree^{7/2}\Mbar{=}3.06{\times}10^{13}$ GeV (canonical); $A_s$ predicted \\
$\Lambda$ typing (Prop.~\ref{prop:lambda}) & \tP & three objects linked by $(8\pi)^2$; branch open \\
$\theta_{12}$ (Prop.~\ref{prop:theta12}) & \tN/\tP & cond.\ derived $\tfrac13-\tfrac{\phiz}{2}{=}0.307$; rests on seam-misalignment $\varepsilon{=}\cthree$ \\
quark $c$-rationals & \tI/\tA & hierarchy + source ratios closed; exact digits reduce to $(U)$ (\veri{v19}) \\
H2 splitting type (Prop.~\ref{prop:h2}) & \tI & $4{=}1{+}1{+}2$ unique; branch coker $\Z_8$ \\
unitarity (U) of $\nabla_F^\star$ & \tA & selects the stable point of a \emph{positive-dim} character variety (\veri{v19}) \\
\bottomrule
\end{tabularx}
\end{center}
...
Fredholm and the truncation bound as lemmas \tI; the Starobinsky scalaron mass fixed by $\cthree^7$ (so
...
% =====================================================================
The Standard-Model identities used here (marked \tI, \tL, \tN) are re-derived from $\{\cthree,\gcar\}$ and
