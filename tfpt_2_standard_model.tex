...
\begin{gapbox}[Status and the open derivation target \tP$\to$\tL\ (advanced)]
...
$\Sigma$-shadows of one $D_4$-equivariant parabolic operator. What \emph{remains} \tP/\tA: the cusp weights
are the established monodromy exponents (v61), and the explicit integer matrix $Q$ (beyond its spectra) still
needs the lattice/SNF datum. So the $Q,\Sigma$ algebra is now an \tI/\tL\ layer with its geometric origin
\emph{derived at the spectral/equivariant level}, the residual reduced to the integer-lift.
\end{gapbox}
...
The residue matrix $R$ and the word-length matrix $L=R+6\,e_1 a^\top$ act \emph{identically} on the
dual of the anchor $a=(1,1,2)$:
\[
  \boxed{\ a^\top R^{-1}=a^\top L^{-1}=(-\tfrac12,-\tfrac12,1)\ }\qquad\tI .
\]
This exact identity (the two equal entries are the $\mathrm{gen}\,1{\leftrightarrow}2$ TBM symmetry, the
$1$ marks the third-generation anchor) ties the solar misalignment to the seam. \textbf{New: the
