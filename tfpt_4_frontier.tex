...
\par\smallskip\noindent\emph{How external is ``external''? A falsifiable clock probe (\veri{v397\_external\_clock\_probe.py}).}
Under the strict No-Free-Pattern rule (an \emph{exact} match, not a wide-band fit), three of the four frontier
scheme-numbers in fact land \emph{on} the $\{2,3,5\}$ clock: the $\eta_B$ decuple $A_\Lambda=10=|Z_2|\,g_{\mathrm{car}}$
(\vref{v212}), the Koide rate $(2/3)^6=(|Z_2|/N_{\mathrm{fam}})^6$ (\vref{v303}) and the axion spine angle
$\theta_i=\pi N_{\mathrm{fam}}/g_{\mathrm{car}}=3\pi/5$ (\vref{v211}). Only the proton $C_p\sim2.83\pm0.15$ is \emph{not} a discriminating
clock value (several simple numbers fit its $\pm5.3\%$ band), so it stays genuinely external --- the irreducible
``external physics'' residue shrinks to essentially the proton's non-perturbative QCD number.
\par\smallskip\noindent\emph{The Paper-E closure certificate (\veri{v402\_ftransfer\_suite.py}, \texttt{FTRANSFER.SUITE.01}).}
The four sectors are certified as ONE functor $F_{\mathrm{transfer}}=F_{\mathrm{observable}}\circ F_{\mathrm{threshold}}\circ F_{\mathrm{RG}}$:
each observable factors source\,$\to$\,threshold\,$\to$\,RG\,$\to$\,observable from an exact atom source ($Q_{\mathrm{src}}$,
$A_\Lambda=2g_{\mathrm{car}}=10$, $f_a=M_{\mathrm{scal}}/128$, $b_3=-7$) and carries a committed kill test, with the firewall
intact (the physical prediction stays \mC/\mO, only the algebraic source \mE) --- a consolidation of the
\texttt{FR.POLE/BOLTZMANN/RELIC/QCD.SOLVE} solvers, not a closure.
\end{gapbox}
...
$k=\cthree/2\Leftrightarrow\ln(m/\mu)=3/4=q(A_3)$ (\veri{v152\_norm\_is\_anchor.py}; the collar data
exhibit this as the anchor --- drifting $\mu_{\mathrm{lat}}$, no \emph{derived} $\ln(m/\mu){=}\tfrac34$,
\vref{v471}), is reached a \emph{second}, independent way: the old torsion-compression ansatz $\kappa^2=\xi\,\phiz/\cthree^2$ with the
...
\texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann --- the one open premise being that the seam net condenses to
the \emph{holomorphic} $|\det K|{=}1$ point that selects $(E_8)_1$; the central charge $c{=}8$ alone does
\emph{not}, being shared by the non-holomorphic $SO(16)_1$ counterexample, \vref{v277}/\vref{v281}), a
certification object rather than missing dynamics; we do
...
($1/4{=}1/|\mu_4|$, central charge $c=g_{\mathrm{car}}{+}\Nfam=8$). (The single-ball CHM input now has a
\emph{mark-resolved} companion: the four seam marks are the exactly solvable \emph{four-interval} multilocal
modular geometry, \vref{v480} --- same atoms, the multi-interval class; \texttt{QGEO.SYM.01} stays \mO.) \textbf{Full covariant equation
...
only the equation-of-state status (Jacobson framing, not a from-action quantisation; an \emph{external} candidate
for the missing action level --- Bianconi's entropic action, Phys.\ Rev.\ D 111, 066001 (2025) --- is quantified in
\vref{v473}: $\beta'_{\mathrm B}=\cthree/6$ pinned, $R^2$ gap $3(8\pi)^9$ pre-registered as kill test, typing
unchanged) and the absolute scale
...
cited theorems} on the MMST route \texttt{SEAM.EQUIV.MMST.01} (lattice \vref{v367}/\vref{v368} $+$ the $S_3$ stack
\vref{v376}--\vref{v379}, Lean
\texttt{FORM.SEAM.MMST.01}; the parent \texttt{SEAM.EQUIV.01} stays \mO);
...
This redundancy is now a machine-checked \emph{dependency} fact, not only prose
(\veri{v423\_no\_ambient\_dependency.py}, \texttt{QGAMB.NODEP.01}): \texttt{QG.AMB.01} is a
certification \emph{sink} --- every claim that lists it is a gravity-gate/QG row, never a frozen
prediction --- and it is \emph{not a value source} --- it has no producing script, while $\alpha^{-1}$,
the mixing angles and the cosmological readouts are each \emph{computed} by a compiler script
(\vref{v3}/\vref{v9}/\vref{v268}/\vref{v7}), none of them an ambient-measure script. The single
transitive link is architectural, not numerical: of the frozen readouts only $\theta_{13}$ references
\texttt{QG.AMB.01}, through the 4d-QFT contract spine
($\texttt{PS.DIRAC}{\to}\texttt{CONTRACT.QFT4D}{\to}\texttt{QG.G2}{\to}\texttt{GATE.METRIC}$) --- a
coherence edge from living inside the unified spectral-action construction, not an input to its value
$\sin^2\theta_{13}=\phiz\,e^{-5/6}$ (\vref{v268}). Honest scope: this kills \texttt{QG.AMB.01}
specifically; it does \emph{not} discharge \texttt{SEAM.EQUIV.01}, on which the readouts still depend.
\smallskip\\
...
holomorphy bit --- now \emph{closed modulo cited theorems} on the MMST route \texttt{SEAM.EQUIV.MMST.01}
(lattice \vref{v367}/\vref{v368} $+$ the $S_3$ stack
...
tests ($\Sigma m_\nu$: DESI/CMB-S4; proton decay: Hyper-K) (\texttt{PRED.LIVE.01}). Two \emph{named post-hoc}
\mO\ candidates sit next to this board without touching the frozen record: the third-generation ladder-base
pattern (the three ${\sim}2\sigma$ mixing tensions $\theta_{13}/\theta_{23}/|V_{cb}|$ as one common $-\phiz$
suppression, \texttt{FLAV.THIRDGEN.PATTERN.01}, \vref{v467}) and the splitting-ratio relation
$\Delta m^2_{21}/\Delta m^2_{31}=|J_{\mathrm{PMNS}}|$ at $-0.19\sigma$ (\texttt{FLAV.DM2RATIO.01}, \vref{v468})
--- both pre-registered with symmetric kills (JUNO / Belle~II), see document~2.
\smallskip\\
...
face of the keystone \texttt{SEAM.EQUIV.01}, which is now \emph{closed modulo cited theorems on its MMST route}
\texttt{SEAM.EQUIV.MMST.01} (lattice $+$ $S_3$
stack, Lean \texttt{FORM.SEAM.MMST.01}; only the cited continuum existence stays \mO\ --- its extension leg since
re-founded on peer-reviewed crossed-product theorems, the realisation input reduced to invariant level R1$'$,
\vref{v469}; the twistor route \texttt{SEAM.EQUIV.TWISTOR.01} and the unconditional parent stay \mO).
\smallskip\\
...
$G_{\mathrm{metric}}$; the primary, machine-checked route stays the seam-net. A third, quantitative
approach to the keystone --- the celestial and twistor continuum route, now named
\texttt{SEAM.EQUIV.TWISTOR.01} (contract \texttt{CELEST.SEAM.01}, WP1--WP5d, both
WP5d stages, plus the
WP5e $\alpha$, $\beta$, $\gamma$, $\delta_1$, $\delta_2$, $\varepsilon_2$ and $\varepsilon_1$
stages and the
M1--M3 back-reaction milestones executed:
the level-2 null ideal derived, its deleting operator explicit, the GNS limit state constructed, the
two-interval index chain measured, split $+$ strong additivity witnessed on the lattice (all three
KLM ingredients of complete rationality; the continuum uplift is Xu's theorem, cited not claimed),
the $q^{-1/3}$ prefactor $+$ level $k{=}1$ pinned on the CFT
side, the equivariant anomaly ledger exact on the twistor side (index bridge, $-78$ defect,
rigid $32\,T_3$ residual, the honest $a_0$ refutation), the sphere-axion exchange sub-branch closed
with a certificate, the collapse confirmed full-tensorially with the one cubic $d$-symbol door
open (its $c_d=1920=|W(D_5)|$ fingerprint honestly typed look-elsewhere-loaded and
convention-contingent, the stable core $c_d=32\times60$, \vref{v513}), ``one level'' lifted to
a theorem of clock invariance with the sector
counter pinning $k=1$ (\vref{v508}/\vref{v509}/\vref{v511}/\vref{v513}), the $O(-2)$ bulk-axion
slot built with $\tilde\lambda=6$ triply pinned (\vref{v514}), the back-reacted $\Omega_N$
closed-form with $(2\pi i)^2$-integral lockstep periods and the lens-forced source charge
$4=|\mu_4|$ (\vref{v515}), the twisted KS measure on the declared completion reading cancelling
the $32\,T_3$ residual and supplying the $\psi=64$ slice without the cubic $d$-channel
(\vref{v516}), the $a_0$ uplift coupled to the centre count on four scales, monodromy $i$
(\vref{v517}), the $\delta_1$ chain decided --- kill under the derived measure, the
declared/derived measure tension stated honestly (\vref{v518}) --- the measure question
since decided at probe level for the declared reading (single-valuedness derived from
F-independence $+$ the Quillen pairing under the typed premises TP-1..TP-4, \vref{v520}),
and the $w_m$ normalisation since derived constructively at probe level ($1/\det_j$ = the
Atiyah--Bott/zeta-determinant fixed-point factor, three independent sources, \vref{v523});
WP5e proper \mO, the global BCOV
quantisation --- narrowed to the global BCOV integral beyond the fibre zero-mode factor ---
the single remaining milestone) --- is
synthesised as a
dedicated section in \texttt{tfpt\_3\_e8\_audit\_bootstrap}; it does not close \texttt{SEAM.EQUIV.01}
either.
\end{keybox}
...
dark matter & \mC & determinant-line axion: $f_a{=}M_{\mathrm{scal}}/128$, $m_a\approx23.8\,\mu$eV. \emph{Post-inflationary} ($H_{\mathrm{inf}}/f_a{\approx}66$, isocurvature): the misalignment angle is a horizon average (\emph{not} observable), the relic follows from $f_a$ (in the post-inflationary window), domain-wall-safe via $N_{\mathrm{DW}}{=}|C|{=}1$ (\vref{v470}/\vref{v472}); the $3\pi/5$ spine value is retained as a structural rational, not a relic prediction --- branch-level kill test, not a theory closure \\
...
finite, UV-softened and tree-unitary, \veri{v386\_grav\_amplitude.py}, and UV-\emph{finite at every loop order} by
power counting --- the super-polynomial falloff drives the superficial degree of divergence to $-\infty$, regulated
by the seam scale $M_{\mathrm{scal}}$, \veri{v389\_grav\_loop\_finiteness.py}, with full loop \emph{unitarity} of the
nonlocal form factor honestly the still-open point; the nonlocal action's IR projection \emph{is} the atom-scaled
$R+R^2$, $M_{\mathrm{scal}}^2/\Mpl^2=\cthree^7$, so the local readout is its shadow, not a truncation with sawn-off legs,
\veri{v400\_grav\_nonlocal\_action.py}), leaving the ambient projective \emph{measure} (G6) as a \mC\ redundancy
...
$\Omega_{\mathrm{DM}}$ (in band) while the hilltop $170^\circ$ over-produces (${\sim}5.4\times$); in the
natural post-inflationary scenario ($H_{\mathrm{inf}}/f_a{\approx}66$) the angle is instead a horizon
average and the relic follows from $f_a$ with $N_{\mathrm{DW}}{=}1$.
