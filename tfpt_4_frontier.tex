...
\input{tfpt_docset}
\noindent\textbf{You are reading document \#5.}
...
\begin{winbox}[Sharpening: the source$\to$pole map is \emph{forced} by that gap \tI/\tP\ (\veri{v82\_koide\_attractor\_splitting.py})]
The previous box pairs the Koide target with $\lambda_2=(2/3)^6$; \veri{v82\_koide\_attractor\_splitting.py} turns the
pairing into a \emph{mechanism}. On the anchor double cover (Paper 2) the two branch points are Koide $q=2$ and carrier
...
chosen branch are all forced. This is still not a closure: that single identification stays \tP. (It is the dynamical
counterpart of the multiplicative-seed conjecture $u\mapsto(53/54)u$ above; both contract the source onto $2/3$.)
\end{winbox}

\begin{winbox}[Relaxation toy: basin, exact rate, and two honest negatives \tI/\tN\ (\veri{v93\_koide\_relaxation\_toy.py})]
The relaxation picture is now stress-tested on the physical values (ledger \texttt{FR.KOIDE.05}).
\textbf{Basin lemma \tI\ (new):} under the canonical dictionary $q=\Nfam\,Q$ the \emph{entire} physical Koide
...
fine-tuning of the start. \textbf{Exact rate \tI:} along the trajectory from the physical source value the
cross-ratio contracts by \emph{exactly} $(2/3)^6$ per step. \textbf{Source/pole \tI/\tN:} the exact ladder
...
the literal discrete iteration $\mathrm{pole}=F^n(\mathrm{source})$ is \emph{excluded}; if the v82 map drives
the transfer at all, the missing \tP\ object is a \emph{continuous-time} generator $\exp(t\log F)$;
...
\begin{winbox}[Flow time: canonical generator, data-fragility, and a conditional $m_\tau$ \tI/\tN/\tP\ (\veri{v99\_koide\_flow\_time.py})]
Three sharpenings of the box above (ledger \texttt{FR.KOIDE.06}); they correct the \emph{robustness} of
negative (i), not its arithmetic. \textbf{(1) The continuous generator has a canonical form \tI.} The v82
...
missing \tP\ object is therefore precisely a QFT mechanism producing \emph{gap $\times$ anchor-block
quadric} --- the form is canonical, only its physical derivation is open. \textbf{(2) The non-integrality
of $t$ is data-fragile \tN.} Because $\rho_{\mathrm{pole}}=-2.2\times10^{-6}$ sits so close to the branch
...
steps \tP\ (registry untouched; not predictions of record):} $n{=}2$ is \emph{excluded} ($-2.9\sigma$,
\tN); $n{=}3{=}\Nfam$ steps --- one per family --- predicts
\[
...
within $0.1\sigma$); it is rejected and recorded so it is not re-fished. P2's type is unchanged ([P], one
...
\smallskip\\
\noindent\emph{Update (QBL chain, v108--v113):} the boundary-net residual now carries \emph{double}
...
already posits. Closing the index-$4$ statement therefore also closes the carrier selection \tL\
