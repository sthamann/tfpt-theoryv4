...
\texttt{v30\_\allowbreak d4\_character\_\allowbreak variety.py} & $C_U^{(2)}$: full $D_4$-fixed $SU(3)$ character variety is positive-dimensional $\Rightarrow$ selector needs the $R(\rho)$ dictionary (H2; seit v118--v122 exakt, Rest $=$ R4$'$ Selektor-Etablierung) \\
...
\texttt{v120\_\allowbreak address\_\allowbreak table.py} & Address table (frozen v18/v20 integers): lepton words $(8,5,3){=}(\operatorname{rank}E_8,\gcar,\Nfam)$; addresses $=$ divmod hexagon ($p_2{=}6$): $e{\to}(2,1)$, $\mu{\to}(5,0)$, $\tau{\to}(3,0)$, $\sum r{=}p_3$; sheet parity: $\tau$ at the real twisted eigenwert $-1$ (= Produktregel-Site); quark sums: up $=p_3$, down $=p_1{+}p_3$, alle Quarks $=24{=}|W(A_3)|$, alle neun $=40{=}p_1p_3{=}a^TRa$; top am Vakuum-Site $(0,0)$; Zuordnungs-Herleitung bleibt offen \tI\,$+$\,\tP \\
\texttt{v121\_\allowbreak address\_\allowbreak pinning.py} & Address pinning: $L{=}R{+}2U$ (v95) $\Rightarrow$ Wort-Tabelle $=$ Residuen-Operator $R$ $+$ Generation-1-Windung, $R$-Spalte 1 $=$ Anker $a$; R\"ander $=$ Atome (Zeilen $(e_1,\mathrm{rank},p_3)$, Spalten $(e_1,\Delta_Q,\gcar)$); \emph{Census-Satz}: unter allen Hexagon-Matrizen mit Atom-R\"andern (17 St\"uck) hat genau \emph{eine} $\det{=}8$ --- und es ist $R$ (auch eindeutig mit SNF $(1,1,8)$; alle 17 Determinanten verschieden) $\Rightarrow$ Zuordnungstabelle tr\"agt \emph{null} Restinformation; H2-Rest $=$ die sechs Atom-R\"ander \tI\,$+$\,\tP \\
\texttt{v122\_\allowbreak margin\_\allowbreak theorem.py} & Margin-Theorem: die \emph{etablierten} Selektoren --- $D_4$-Annihilator $n{=}(5,-9,6)$ mit $n^TR{=}(8,0,0)$ (v94), $\det R{=}8$, $\det K{=}4$ (v71-Quartett) --- pinnen $R$ \emph{eindeutig} unter Hexagon-Matrizen ($64\to12\to1$) $\Rightarrow$ Atom-R\"ander, Anker-Spalte und alle Summenregeln sind \emph{Folgen} (Input-Status aufgehoben); Bonus: $\det L{=}20$ auf dem Locus automatisch (Quartett redundant); H2 f\"uhrt \emph{keine} neue Restklasse ein \tI\,$+$\,\tP \\
\texttt{v123\_\allowbreak inventory\_\allowbreak update.py} & Residual-Inventar aktualisiert (post v110--v122): dreizehn neue Ledger-Zeilen maschinell gepinnt; R2 tr\"agt \emph{drei} Lasten (Metrik $+$ Carrier $+$ QBL: ein Satz, drei T\"uren); Tabelle re-pinnt auf \emph{f\"unf} Klassen --- R1 Uhr, R2 Index-4-Netz, R3 EH, R4$'$ Selektor-Etablierung (ersetzt das \emph{erledigte} H2-W\"orterbuch), R5 $Q$-Realisierung --- $+$ $v_{\mathrm{geo}}$, $\pi$; eine sechste Klasse l\"asst das Skript scheitern \tI \\
\texttt{v124\_\allowbreak resummed\_\allowbreak clock.py} & Resummed clock (R1-Bend in geschlossener Form): Transfer-Eigenwerte $=$ exakt $(1{-}n/\Nfam)^{p_2}$ $\Rightarrow$ $\mathrm{rate}(n){=}{-}p_2\ln(1{-}n/\Nfam)$ reproduziert $\{0,\Delta,6\ln3\}$, Bend $\log_{3/2}3$ einzeilig; Linearisierung tr\"agt Steigung $|\Z_2|{=}2$ relativ zur klassischen GP-Uhr $\{0,-1,-2\}$; Pol bei $n{=}\Nfam$ erzwingt das Drei-Niveau-Spektrum; R1-Ziel jetzt geschlossen-formig \tI\,$+$\,\tP \\
\texttt{v125\_\allowbreak glue\_\allowbreak qsystem.py} & Glue-Q-System: $\mathbb C[\Z_4]$ erf\"ullt alle Longo-Q-System-Axiome exakt (Assoziativit\"at, Unit, Frobenius, Specialness $mm^*{=}|\Z_4|\mathbf1$) $\Rightarrow$ Jones-Index $4{=}|\mu_4|$ (v89-Wert); Lokalit\"at $=$ Isotropie ($q(k(1,1)){=}k^2\in\Z$, v92); Halfway-Sub-Q-System $\mathbb C[\Z_2]$ $=$ $SO(16)_1$, $4{=}2{\times}2$; R2 sch\"arft sich von ``finde'' zu ``identifiziere'' \tI\,$+$\,\tP \\
\texttt{v126\_\allowbreak clock\_\allowbreak wall\_\allowbreak bridge.py} & Uhr und Wand teilen ein Spektrum: die Resummations-Gewichte $n/\Nfam{=}\{0,\tfrac13,\tfrac23\}$ $=$ exakt $\operatorname{spec}A_0^\star$ (parabolische MS-Gewichte); $\lambda_n{=}(1{-}\alpha_n)^{p_2}$; Checkpoint 1 \emph{klassisch} bestanden ($p_2/\Nfam{=}2{=}|\Z_2|$ $=$ v104-Entropie-Rate $2H$); $\det(\mathbf1{-}A_0^\star){=}\tfrac29$ $=$ Entropie-Kr\"ummung (v102); Quanten-Inhalt von R1 isoliert auf den geometrischen Schwanz \tI\,$+$\,\tP \\
\texttt{v127\_\allowbreak ring\_\allowbreak resummation.py} & Die Uhr ist eine Log-Determinante: rate $=-p_2\operatorname{Tr}\ln(\mathbf1{-}\alpha P)$ --- sechs identische Ring-(RPA-)T\"urme, \emph{einer pro Hexagon-Site}, Kopplung $=$ parabolisches Gewicht; $k{=}1$-Ring $=$ klassischer Term, $k{=}2$-Ring $=$ erste Quantenkorrektur; $\kappa_{\mathrm{seam}}{=}\tfrac1{3\pi}{=}\alpha_1/\pi$ (Audit); Wand $=$ RPA-Instabilit\"at bei $\alpha{\to}1$; v107s ``O(1)-Resummation'' ist damit \emph{typisiert} (Ring/Log-Det), Herleitung offen \tI\,$+$\,\tP \\
\texttt{v128\_\allowbreak graded\_\allowbreak hull.py} & Graded Hull $+$ Nullmoden-Multiplizit\"at: die $240$ $E_8$-Wurzeln explizit in $D_5{\oplus}A_3{+}$Glue-Koordinaten, Coset-Zerlegung $240{=}52{+}64{+}60{+}64$; Graduierung auf \emph{allen} $6720$ Wurzelpaar-Summen exakt additiv $\Rightarrow$ $E_8$ ist eine $\Z_4$-\emph{graduierte} Lie-Algebra \"uber dem Carrier (Graduierung $=$ Glue $=$ Q-System); Ring-Multiplizit\"at $6{=}2(2l{+}1)|_{l=1}$ $=$ blatt-verdoppelte GP-Nullmoden \tI\,$+$\,\tP \\
\texttt{v129\_\allowbreak entropy\_\allowbreak power\_\allowbreak law.py} & Die Uhr ist ein Entropie-Potenzgesetz: $\lambda_n{=}(S_n/S_{dS})^{p_2}$ mit den drei Nariai-Entropie-Niveaus $S/S_{dS}{\in}\{1,\tfrac23,\tfrac13\}$ (v101); rate $=p_2\ln(S_{dS}/S)$; Dreifach-Identit\"at $\alpha{=}n/\Nfam{=}$ MS-Gewicht $=$ Entropie-Defizit-Anteil $\Rightarrow$ die RPA-Kopplung ist nirgends frei; Orientierung konsistent (Attraktor $=$ max.\ Entropie); R1 thermodynamisch: $\Gamma{\propto}(S/S_{dS})^{p_2}$ (Gibbons--Hawking-Potenzgesetz, $h{=}\Nfam$ Audit) \tI\,$+$\,\tP \\
\texttt{v130\_\allowbreak born\_\allowbreak square.py} & Born-Quadrat: Amplitude $\sim(S/S_{dS})^{n_{\mathrm{zero}}/2}{=}(S/S_{dS})^3$ (ein $S^{1/2}$ pro Nullmode), Wahrscheinlichkeit $=|A|^2{=}(S/S_{dS})^6$ $\Rightarrow$ $h{=}n_{\mathrm{zero}}/2{=}3{=}\Nfam$, $p_2{=}2h$ (Born-Regel) --- v129-Gewichts-Audit \emph{hergeleitet}; die $3$ $=$ Dimension des $l{=}1$-Raums $=$ Zahl der eigentlichen konformen Killing-Vektoren der $S^2$; $\times2$ $=$ Horizont-Paar (v101-Deck); R1-Rest $=$ Standard-Nullmoden-Ma\ss{} \tI\,$+$\,\tP \\
\texttt{v131\_\allowbreak measure\_\allowbreak is\_\allowbreak area.py} & Das Ma\ss{} ist die Fl\"ache: $\|Y_{1m}\|^2{=}r^2{=}A/(4\pi)$ \emph{exakt} (alle drei $l{=}1$-Moden integriert) $\Rightarrow$ Jacobian pro Mode $=$ Modennorm $\sim S^{1/2}$ $\Rightarrow$ Amplitude $S^3$, Born $S^6$ --- jeder Exponenten-Faktor mit Herleitungs-Ursprung; das $4\pi$ $=$ dieselbe Gauss--Bonnet-Primitive wie in $c_3$, k\"urzt sich in allen Ratios; R1-Rest $=$ \emph{eine} Det-Ratio-K\"urzung $\det'_n/\det'_{dS}{=}1{+}O(\mathrm{Defizit})$ \tI\,$+$\,\tP \\
\texttt{v132\_\allowbreak detratio\_\allowbreak anomaly.py} & Det-Ratio-Anomalie $=$ Koide-Konstante: $\zeta(0)|_{\det'}$ des GP-Sph\"aren-Operators $-\Delta{-}2$ (ohne die 3 Nullmoden) $=a_1{-}3=\tfrac73{-}3={-}\tfrac23={-}|\Z_2|/\Nfam$ exakt ($a_1{=}\tfrac1{\Nfam}{+}|\Z_2|$; numerische Kontinuation $\sim10^{-7}$) --- achter $2/3$-Auftritt, erster als \emph{spektrale} Anomalie; Konsequenz $\det'(cL){=}c^{\zeta(0)}\det'(L)$; R1-Rest $=$ \emph{ein} $\zeta(0)$-Budget-Statement (4d-Summe) \tI\,$+$\,\tP \\
\texttt{v133\_\allowbreak zeta\_\allowbreak budget.py} & $\zeta$-Budget, beide Routen exakt (euklid.\ Nariai $=S^2{\times}S^2$): reduzierte Route $-\tfrac23$ pro Sektor, total $-\tfrac43{=}{-}e_1/p_0$ ($=$ minus Seed-Gain --- zwei der drei Triad-Werte); naive 4d-Route $a_2{=}\tfrac{161}{45}$, $\zeta(0){=}{-}\tfrac{109}{45}$ (\emph{kein} Atom; Kontinuation $\sim10^{-8}$); Nullmoden $3{+}3$ $=$ Horizont-Paar $\Rightarrow$ Budget favorisiert strukturell die \emph{reduzierte} Seam-Lesart; Rest $=$ Graviton/Geist-Anteile (Volkov--Wipf-Klasse) \tI\,$+$\,\tP \\
\texttt{v134\_\allowbreak dual\_\allowbreak anchor.py} & Dual-Anchor-Lemma: $d{:=}a^\top R^{-1}{=}a^\top L^{-1}{=}(-\tfrac12,-\tfrac12,1)$, $d{\cdot}\mathbf 1{=}0$, $d{\cdot}a{=}1$, $(1,1,-2){=}{-}2d$ --- die inverse Flavor-Antwort \emph{ist} die Nariai-Wurzel (v101); Invarianz exakt via Sherman--Morrison ($\Leftrightarrow$ Orthogonalit\"at zu $R^{-1}\mathbf 1{=}\tfrac14(1,1,-1)$; Kontrollen: $\mathbf 1,e_1,n$ liegen \emph{nicht} auf der Ebene); $(d,n)$ $=$ duales Normalen-Paar der Flavor-Boundary; dritter rein \emph{algebraischer} Br\"ucken-Schenkel Flavor$\leftrightarrow$Horizont \tI\,$+$\,\tP \\
\texttt{v135\_\allowbreak det\_\allowbreak surface.py} & Determinanten-Fl\"ache: $\det M(s,t){=}3s(t{+}1)(t{+}2){+}t^2{+}5t{+}8$ --- zwei $s$-unabh\"angige Iso-Volumen-W\"ande $t{=}{-}2{\Rightarrow}2{=}|\Z_2|$, $t{=}{-}1{\Rightarrow}4{=}|\mu_4|$ ($\det K{=}4$ ist eine \emph{Schicht}); Winding-Linie $6s{+}8\Rightarrow(8,14,20)$, $\det F{=}32{=}2^{\gcar}$; $\det B(s,0){=}2\det M(s,0)$ exakt, $B$-Wand nur bei $t{=}{-}2$; Rowsum-Achsen $C{=}(7,11,13)$, $U{=}(3,3,3)$, $V{=}(1,2,3){=}\mathrm{Spec}(Q_+)$; Mikro-Identit\"aten $e_1^2{+}e_2^2{=}41{=}10b_1$, $p_1^2{+}p_2^2{=}52{=}$ Carrier-Coset-Wurzeln (v128) \tI\,$+$\,\tP \\
