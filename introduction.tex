...
physical TOE. The remaining strict gates are: the \emph{absolute} amplitude normalisation of the $R$ bridge
($U_{\mathrm{point}}$, an anchor --- the ratio $c_u/c_d$ itself is rigid, v49, and the selector stratum is now
\emph{derived}, v69/v70/v71), the ambient metric projective measure ($G_{\mathrm{metric}}$), and the named
QFT/cosmology transfer interfaces ($F_{\mathrm{frontier}}$).
\smallskip\\
\noindent\textbf{Two points that are \emph{not} in the residual (closed, but easy to mis-list as open).}
\emph{(i) Parameter-freeness is a theorem, not a hope:} the boundary transport is gapped
($\Delta=6\log\tfrac32>0$), so by Perron--Frobenius its iteration has a \emph{unique} attracting fixed point
--- the constants are \emph{selected}, not tuned (\tI/\tL, \veri{v56\_unique\_attractor.py}), reinforcing the
static bootstrap web ($\gcar=5$ forced three ways, \veri{v6\_bootstrap.py}, \veri{v14\_carrier\_uniqueness.py}).
...
\texttt{tfpt\_3}), and \emph{one} dimensionful scale (dimensional analysis; the induced-gravity anchor,
\veri{v68\_seeley\_dewitt\_residual.py}).
\end{keybox}
...
QFT/cosmology interfaces. The status card:
...
$(U)$ flavor gate & \tA & only the \emph{absolute} amplitude scale ($U_{\mathrm{point}}$); ratios are closed \\
...
ambient QG measure & \tA & strict-TOE completion target (G6); bounded IR unaffected, blocks strict-TOE cert.\ ($G_{\mathrm{metric}}$) \\
...
fine structure $\ainv$ & \tI & --- \textbf{now existence+uniqueness proved} (this round) \\
...
\texttt{v67\_\allowbreak area\_\allowbreak law\_\allowbreak coefficient.py} & Central-theorem \emph{structural closure}: Fursaev--Solodukhin $S{=}4\pi k A$, $k{=}\cthree/2$ $\Rightarrow$ $S{=}2\pi\cthree A{=}\tfrac14 A$ ($2\pi\cthree{=}\tfrac14$); $\cthree{=}\tfrac1{8\pi}$ is the \emph{unique} value with replica-coeff ${=}$ BH $\tfrac14$. Residual: one coeff $k{=}\cthree/2$ (Seeley--DeWitt) \tI/\tL\,$+$\,\tA \\
\texttt{v68\_\allowbreak seeley\_\allowbreak dewitt\_\allowbreak residual.py} & Residual \emph{resolved as an anchor}: $a_2{=}{-}\tfrac{d}{192\pi^2}R$ $\Rightarrow$ $\tfrac1{16\pi G}$ UV-sensitive ($\propto d\Lambda^2 f_2$), so $k{=}\cthree/2$ is a normalization not a derivation; cutoff-indep.\ $R^2$/scalaron derived; $1/G$ is the one anchor. Central theorem maximally closed \tI/\tL \\
