...
equivalence web in which bare mark-transitivity is automatic for every configuration and no local
...
discrete input (\vref{v521}, \texttt{SEAM.BIT.RPBLIND.01}, markers unchanged); and the
mark-decorated \emph{state} class (twist/defect insertions --- everything Wick-computable)
is since documented as the \emph{ninth}: the free-plus-twist class is exhausted, and only a
genuinely interacting algebra could still see the bit (\vref{v525},
\texttt{SEAM.BIT.TWISTBLIND.01}, markers unchanged).
...
reduction state (2026-07-22): the compiler's dimensionless inputs reduce to \emph{four marks} (derived
from P1-side topology), \emph{one discrete symmetry-lift bit} (flag transitivity $\iff\tau=i$;
equivalently the clock, the CM-fixed half-period, or the non-split extension class on the free seam
circle) and $\pi$ --- a reduction state, not an abolition of the axiom:
...
half is one discrete symmetry-lift bit (flag transitivity $\iff\tau=i$, \vref{v512}), so the
dimensionless compiler inputs read \emph{four marks $+$ one discrete symmetry-lift bit (flag
