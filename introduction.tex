...
\large The discrete compiler, its status, and how to read the document set ---\\
...
The theory tips from ``many sectors with surprising hits'' to ``\emph{one small discrete compiler
generates those sectors}'': $E_8$ is \emph{built} from two TFPT atoms and sits at the centre as the
audit hull; the flavor matrix, the fine-structure constant and the scale grammar follow as
