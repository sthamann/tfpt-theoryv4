...
dial left to tune}. Pushed to the end (\veri{v353\_selfloop\_capstone.py}), this means TFPT is best read not as a
\emph{linear} theory (axioms $\to$ theorems) but as a \emph{closed self-consistent loop} whose outputs fix its own
inputs ($\gcar{=}5$ is the largest prime of the output $h(E_8){=}30$; the $8$ in $\cthree$ is $\operatorname{rank}E_8$),
with a \emph{unique} fixed point (\vref{v6}/\vref{v56}) and \emph{zero} free adjustable dimensionless parameters.
Even $\pi$ is not a free dial: it is the fixed geometric constant of the boundary $2$-sphere
($\oint_{S^2}K=4\pi$), the one transcendental the continuous boundary contains. ``Which is the axiom?'' is the
wrong question for a loop --- every part is fixed by the whole; the one remaining structural question, whether the loop
also closes in the \emph{continuum} (the keystone \texttt{SEAM.EQUIV.01}), now carries two named routes (route
split 2026-07-22, an ID restructuring with \emph{no} status change in substance): its MMST route
\texttt{SEAM.EQUIV.MMST.01} is \emph{closed modulo cited
theorems} --- the lattice model and $S_3$ stack pin it at every computable level and it is Lean-formalised
(\texttt{FORM.SEAM.MMST.01}); only the cited continuum-existence theorems stay \mO\ (\vref{v367}/\vref{v368},
\vref{v376}--\vref{v379}, \vref{v336}), their extension leg since re-founded on peer-reviewed crossed-product
theorems with the realisation input reduced to invariant level (\vref{v469}) --- while its twistor route
\texttt{SEAM.EQUIV.TWISTOR.01} (the open Costello--Li construction, prepared by \texttt{CELEST.SEAM.01}) stays
\mO; the parent \texttt{SEAM.EQUIV.01} is closed if either route closes and, as an unconditional claim, stays \mO.
...
positive parts --- machine-checked with all its negative controls as the partition corollary
(\veri{v491\_p2\_partition\_corollary.py}, \texttt{P2.PARTITION.01}): the unique multiset $\{1,1,2\}$
forces $e=(4,5,2)$, hence $\gcar=e_2=5$, $|\Z_2|=e_3=2$, $\Nfam=3$ as \emph{corollaries} (dropping
positivity, the sum, the part count or integrality each destroys uniqueness --- Mehta--Seshadri-rational
weights give $e_2=\tfrac{16}3$ or $\tfrac{44}9\neq5$), sharpening \vref{v53} which needs all three targets.
The typing itself is now hardened (\veri{v499\_p2\_weights\_deligne\_bg.py}, \texttt{P2.TYPING.01}): given
the four marks, rank $3$, the cusp class and unitarity (U), the Deligne canonical extension of the flavor
connection has $\deg E=-4$ (residue traces) and Mehta--Seshadri stability forces $h^0(E)=0$, so
integrality (Birkhoff--Grothendieck), positivity and the sum $4$ are \emph{theorems} --- the residual
shrinks to the module identification and (U), both \mC; and that module-identification rest is now
\emph{one} residual with the QGEO modulus rest (the order-$4$ clock $=$ the Coxeter/cusp-class carrier
datum; \veri{v503\_qgeo\_emergence\_light.py}, \texttt{QGEO.EMERGE.LIGHT.01}, no marker moves).
That common residual is itself now reduced to \emph{one alignment bit} --- the collar deck central in
the mark-$D_4$; the clock's \emph{order} is fermionically forced, $U^2=(-1)^F$ exactly
(\veri{v506\_seam\_clock\_rigidity.py}, \texttt{SEAM.CLOCK.RIGIDITY.01}, markers unchanged); and the
bit is no tautology --- the deck's \emph{class} is derived (every mark-free collar deck is a
half-period translation of the seam double), only its \emph{position} stays the \mC\ input
(\veri{v507\_seam\_bit\_origin.py}, \texttt{SEAM.BIT.ORIGIN.01}, markers unchanged); and that
position half is now \emph{topology} --- the covering deck is free on the seam circle, the
edge class is excluded, and the bit reduces to the square-modulus datum $\tau=i$ alone
(\veri{v510\_seam\_bit\_freedom.py}, \texttt{SEAM.BIT.FREEDOM.01}, markers unchanged); and that
modulus datum now carries an equivalent \emph{discrete} form --- flag transitivity of the four marks
(marks \emph{and} their two sides indistinguishable, $V_4\to D_4$) $\iff\tau=i$, a $13$-fold exact
equivalence web (since $15$-fold by \vref{v528} facets \#14/\#15) in which bare mark-transitivity is
automatic for every configuration and no local
jet sees the side bit (\veri{v512\_seam\_tau\_flag.py}, \texttt{SEAM.TAU.FLAG.01}, markers
unchanged); free OS positivity is since documented as the \emph{eighth} side-blind test on that
scoreboard --- the bit is not derivable from free RP/$\Theta$ existence and remains genuine
discrete input (\vref{v521}, \texttt{SEAM.BIT.RPBLIND.01}, markers unchanged); the
mark-decorated \emph{state} class (twist/defect insertions --- everything Wick-computable)
is since documented as the \emph{ninth}: the free-plus-twist class is exhausted
(\vref{v525}, \texttt{SEAM.BIT.TWISTBLIND.01}, markers unchanged); and the first genuinely
interacting Fidkowski--Kitaev seam toy is the \emph{tenth} side-blind test ($9\to10$), while
Kill-Test 2 fires at toy level under a typed fence (\vref{v529}, \texttt{SEAM.INT.FKTOY.01},
markers unchanged); the straddle filter it produced, since run as a \emph{selector} on the
interacting mark family, keeps exactly \emph{one} member alive --- reflection positivity
dynamically selects $\delta=\pi/2$ with positive coupling, the same member that carries the
twist-class order parameter: the first \emph{positive} selection datum for the bit
(\vref{v534}, \texttt{SEAM.STRADDLE.CONE.01}, markers unchanged; toy-level evidence,
not a derivation). The bit itself is now the \emph{twist-class choice}, in principle
interferometrically readable (\vref{v528}, \texttt{SEAM.BIT.TWISTCLASS.01}; it remains formal
input). Temperature joins geometry and anomaly as the \emph{third leg} of $\cthree$
(\vref{v526}, \texttt{SEAM.THERMAL.KMS.01}, \mC-typed reading).
The genuine remaining axiom is therefore not ``why $a=(1,1,2)$'' but ``why the carrier
$\gcar=5$ / $(|\mu_4|,\Nfam)=(4,3)$'' --- that interface (with $\cthree$), i.e.\ the weight-\emph{typing}
postulate itself, is the \mO\ input layer (\texttt{AX.P2.01} stays a declared axiom). The consolidated
reduction state (2026-07-23): the compiler's dimensionless inputs reduce to \emph{four marks} (derived
from P1-side topology), \emph{one discrete symmetry-lift bit} --- the twist-class choice (flag
transitivity $\iff\tau=i$; equivalently the clock, the CM-fixed half-period, or the non-split
extension class on the free seam circle) --- and $\pi$ --- a reduction state, not an abolition of the axiom:
\texttt{AX.P2.01} remains declared, and the \mC\
identifications R1/R2 of \vref{v499} remain conditional.
\par\smallskip
...
is the keystone \texttt{SEAM.EQUIV.01}, whose conformal-deck face \texttt{QGEO.SYM.01} (the carrier $\mu_4$
clock is the seam's conformal deck, see
\texttt{tfpt\_research\_contracts}) forces the $\mu_4$ deck, hence $\Nfam=3$, $\deg E=-4$ and the
...
\end{keyeq}
TFPT~v\TFPTversion\ closes the discrete compiler, the algebraic Standard-Model readout, the EM fixed point, the
...
shared by flavor and gravity --- the boundary net identification (now \emph{closed modulo cited theorems} on its
MMST route \texttt{SEAM.EQUIV.MMST.01},
\vref{v367}/\vref{v368}, \vref{v376}--\vref{v379}, Lean \texttt{FORM.SEAM.MMST.01}; the parent
\texttt{SEAM.EQUIV.01} stays \mO), and the named QFT/cosmology transfer
...
\subsection*{The hard reduction: the live residual as typed interfaces}
\noindent After the compiler closure, the entire residual is
\begin{keyeq}
$\displaystyle \text{Rest}=v_{\mathrm{geo}}\ \oplus\ G_{\mathrm{net}}\ \oplus\ F_{\mathrm{transfer}}$
\end{keyeq}
--- one dimensionful scale anchor, one metric-sector inclusion, and a set of deliberately typed
QFT/cosmology interfaces (the historical gate labels $(U_{\mathrm{wall}})/(G_{\mathrm{metric}})/(F_{\mathrm{frontier}})$ are
kept only for ledger continuity). In current terms: $(U_{\mathrm{wall}})\to v_{\mathrm{geo}}$ --- the flavor ratios are
...
(\texttt{CONTRACT.F.01}, \vref{v213}) and \emph{one shape} --- each is a gapped relaxation to a unique attractor
(the main-branch discrete$\to$dynamic motif), with F\_pole at the \emph{seam} rate $(2/3)^6$ and the other three
external (\vref{v303}). The metric-sector reduction lands on a \emph{single named keystone}. The boundary inclusion
$G_{\mathrm{net}}$ is the index-$4$ $\mu_4$ simple-current extension
$(D_5)_1{\otimes}(A_3)_1{\rtimes}\langle(1,1)\rangle\cong(E_8)_1$ (Simple-Current Extension Theorem,
\vref{v154}, exact: index $4$, $c{=}8$, $\mu{=}1\Rightarrow$ holomorphic $\Rightarrow E_8$); net
existence and full-cone reflection positivity are discharged to \mE\ (\vref{v175}); and the
\textbf{No-Unit Theorem} (\vref{v153}) makes $v_{\mathrm{geo}}$ an irreducible metrology primitive
($U_{\mathrm{point}}\sim v_{\mathrm{geo}}$, $1/G\sim v_{\mathrm{geo}}^2$, $m/\mu=e^{3/4}$ --- one unit in
three dimensions). The flavor selector and Q-geometry residues are \emph{derived} (no discrete freedom
left). The \emph{one} seam-side identification, packaged as the single named
theorem \texttt{SEAM.EQUIV.01} --- \emph{the raw reflection-positive seam state is the holomorphic
$(E_8)_1$ net at $\tau{=}i$} --- is now \emph{closed modulo cited theorems on its MMST route}
(\texttt{SEAM.EQUIV.MMST.01}; the twistor route \texttt{SEAM.EQUIV.TWISTOR.01} and the unconditional parent stay
\mO): the explicit lattice model
(\vref{v367}/\vref{v368}) and the $S_3$ stack ($c{=}8$, $248$ currents/$1$ primary, torus $\mathrm{GSD}{=}1$, RP;
\vref{v376}--\vref{v379}) pin it at every computable level, Lean-formalised as \texttt{FORM.SEAM.MMST.01}, with only
the cited MMST/OS continuum-existence theorems left \mO\ (\vref{v336}); its conformal-deck premise
\texttt{QGEO.SYM.01} is its \emph{downstream} definitional face (per \vref{v323}; the residual is Lean-pinned to named
cited steps, \vref{v308}). The
sprint-by-sprint reduction that produced it is recorded in the changelog and in
\texttt{tfpt\_research\_contracts}, not replayed here.
The status card:
...
flavor absolute scale & \mE/\mO & ratios \emph{closed} (\vref{v49}/\vref{v71}); only the absolute scale remains $=v_{\mathrm{geo}}$ (Gate~1, \vref{v75}), the same anchor as $1/G$ \\
$R+R^2$ gravity & \mE/\mC & covariant $f(R)$ field equation closed in regime; heat-kernel grounded (G2) \\
Einstein eq.\ (covariant, local) & \mE/\mC & fixed-volume entanglement equilibrium $\Rightarrow$ \emph{full} $G_{ab}{+}\Lambda g_{ab}{=}\cthree^{-1}T_{ab}$ with BOTH coefficients parameter-free: $8\pi{=}1/\cthree$ (\vref{v358}, thermo$=$geo via $|\mu_4|{=}|\Z_2|\chi{=}4$) and $\Lambda$ from $\alpha$ (\vref{v60}); Einstein tensor via Lovelock, conservation an output (\vref{v359}); residual $=$ equation-of-state status $+$ $v_{\mathrm{geo}}$, global measure (C7) now a \mC\ redundancy (\vref{v369}) \\
physical QG sector & \mE/\mC & gap-decoupled admissible measure ($\Delta_{\mathrm{eff}}{=}1.648{>}0$) \\
ambient QG measure & \mC\ (redundancy) & \textbf{certification object, not missing dynamics} (\vref{v369}; conditional on \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann): holographically reduced to a finite seam-\emph{boundary} measure (\vref{v76}); IR tier closed (decoupling); strict-TOE cert.\ $=$ the simple-current extension $(D_5)_1{\otimes}(A_3)_1{\rtimes}\langle(1,1)\rangle\cong(E_8)_1$ (\vref{v154}); the free-bulk premise (A) is itself a fixed-point theorem with the cone eliminated (cone gap $=$ one-particle gap $(2/3)^6$) and factors into the $A_2$ net-existence $+$ the keystone \texttt{SEAM.EQUIV.01} (MMST route closed mod.\ cited; parent \mO) --- zero new gates (\vref{v160}--\vref{v165}) \\
...
Koide & \mC & $Q_{\mathrm{src}}+\varphi_0/24$ source$\to$pole: typed runnable solver with kill test (QED-excluded, \vref{v371}) \\
axion DM & \mC & $f_a=M_{\mathrm{scal}}/128$: finite-$T$ spine-branch typed runnable solver (\vref{v373}) \\
$\eta_B$ & \mC & readout closed; integrated BDP leptogenesis Boltzmann ODE, typed runnable solver (\vref{v372}) \\
$m_p/m_e$ & \mC & cross-sector QCD/EW ratio: typed runnable solver with uncertainty budget (\vref{v374}) \\
...
sheet/parity (Q-geometry derived) & \mE/\mC & $H^1$ characters $=$ $\operatorname{Spec}(Q_+)$ $=$ $A_3$ exponents (\vref{v137}); $\Z_3$ choice \emph{derived} (\vref{v141}); full M\"obius $D_4$ matched parity by parity (\vref{v146}); residue $=$ the module identification, folded into \texttt{SEAM.EQUIV.01} \\
...
\end{center}
\markerkey
\noindent Card addendum (2026-07-22): the $a=(1,1,2)$ residual now carries the consolidated P2
reduction state --- the position half of the alignment bit is topology (\vref{v510}) and the modulus
half is one discrete symmetry-lift bit --- the twist-class choice (flag transitivity
$\iff\tau=i$, \vref{v512}/\vref{v528}), so the
dimensionless compiler inputs read \emph{four marks $+$ one discrete symmetry-lift bit (the
twist-class choice, in principle interferometrically readable; flag
transitivity $\iff\tau=i$) $+$ $\pi$}
(\texttt{AX.P2.01} stays a declared axiom).
\par\smallskip
\noindent The former structural open core, the \emph{single} named keystone \texttt{SEAM.EQUIV.01} (the
$G_{\mathrm{net}}$ inclusion: the raw RP seam $=(E_8)_1$ at $\tau{=}i$), is now \emph{closed modulo cited theorems
on its MMST route} (\texttt{SEAM.EQUIV.MMST.01}; the twistor route \texttt{SEAM.EQUIV.TWISTOR.01} and the
unconditional parent stay \mO):
the explicit lattice model (\vref{v367}/\vref{v368}) and the $S_3$ stack ($c{=}8$, $248$ currents/$1$ primary, torus
$\mathrm{GSD}{=}1$, RP; \vref{v376}--\vref{v379}) pin it at every computable level, it is Lean-formalised as
\texttt{FORM.SEAM.MMST.01}, and only the cited MMST/OS continuum-existence theorems stay \mO\ (\vref{v336}); its
conformal-deck face \texttt{QGEO.SYM.01} is a \emph{corollary} (a conformal net's vacuum is rotation-invariant by axiom,
\vref{v323}/\veri{v335\_seam\_equiv\_unify.py}), not a second item. $v_{\mathrm{geo}}$ is an irreducible metrology
unit and the frontier items are typed interfaces, not structural holes. $G_{\mathrm{net}}$ is heat-kernel
grounded (G2: the spectral action gives $R+R^2$ structurally) and \emph{gap-decoupled} (G5:
$\Delta_{\mathrm{eff}}{=}1.648{>}0$ isolates the closed admissible sector from the ambient), so the
ambient measure \texttt{QG.AMB.01} is no longer carried as an open structural item: it is a \mC\ \emph{redundancy}
(\vref{v369}, conditional on \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann), a certification object rather than
missing dynamics --- it does not affect any TFPT readout. Both are written up
as numbered \emph{research contracts} (lemma chains $+$ closing theorem $+$ machine-certifiability) in the
companion \texttt{tfpt\_research\_contracts}, with priority on the $v_{\mathrm{geo}}$ scale anchor and the
$G_{\mathrm{net}}$/\texttt{SEAM.EQUIV.01} inclusion theorem.
...
condensation (the Kitaev $E_8$ state); the one analytic step ``the free RP seam condenses the
order-$|\mu_4|$ Lagrangian glue'' $=$ the single named keystone \texttt{SEAM.EQUIV.01} (with
\texttt{QGEO.SYM.01} its downstream conformal-deck face) is now \emph{closed modulo cited theorems} on its MMST
route \texttt{SEAM.EQUIV.MMST.01}
(\vref{v367}/\vref{v368}, \vref{v376}--\vref{v379}, Lean \texttt{FORM.SEAM.MMST.01}; cited continuum existence \mO,
\vref{v336}; the parent \texttt{SEAM.EQUIV.01} stays \mO). So beneath the gate \emph{labels} there is,
...
the boundary QFT is closed as a single relative object modulo the keystone \texttt{SEAM.EQUIV.01} and adds
\emph{no new open item}, while the ambient quantum-gravity measure is a \mC\ redundancy (\vref{v369}), not a separate
structural gap.
...
\item \textbf{a network reading (computational substrate).} On the same McKay $(2,3,5)$ graph the affine-$E_8$
      marks are the \emph{unique dynamical attractor} of a minimal local-averaging update
      $m\!\leftarrow\!(A{+}2I)/4\cdot m$ (it converges from \emph{any} positive start to $(1,2,3,4,5,6,4,2,3)$);
      the spherical tower terminates at $E_8$ and the cascade $E_8\!\supset\!E_7\!\supset\!E_6\!\dots$ is
      nested-subgraph coarse-graining. So $E_8$ reads as the \emph{universal attractor of a minimal $(2,3,5)$
      network} (a Wolfram-style computational-substrate picture) at the Coxeter-skeleton level. \mC\ A minimal
      hypergraph \emph{rewrite} reproducing the \emph{full} structure (lattice $+$ recovery $+$ readouts) stays
      open, and $\phiz$ is the analytic seed $\tfrac{4}{3}c_3=1/(6\pi)$, not an edge fraction. \mO\
      \veri{v298\_e8\_network\_attractor.py}
\item \textbf{the bridge, hardened.} A local witness-based growth dynamics (a \emph{diffusion-generated} witness
      $+$ the local balance gate $A m\le2m$, Collatz--Wielandt $=$ Smith's $\rho\le2$; the Perron eigenvector
      enters only as an audit) \emph{reaches} $E_8$ as the last finite point ($E_6\!\to\!E_7\!\to\!E_8$, then
      stops at the affine boundary $\hat E_8$) --- but \emph{only} from a 3-arm $(2,3,\cdot)$ seed (the controls
      $\rho(A_n)=2\cos\frac{\pi}{n+1}$ and $\rho(D_n)=2\cos\frac{\pi}{2n-2}$ are $<2$ for \emph{all} $n$, so
      neither family ever reaches $E_8$). So $P_2$ is exactly the irreducible seed on which the rule reaches
      $E_8$ as the last finite point --- a second network reading of the $(2,3,5)$ hull, not a derivation of
      $P_2$. \mO\ \veri{v299\_hypergraph\_0d\_autonomous.py}
\item \textbf{what a rewrite must inject.} Testing \emph{which} load-bearing quantities are graph-spectral
      sharpens the open question: \mE\ the Kac marks are the Perron eigenvector ($A\,m{=}2m$) and the subleading
      eigenvalue is exactly $2\cos(\pi/5)=$ the golden ratio (the $5$-fold/icosahedral signature), but \mE\ the
      recovery rate $(2/3)^6$ is \emph{not} any eigenvalue of the adjacency and $\phiz$ is not a rational edge
      fraction. The cusp weight $2/3{=}|\Z_2|/\Nfam$ is now \emph{derived} from a minimal branching rule
      (\veri{v327\_hypergraph\_rewrite.py}); what remains non-graph-spectral for a \emph{full}-structure rewrite
      at leading order is sharpened by \veri{v396\_phi0\_icosahedral.py} (tree-level $\varphi_0^{\mathrm{tree}}=F/(4h\pi)$);
      the puncture $48c_3^4$ alone stays open. \mO\ \veri{v312\_hypergraph\_substrate.py}
\item \textbf{one coupled local rule (carrier $\times$ family).} On a $9\times3$ labelled grid (affine-$E_8$
      nodes $\times$ family channels) a \emph{single} coupled micro-step --- network lazy diffusion on each cusp
      column plus the \veri{v327\_hypergraph\_rewrite.py} branching rule $M$ on each node's family vector --- has joint attractor
      marks$\otimes(w{=}0)$ and recovery rate $(2/3)^6$ after one clock hand ($2\Nfam$ family steps); the
      \veri{v324\_hypergraph\_fiber.py} fiber product $T_{\mathrm{net}}\otimes T_{\mathrm{cusp}}$ emerges as the cusp-readout basis,
      and the \veri{v299\_hypergraph\_0d\_autonomous.py} growth trajectory $E_6\!\to\!E_7\!\to\!E_8\!\to\!\hat E_8$ is unchanged with the
      fiber attached. \mE\ \veri{v395\_hypergraph\_coupled.py}
\item \textbf{the seed at leading order.} The tree-level retained seed
      $\varphi_0^{\mathrm{tree}}=1/(6\pi)$ is exactly $F/(4h\pi)$ on the icosahedral hypergraph
      ($F=20$ triangular hyperedges, $h=30$, $|\Aut|=120$): equivalently $(F/(\gcar\Nfam))c_3$, with
      $F/h=|\Z_2|/\Nfam=2/3$ the same survival ratio as \veri{v327\_hypergraph\_rewrite.py}; Gauss--Bonnet
      consistent with $c_3=1/(|\Z_2|4\pi)$. The puncture $48c_3^4$ remains analytic, not a graph
      fraction. \mC\ \veri{v396\_phi0\_icosahedral.py}
\end{itemize}
...
reducing to the single dimensionful anchor $v_{\mathrm{geo}}$.
...
\item \textbf{Flavor residual --- ratios closed; only the absolute scale is the wall-selection.}
      ...
      Contract 1). \mE/\mO
\item \textbf{Gravity --- the local field equation is parameter-free.} The metric-sector \emph{field
      equation} is supplied by the entanglement first law $\delta S=\delta\langle K\rangle$ at fixed
      \emph{volume} (Jacobson 2015): equilibrium forces the \emph{full} covariant Einstein equation
      $G_{ab}{+}\Lambda g_{ab}=\cthree^{-1}T_{ab}$ with \emph{both} coefficients fixed --- $8\pi{=}1/\cthree$
      (\vref{v358}) and $\Lambda$ from $\alpha$, $\rho_\Lambda{=}(3/4\pi^2)e^{-2\alpha^{-1}}$ (\vref{v60}) ---
      the Einstein tensor (not Ricci) forced by Lovelock, so $\nabla^aT_{ab}{=}0$ is an \emph{output}
      (\vref{v359}). The $R{+}R^2$ scalaron is the \emph{action}/inflation layer (the next-order term of the
      same equilibrium route, \vref{v360}), and $v_{\mathrm{geo}}$ is the theory's single dimensionful input
      (\vref{v364}). What remains is \emph{not} the field equation but the equation-of-state status (Jacobson
      framing, not a from-action quantisation; an \emph{external} candidate action --- Bianconi's entropic
      action, arXiv:2408.14391 --- is quantified in \vref{v473} without changing this typing) and the one
      unit $v_{\mathrm{geo}}$; the ambient measure
      \texttt{QG.AMB.01} is now a \mC\ redundancy (\vref{v369}, conditional on
      \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann), not missing dynamics, and \emph{perturbative} graviton
      unitarity is established --- the Stelle ghost is a Seeley--DeWitt truncation artefact (\vref{v304}), the
      spin-$2$ sector ghost-free via the entire KMS form factor (\vref{v370}) with the truncation tower decoupling
      (\vref{v380}). \mE\ (local field equation) / \mC\ (perturbative unitarity, ambient redundancy)
...
\item \textbf{Full dynamics --- the admissible transfer model is closed, and the boundary QFT is now one
      ...
      closed modulo the single keystone \texttt{SEAM.EQUIV.01}, whose MMST route \texttt{SEAM.EQUIV.MMST.01} is
      \emph{closed modulo cited theorems}
      (lattice $+$ $S_3$ stack, Lean \texttt{FORM.SEAM.MMST.01}; \vref{v367}/\vref{v368}, \vref{v376}--\vref{v379};
      the unconditional parent stays \mO).
      \emph{What this does \emph{not} fully close:} the full
      measured pole masses (the absolute scale $v_{\mathrm{geo}}$ $+$ standard RG), the absolute QCD scales
      ($m_p/m_e$, now a typed \mC\ transfer, \vref{v374}), the complete PMNS dynamics (the angles are typed; the full
      mass/CP dynamics stay \mC/\mO), and the real $4$D interacting QFT --- the continuum limit, whose cited
      existence theorems stay \mO\ --- with the ambient measure a \mC\ redundancy (\vref{v369}). \mE\ (transfer model
      $+$ boundary relative object) / \mC\ (continuum modulo cited, transfers, ambient redundancy)
\item \textbf{Axioms P1/P2 --- P2 machine-verified.} The Lean proof builds cleanly (\texttt{3357 jobs},
...
\midrule
boundary QFT (Modular Spectral Closure) & \mE/\mC & \textbf{one relative object} $\mathsf{TFPT}_{\mathrm{QFT}}$: $D_F=$ covariance induction (\vref{v258}), cutoff $=$ KMS weight $f_2/f_0{=}1$ (\vref{v259}), seam/carrier/$E_8$ on one K3 (\vref{v260}), certified (\vref{v261}) --- closed modulo \texttt{SEAM.EQUIV.01}, whose MMST route \texttt{SEAM.EQUIV.MMST.01} is closed modulo cited theorems (\vref{v376}--\vref{v379}; parent \mO) \\
perturbative $4$D S-matrix ($S_{\mathrm{pert}}$) & \mE/\mC & \textbf{Epstein--Glaser-constructible} (\vref{v269}): one-loop quartic (\vref{v271}) $+$ gauge $\beta=(41/10,-19/6,-7)$ (\vref{v273}), $S_{\mathrm{pert}}\!\xrightarrow{\mathrm{LSZ}}\!S_{\mathrm{phys}}$ with one-loop unitarity (optical theorem, \vref{v278}); the spin-$2$ graviton sector is perturbatively unitary too --- the Stelle ghost is a Seeley--DeWitt truncation artefact (\vref{v304}/\vref{v370}/\vref{v380}) \\
\midrule
\multicolumn{3}{@{}l}{\textit{the actual TOE remainder --- dimensionful units, typed transfers, and the cited-theorem ceiling on the seam}}\\
QFT measure (admissible sector) & \mE & \textbf{closed: RP from cusp spectrum, OS reconstruction} \\
full measured pole masses & \mC & ladder $\times\,v_{\mathrm{geo}}$, dressed by standard RG (no new physics); $v_{\mathrm{geo}}$ over-determined (\vref{v274}) \\
absolute QCD scales ($m_p/m_e$, $\Lambda_{\mathrm{QCD}}$) & \mC & typed runnable transfer with uncertainty budget (\vref{v374}, contract \texttt{FR.MPME.01}); not a compiler number \\
full PMNS dynamics (mass scale, CP) & \mC/\mO & angles typed; CP strength $J_{\mathrm{PMNS}}{=}{-}0.0297$ derived (\vref{v270}); absolute $\nu$-scale $=$ one seesaw ratio (\vref{v272}); three named post-hoc \mO\ candidates: third-gen ladder-base (\vref{v467}), $\Delta m^2$ ratio $=|J|$ (\vref{v468}), carrier-normalised one-parameter seesaw (\vref{v481}) --- record unchanged \\
\textbf{(1)} \texttt{SEAM.EQUIV.01} --- one named theorem (\emph{raw seam $=(E_8)_1$ at $\tau{=}i$}) & \mO/\mC & \textbf{route split 2026-07-22: closed if MMST or TWISTOR route closes. MMST route \texttt{SEAM.EQUIV.MMST.01} closed modulo cited theorems}: lattice model (\vref{v367}/\vref{v368}) $+$ $S_3$ stack (\vref{v376}--\vref{v379}) pin it at every computable level, Lean \texttt{FORM.SEAM.MMST.01}; only residual \mO\ $=$ the cited continuum existence (\vref{v336}); extension leg certified (\vref{v469}). Twistor route \texttt{SEAM.EQUIV.TWISTOR.01} (Costello--Li, via \texttt{CELEST.SEAM.01}) \mO\ $\Rightarrow$ parent stays \mO\ as an unconditional claim \\
\textbf{(2)} \texttt{QG.AMB.01} --- ambient measure (\emph{redundancy, not missing dynamics}) & \mC & \textbf{discharged as a redundancy} (\vref{v369}; \vref{v379} RP$\to$OS), conditional on \texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann: a certification object, gap-decoupled (margin $1.648{>}0$, \vref{v330}/\vref{v335}) so no readout depends on it; obstruction characterised (\vref{v332}) $+$ conditionally resolved by GHP$+$IDG (\vref{v334}); Tier-B's holomorphy bit ($\det K{=}1$, \vref{v277}) is absorbed into \texttt{SEAM.EQUIV.01} \\
...
$\eta_B$, $m_p/m_e$, Koide, dark matter & \mC & typed runnable transfer solvers with kill tests (\vref{v371}--\vref{v374}, observatory CI \vref{v375}; see document~4) \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
the single keystone \texttt{SEAM.EQUIV.01}. \emph{What that closure does \emph{not} reach} is exactly the
dimensionful/ambient remainder: the full measured pole masses (ladder $\times\,v_{\mathrm{geo}}$ $+$ standard
RG, \mC), the absolute QCD scales ($m_p/m_e$, now a typed \mC\ transfer, \vref{v374}), the complete PMNS dynamics (\mC/\mO), and the
nonperturbative \emph{ambient} measure \texttt{QG.AMB.01}, now itself a \mC\ redundancy (\vref{v369}). (The
metric-sector \emph{field equation} is parameter-free at the local level --- the full covariant
$G_{ab}{+}\Lambda g_{ab}{=}\cthree^{-1}T_{ab}$ from fixed-volume entanglement equilibrium, \vref{v358}/\vref{v359}
--- and the ambient \emph{measure} is no longer an open structural item but a certification object, conditional on
\texttt{SEAM.EQUIV.01}${+}$Bisognano--Wichmann.) The
\emph{perturbative} $4$D QFT is now built: the spectral-action S-matrix is Epstein--Glaser-constructible and
LSZ-bridged to the physical asymptotic S-matrix with one-loop unitarity for the matter$+$gauge sector
(\vref{v269}--\vref{v278}), and the spin-$2$ graviton sector is perturbatively unitary too --- the $R^2/$Weyl$^2$
Stelle ghost is a Seeley--DeWitt \emph{truncation artefact} (\vref{v304}), ghost-free via the entire KMS form factor
and the Barnes--Rivers decomposition (\vref{v370}) with the truncation tower decoupling (\vref{v380}); the whole
bedrock reduces to one geometric postulate, \emph{the raw seam is the flat $\tau{=}i$ pillowcase} (\vref{v276}), with
Tier-B's holomorphy bit $\det K{=}1$ (\vref{v277}) absorbed into \texttt{SEAM.EQUIV.01}.
The former single open lemma, the named theorem \texttt{SEAM.EQUIV.01} --- \emph{the raw reflection-positive
seam state is the holomorphic $(E_8)_1$ net at $\tau{=}i$} --- is now \emph{closed modulo cited theorems on its
MMST route} \texttt{SEAM.EQUIV.MMST.01} (the twistor route \texttt{SEAM.EQUIV.TWISTOR.01} and the unconditional
parent stay \mO). (The
flat $\tau{=}i$ geometry and the $E_8$ holomorphy are \emph{two faces of one object}: the $(E_8)_1$ character is
$\chi_{E_8}{=}j^{1/3}$, so $\chi_{E_8}(i){=}1728^{1/3}{=}12$ and $\tau{=}i$ is at once the order-$4$ CM point and the
$(E_8)_1$ partition-function modulus, \vref{v282}.) The explicit lattice model (\vref{v367}/\vref{v368}) and the
$S_3$ stack ($c{=}8$, $248$ currents/$1$ primary, torus $\mathrm{GSD}{=}1$, RP; \vref{v376}--\vref{v379}) pin it at
every computable level, and it is Lean-formalised as \texttt{FORM.SEAM.MMST.01} (collar hypotheses kernel-proved,
the MMST scaling-limit and Adamo--Moriwaki--Tanimoto OS-reconstruction theorems as named cited axioms, clean
\texttt{\#print axioms}); the \emph{only} residual that stays \mO\ is the abstract continuum existence of the scaling
limit (\vref{v336}) --- its $128$-spinor extension leg since re-founded on the peer-reviewed crossed-product package
($h_s{=}16/16{=}1{\in}\Z$ is the Longo--Rehren locality criterion; KLM $\mu{=}4/2^2{=}1$; AGT/AMT the second
witness), the realisation input reduced to invariants (R1$'$; computed $|C|{=}1$, $\nu{=}16$), Lean parallel route
\texttt{seamResidualClosed$'$} (\vref{v469}) --- so it is closed \emph{modulo a cited theorem},
not solved, Lean-pinned to exactly those named steps plus the derived Recovery gap
$\Delta=6\ln(3/2)\approx2.43>0$ (\vref{v308}); the conformal-deck premise
\texttt{QGEO.SYM.01} is its \emph{downstream} definitional face (\vref{v323}). This is by design the
\emph{one} irreducible structural input of the theory --- the role the \emph{constancy of $c$} plays in
relativity. It is not a defect to be removed but the single, sharply-localised, falsifiable premise on which
the whole boundary closure turns; with the lattice/$S_3$ pinning and the Lean formalisation in place,
``closing the theory'' has reduced to invoking exactly the two cited continuum-existence theorems, not an
open-ended search. The full sprint-by-sprint reduction that pinned
\texttt{SEAM.EQUIV.01} is recorded in the changelog, not replayed here. The seam seed is \emph{hardened}:
...
single irreducible primitive. What remains for a full TOE is then the cited continuum-existence theorem on
the seam and the one unit $v_{\mathrm{geo}}$, plus the standard-RG
dimensionful masses and the typed frontier transfers (the ambient QG measure being a \mC\ redundancy, \vref{v369}). No load-bearing discrete parameter remains free, and
...
% =====================================================================
\section{TFPT in the light of algorithmic physics: Zuse--Schmidhuber--Wolfram}
% =====================================================================
\noindent The ``compiler'' framing places TFPT in an older lineage --- the idea that
physics is, at bottom, the output of a short computation --- and it is worth stating
precisely what TFPT shares with that lineage and where it deliberately parts company, so
the family resemblance is not mistaken for a claim the theory does not make. Three
landmarks fix the coordinates. \textbf{Zuse} (\emph{Rechnender Raum}, 1969) proposed that
the universe is literally a (cellular-automaton) computation. \textbf{Schmidhuber}
sharpened this into an \emph{algorithmic} theory of everything (1997, 2000): the shortest
program that outputs a universe is the natural prior --- a physics reading of Kolmogorov
complexity and Solomonoff induction --- while his \emph{Speed Prior} (2002) adds that a
computed universe should be one that is \emph{fast} to compute, and his
low-complexity-art programme (1997) identifies beauty with compressibility.
\textbf{Wolfram} (\emph{A New Kind of Science}, 2002; the Physics Project) recasts the
same instinct combinatorially: space--time as a hypergraph updated by simple rewrite
rules.

\subsection*{Three shared instincts}
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,topsep=2pt]
\item \textbf{Minimal description length as the selection principle.} ``No load-bearing
      free number, only $\pi$'' is, in these terms, a Kolmogorov-style compression
      statement --- the plausibility discussion below already names it
      \emph{Kolmogorov-like compression: many numbers from little} --- and the
      grammar-internal null model of the next section turns it from rhetoric into a
      measured bound.
\item \textbf{Deterministic, resource-bounded generation.} The readouts fall out of a
      chain of exact identities in about a second, not a search over a model space ---
      the same spirit as the Speed Prior's preference for \emph{fast}-computable worlds,
      though TFPT makes no claim to instantiate a speed prior.
\item \textbf{Compressibility as the aesthetic.} ``Choosing $E_8$ is choosing the
      icosahedron'' selects the maximally symmetric, shortest-to-describe object; the
      affine Kac marks $(1,2,3,4,5,6,4,2,3)$ carry a fourfold identity (representation
      dimensions $=$ network attractor $=$ spectral certificate $=$ local balance
      weight). This is Schmidhuber's \emph{beauty $=$ compressibility}, realised on the
      Standard-Model alphabet rather than on pixels.
\end{enumerate}

\subsection*{Three deliberate divergences}
\begin{enumerate}[leftmargin=1.6em,itemsep=2pt,topsep=2pt,label=\textbf{(\Alph*)}]
\item \textbf{One forced program, not an ensemble.} Schmidhuber's shortest program
      computes \emph{all} universes and privileges none; TFPT bets the opposite --- an
      over-determined bootstrap fixed point that \emph{forces} a single dimensionless
      skeleton ($g_{\mathrm{car}}{=}5$ fixed three independent ways). TFPT is a candidate
      for \emph{the} short program of our constants, not a measure over programs.
\item \textbf{The description language is algebraic.} TFPT's compression is measured in a
      Lie/lattice/VOA language, not in Turing bit-strings; Kolmogorov complexity is
      language-dependent up to an additive constant, so the honest question --- is the
      shortness real, or hidden in the $E_8$ machinery? --- is exactly what the null model
      and the ``forced three ways'' bootstrap are built to answer. A Wolfram-style rewrite
      reading sharpens the same point rather than dissolving it: the $(2,3,5)$ seed is the
      terminal of a minimal $T(2,3,r)$ growth rule (Cartan determinants $3,2,1,0,-1$ for
      $E_6,E_7,E_8,\hat E_8,E_{10}$), so the rewrite picture \emph{re-formulates}
      axiom~P2 rather than removing it --- both pictures bottom out on the same single
      irreducible seed.
\item \textbf{Falsifiability --- the one thing TFPT adds.} As \emph{physics}, the
      Zuse/Schmidhuber algorithmic TOE and Wolfram's rule search are essentially
      unfalsifiable: no dated measurement can kill them. TFPT freezes dated, kill-tested
      predictions (the registry \texttt{predictions\_frozen.json}, the predictions section
      below) and so puts the computational-universe intuition at genuine empirical risk.
      That, not the compression rhetoric, is the load-bearing difference.
\end{enumerate}

\noindent\emph{Status.} This section is \emph{positioning}, not a new result: it
introduces no formula and moves no status marker. Its machine-checked hooks already live
in the suite --- the grammar-internal null-model bound and the icosahedral/McKay bedrock
with its network attractor; the Wolfram-style rewrite correspondence is worked out
separately in the repository's hypergraph note.
...
\input{tex-artefacts/predictions}
...
\caption{The development timeline of the verification suite --- the phases of the journey and what
...
interfaces, the QGEO pillowcase and Sheet Diamond, the \emph{icosahedral capstone} (the single
$(2,3,5)$ Brieskorn singularity generates the whole skeleton), the Modular Spectral Closure, the
\texttt{SEAM.EQUIV.01} closing arc (\texttt{v276}--\texttt{v302}), the typed solvers and the
parameter-free gravity round (\texttt{v303}--\texttt{v407}), to the certification round
(\texttt{v408}--\texttt{v481}: the $\alpha$-Quillen residual narrowed to the continuum $\zeta$-det face,
the entropic-action bridge typed, the Kronheimer quiver bridge, the four-interval $\mu_4$ geometry).
The master script$\to$check map follows.}
\label{fig:script-timeline}
\end{figure}

\input{tex-artefacts/verification}
