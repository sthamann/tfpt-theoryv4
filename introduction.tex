...
also closes in the \emph{continuum} (the keystone \texttt{SEAM.EQUIV.01}), is now \emph{closed modulo a cited
theorem} --- the lattice model and $S_3$ stack pin it at every computable level and it is Lean-formalised
...
theorems with the realisation input reduced to invariant level (\vref{v469}).
...
reduction state (2026-07-21): the compiler's dimensionless inputs reduce to \emph{four marks} (derived
from P1-side topology), \emph{one square-modulus datum} ($\tau=i$; equivalently the clock, the
CM-fixed half-period, or the non-split extension class on the free seam circle) and $\pi$ --- a
reduction state, not an abolition of the axiom.
Consolidated (2026-07-21, text only): the compiler's dimensionless inputs reduce to \emph{four marks}
(derived from P1-side topology), \emph{one square-modulus datum} $\tau=i$ (equivalently: the clock /
the CM-fixed half-period / the non-split extension class on the free seam circle), and $\pi$ --- a
reduction \emph{state}, not an abolition: \texttt{AX.P2.01} remains declared, and the \mC\
...
shared by flavor and gravity --- the boundary net identification (now \emph{closed modulo a cited theorem},
\vref{v367}/\vref{v368}, \vref{v376}--\vref{v379}, Lean \texttt{FORM.SEAM.MMST.01}), and the named QFT/cosmology transfer
...
$(E_8)_1$ net at $\tau{=}i$} --- is now \emph{closed modulo a cited theorem}: the explicit lattice model
...
\end{center}
\markerkey
\noindent Card addendum (2026-07-21): the $a=(1,1,2)$ residual now carries the consolidated P2
reduction state --- the position half of the alignment bit is topology (\vref{v510}), so the
dimensionless compiler inputs read \emph{four marks $+$ one square-modulus datum $\tau=i$ $+$ $\pi$}
...
$G_{\mathrm{net}}$ inclusion: the raw RP seam $=(E_8)_1$ at $\tau{=}i$), is now \emph{closed modulo a cited theorem}:
...
\texttt{QGEO.SYM.01} its downstream conformal-deck face) is now \emph{closed modulo a cited theorem}
(\vref{v367}/\vref{v368}, \vref{v376}--\vref{v379}, Lean \texttt{FORM.SEAM.MMST.01}; cited continuum existence \mO,
\vref{v336}). So beneath the gate \emph{labels} there is,
...
\item \textbf{Full dynamics --- the admissible transfer model is closed, and the boundary QFT is now one
      ...
      closed modulo the single keystone \texttt{SEAM.EQUIV.01}, itself now \emph{closed modulo a cited theorem}
...
\midrule
boundary QFT (Modular Spectral Closure) & \mE/\mC & \textbf{one relative object} $\mathsf{TFPT}_{\mathrm{QFT}}$: $D_F=$ covariance induction (\vref{v258}), cutoff $=$ KMS weight $f_2/f_0{=}1$ (\vref{v259}), seam/carrier/$E_8$ on one K3 (\vref{v260}), certified (\vref{v261}) --- closed modulo \texttt{SEAM.EQUIV.01}, itself closed modulo a cited theorem (\vref{v376}--\vref{v379}) \\
...
\textbf{(1)} \texttt{SEAM.EQUIV.01} --- one named theorem (\emph{raw seam $=(E_8)_1$ at $\tau{=}i$}) & \mC & \textbf{closed modulo a cited theorem}: the explicit lattice model (\vref{v367}/\vref{v368}) and the $S_3$ stack ($c{=}8$, $248$ currents/$1$ primary, torus $\mathrm{GSD}{=}1$, RP; \vref{v376}--\vref{v379}) pin it at every computable level, Lean-formalised as \texttt{FORM.SEAM.MMST.01} (collar hypotheses kernel-proved, MMST/OS as named cited axioms, clean \texttt{\#print axioms}); the \emph{only} residual that stays \mO\ is the abstract continuum existence of the scaling limit (\vref{v336}); extension leg crossed-product-certified, realisation at invariant level (\vref{v469}) \\
...
seam state is the holomorphic $(E_8)_1$ net at $\tau{=}i$} --- is now \emph{closed modulo a cited theorem}. (The
