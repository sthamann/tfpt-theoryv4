...
\begin{keybox}[The dual anchor: the inverse flavor response \emph{is} the Nariai root \tI\ (\veri{v134\_dual\_anchor.py})]
The Nariai pattern is not only the traceless anchor --- it is stored \emph{inside the flavor compiler} as a
dual invariant. For the residue/winding operators $R,L$ of Paper~2 and the anchor $a=(1,1,2)$,
\[
...
torsion ($n^\top R=(8,0,0)$, $n^\top L=(20,0,0)$). \emph{Typing:} all identities \tI; the reading --- a
...
re-sharpened \tP:} the semiclassical job is now ``derive $-p_2\ln(1-n/\Nfam)$ from the near-Nariai
...
$\det A_0^\star=0$. \emph{R1 re-targeted \tP:} derive ``rate $=-p_2\ln(1-\alpha)$'' with $\alpha$ the
...
$2l+1=3$ modes, and sheet doubling gives $2\times3=6=p_2$ \tI: the ring multiplicity equals the
...
\begin{winbox}[The clock is an entropy power law \tI/\tP\ (\veri{v129\_entropy\_power\_law.py})]
What the resummation argument $1-n/\Nfam$ \emph{is} physically: the three established Nariai entropy
levels (v101) are $S/S_{dS}\in\{1,\tfrac23,\tfrac13\}$ (pure dS; two-horizon total; single horizon ---
deficit $S_{dS}/3$ per step) --- exactly the complementary cusp weights. Hence \tI:
\[
...
hexagon-weighted log-entropy deficits. \emph{The coupling is the entropy deficit \tI\ (triple
identity):} $\alpha=n/\Nfam$ $=$ the Mehta--Seshadri parabolic weight (v115/v126) $=$
...
\tI:} weight $0\leftrightarrow$ pure dS (maximal entropy, stationary attractor), weight
$2\leftrightarrow$ single horizon (fastest decay) --- the established repeller$\to$attractor flow.
\emph{R1, thermodynamic form \tP:} derive $\Gamma_n\propto(S_n/S_{dS})^{p_2}$ from the near-Nariai
...
from mode counting \tI. In the standard collective-coordinate treatment each zero mode contributes a
...
while the $\times2$ is the \emph{horizon pair} $=$ the SdS mass-line deck (v101). The same $3$ counts
the generations and the sphere's conformal directions. \emph{Residue \tP:} justify the per-zero-mode
...
that normalisation is exact \tI: on $S^2(r)$ the $L^2$ norm of \emph{each} of the three normalised
$l{=}1$ harmonics is
\[
...
ratios --- exactly why the clock sees only entropy fractions. \emph{The last step \tP:} show
$\det'_n/\det'_{dS}=1+O(\text{deficit})$ (non-zero-mode determinant cancellation; same local
near-Nariai geometry) --- the one remaining computation of R1.
\end{winbox}

\begin{winbox}[The det-ratio anomaly is the Koide constant \tI/\tP\ (\veri{v132\_detratio\_anomaly.py})]
The det-ratio question acquires its first \emph{exact} invariant. The Ginsparg--Perry sphere operator
on the unit $S^2$ is $L=-\Delta-2$ (the $-2=-|\Z_2|$ is the v104 modulus mass), with eigenvalues
$(l{+}2)(l{-}1)$ and kernel the three $l{=}1$ zero modes. Its heat coefficient is made of atoms \tI:
...
\emph{spectral} anomaly. \emph{Consequence \tI:} $\det'(cL)=c^{\zeta(0)}\det'(L)$ --- the determinant
...
configuration-independent (pure-number GP spectrum). \emph{The budget statement \tP\ (the remaining
...
\begin{winbox}[The zeta budget: the reduced route carries the anchor ratios \tI/\tP\ (\veri{v133\_zeta\_budget.py})]
Euclidean Nariai is $S^2\times S^2$ --- the temporal sector continues to the \emph{same} unit sphere,
so both candidate determinant routes are exactly computable. \emph{(i) The reduced route \tI:} each
2d sector carries the v132 operator with $\zeta(0)|_{\det'}=-\tfrac23$; the two sectors give
\[
  \boxed{\ \text{budget}_{\mathrm{red}}=-\tfrac23-\tfrac23=-\tfrac43=-\frac{e_1}{p_0}\ \text{(minus
  the seed gain)}\ }
\]
--- the per-sector share and the total are \emph{two of the three v119 anchor-ratio triad values};
the six zero modes split $3+3$ across the two spheres: the sheet doubling \emph{is} the two-sphere
structure of Euclidean Nariai ($=$ the horizon pair, v101). \emph{(ii) The naive 4d route \tI:} with
...
discrimination \tP:} the budget computation structurally favours the \textbf{reduced seam reading}
(pure anchor-ratio anomalies), consistent with the theory's 2d-seam architecture throughout ---
route-selection evidence, not proof. \emph{Remaining \tP:} the graviton/ghost shares on $S^2\times
S^2$ (Volkov--Wipf class, external standard) --- a finite list of known heat coefficients.
\end{winbox}

\begin{winbox}[The Volkov--Wipf firewall: the external edge is two copies of the reduced budget \tI/\tP\ (\veri{v138\_vw\_firewall.py})]
...
contain; two copies $=$ the horizon pair $=$ the sheet doubling (v101; the $3{+}3$ zero-mode split).
\emph{Typing decision \tP\ (recorded):} R1 $=$ the reduced zero-mode/RPA seam clock $=$ an edge-sector
...
edge factor enters inverted) and the Lorentzian edge-mode reading stay \tP.
\end{winbox}
