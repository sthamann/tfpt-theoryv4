...
genuine compiler fingerprints ($1920=|W(D_5)|$, $|\mu_4|=4$). The Hawking normalisation itself is
since \emph{measured} on the seam: the reconstructed free OS quotient is KMS with
$\beta_{\mathrm{angle}}=2\pi$ exact, $T_{\mathrm{seam}}=4c_3$
(\veri{v526\_seam\_thermal\_kms\_nariai\_bridge.py}) --- temperature joins geometry and anomaly
as the \emph{third leg} of $c_3$ (\mC-typed reading; the entropy-fraction bridge honestly does
not close).
\end{keybox}
...
The same normalisation is now \emph{measured} on the reconstructed free seam OS quotient
(\texttt{SEAM.THERMAL.KMS.01}, \veri{v526\_seam\_thermal\_kms\_nariai\_bridge.py}): detailed balance
gives $\beta=N$ clock steps at both levels, hence $\beta_{\mathrm{angle}}=2\pi$ and
$T_{\mathrm{seam}}=4\cthree$ --- temperature joins geometry and anomaly as the \emph{third leg}
of $\cthree$ (\mC-typed reading: seam euclidean circle $=$ thermal circle of the reconstructed
horizon dynamics; the entropy-fraction bridge to the Nariai ledger honestly does not close).
...
is \mE; the explicit Petz recovery map and the rank-one (one-dimensional baby-universe, cf.\ Engelhardt
2025) contraction limit $\|T^n-P_\infty\|=(2/3)^{6n}$ are now realised numerically
(\texttt{experiments/recovery-channel}), so the Petz identification of $T$ with the Page recovery map is
verified at the channel level \mC; the full holographic reading stays gated on the Seam--Horizon theorem. The scrambling prefactor carries a $\mu_4$ signature: with
...
\]
--- exactly the grading that carries the transfer operator's eigenspaces; the $l\ge2$ modes
...
\begin{itemize}\setlength\itemsep{2pt}
\item \textbf{Black-hole echoes / horizonless compactness:} any near-horizon echo amplitude ratio would be
a natural transfer-rate target, $\mathcal A_{n+1}/\mathcal A_n\lesssim(2/3)^6\approx0.0878$. The \emph{same}
window $Q_{\mathrm{geom}}\in[\tfrac38,\tfrac12]$ that the SdS roots trace surfaces independently as the
maximum horizonless gravastar compactness $\mathcal C=\tfrac38$ (de Sitter endpoint $\tfrac12$) of
Jampolski--Rezzolla (2026); since $\tfrac13<\tfrac38<\tfrac49<\tfrac12$ such an object is a horizonless
light-trapping ECO. \texttt{experiments/gravastar-compactness} turns this into an echo template (round-trip
delay $\sim0.7$\,ms at $62\,M_\odot$, amplitude $\le(2/3)^6$) --- a \mC\ structural echo, no
$\mathcal C\!\leftrightarrow\!Q_{\mathrm{geom}}$ map proven. A search ansatz, no prediction.
...
\item \textbf{BH HFQPO ladder tooth ($\times1.5$):} the four published black-hole HFQPO twin pairs are
consistent with exactly $3{:}2$, but that cluster is \emph{cheap} (an anchored detection-selection null
reproduces it in $18.5\%$ of trials, and XTE J1859+226 breaks universality at $+9.2\sigma$), and mapping the
TFPT relaxation-ladder step $3/2$ (from $\mathrm{rate}(n)=-6\ln(1-n/N_{\mathrm{fam}})$, $N_{\mathrm{fam}}=3$)
onto a two-oscillator frequency ratio is \emph{non-canonical} --- GR parametric resonance is the standing
favorite. The one discriminating target is a \emph{third tooth} at $\nu_3=(3/2)\,\nu_u$
($661.5/414/252/363$\,Hz for the four sources) with integer harmonics forbidden on the geometric ladder.
No published search ever targeted these frequencies; the preregistered archival RXTE PCA scan (executed
2026-07 on 77 ObsIDs across the four sources; sanity gate 11/12 published pair lines reproduced,
injection-calibrated in every source) finds \emph{neither} the tooth \emph{nor} the integer control line
$4\nu_0$ anywhere ($\sim\!0\sigma$, trials-corrected), with $3\sigma$ rms upper limits of
$0.53$--$3.06\%$ --- in every source at or below the strength of the detected upper pair line. A
well-powered null: the ladder reading gains no support (GR resonance stays favored) but is not killed,
since the harmonic branch did not fire either (the published integer lines $92{=}184/2$, $34/68$\,Hz
stand). \texttt{experiments/hfqpo-ladder} carries the preregistered census, selection-null Monte Carlo
and the executed archival scan. A search ansatz, no prediction; even a future tooth hit would be \mC\
until the mapping is derived.
...
\item \textbf{Cosmological coupling ($w=-1$):} if the de Sitter--vacuum seam interior (the regular-core
reading above) makes a seam black hole carry the $\Lambda$ vacuum, $w_{\mathrm{in}}=-1$, then under
Croker--Weiner cosmological coupling $k=-3w_{\mathrm{in}}$ the mass-growth index is $k=3$ exactly, i.e.\ a
population $w=-1$. \texttt{experiments/ccbh-dark-energy} confronts this with Farrah+2023 ($k=3.11\pm0.79$,
$-0.14\sigma$) as a \emph{contested} downstream bridge and an \emph{alternative reading} of the same $w=-1$
the DESI $w$-watchdog tests --- not two independent hits.
\item \textbf{Cosmic spin parity (frontier):} the $\mu_4$ clock and the $PSL(2,\mathbb{C})\!\cong\!SO^+(3,1)$
boundary orientation allow at most a tiny global remnant, so TFPT predicts \emph{approximate parity}, not a
large handedness excess. \texttt{experiments/cosmic-handedness} watches the galaxy-spin asymmetry
(Shamir JADES $\sim3.3\sigma$ monopole) --- most likely a Milky-Way-rotation \emph{dipole} systematic and
isotropic in Galaxy Zoo --- a frontier watchdog, explicitly \emph{not} promoted.
\end{itemize}
\end{gapbox}
