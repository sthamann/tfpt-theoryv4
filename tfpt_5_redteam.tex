...
%  Backed by verification/redteam/rt_A..rt_E + run_redteam.py.
% ---------------------------------------------------------------------
...
This is the deliberately \emph{adversarial} layer requested in the v78 review. Its purpose is
...
by \veri{redteam/rt\_A\_e8net.py} \dots \veri{redteam/rt\_E\_vgeo.py} and
...
\midrule
A & seam--Calder\'on measure $=$ $(E8)_1$ net & \textcolor{tfred}{\textbf{reduced, not closed}} & holomorphy proof $+$ constructive map $+$ bulk uniqueness \\
...
criterion $c_{\mathrm{coset}}=c(E8)-c(D5)-c(A3)=0$ holds \tI. This is \emph{compatibility}.
...
\tP. Residual: holomorphy proof $+$ constructive boundary map $+$ bulk-reconstruction uniqueness.
...
\tF). This arithmetic is \emph{not} attacked. The attack is upstream: why must the carrier obey the
...
typed \tA/\tP, not \tI. Residual: a boundary/seam derivation of half-spinor exhaustion.
...
scan of \texttt{v67/v68/v73} finds every $\cthree/2$ used as the \emph{reduced} coefficient and
\textbf{zero} naked dimensionful identities; the firewall's teeth are verified against a synthetic
violation. Internally consistent with $1/G$ being UV-sensitive (Sakharov/Connes, v68).
\end{winbox}

\noindent\textbf{Verdict.} Safe as worded (reduced coefficient). Status \tI\,(structure) $+$ \tA\,for
...
zeros / odd sector size) are made explicit; they are silent in v75 and must be named. Status \tI\,
(reduction) $+$ \tA\,($\vgeo$). Residual: the CP-phase sector.
...
$=(\pi/\sqrt2)\,c_f\,\varphi_0^{k}\,\vgeo$, and $1/G$ shares the same anchor (v68/v75). The audit
...
reheating temperature, and the leptogenesis scale --- all frontier \tP/\tA\ inputs that must not be
...
honestly typed. Status \tI\,(ratios) $+$ \tA\,(one scale). Residual: explicit reduction (or honest
frontier typing) of the EW / reheating / leptogenesis scales $+$ the seam$=$Planck cutoff.

% =====================================================================
\section*{Outcome}
% =====================================================================
\begin{keybox}[What the red team changed]
No target broke. The hardest gate (A) is confirmed \emph{open} with the conservative wording;
three targets (B, D, E) \emph{survive once a silent assumption is named}; one (C) survives as
...
\begin{winbox}[Follow-up --- the verdicts were then used as a worklist \tL/\tI\ (\veri{v83\_e8net\_holomorphic\_uniqueness.py})]
...
\smallskip\noindent\textbf{Target A, residual 1 --- closed \tL.} At level 1 the number of primaries equals
...
($\Delta_{\mathrm{eff}}>0$ supports, not proves), and (ii)~bulk-reconstruction uniqueness --- both still \tP/\tA.

\smallskip\noindent\textbf{Target B --- reduced \tI/\tL.} The ``$K=2$ Pascal truncation'' is not free:
...
\begin{winbox}[Second follow-up round \tL/\tN\ (\veri{v87\_bulk\_uniqueness\_reduction.py}, \veri{v88\_cp\_phase\_audit.py}, \veri{v86\_nstar\_reheating.py})]
\smallskip\noindent\textbf{Target A, residual (ii) --- conditionally closed \tL: Target A $=$ ONE residual.}
...
Target~A reduces to the single theorem ``seam--Calder\'on boundary net is holomorphic with $c=8$'' (\tP/\tA;
ledger \texttt{GATE.METRIC.05}).

\smallskip\noindent\textbf{Target D --- the CP-phase residual quantified \tN\ (not closed).} With the frozen leading
...
\smallskip\noindent\textbf{Target E --- one flagged scale computed \tP.} The reheating temperature is no longer a
silent input: $M_{\mathrm{scal}}=\cthree^{7/2}\bar M_{\mathrm{Pl}}$ \tI\ $+$ standard physics give
$T_{\mathrm{reh}}=9.6\times10^9$ GeV and $N_\star(0.05/\mathrm{Mpc})=51.4$ \tP\ (ledger \texttt{COSMO.NSTAR.01});
the leptogenesis scale and the seam$=$Planck identification remain typed inputs. \textbf{Again: no target was
