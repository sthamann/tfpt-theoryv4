...
\emph{nonperturbative} \texttt{QG.AMB.01} is a \emph{decoupled general} QG problem (the Euclidean-QG conformal-factor
problem; gap-decoupled from every TFPT readout, not a TFPT structural item, \vref{v335}), not a new gap.
...
Stelle ghost) --- this is the \emph{decoupled general} \texttt{QG.AMB.01} problem (gap-decoupled from every TFPT
readout, not a TFPT structural item, \vref{v335}); and the anchor over-determination is
...
\mE. Target~A's single residual is therefore the seam realisation, now the named keystone
\texttt{SEAM.EQUIV.01} --- \emph{the raw RP seam is the holomorphic $(E_8)_1$ net at $\tau{=}i$} --- whose
...
\caption{The eight $E_8$ Casimir invariant degrees, audited (\veri{v66\_e8\_casimir\_degrees.py}, \veri{v354\_e8\_reverse\_audit.py}, \veri{v355\_e8\_unmapped\_bandwidth.py}). Only three feed a \emph{primary} physical-constant readout; the rest coincide with internal atoms but those per-degree matches are mostly \emph{unforced}. The genuinely forced content is \emph{collective}: $\sum\deg = 128 = \dim S^+$, $\prod\deg = |W(E_8)|$, exponents $=$ totatives of $30$.}
...
$12$ & $11$ & $h(E_6)=|R(A_3)|$        & suggestive & no (overhead) \\
$14$ & $13$ & $\dim G_2$               & declined   & no (overhead) \\
$18$ & $17$ & $h(E_7)=p_4(a)$          & suggestive & no (overhead) \\
$20$ & $19$ & $\det L$                 & declined   & no (overhead) \\
$24$ & $23$ & $|W(A_3)|=4!$            & declined   & no (overhead) \\
...
$\sin^2\theta_{13}$ ($\sim2\sigma$ low, a \emph{separate} carrier-trace channel, \vref{v268}); a
