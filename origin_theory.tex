...
\[
  ...
  \Rightarrow
  \underbrace{S=A/(4G_\Sigma)}_{\textbf{\tA\ open: area-law from the seam determinant}}\Rightarrow
  \underbrace{\text{Einstein }8\pi G}_{\tP\,(\text{Jacobson})}.
  \end{gathered}
\]
Links 1--3 (and RT in seam units) are exact; the single missing analytic step is the replica/area-law from
the seam kernel. If it closes, $\cthree=\tfrac1{8\pi}$ stops being ``the known GR number in a new hat'' and
becomes the visible imprint of the boundary geometry from which gravity is read thermodynamically.
...
remaining coefficient statement $k=\tfrac{\cthree}2$ --- the seam determinant's heat-kernel Einstein--Hilbert
coefficient (the induced $\tfrac1{16\pi G}$ of Susskind--Uglum/Callan--Wilczek, fixed by the Seeley--DeWitt
$a_2$ of the carrier content). That is one sharp spectral computation \tA, no longer a diffuse ``derive
$\tfrac1{8\pi}$''.
...
the seam cutoff $\Lambda$ and the moment $f_2$). So $1/G$ is \emph{not} a pure number (Sakharov/Connes induced
gravity): $k=\tfrac{\cthree}2$ is the seam normalisation P1 carried through the heat kernel, and the physical
identification ``seam scale $=$ observed Planck scale'' is the \emph{one} irreducible dimensionful anchor
($v_{\mathrm{geo}}$; no metre from pure mathematics). What \emph{is} cutoff-independent --- the $R^2$
