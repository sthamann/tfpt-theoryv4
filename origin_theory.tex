...
\newtcolorbox{intbox}[1][]{enhanced,breakable,colback=tfgold!7!white,colframe=tfgold!80!black,fonttitle=\bfseries\small,coltitle=white,title={#1},fontupper=\small}
...
\begin{keybox}[The maximal black hole is the anchor \tI\ (\veri{v101\_horizon\_anchor.py})]
...
textbook, the bulk reading stays \tP. Details and figures: Appendix~H.
\smallskip\\
...
(curvature $2/9=|\Z_2|/\Nfam^2$). \tI\ (\veri{v102\_seam\_orientation.py}); the common-principle reading
stays \tP.}
...
\smallskip\noindent\textbf{$N_\star$ --- the band sharpened to a point \tP\ (this round).} The demotion
stands, but the announced ``reheating model'' is now computed (\veri{v86\_nstar\_reheating.py}): since the
compiler fixes $M_{\mathrm{scal}}=\cthree^{7/2}\bar M_{\mathrm{Pl}}=3.06\times10^{13}$ GeV \tI,
...
Every step beyond $M_{\mathrm{scal}}$ is \emph{imported standard physics}, so the result is typed \tP, the
...
reheating}, and the slow Higgs-channel point is $A_s$-disfavoured: the \tP\ point is \emph{conditional on
...
\begin{keybox}[The residual inventory: one constant, one anchor, one clock to find \tI/\tP\ (\veri{v105\_residual\_inventory.py})]
...
the bridge reaches its final form: \emph{one} \tP\ identification, ``the transfer operator is the seam's
own Nariai clock''. \emph{(Since constructed, v124--v133: the resummed clock has the closed form
...
objects stand between the certified core and a strict TOE: the seam quantum clock \tP, the
holomorphy$+c{=}8$ theorem \tP/\tA, the seam-determinant$\to$EH step \tA, the H2 dictionary \tA/\tP, and
the parabolic realisation of $Q$ (which now also carries the sheet question) \tP\ --- plus the two
...
\smallskip\\
\noindent\emph{Update (post v110--v122, \veri{v123\_inventory\_update.py}):} the list keeps its
cardinality but its \emph{contents} sharpened. The old R4 (``H2 dictionary'') is \textbf{discharged}
--- its values, addresses and margins are now exact theorems (v118--v122); what replaces it is the far
...
\begin{keybox}[The carrier chapter: the interior is free, the structure is the certificate \tI/\tP\ (\veri{v113\_quasifree\_kernel.py})]
The QBL round (v108--v113) adds the missing chapter of the origin story --- \emph{why this carrier}. The
