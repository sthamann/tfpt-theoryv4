"""v513 -- CELEST.DTERM.NONDERIV.01: the c_d negative certificate -- CAN
GEOMETRY DERIVE c_d = 1920?  An anti-numerology certificate in the v508
style (an honest rigid NEGATIVE result).  The v511 full-tensor ledger
found exactly ONE legal cubic channel (the su(4) d-symbol with exchange
quartic Q_dd = (1/60)(T3 - P3/4)) and the relaxed pairing solves EXACTLY
with A_fix = -u + 8v + 2w + 1920 Q_dd; the v511 [C] fence notes 1920 =
|W(D5)|.  This module is the honest stress test of that fingerprint:
derivation or numerology?  Four candidate routes (flux products,
quantisation, character/index, controls) with a PRE-DECLARED
look-elsewhere ledger.  Exact sympy/Fraction arithmetic, no floats.

[E] CONVENTION TABLE (the precondition of any "hit"): c_d is
      convention-dependent, c_d -> c_d/kappa^2 under d -> kappa d and
      c_d ~ propagator normalisation -- the five-way table gives c_d in
      {1920, 120, 256, 32, 69120} (probe tr-fund/e8-Killing; Gell-Mann;
      su(4)-Killing; plain trace; ch3-normalised d/6): only 1 of 5
      conventions hits |W(D5)|.  The convention-STABLE content is the
      factorisation c_d = Phi_T3(A_fix) x 2 h_vee(E8) = 32 x 60, both
      factors [E]: the Killing propagator 60 from sum_alpha <alpha,e>^2
      = 120 over the actual roots, and the T3-slot fixing via
      det(P-block) = -64 != 0 (c_d is pinned by the T3 slot alone).
[E] K1 DEAD (flux products): the charge-legal flux pairings (1,3)/(2,2)
      give 2048/1800 -- they BRACKET 1920 and both MISS; the only flux
      hit 64*60/2 is the FORBIDDEN (1,2) pairing (charge 3 mod 4, killed
      by the v511 innerness theorem).  LOOK-ELSEWHERE LEDGER: 924
      expressions from 21 [E] numbers x grammar {a*b, a*b/2, a*b/4,
      a+b}; 11 hits on 1920; control targets 1800 -> 8, 2048 -> 5,
      2016 -> 0 -- the catalog's multiplicative structure makes any
      smooth number near 2000 easy to hit.
[E] K2 (quantisation gives only a lattice): integer couplings in
      tr-fund units + e8-Killing propagator put c_d in 60 Z -- 1920 is
      the 32ND multiple, no minimality; strict ch3-unit couplings (6 Z)
      give 2160 Z and EXCLUDE 1920 outright; the cubic quantum is
      lattice-dependent (6 on coroots, 3/8 on weights); the residual
      integer 32 has 12 signed factorisations, nothing forces it.
[E] K3 PROVENANCE CLASH (the death blow to the index route): the su(4)
      cubic indices per class are A_m = (0, 16, 0, -16) with twisted sum
      sum_m i^m A_m = 32i, NUMERICALLY equal to Phi_T3(A_fix) = 32 --
      BUT the quartic class decomposition (10, -4, 30, -4) is dominated
      by classes 0/2 while the cubic 32 is built 16 + 16 from classes
      1/3: same number, disjoint mechanism = coincidence exposed; the
      Weyl side is WRONG (the d-channel is an A3-side object, |W(A3)| =
      24; D5 has no cubic invariant at all, Inv(S^3 45) = 0;
      |W(E8)|/|W(A8)| = 1920 is a second accidental 1920 inside E8
      combinatorics).
[E] K4 NEGATIVE CONTROLS: SO(16)/D8 fails every candidate formula
      consistently (no d-channel, cubic class sums 0, dark spheres);
      k = 2 flux doubling scales the flux formula x4 (7680) while c_d
      is root-theoretic and static; false g0 = d5+a2+u(1) has SIX
      symmetric cubics (the question cannot be posed); so(10) sanity
      (Tr X^3 = 0, T5 protection).

VERDICT: NUMEROLOGY as a derivation claim -- the v511 [C] fence stays a
fence, now typed look-elsewhere-loaded; the honest [E] core is c_d =
32 x 60; the physical generation of the 32 stays [O].

Status: [E] exact convention/enumeration/lattice/index arithmetic
(sympy/Fraction, no floats); the v511 [C] fence "1920 = |W(D5)|" is
TEXT-SHARPENED to look-elsewhere-loaded and convention-contingent --
NO marker moves (the fence stays [C], the physical justification of the
cubic GS term stays [O], CELEST.SEAM.01 stays Open).  Python;
Wolfram-mirrored (convention table, flux enumeration, look-elsewhere
counts, provenance clash, K2 lattices), counted per GATE.WOLFRAM.02.
Discovery provenance: experiments/tfpt-discovery/
celestial_seam_wp5e_dterm_coefficient_derivation_probe.py (2026-07-22,
24/24)."""
...
def section_k1(roots):
    ...
    controls = {t: len(enumerate_hits(t)[0])
                for t in (1800, 1920, 2016, 2048)}
...
def section_k2():
    ...
    vals = set()
    for a in range(-MU4, MU4 + 1):
        for b in range(-MU4, MU4 + 1):
            for c in range(-MU4, MU4 + 1):
                d = -(a + b + c)
...
def section_k3(K, Q, C, dA):
    ...
    for m in range(4):
        ...
        cub[m] = sp.Rational(pure.subs(pt), t3ref) if pure != 0 else \
            sp.Rational(0)
        assert sp.expand(pure - cub[m] * T3CUB) == 0, "not prop. to t3"
...
def run():
    reset()
    print("v513  CELEST.DTERM.NONDERIV.01: the c_d negative certificate "
          "-- can geometry derive c_d = 1920? (convention table / flux "
          "look-elsewhere / quantisation lattice / index provenance + "
          "controls)")
    ...
    return summary("v513 CELEST.DTERM.NONDERIV.01: the fingerprint c_d = "
                   "1920 = |W(D5)| does NOT survive the honest test "
                   "battery -- NUMEROLOGY as a derivation claim.  (1) "
                   "CONVENTION TABLE: c_d in {1920, 120, 256, 32, 69120} "
                   "across five normalisations, only 1 of 5 hits "
                   "|W(D5)|; the convention-stable content is c_d = "
                   "Phi_T3(A_fix) x 2 h_vee(E8) = 32 x 60 (both factors "
                   "[E]: Killing propagator 60 from sum <alpha,e>^2 = "
                   "120, T3-slot fixing via det(P-block) = -64 != 0).  "
                   "(2) K1 DEAD: charge-legal flux pairings (1,3)/(2,2) "
                   "give 2048/1800 -- bracket and MISS; the 64*60/2 hit "
                   "is the forbidden (1,2) pairing (innerness); look-"
                   "elsewhere ledger: 924 expressions from 21 [E] "
                   "numbers, 11 hits on 1920, controls 1800 -> 8, "
                   "2048 -> 5, 2016 -> 0.  (3) K2: quantisation delivers "
                   "only lattices (60 Z: 1920 = 32nd multiple, no "
                   "minimality; strict ch3 units 2160 Z EXCLUDE 1920); "
                   "the cubic quantum is lattice-dependent (6 on "
                   "coroots, 3/8 on weights); the integer 32 has 12 "
                   "factorisations.  (4) K3 PROVENANCE CLASH: cubic "
                   "indices A_m = (0, 16, 0, -16), sum i^m A_m = 32i "
                   "numerically = Phi_T3(A_fix) -- BUT the quartic class "
                   "decomposition (10, -4, 30, -4) comes from classes "
                   "0/2 vs the cubic 16 + 16 from classes 1/3: same "
                   "number, disjoint mechanism = coincidence; the Weyl "
                   "side is wrong (d-channel is A3, |W(A3)| = 24; D5 has "
                   "no cubic; |W(E8)|/|W(A8)| = 1920 a second accidental "
                   "1920).  (5) K4 controls (SO(16), k = 2, false g0, "
                   "so(10)) separate consistently.  VERDICT: the v511 "
                   "[C] fence stays a fence, now typed look-elsewhere-"
                   "loaded; the honest [E] core is c_d = 32 x 60; the "
                   "physical generation of the 32 stays [O].  No marker "
                   "moves")
