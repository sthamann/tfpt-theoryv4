...
% ---- tcolorbox environments (union of all merged notes) ----
...
\newtcolorbox{auditbox}[1][]{enhanced,breakable,colback=tfgold!8!white,colframe=tfgold!80!black,fonttitle=\bfseries,coltitle=white,title={#1}}
\newtcolorbox{audbox}[1][]{enhanced,breakable,colback=tfgold!8!white,colframe=tfgold!80!black,fonttitle=\bfseries,coltitle=white,title={#1}}
\newtcolorbox{resbox}[1][]{enhanced,breakable,colback=tfgreen!5!white,colframe=tfgreen!55!black,fonttitle=\bfseries,coltitle=white,title={#1}}
\newtcolorbox{roadbox}[1][]{enhanced,breakable,colback=tfgreen!5!white,colframe=tfgreen!55!black,fonttitle=\bfseries,coltitle=white,title={#1}}

% ---- status markers (colored) ----
...
% ---- central marker legend in the page footer (so [I],[L],... are documented on every page) ----
\usepackage{fancyhdr}
\newcommand{\footmarkers}{\scriptsize\textcolor{black!55}{\textbf{Markers:}\ \tI~identity\quad\tL~Lie/lattice\quad\tF~formalised\quad\tN~numerical\quad\tP~phys./cond.\quad\tA~axiom/open}}
\pagestyle{fancy}
\fancyhf{}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0.3pt}
\fancyfoot[C]{\footmarkers}
\fancyfoot[R]{\small\thepage}
\fancypagestyle{plain}{\fancyhf{}\renewcommand{\headrulewidth}{0pt}\renewcommand{\footrulewidth}{0.3pt}\fancyfoot[C]{\footmarkers}\fancyfoot[R]{\small\thepage}}
...
\end{enumerate}
\textbf{You are reading document \#2.}
\end{keybox}
...
\begin{abstract}
This note reorganises a set of results sitting on top of the existing TFPT 4.5 series
(Papers 1--7). It is written for someone familiar with the series and is meant as an
\emph{update for comments}, not a finished paper. The thesis is that the bridge layer of TFPT is
...
\tI~exact identity, \tL~Lie/lattice theorem, \tF~formalised, \tN~numerical fixed point,
\tP~physical/conditional, \tA~axiom/open. (All markers are explicit; the legacy \texttt{[M]}/\texttt{[C]} compatibility aliases have been removed.)
\end{abstract}
...
% =====================================================================
\section{How to read this, and where it sits in Papers 1--7}
% =====================================================================

\begin{center}\small
\begin{tabularx}{\linewidth}{@{}p{0.05\linewidth}p{0.30\linewidth}X@{}}
\toprule
\textbf{P} & \textbf{Title (short)} & \textbf{What it fixes (used here)} \\
\midrule
1 & Boundary polarization \& primitive kernel & $\cthree=1/(8\pi)$; winding $[u_\Sigma]=1$ \\
2 & Carrier rigidity \& SM packet & $\gcar=5$, $(b,s)=(3,2)$, $Y$, $\dim S^+=16$, $\Nfam=3$, $\Oadm=48$, $b_1=41/10$ \\
3 & EM fixed point \& flavor transport & $\alpha$, transport pole $\dph$, seed $\useed$, CKM/PMNS \\
4 & Admissibility, strong CP, QFT closure & selector, $\theta_{\mathrm{eff}}=0$ \\
5 & Geometric Hodge closure \& metrology & boundary unit $\lamsig$, $\Mbar/\lamsig$, $\GN\lamsig^2$, $\xi$ \\
6 & Cosmology interfaces & $\LamIR$, axion, inflation interface \\
7 & CMB operational closure & Stage-1/2 pipeline (programmatic) \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{equation}
  \boxed{\ \cthree=\frac{1}{8\pi}\ \ (\text{Paper 1}),\qquad \gcar=5\ \ (\text{Paper 2}).\ }
\end{equation}
...
\begin{description}[leftmargin=2.2em,style=nextline]
  ...
  projector / determinant-character chain (Paper 1) and the Lean carrier-rigidity development
  ...
  \item \textbf{P2$_{\mathrm{interface}}$ \tA\ (axiom):} the seam is read out by a five-slot carrier
  $\gcar=5$ ($3$ colour $+\,2$ weak) --- a postulate (the readout interface).
  \item \textbf{P2$_{\mathrm{algebra}}$ \tF\ (theorem):} \emph{given} the carrier, hypercharge
...
\begin{keybox}[Seed normal form, the factorial spine, and the degree-$2$ pattern \tI/\tP\ (\veri{v106\_review\_validation.py})]
Three review-validated sharpenings of the anchor picture. \textbf{(1) Seed normal form \tI.} The seed
formula is exactly
\[
...
states --- a structured anchor readout, not a fitted expression. \textbf{(2) Factorial spine \tI.} With
...
\tP\ (recorded, not claimed):} \textbf{Quadratic Boundary Locality} --- the seam supports only bilinear
data, so the truncation $K=2$ is \emph{forced} rather than chosen; if proven, the Pascal \emph{selection}
(red-team Target~B residual, typed \tA/\tP) upgrades to \tL\ and the chain locality $\Rightarrow K{=}2
...
(Paper~3). \emph{Consequence \tP:} the seam's single scalar two-point kernel is necessarily a
...
The lemma's singlet counts per sheet block, $(0,0,1,1)$, give the exact selection statement \tI:
\[
...
sheet choice. \emph{Anti-overclaim control \tI:} the parity theorem is generic over the odd ladder
...
parts \tP\ (recorded, not claimed): \textbf{(a) interface} --- ``the seam Calder\'on involution is
...
valid over $\mathbb Q$). \emph{Parity} \tI: all linear words are sheet-odd, all $45$ quadratic words
sheet-even --- code-preserving transport has \emph{even} Clifford degree (the same $\Z_2$ again).
\emph{Minimality} \tI: sheet-even words of degree $\le1$ are the scalars alone. \emph{Generation and
completeness} \tI: the $45$ quadratics ($=\mathfrak{so}(10)$, exactly the $\Lambda^2$ term of the
...
\emph{Anti-overclaim control \tI:} the $g=3$ world behaves identically ($15$ quadratics generate
$\operatorname{End}(S^+)=16$) --- the theorem selects the \emph{degree}, not the rank; the
$g$-selection stays with Pascal closure $+$ rank-$8$. \emph{Residue after v109--v111} \tP: QBL's
...
$\#\{\text{neutral kernels}\}=\dim S^+=2^{g-1}$ exactly \tI. Grading the \emph{same} neutral set by
pair-degree $m$ gives sizes $\binom gm$, $m=0..K$ --- for $g=5$ the triple $(1,5,10)$ \tI. Two
countings of one set:
\[
  \boxed{\ 2^{g-1}=\sum_{m\le K}\binom gm\ \text{is the channel counting itself}\ }
\]
(symbolically verified for all odd $g=3..13$) --- the Pascal \emph{closure} of Paper~3 is not an
extra requirement but an identity of the certified channel. \emph{Hodge fold} \tI: the code's
...
rank-$8$ closure $g+\Nfam=8$ and ladder integrality. \emph{Residue re-type \tP\ (recorded, not
...
objects. \emph{Bookkeeping} \tI: $c(D_5)_1=\tfrac{45}9=5=g_{\mathrm{car}}$, $c(A_3)_1=\tfrac{15}5=3=\Nfam$,
...
$2\times2=4=|\mu_4|$). \emph{Wick/Pfaffian} \tI: in the exact ten-Majorana model \emph{all} $210$
...
\tI: $M=\mathbf1+iA$ with $A^2=-\mathbf1$, so $P=M/2$ is a projection with $\operatorname{rank}P=5=
...
annihilator kernel one-dimensional). \emph{Premise merge \tP\ (recorded, not claimed):} ``single
...
when the gate closes, the carrier choice closes \tL\ along free net $\Rightarrow$ one kernel
$\Rightarrow$ sheet-odd $\Rightarrow$ self-counting tower $\Rightarrow$ complete transport
$\Rightarrow$ carrier. Gate typing \tP/\tA\ unchanged.
\end{keybox}
...
the seam}. Paper~5 compactifies the oriented two-dimensional normal slice $N_\Sigma$ to a sphere $S^2$,
...
``$8$'' with $E_8$ --- but does \emph{not} eliminate $\pi$; $\pi$ remains the one continuous primitive. \tP
\end{keybox}
...
\begin{keybox}[Carrier uniqueness theorem: $\gcar=5$ and the $3{+}2$ split are forced \tI]
...
\begin{auditbox}[The Pascal grammar: one $\Lambda^\bullet$ mechanism in four places (anti-numerology) \tI]
...
\begin{equation}
  ...
  \end{aligned}\ }
\end{equation}
with $\Nfam=(2^{\gcar-1}-1)/\gcar=15/5=3$ and $\operatorname{rank}E_8=\gcar+\Nfam=8$. \tI. The
...
\begin{keybox}[The compiler in one line]
\[
  \boxed{\ \text{five carrier slots}\Rightarrow D_5,\quad
...
\begin{equation}
  \boxed{\ \operatorname{Aut}_{\mathrm{aff}}(C^+)=C^+\rtimes S_5,\quad |\operatorname{Aut}_{\mathrm{aff}}(C^+)|=2^{\gcar-1}\gcar!=1920=|W(D_5)|=r\,|R(E_8)|\ }\qquad\tI ,
\end{equation}
...
Paper 2 fixes $X_f^\circ\cong\mathbb P^1\setminus\mu_4$, $H_1=\mathbb Z^3$, rigid $D_4$ generated by
...
\begin{equation}
  ...
  \mathrm{Gram}=\mathrm{Cartan}(A_3),\ \det=4\ }\qquad\tI .
\end{equation}
...
\begin{auditbox}[The metric, settled (former \tP)]
...
element. So the lattice structure is exact \tI; the analytic metric is a characterised deformation,
not an open gap.
\end{auditbox}
...
\begin{equation}
  ...
  \end{aligned}\ }\qquad\tI\ \veri{v1\_e8\_glue.py}
\end{equation}
...
$240$-metrisation is no longer blind --- $E_8$ is built}. \tI
...
\begin{keybox}[Spine Quotient Lemma: the recurring factors are adjacent quotients of $(2,3,4,5)$ \tI\ (\veri{v74\_compiler\_micro\_lemmas.py})]
...
are five projections of one discrete chain --- not scattered constants.
\end{keybox} The carrier adjoint $\dim D_5=|R(D_5)|+\gcar=45$
then gives a second reading of the $U(1)$ integer,
\begin{equation}
  \boxed{\ 10b_1=\dim D_5-|\mu_4|=45-4=41=\underbrace{40}_{\sum L}+\underbrace{(\gcar-|\mu_4|)}_{=N_\Phi=1}\ }\qquad\tI ,
\end{equation}
i.e.\ the single Higgs doublet is the carrier rank minus the glue corners, $5-4=1$. The start defect
...
\begin{equation}
  ...
  I_1^{\mathrm{EW}}=\frac{|E(K_5)|}{|R(A_3)|}+\frac{1}{|\mu_4||R(A_3)|}=\frac{10}{12}+\frac1{48}=\frac{41}{48}\ }\qquad\tI .
\end{equation}
...
\begin{equation}
  ...
  L_u=(7,3,0),\ L_d=(7,5,2),\ L_e=(8,5,3)\ }\qquad\tI\ (\text{given anchors}) .
\end{equation}
...
\begin{equation}
  ...
  \text{lepton}\leftrightarrow 2y_+,\ \text{up}\leftrightarrow -2y_-,\ \text{down}\leftrightarrow 2(y_++y_-)\ }\qquad\tI .
\end{equation}
Paper~3 (Thm.~``canonical transport kernel'') derives the residue packets --- and hence the anchors ---
...
\begin{equation}
  \boxed{\ a_f=\operatorname{round}(2\,\mathrm{cusp}_f):\quad (a_u,a_d,a_e)=\operatorname{round}\bigl(\tfrac43,\tfrac23,2\bigr)=(1,1,2)\ }\qquad\tP ,
\end{equation}
...
whose corner rotation is the $A_3$ Coxeter element), rather than from the Paper-3 family-connection
theorem.

\begin{auditbox}[Riemann--Hilbert check, and what it does (and does not) fix]
...
of Paper~2's family connection and the corner-rotation $=$ Coxeter identity. However, the flat
...
Hodge/metric data on the spine, not topological monodromy. So the residual \tA\ is now identified
sharply as a \emph{parabolic-weight computation} on $\mathbb P^1\setminus\mu_4$ --- the cusp exponents
$\{0,\tfrac13,\tfrac23\}$ promoted to parabolic degrees --- not a free choice and not a pure
local-system question. \tA
\end{auditbox}
...
\midrule
Number of generations & $\Nfam=3=\operatorname{rank}A_3=\dim H^1(\mathbb P^1\setminus\mu_4)$ & $A_3$ & \tI \\
Gauge algebra dimension & $\dimg=12=|R(A_3)|=\gcd(6r,2h)$ & $A_3$ & \tI \\
One generation ($16$, incl.\ $\nu^c$) & $S^+=D_5$ half-spinor $=\Lambda^{0,2,4}E$ & $D_5$ & \tI \\
Hypercharge spectrum & all $Y$ from $y_\pm=\{\tfrac12,-\tfrac13\}$; $\operatorname{Tr}X^2=120$ & $D_5$ root data & \tI \\
Abelian index $b_1$ & $10b_1=1+|\mu_4||E(K_5)|=1+4\cdot10=41$ & $\mu_4\times K_5$ & \tI \\
Fermion family occupancy & $\Oadm=\Nfam\dim S^+=(\Nfam{+}N_\Phi)|R(A_3)|=48$ & $A_3$ & \tI \\
Fine structure $\ainv=137.0359992168$ & root of $F_{U(1)}(\alpha)=0$ (explicit, unique) & EM closure & \tN \\
Cabibbo angle & $\lC=\sqrt{\useed(1-\useed)}$, $\useed=\tfrac43\cthree+48\cthree^4$ & seed & \tI \\
CKM CP phase & $\delta_{\rm CKM}=\tfrac{2\pi}{|R^+(A_3)|}+\Nfam\useed(1-\useed)=\tfrac\pi3+3\lC^2$ & $A_3^+$, seed & \tI/\tP \\
PMNS angles/phase & $\sin^2\theta_{12}=\tfrac13-\tfrac\useed2$, $\sin^2\theta_{13}=e^{-5/6}\useed$, $\delta^\nu=\tfrac{4\pi}{3}$ & $A_3^+$, seed & \tI/\tP \\
Charged-lepton hierarchy & $\det M_\ell\propto\useed^{\,h/\Nfam}=\useed^{10}$; $(5,3,2)$ & decuple & \tI/\tP \\
Mass-hierarchy transport & $\sum L=40=|R(D_5)|$; entries on $C_6=|R^+(A_3)|$ & $D_5\times A_3$ & \tI \\
Strong CP & $\theta_{\rm eff}=0$ (determinant erasure, Paper 4) & --- & \tI \\
Weak CP & holonomy: $\delta_{\rm CKM}-\tfrac\pi3=\Nfam\,\useed(1-\useed)$ & $A_3^+$ & \tP \\
Gravity tree normaliser & $\xi_{\rm tree}=\tfrac{\dimg}{\dim S^+}=\tfrac34=q(A_3)$ & $A_3$ glue & \tI \\
Cosmological constant & $\rho_\Lambda/\Mbar^4=\tfrac{3}{4\pi^2}e^{2\dph}[v_{\rm geo}/(5\betarad^2\Mbar)]^{10}$ & decuple & \tI \\
Hubble anchor & $H_0\sqrt{\Omega_\Lambda}=\Mbar e^{-\ainv}/(2\pi)$ & action & \tI \\
Baryon density & $\omega_b=\lC/\AL=\lC/10$ & decuple & \tI/\tP \\
Inflation (scalaron) & $M_{\rm scal}^2/\Mbar^2=\cthree^{\,\Oadm-10b_1}=\cthree^{7}$ & deficit $r-1$ & \tI \\
Inflation amplitude & $A_s=N_\star^2\cthree^7/24\pi^2\approx2.2\times10^{-9}$ & deficit & \tP/\tA \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
From $\gcar=5$ and the split $(b,s)=(3,2)$ the Paper-2 data follow:
\begin{equation}
\begin{aligned}
...
\begin{resbox}[Two carrier roots generate the family and the transport cusps \tI]
...
\begin{keybox}[The $X=6Y$ quadratic and the hypercharge Pascal-sign duality \tI]
...
$\operatorname{Tr}X^4/\operatorname{Tr}X^2=2280/120=19$ (an $E_8$ exponent; audit). \tI
\end{keybox}
...
\begin{align}
  10\,b_1 &= 41 = \gcar^2+(\gcar-1)^2 = \Oadm\,\gamma_{\mathrm{car}}+1
      = 1+h_{E_8}\tfrac{\dim S^+}{\dimg} = 1+30\cdot\tfrac{16}{12} = 40+1, &&\tI\\
  I_1^{\mathrm{EW}} &= \frac{41}{48}=\operatorname{Var}_{S^+}(Y)\cdot b_1=\frac{5}{24}\cdot\frac{41}{10}
      = \gamma_{\mathrm{car}}+\frac{1}{\Oadm}, &&\tI\\
  \operatorname{Tr}_{S^+}(X^2) &= 120 = 5!\quad(X=6Y), \qquad
  \dtwo = \tfrac54\dtop^2 = 2880\,\cthree^8 = 4!\cdot 5!\,\cthree^8 = 4!\,\operatorname{Tr}_{S^+}(X^2)\,\cthree^8, &&\tI\\
...
(the value is the standard one; the rewrite is a coincidence to note, not a claim). \tP

% =====================================================================
\section{Grammar II --- the seed grammar (one decoder $\useed$)}
% =====================================================================

The Paper-3 seed is, exactly, a pure function of $\cthree$:
\begin{equation}
  \boxed{\ \useed=\varphi^{\mathrm{ret}}_0=\frac43\,\cthree+\Oadm\,\cthree^4
  =\underbrace{\frac{1}{6\pi}}_{\phibase}+\underbrace{\frac{3}{256\pi^4}}_{\dtop}=0.0531719521768\ }\qquad\tI.
\end{equation}
...
\begin{resbox}[One seed, four readouts (and their inverses) \tI]
\[
...
\subsection{The EM closure: $\ainv$ as an explicit fixed point (Paper 3, made explicit here)}
The engine $\ainv$ is not an input --- it is the unique root of an explicit closure equation
(Paper~3, Thm.\ ``exact electromagnetic closure''). With $\cthree=1/(8\pi)$, $\phibase=1/(6\pi)$,
$q(\alpha)=\dtop e^{-2\alpha}$ and the seam response $\phiseam(\alpha)=\phibase+q(\alpha)(1-q(\alpha))^{-5/4}$,
\begin{equation}
  \boxed{\ F_{U(1)}(\alpha)=\alpha^3-2\cthree^3\alpha^2-\tfrac{4}{5}\cthree^6\Bigl(\textstyle\sum_{f,j}L_{f,j}+N_\Phi\Bigr)\log\frac{1}{\phiseam(\alpha)}=0\ }\qquad\tI/\tP ,
\end{equation}
...
\noindent Solving:
\begin{equation}
  \boxed{\ \alpha_\star=0.00729735256220985\ldots,\quad \ainv=137.0359992168407\ldots\ }\quad\tN
\end{equation}
\veri{v3\_em\_alpha.py}\;
...
\begin{auditbox}[Why \emph{this} function: the boundary $U(1)$ Ward reading (Theorem C) \tI/\tP]
...
decomposition and the coefficient identities ($8b_1=\tfrac45 M$, $-\tfrac54=q(D_5)$) are exact \tI; the
...
is a conjectured \tP\ interpretation, not yet machine-proven. \veri{v48\_em\_ward.py}
\end{auditbox}
...
\begin{keybox}[Decuple bridge: $\Lambda$ is the tenth power of the stripped EW scale \tI]
\begin{equation}
...
\tI. The exponents are the complete-graph counts of the five-slot carrier $K_5$:
\[
...
$\Omega_\Lambda$-dependent; not a claim. \tA)
...
\begin{winbox}[High-precision cosmological constant: a $\sim10^{-13}$ prediction \tN]
...
\begin{equation}
  \boxed{\ \frac{\rho_\Lambda}{\Mbar^4}=\frac{3}{4\pi^2}\,e^{-2\ainv}=7.12533\times10^{-121}\ }\qquad\tN\ \veri{v7\_gravity\_cosmo.py} ,
\end{equation}
...
\begin{equation}
  \boxed{\ 2\gcar=\binom{\gcar}{2}\quad\Longleftrightarrow\quad \gcar=5\ }\qquad\tI\ \veri{v2\_carrier\_pascal.py},
\end{equation}
...
the carrier (hypercharge) polynomial. For a general split $b+s=g$ the Paper-2 polynomial is
...
\begin{equation}
  ...
  \AL=\binom{\sqrt{\Delta_Y}}{2}\AEW\ }\qquad\tI .
\end{equation}
...
of TFPT scales: two integer labels per scale. \tI\ (charges) / \tP\ (residues).
...
L_0=\ln(1/\useed)\ (\text{flavor}),\,L_c=\ln(1/\cthree)\ (\text{seam/gravity})\}$. \tI

% =====================================================================
\section{The Einstein normaliser as a corrected tree value}
% =====================================================================

Paper 5's gravitational normaliser $\xi=\cthree/\useed$ has a transparent exact form. Writing
$\useed=\tfrac{1}{6\pi}\bigl[1+\tfrac{9}{128\pi^3}\bigr]$,
\begin{equation}
  \boxed{\ \xi=\frac{\cthree}{\useed}=\frac34\cdot\frac{1}{1+\dfrac{9}{128\pi^3}}=0.748303\ }\qquad\tI .
\end{equation}
...
Paper 5 leaves the absolute SI scale as a calibration. The dimensionless vacuum quotient lets one
...
\begin{auditbox}[Non-circularity (\tA, essential)]
...
\begin{enumerate}
\item \emph{Electroweak anchor} (Paper 5): the dimensionless $\GN v_{\mathrm{phys}}^2=4.06717\times10^{-34}$
...
\end{equation}
Closure algebra \tI; SI anchor and numerical match \tP\ (the cosmological anchor is model dependent;
...
\begin{auditbox}[Prefactor branch (\tA)]
...
\begin{auditbox}[Mandatory $\Lambda$-layer typing (\tA)]
...
(Paper 5, charged-current residue, not a fit), the decuple bridge in the observable vev reads
\begin{equation}
...
source scale; both forms should appear side by side. \tI/\tP
...
$\tfrac{g(g+1)}{2}=2^{g-1}-1$ has $g=5$ as its non-trivial solution (\tP).
...
\subsection{Level 1 --- original $E_8$ data of the series (\tI)}
\begin{align}
...
(Coxeter principal part plus a rank singlet). \tI\ (arithmetic) / \tP\ (typing).

\subsection{Level 2 --- new exact counting identities (\tI)}
\begin{align}
...
\end{equation}
and the $120$ undirected flips match the $120$ $E_8$ root \emph{pairs} (\tI, counting). This makes the
...
\begin{auditbox}[Honest obstruction (\tA)]
...
than B, so it is the right first falsifier. \tA

\begin{auditbox}[Audit only --- \emph{non-load-bearing} speculation: the $\dim 496$ heterotic count \tA]
...
\emph{is} either gauge group is unproven and not used anywhere. \tA\ (audit-only).
\end{auditbox}
...
\begin{equation}
  ...
  =\Nfam\binom{\gcar}{2}=\Nfam\,A_\Lambda=2\gcar\Nfam=2^{\gcar}-2\ }\qquad\tI .
\end{equation}
...
\end{equation}
so Hubble reads half a Coxeter period per family and $\Lambda$ a full Coxeter period per family. \tP
...
rank--Coxeter coupling, not an optional singlet. \tI/\tP
...
\begin{equation}
  \boxed{\ |R^+(E_8)|=120=\operatorname{Tr}_{S^+}X^2=5!\ }\qquad\tI,
\end{equation}
...
slice of the $E_8$ root atlas is exactly the full flavor transport budget. \tI\ (arithmetic) / \tP\
(interpretation).
...
the $2,3,5$ carrier}. \tI\ (arithmetic) / \tP\ (interpretation).

\subsection{Level 3 --- conjectural geometric readings (\tP)}
...
\end{conjecture}
\begin{auditbox}[Two routes to $E_8$ --- keep their status separate]
Levels 1--2 are exact arithmetic. There are now \emph{two} distinct routes to $E_8$, and they must not
be conflated:
\begin{itemize}
\item \textbf{Glue route (closed, \tL):} $E_8=(D_5\oplus A_3)+\mu_4$-glue is built explicitly in
...
\item \textbf{Direct flip-atlas route (open, \tA, optional bonus):} the even-flip transition set
...
a near-maximal $|J^\nu_{\mathrm{CP}}|\approx0.030$ (sharp readout for DUNE/Hyper-Kamiokande). \tP
...
holonomy of Paper 3, $\delta_{\mathrm{CKM}}=\arg\operatorname{Tr}\Pi_\triangle(\nabla_F^\star)$; on the
small-area branch this has the compact readout form (with an $O(\lC^4)$ remainder)
\begin{equation}
  \lC=\sqrt{\useed(1-\useed)},\qquad
  \boxed{\ \delta_{\mathrm{CKM}}-\frac{\pi}{3}=\Nfam\,\useed(1-\useed)=3\lC^2\ }\qquad\tI/\tP\ (\text{readout; exact}=\text{holonomy}) .
\end{equation}
...
\begin{equation}
  ...
  \delta^{\mathrm{PMNS}}_{\mathrm{CP}}=4\cdot\frac{2\pi}{|R^+(A_3)|}=\frac{4\pi}{3}\ }\qquad\tI/\tP .
\end{equation}
...
PMNS $=$ carrier root $+$ seed $+$ $A_3$ holonomy phase. \tP
...
\emph{determinant} CP is. \tP\ --- the strong-CP closure lives on the admissible QFT branch and is
\emph{conditional} on the analytic hypotheses of Paper 4 (reflection positivity, temperedness,
...
\end{equation}
One measured member of $\{\lC,\theta_{13},\Omega_b,\betarad\}$ fixes the other three. \tI\ (the four
...
The Paper-3 transport length packets are
\begin{equation}
...
and explaining why the first generation is so light --- ``one turn more'', not a free fit. \tP
\emph{Flagged proximity (\tA, not exact):} the full-turn cost is close to the topological seam defect,
...
windings; generations 2 and 3 are the winding complement. \tI\ (arithmetic).

\subsection{Lepton flavor decuple: the charged-lepton determinant carries $u^{10}$}
The Paper-3 source masses on the $v_{\mathrm{geo}}$ branch are
...
$E_8$ Coxeter period read the same carrier core additively ($=10$) and multiplicatively ($=30$). \tI/\tP
...
\begin{equation}
  \boxed{\ y_f\simeq\lambda_C^{\,L_f}\times\Lambda_f\ (\Lambda_f=\mathcal O(1)),\qquad y_t\simeq1\ \text{because}\ L_t=0\ }\qquad\tI/\tP .
\end{equation}
...
% =====================================================================
\section{Inflation interface (Paper 6)}
% =====================================================================
...
\begin{equation}
  \boxed{\ \frac{M_{\mathrm{scal}}^2}{\Mbar^2}=\cthree^{\,r-1}=\cthree^{\,\Oadm-10b_1}=\cthree^7\ }\qquad\tI ,
\end{equation}
an \emph{exact} identity (the exponent $7=48-41$ is the post-flavor, post-Higgs occupancy deficit).
Paper 6 reduces the geometric gravity action to an FRW $R^2$/scalaron branch in which $M_{\mathrm{scal}}$
...
\begin{equation}
  A_s=\frac{N_\star^2\,\cthree^7}{24\pi^2}=\frac{N_\star^2\,\dtwo}{8640\pi}=2.17\times10^{-9}\qquad\tP/\tA ,
\end{equation}
matching Planck ($\simeq2.1\times10^{-9}$); $\dtwo=2880\,\cthree^8$ is the second seam defect. The
seam-power form is exact \tI; the amplitude is then determined \emph{within} the $R^2$ sector, the only
residual being the geometric-gravity (Hodge) closure on which that sector rests \tP. \emph{Convention
...
for the Higgs/$W$/$Z$ masses yet. \tA
\end{itemize}
...
\item \textbf{The $\phiz$-ladder (whole spectrum, one formula).} Every fermion mass is
  ...
  $t(0)>\{b,c,\tau\}(2)>\{s,\mu\}(3)>\{u,d\}(4)>e(5)$. \tI\ (leptons exact, quark ratios closed v49/v71) / \tA\ (absolute quark scale)
...
\item \textbf{H2 reduced via the Mehta--Seshadri framework.} Mehta--Seshadri supplies the equivalence
  ...
  $\nabla_F^\star$ (Paper 3) and the parabolic-degree$\to C_6$ dictionary. H2 is the sharp realisation
  statement: the $D_4$-equivariant stable structure on $\mathcal O(-2)\oplus\mathcal O(-1)^2$ realises the
  branch with $\det R=h(D_5)=8$, principal $2$-minors $(2,3,5)$. \tI\ (given (U))
...
\item \textbf{Strong CP / dynamics via the explicit gap.} $\theta_{\mathrm{eff}}=0$ needs only
  ...
  $\Rightarrow$ OS reconstruction $\Rightarrow$ measure on the admissible sector. \tI\ (phys.\ sector)
...
\item \textbf{SM completeness.} The discrete structure and dimensionless core are closed; the
  ...
  with $m_H$ resting on the closed UV quartic $\lambda_\Phi=1/(16\pi^2)$ (near-criticality). The only
  genuinely open SM point is the \emph{exact} solar angle $\theta_{12}$ (leading $1/\Nfam=\tfrac13$). \tP
\end{enumerate}
...
\midrule
Dual-root dictionary ($Y$, cusps, $\gamma_{\mathrm{car}}$, $\phibase$) & \tI & exact, from $\{1/2,-1/3\}$ \\
$10b_1=5^2+4^2=\Oadm\gamma+1$ & \tI & Pythagorean carrier identity \\
$I_1^{\mathrm{EW}}=41/48=\operatorname{Var}(Y)\,b_1$ & \tI & exact \\
$\dtwo=4!\cdot5!\,\cthree^8$, $\operatorname{Tr}X^2=5!$ & \tI & factorial traces \\
$5/4=\gcar/(\gcar-1)$ universal compression & \tI & exact \\
$\useed=\tfrac43\cthree+48\cthree^4$; one-decoder identities & \tI & exact \\
$\xi=\tfrac34/(1+9/128\pi^3)$ & \tI & corrected tree value \\
Ladder $\AEW:\AH:\AL=1:\gcar:2\gcar$ & \tI/\tP & Hubble rung is a corollary \\
Ladder $=\binom{\gcar}{0}:\binom{\gcar}{1}:\binom{\gcar}{2}$; $2\gcar=\binom{\gcar}{2}\Leftrightarrow\gcar=5$ & \tI & new overdetermination of $\gcar=5$ \\
2D action lattice $S_x=a(\ainv/\gcar)+bL_0$ & \tI/\tP & two integer labels per scale \\
\textbf{Decuple bridge} $\rho_\Lambda/\Mbar^4=\tfrac{3}{4\pi^2}e^{2\dph}[v/(5\betarad^2\Mbar)]^{10}$ & \tI & exact, $10=2\gcar=\dim\Lambda^2E$ \\
Family $=$ ladder $=$ Pascal row $\{1,5,10\}$; $16=1+5+10$ & \tI & central new identification \\
$\gcar=5$ via $2^{\gcar-1}=\binom{\gcar}{0}{+}\binom{\gcar}{1}{+}\binom{\gcar}{2}$ & \tI & Pascal closure (unique) \\
$\operatorname{Tr}_{S^+}X^2=5!$ generates $40,41,240,\dtwo$ & \tI & master moment \\
$|R(E_8)|=16\cdot5\cdot3$; $\dim E_8=240+8$ & \tI & $E_8$ compression \\
Even-flip atlas $240=160+80$ & \tI/\tA & $E_8$ metrization open ($\neq112+128$) \\
$\delta_{\mathrm{CKM}}-\pi/3=3\useed(1-\useed)$ & \tI & CP $=$ triple seed variance \\
Observable quadrilateral ($\useed=\Omega_b+\betarad$) & \tI & closed UV-shadow cycle \\
Power tower $v{=}x^1,\,H{=}x^5R_H,\,\Lambda{=}x^{10}R_\Lambda$ & \tI & $K_5$ vertices/edges \\
$496=16\cdot31=\dim(E_8{\times}E_8)=\dim SO(32)$ & \tI/\tP & heterotic-dimension count \\
$\AEW=\ainv/\sqrt{\Delta_Y}$, $\Delta_Y=(b{+}s)^2=25$ & \tI & discriminant action theorem \\
$\hat m_e\hat m_\mu\hat m_\tau\propto\tfrac{32}{9}\useed^{10}$; $(5,3,2)=(\gcar,b,s)$ & \tI & lepton flavor decuple \\
$M_{\mathrm{scal}}/\Mbar=\cthree^{7/2}$ & \tI & scalaron seam power \\
$\delta_{\mathrm{CKM}}-\pi/3=\Nfam\,\useed(1-\useed)$ & \tI & weak CP $=\Nfam\times$ seed variance \\
$L=R+W$: $\sum R=22$, $\sum W=18$ & \tI & first generation $=$ residue store \\
$|R(E_8)|/6=\sum L=40$ ($\mathbb Z_6$ slice) & \tI & one $\mathbb Z_6$ slice $=$ flavor budget \\
$h_{E_8}=|R|/\operatorname{rank}=30=\Nfam\binom52=\Nfam A_\Lambda$ & \tI/\tP & carrier Coxeter theorem \\
$|R^+(E_8)|=\operatorname{Tr}_{S^+}X^2=120=5!$ & \tI & positive-root moment \\
common decuple $\binom52{=}A_\Lambda{=}u^{5+3+2}{=}h_{E_8}/\Nfam{=}10$ & \tI/\tP & one carrier pair-decuple \\
$\lambda_C^6/\dtop=1.061$ (winding $\approx$ seam defect) & \tA & flagged proximity (not exact) \\
$y_f\simeq\lambda_C^{L_f}$, top $L_t=0\Rightarrow y_t\simeq1$ & \tI/\tP & Froggatt--Nielsen ladder \\
$w=-1$ (true $\Lambda$); $N_{\mathrm{eff}}\simeq3$ & \tI & dark energy / neutrino species \\
3-generator lattice $\{\ainv,L_0,L_c\}$; $M_{\mathrm{scal}}=e^{-\frac72 L_c}$ & \tI & seam axis $L_c=\ln(1/\cthree)$ \\
$\dim S^+=2\operatorname{rank}(E_8)$, $h_{E_8}=2(\dim S^+{-}1)=2^{g}{-}2$ & \tI & rank--Coxeter coupling ($\nu^c$ closes it) \\
$\gcd(2h,6r)=\dimg=12$, $\operatorname{lcm}=240$ & \tI/\tP & SM algebra $=$ gcd; $E_8$ count $=$ lcm \\
$I_1^{\mathrm{EW}}=h_{E_8}/36+1/(6r)=41/48$ & \tI/\tP & Coxeter part $+$ rank singlet \\
$M_{\mathrm{scal}}^2/\Mbar^2=\cthree^{r-1}=\cthree^7$ exact; $A_s=N_\star^2\cthree^7/24\pi^2$ & \tI\,/\,\tP/\tA & seam power exact; $A_s$ conditional (Starobinsky) \\
$h_{E_8}=2{\cdot}3{\cdot}5=30$, $r=\varphi(30)=8$ & \tI/\tP & $2,3,5$ Coxeter cyclotomic compiler \\
$\operatorname{Exp}(E_8)=(\mathbb Z/30)^\times$, $\sum=120$, $\chi_C=\Phi_{30}$ & \tI/\tP & exponents $=$ primitive residues \\
lepton $(5,3,2)$: $\sum{=}10{=}A_\Lambda$, $\prod{=}30{=}h_{E_8}$ & \tI/\tP & sum$=$action, product$=$Coxeter \\
Hubble $H_0/\Mbar=e^{-\ainv}/(2\pi\sqrt{\Omega_\Lambda})$ & \tI & exact corollary \\
$\Lambda$-anchored closure $\GN=\tfrac{3c^3}{32\pi^3\hbar\Lamgeom}e^{-2\ainv}$ & \tI/\tP & $\GN$ to $-0.35\%$ \\
$\gcar=5$ overdetermined & \tP & monotone $\alpha^{-1}(\gcar)$ \\
$|R(E_8)|=240=16\cdot15$, $248=8\cdot31$ & \tI & counting identities \\
$E_8\leftrightarrow$ carrier-transition map & \tP & needs structure map \\
$\delta_{\mathrm{CP}}=4\pi/3$, $\theta_{12}=\tfrac13-\tfrac{\useed}{2}$ & \tP & PMNS readouts \\
$|V_{td}|,|V_{ts}|,J_{\mathrm{CKM}},J^\nu_{\mathrm{CP}}$; $n_s,r$ & \tP & derived readouts \\
\bottomrule
\end{longtable}
\end{center}
...
the \emph{anchor multiset} $\{1,1,2\}$ is derived (Schur--Horn $+$ stability) \tI; the \emph{sector
assignment} $(1,1,2)$ follows from the $D_5$ cusp ordering \tI/\tP; the \emph{first-generation $+6$
winding} is the hierarchy convention \tP; the six \emph{non-anchor residues} are then forced (residue
sets $=D_6$ orbit of $\{0,1,3\}$, minus anchor, monotone) \tI. The single remaining geometric input is
(H2), the specific $D_6$ cusp labelling on $\mathbb P^1\setminus\mu_4$ --- sharply characterised by the
residue-matrix spectral selector ($\det R=h(D_5)$, principal minors $(2,3,5)$), with a naive
cusp-position shortcut explicitly ruled out \tP. The explicit Riemann--Hilbert construction confirms
the rigid $SU(3)$ system and its $A_3$ Coxeter rotation; the genuine open step is the global
parabolic-weight (Hodge) realisation of that selector.
...
\begin{keybox}[The three nested grammars]
\[
  \begin{aligned}
...
\begin{keybox}[The compiler in one line]
\[
  \boxed{\ \text{TFPT}=\text{even carrier code}+\text{seed decoder}+2{\cdot}3{\cdot}5\ \text{Coxeter compiler}\ }
\]
From the three atoms $(2,3,\gcar{=}5)$:
\[
...
\begin{abstract}
This short companion compresses the TFPT 4.5 bridge layer to a single discrete object. The claim is
...
residue pairing on $H^1(\mathbb P^1\setminus\mu_4)$ is the $A_3$ Cartan form, and the Paper-2 corner
...
\[
  ...
  \end{aligned}\ }
\]
with $\cthree=\dfrac{1}{8\pi}$, $\gcar=5$, $h:=2\cdot3\cdot\gcar=30$, $r:=\varphi(h)=\varphi(30)=8$,
...
$\{1,\tfrac23,\tfrac13\}=\{2y_+,-2y_-,2(y_++y_-)\}$, with $\gamma_{\mathrm{car}}=\operatorname{Tr}_EY^2=3y_-^2+2y_+^2=\tfrac56$. \tI
\smallskip\\
...
\begin{keybox}[Hypercharge Lucas--Binet resonances \tI\ (mode reading \tP, \veri{v79\_review\_identities.py})]
...
hypercharge generator, not a free integer. \tI\ for the arithmetic; the per-mode physical identification is a
\tP\ cross-domain reading (only $n\le6$, the carrier-slot range, is meaningful).
\end{keybox}
...
\begin{aligned}
  ...
  &\boxed{\ R_4(C^+)=\text{Clebsch graph, } \mathrm{SRG}(16,5,0,2)\ }\qquad\tI .
\end{aligned}
\end{equation}
\begin{remark}
...
binomial'' --- it is the spectral dual of the even exterior family. \tP
\end{remark}
...
\tL\ (code identity) / \tP\ (holographic reading).
...
\begin{equation}
  ...
  \boxed{\ P^{\!\top}\operatorname{diag}(1,5,10)\,P=\operatorname{diag}(16,160,80)\ }\qquad\tI ,
\end{equation}
...
\begin{equation}
  \boxed{\ \operatorname{Aut}_{\mathrm{aff}}(C^+)=C^+\rtimes S_5,\qquad |\operatorname{Aut}_{\mathrm{aff}}(C^+)|=2^{\gcar-1}\,\gcar!=16\cdot120=1920=|W(D_5)|\ }\qquad\tI ,
\end{equation}
...
\begin{equation}
  \boxed{\ 2^{g}-2=g(g+1)\ \Longleftrightarrow\ g=5\ }\qquad\tI,
\end{equation}
...
\begin{equation}
  \boxed{\ |R(E_8)|=rh=240,\qquad \dim E_8=r(h+1)=8\cdot31=248,\qquad h+1=2^{\gcar}-1=31\ }\qquad\tI .
\end{equation}
...
\begin{equation}
  ...
  |R(E_8)|=2\binom{16}{2}=240\ \end{gathered}}\quad\tI .
\end{equation}
...
\begin{equation}
  \boxed{\ \operatorname{Tr}_{S^+}X^2=(N_{\mathrm{fam}}+N_\Phi)\,h=4\cdot30=120=\dim\mathfrak{so}_{16}=\underbrace{45}_{D_5}+\underbrace{15}_{A_3}+\underbrace{60}_{(10,6)}\ }\qquad\tI ,
\end{equation}
...
\begin{auditbox}[How the root \emph{origin} is now obtained --- see Theorem 7]
...
four punctures (Theorem 7, \tA), $E_8$ remains a Coxeter atlas with a concrete, falsifiable lift.
\end{auditbox}
...
\begin{equation}
  \boxed{\ 40=10\cdot4,\quad 60=10\cdot6,\quad 120=10\cdot12,\quad 240=10\cdot24\ }\qquad\tI ,
\end{equation}
...
\begin{aligned}
  ...
  &\boxed{\ \Oadm=\Nfam\dim S^+=(\Nfam{+}N_\Phi)|R(A_3)|=3\cdot16=4\cdot12=48\ }\qquad\tI ,
\end{aligned}
\end{equation}
...
\begin{equation}
  ...
  L_{f,j}\bmod 6\ \text{lives on}\ C_6=Z_6=|R^+(A_3)|\ }\qquad\tI ,
\end{equation}
...
The scalaron mass is a pure seam power \tI; the amplitude is a \emph{conditional} readout, kept
\tP/\tA\ until the scalaron variable is formally identified with the Starobinsky $M$ parameter. That
...
\begin{equation}
  \underbrace{\frac{M_{\mathrm{scal}}^2}{\Mbar^2}=\cthree^{\,r-1}=\cthree^7}_{\tI},\qquad
  \underbrace{A_s=\frac{N_\star^2}{24\pi^2}\frac{M_{\mathrm{scal}}^2}{\Mbar^2}=\frac{N_\star^2\cthree^7}{24\pi^2}=2.17\times10^{-9}}_{\tP/\tA\ (\text{if }M_{\mathrm{scal}}=M_{\mathrm{Star}})} .
\end{equation}
...
\emph{Stage 1 ($D_5$, \tI):} $C^+$ is the $D_5$ half-spin orbit, $R_4=$ Clebsch, $\operatorname{Aut}=W(D_5)$,
$|E(R_4)|=40=|R(D_5)|$ --- Theorem 2.
\emph{Stage 2 ($A_3$, \tI):} Paper 2 fixes $X_f^\circ=\mathbb P^1\setminus\mu_4$ with
...
\emph{The corner rotation realises the $A_3$ Coxeter element.} The Paper-2 generator $z\mapsto iz$
...
\begin{equation}
  ...
  \chi(t)=\tfrac{t^4-1}{t-1}=\Phi_4(t)\Phi_2(t)\ }\qquad\tI ,
\end{equation}
...
embeds in $W(A_3)=S_4$ (order $8$, index $3$) by isometries. \emph{Stage 3 (standard, \tI):} assemble
...
\begin{equation}
  ...
  [\,E_8:D_5\oplus A_3\,]=\sqrt{\tfrac{\det D_5\cdot\det A_3}{\det E_8}}=\sqrt{\tfrac{4\cdot4}{1}}=4=|\mu_4|\ }\qquad\tI .
\end{equation}
...
\begin{equation}
  ...
  \xi_{\mathrm{tree}}=\frac{\dimg}{\dim S^+}=\frac{12}{16}=\frac34=q(A_3)\ }\qquad\tI ,
\end{equation}
...
\begin{keybox}[The whole integer skeleton from one pair $(\gcar,\Nfam)=(5,3)$ \tI\ (\veri{v53\_compiler\_core.py})]
...
grand-mass-volume exponent $\Delta_Y=\gcar^2=25$ ($\det M_{\mathrm{SM}}\sim(\phiz)^{25}$, \S Paper~2)
...
\begin{keybox}[Boundary Carrier Selection Theorem (Theorem A) \tL/\tP]
...
lattice/classification facts are \tL; the physical selection (reflection-positive seam $\cthree=1/8\pi$ +
minimal chiral carrier $\Rightarrow$ exactly these inputs) is the \tP/\tA\ selection axiom (P1 analytic
...
\begin{equation}
  ...
  G_{\mathrm{closure}}=\frac{\mathrm{Spin}(10)\times SU(4)}{\Delta\mathbb Z_4}\subset E_8\ }\qquad\tI ,
\end{equation}
...
\begin{equation}
  ...
  \ =\ 240\ \text{roots, all norm }2,\ \text{Cartan }\det=1\ }\qquad\tL .
\end{equation}
...
corner-rotation $=$ $A_3$ Coxeter element are exact (residue pairing $+$ explicit Paper-2 symmetry).
...
preserves the root system, Weyl group and Coxeter element. So the lattice data is exact \tI; the
...
\begin{keybox}[The anchor power compiler]
\[
...
\begin{keybox}[One line]
\[
  \boxed{\ \text{Five carrier slots give }D_5,\ \text{four family corners give }A_3,\ \text{and }D_5\times A_3\text{ gives the }E_8\text{ atlas.}\ }
\]
Equivalently: $\AL=\tfrac{h}{\Nfam}=10=\binom52$, $h=30=2\cdot3\cdot5$, $(5,3,2):\ \sum=10,\ \prod=30$;
...
Statement & Status & Note \\
\midrule
Even code $C^+$, $\dim S^+=16$, dual-root $Y$ & \tI & Theorem 1 \\
Exterior--Action duality $(1,10,5)\!\leftrightarrow\!(1,5,10)$ & \tI & scheme eigenvalues verified \\
$R_4=$ Clebsch $(16,5,0,2)$, $R_2=(16,10,6,6)$; $\operatorname{Aut}_{\mathrm{aff}}=W(D_5)=1920$ & \tI & carrier $=D_5$ half-spinor \\
$2^g-2=g(g+1)\Leftrightarrow g=5$ & \tI & Coxeter closure (unique) \\
$h{=}30{=}2{\cdot}3{\cdot}5$, $r{=}\varphi(30){=}8$, $\chi_C{=}\Phi_{30}$ & \tI/\tP & cyclotomic compiler \\
$|R^+|=\binom{16}{2}=\operatorname{Tr}X^2=4h=120$ & \tI & positive-edge moment \\
$40,60,120,240=10\cdot(4,6,12,24)$; $41{=}1{+}4{\cdot}10$ & \tI & $K_5\times K_4$ product table \\
$10b_1{=}5r{+}1$, $u$ totient, $M_{\mathrm{scal}}^2{=}\cthree^{r-1}$, $\omega_b{=}\lC/10$ & \tI/\tP & downstream readouts \\
$A_s=N_\star^2\cthree^7/24\pi^2$ & \tP/\tA & if $M_{\mathrm{scal}}=M_{\mathrm{Star}}$ \\
$\mathfrak e_8=(45,1)\oplus(1,15)\oplus(10,6)\oplus(16,4)\oplus(\overline{16},\overline4)$ & \tI/\tP & $D_5\times A_3$ lift \\
$240$ roots built from $D_5{\oplus}A_3{+}\mu_4$ glue, $\det 1$ & \tL & explicit $E_8$ construction \\
flavor $\sum L{=}40{=}|R(D_5)|$, entries on $C_6{=}|R^+(A_3)|$ & \tI & transport $=D_5{\times}A_3$ \\
$A_3$ from $\mathbb P^1\setminus\mu_4$: residue pairing $=$ Cartan, $\det 4$ & \tI & Stage 2 (Theorem 7) \\
corner $z\mapsto iz=$ $A_3$ Coxeter el.: $\{i,-1,-i\}$, $\Phi_4\Phi_2$ & \tI & order $4=h(A_3)=|\mu_4|$ \\
$\operatorname{disc}(D_5)=\operatorname{disc}(A_3)=\mathbb Z_4$, glue $=\mu_4$ & \tI & $E_8=(D_5{\oplus}A_3){+}\mu_4$ \\
$q(D_5){=}\tfrac54{=}\dtwo/\dtop^2$, $q(A_3){=}\tfrac34{=}\xi_{\mathrm{tree}}$, sum $2$ & \tI & discriminant-norm glue \\
$120{=}\dim\mathfrak{so}_{16}$; $\Oadm{=}40{+}1{+}7$ & \tI & adjoint half; deficit ladder \\
$h(D_5){=}8{=}r(E_8)$, $h(A_3){=}4{=}|\mu_4|$, $|R|{=}h\cdot r$ & \tI & uniform Coxeter law \\
$\operatorname{Exp}(E_8)$ as $41/48$ budget deficits & \tA & exponent ledger (fingerprint) \\
geometric metric $D_4$-deform (anisotropy $-\log2$), residue $=A_3$ & \tI & metric computed/characterised \\
\bottomrule
\end{tabularx}
\end{center}

\noindent\emph{Scope.} This note compresses; it does not re-derive the upstream boundary kernel
(Paper 1) or the carrier rigidity theorem (Paper 2), and it makes no claim of a final theory. The
...
$\mu_4$ glue ($240$ roots, $\det1$). The only residual \tP\ is the analytic identification of Paper 2's
...
\begin{equation}
  \boxed{\ \dim S^+\bigl(1-q(D_5)q(A_3)\bigr)=16\cdot\tfrac1{16}=1\ }\qquad\tI ,
\end{equation}
...
$\dim E_8=\binom{|\mu_4|^2}{2}+2|\mu_4|^3=\binom{16}{2}+2\cdot64=120+128=248$ \tP.
...
assembled lattice is $E_8$, not merely a $240$-count. \tF\ (The shifted index $e_{k-2}{-}e_{k-3}$ would
...
--- that is honest, and it is the right level of claim. \tN
\end{keybox}
...
% =====================================================================
The load-bearing identities of this document (marked \tI, \tL, \tN) are re-derived from
