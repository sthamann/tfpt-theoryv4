...
\begin{keybox}[The current reduction state of P2 (2026-07-21) --- text consolidation, markers unchanged]
...
\veri{v510\_seam\_bit\_freedom.py}). What remains as carrier input is the \emph{square-modulus datum}
$\tau=i$ (equivalently: the clock / the CM-fixed half-period / the non-split extension class on the
free seam circle), plus the \mC\ identifications R1/R2 of \vref{v499}. In one sentence: \textbf{the
compiler's dimensionless inputs reduce to four marks (derived from P1-side topology), one
square-modulus datum ($\tau=i$), and $\pi$.} This is a reduction \emph{state}, not an abolition of
...
\mO, \vref{v367}/\vref{v368}/\vref{v376}--\vref{v379}/\vref{v336}). The extension leg of that citation package is
...
(\veri{v510\_seam\_bit\_freedom.py}, \texttt{SEAM.BIT.FREEDOM.01}).
