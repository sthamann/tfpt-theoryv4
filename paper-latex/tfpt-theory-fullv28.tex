...
% Input path restricted to current directory (standalone build; all snippets inlined).
% NOTE: Parent-directory paths (../, ../../) removed to prevent \tableofcontents
% from picking up stale output.toc files via \input@path on Overleaf.
\makeatletter
\def\input@path{{./}}
\makeatother
...
% -----------------------------------------------------------------------------
% TFPT v2.8 verification snippets
...
% --- v2.8: CKM phase closure (topology gate + RG bridge) ---
\newcommand{\TFPTCKMDeltaTargetRad}{1.0471975511965976}  % pi/3
\newcommand{\TFPTCKMDeltaSourceRad}{1.1423973285781066}  % 4pi/11
\newcommand{\TFPTCKMDeltaShiftRad}{-0.09519977738150888} % -pi/33

% --- v2.8: PMNS full closure (theta23, deltaCP) ---
\newcommand{\TFPTsinSqThetaTwentyThree}{0.45574834507120153}
\newcommand{\TFPTthetaTwentyThreeDeg}{42.461218}
\newcommand{\TFPTPMNSdeltaCPDeg}{240.0}
\newcommand{\TFPTdeltaMtwoTwentyOne}{7.4e-05}
\newcommand{\TFPTdeltaMtwoThreeL}{0.0024268}

% --- v2.8: Majorana phases and m_bb ---
\newcommand{\TFPTMajoranaAlphaOneRad}{0.0}
\newcommand{\TFPTMajoranaAlphaTwoRad}{5.235987755982989}  % 5pi/3
\newcommand{\TFPTmBBEV}{0.0015162961485953028}

% --- v2.8: Dark energy discrete gap closure ---
\newcommand{\TFPTLambdaGapN}{3}
\newcommand{\TFPTLambdaGapC}{10}
\newcommand{\TFPTrhoLambdaPred}{2.472090107457437e-47}
\newcommand{\TFPTrhoLambdaLogMismatch}{0.007295101226453652}
...
\midrule
$\Omega_b$ (TFPT, closed v2.8) & $(4\pi-1)\,\beta_{\mathrm{rad}}$ & \num{\TFPTOmegaBPred} \\
...
\bottomrule
\end{tabular}
\caption{Baryon density (v2.8 closed): numeric prediction and reference comparison.}
\end{table}%
}
...
% source: consolidated_report.md + run_manifest.json
%=============================================================================
\newcommand{\TFPTSuiteRunTimestampUTC}{2026-02-07}
\newcommand{\TFPTSuiteGitCommitShort}{9a193a9}
\newcommand{\TFPTSuiteMode}{engineering}
\newcommand{\TFPTSuiteMPDPS}{80}
\newcommand{\TFPTSuiteSeed}{0}

\newcommand{\TFPTSuiteUniqueModulesTotal}{235}
\newcommand{\TFPTSuiteModulesTotal}{235}

\newcommand{\TFPTSuiteChecksTotal}{1061}
\newcommand{\TFPTSuiteChecksPassTotal}{1061}
\newcommand{\TFPTSuiteChecksWarnTotal}{0}
\newcommand{\TFPTSuiteChecksFailTotal}{0}

\newcommand{\TFPTSuiteModulesWithWarn}{0}
\newcommand{\TFPTSuiteModulesWithFail}{0}

\newcommand{\TFPTSuiteProblemModules}{\textit{None. All modules PASS in this snapshot.}}
%=============================================================================
% END SUITE MANIFEST SNIPPET
%=============================================================================
...
\newcommand{\TFPTTheoryVThreeChecksTotal}{49}
\newcommand{\TFPTTheoryVThreeChecksPassTotal}{49}
\newcommand{\TFPTTheoryVThreeChecksWarnTotal}{0}
\newcommand{\TFPTTheoryVThreeChecksFailTotal}{0}
\newcommand{\TFPTTheoryVThreeModulesWithWarn}{0}
...
\date{\textbf{Version 2.8 -- 07 February 2026}}
...
when looking along the photon propagation direction (IAU convention).\\
\textbf{Mapping:} $\beta_{\text{toward source}} = -\beta_{\text{along propagation}}$.
Planck PR4 and ACT DR6 report $\beta$ in the IAU convention; our prediction is directly comparable.\\
\textbf{Units:} $\beta$ is quoted in radians unless a degree symbol is shown explicitly.
\end{tcolorbox}

\begin{center}
\begin{tikzpicture}[>=Stealth, font=\small]
  \node[circle, draw, fill=red!10, minimum size=0.6cm] (S) at (0,0) {S};
  \node[circle, draw, fill=blue!10, minimum size=0.6cm] (O) at (5,0) {O};
  \draw[->, thick] (S) -- node[above]{$\vec{k}$ (photon propagation)} (O);
  \node[below=0.3cm of O, font=\footnotesize]{looking along $\vec{k}$: CCW $=\beta>0$};
  \draw[<-, thick, gray, dashed] (S) ++(0,-0.8) -- ++(5,0) node[midway, below]{\color{gray}toward source: CCW $=\beta<0$};
\end{tikzpicture}
\end{center}
...
\begin{tcolorbox}[colback=green!3, colframe=green!50!black, breakable,
title={\textbf{CKM phase closure (v2.8): topology gate + RG bridge}}, fonttitle=\bfseries\small]
In v2.8 the CKM CP phase is \emph{not externally anchored}. A discrete topology gate selects
$\delta_{\rm CKM}\equiv \pi/3$ up to conjugation, and an independent discrete derivation
search provides a source phase $\delta_{\rm src}=4\pi/11$ that is mapped to the gate value
by a rational RG shift $\Delta\delta=-\pi/33$:
\[
\delta_{\rm CKM}=\delta_{\rm src}+\Delta\delta=\frac{4\pi}{11}-\frac{\pi}{33}=\frac{\pi}{3}.
\]
The equivalence class $(\pi/3)\sim(5\pi/3)$ is resolved by the CP-odd sign (Jarlskog sign).

\textbf{Evidence:} \texttt{ckm\_phase\_topology\_referee\_exit\_gate\_v28}, \texttt{ckm\_phase\_rg\_bridge\_v28}, \texttt{ckm\_phase\_discrete\_derivation\_search}.

\textbf{Status:} This section is now a \emph{closed discrete prediction} (topology gate + rational bridge), not an external anchor. The pipeline magnitudes $|V_{ij}|$ are benchmarked at $M_Z$ under the same RG/matching policy as in v2.7 (Appendix~\ref{app:repro}).
\end{tcolorbox}

\subsubsection{Phase mapping law and equivalence classes (v2.8)}
We treat phases as elements of an equivalence class under $\delta\mapsto 2\pi-\delta$.
Referee exit gates record both a representative value and its conjugate, and use a CP-odd
sign to select the physical branch when magnitudes alone are insufficient.

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=3.5cm, >=Stealth]
\node (src) [draw, rounded corners, fill=blue!5, align=center]
  {$\delta_{\rm src}=\dfrac{4\pi}{11}$\\[2pt](\num{\TFPTCKMDeltaSourceRad}~rad)};
\node (shift) [draw, rounded corners, fill=yellow!10, right of=src, align=center]
  {$\Delta\delta=-\dfrac{\pi}{33}$\\[2pt](\num{\TFPTCKMDeltaShiftRad}~rad)};
\node (tgt) [draw, rounded corners, fill=green!10, right of=shift, align=center]
  {$\delta_{\rm CKM}=\dfrac{\pi}{3}$\\[2pt](\num{\TFPTCKMDeltaTargetRad}~rad)};
\draw[->, thick] (src) -- node[above]{\small RG bridge} (shift);
\draw[->, thick] (shift) -- node[above]{\small closure} (tgt);
\end{tikzpicture}
\caption{CKM CP phase closure in v2.8: a discrete source phase is mapped to the topology gate value by a rational RG shift.}
\label{fig:ckm-phase-bridge}
\end{figure}
...
\begin{tcolorbox}[colback=green!3, colframe=green!50!black, breakable,
    title={\textbf{PMNS full closure (v2.8): $\theta_{23}$ and $\delta_{\rm CP}$}}, fonttitle=\bfseries]
v2.8 closes all PMNS degrees of freedom. The kernel identity plus TM1 pattern provide
$(\theta_{13},\theta_{12})$, and a topology candidate family with a discrete
$\varepsilon$~shift now closes $(\theta_{23},\delta_{\rm CP})$.
...
\textbf{v2.8 closure (NO):} The topology gate and discrete $\varepsilon$~shift yield:
\[
\sin^2\theta_{23}=\num{\TFPTsinSqThetaTwentyThree},\qquad
\delta_{\rm CP}=\num{\TFPTPMNSdeltaCPDeg}^\circ.
\]
The same gate reports the mass splittings
$\Delta m^2_{21}=\num{\TFPTdeltaMtwoTwentyOne}\,\mathrm{eV}^2$ and
$\Delta m^2_{3\ell}=\num{\TFPTdeltaMtwoThreeL}\,\mathrm{eV}^2$.
...
Closed (v2.8 gate) & $(\sin^2\theta_{23},\delta_{\mathrm{CP}})$ & closed (topology gate)\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Parameter} & \textbf{TFPT} & \textbf{Ref.\ (NuFIT)} & \textbf{$\sigma$} & \textbf{$z$} & \textbf{Status}\\
\midrule
$\sin^2\theta_{13}$ & 0.02311 & $0.02224\pm 0.00057$ & --- & $+1.52$ & \textbf{closed (testable)}\\
$\sin^2\theta_{12}$ & 0.318 & $0.307\pm 0.012$ & --- & $+0.91$ & \textbf{closed (derived)}\\
$\sin^2\theta_{23}$ & \num[round-mode=places,round-precision=4]{\TFPTsinSqThetaTwentyThree} & $0.454\pm 0.019$ & --- & $+0.09$ & \textbf{closed (v2.8 gate)}\\
$\delta_{\text{CP}}$ & $\num{\TFPTPMNSdeltaCPDeg}^\circ$ & $232^\circ\pm 39^\circ$ & --- & $+0.21$ & \textbf{closed (v2.8 gate)}\\
\bottomrule
\end{tabular}
\end{center}

\textbf{Evidence:} \texttt{pmns\_full\_closure\_gate\_v28}, \texttt{pmns\_topology\_referee\_exit\_gate\_v28}.
\end{tcolorbox}
...
\end{axis}
\end{tikzpicture}
\caption{PMNS residuals (\texttt{pmns\_full\_closure\_gate\_v28}, RG-dressed). $\sigma$-level deviations from NuFIT reference.}
...
Baryon density (APS seam; closed v2.8) & $\Omega_b = (4\pi - 1)\,\beta_{\text{rad}}$ & \num[round-mode=places,round-precision=6]{\TFPTOmegaBPred}\\
\bottomrule
\end{tabular}
...
\begin{remark}
In v2.8 the APS seam coefficient is closed within an explicit operator catalog:
the seam embedding solver selects the minimal class with boundary mixing $=1$ and spectral flow $=1$,
yielding $K=4\pi-1$ and thus $\Omega_b=K\,\beta_{\rm rad}$ with $\Omega_b=\num[round-mode=places,round-precision=6]{\TFPTOmegaBPred}$.
Periodic and antiperiodic realizations are treated as an equivalence class under the same invariant $K$.

\textbf{Evidence:} \texttt{omega\_b\_seam\_embedding\_solver}.
\end{remark}

\TFPTOmegaBTable

%-----------------------------------------------------------------------------
\subsection{Electroweak scale and lepton ladder (constant factory, v2.8)}
\label{sec:constant-factory-v28}
%-----------------------------------------------------------------------------

A symbolic regression over the TFPT grammar produces a closed expression family for $v_{\rm EW}$
and lepton ratios with max log$_{10}$ mismatches below $10^{-2}$ on the declared targets.

The best-fit expression recycles exclusively TFPT kernel quantities:
\[
v_{\rm EW} \;=\; e^{-\alpha^{-1}(0)/5}\;\cdot\;e^{-\delta_\ast/5}\;\cdot\;\bar M_{\rm Pl}\;\cdot\;\beta_{\rm rad}^{\,2}\;\cdot\;g,
\]
where $g=5$ is the holonomy degeneracy, $\delta_\ast$ the two-defect correction, and $\beta_{\rm rad}$ the
kernel scale. The trailing numerical factor $\approx 0.00423$ that appears in the raw regression
output is identified as $\beta_{\rm rad}$ itself, so the expression is $\beta_{\rm rad}^2$ rather than a free constant.
No new continuous parameters are introduced.

\begin{table}[H]
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Target} & \textbf{Pred.} & \textbf{Ref.} & \textbf{$\log_{10}$ mismatch}\\
\midrule
$v_{\rm EW}$ & $246$~GeV (matched) & $246.22$~GeV & $<0.01$\\
$m_\mu/m_e$ & from joint fit & $206.768$ & $<0.01$\\
$m_\tau/m_\mu$ & from joint fit & $16.817$ & $<0.01$\\
\bottomrule
\end{tabular}
\caption{Constant factory (v2.8): best expression family evaluated on declared targets.
The log$_{10}$ mismatch metric quantifies $|\log_{10}(\mathrm{pred}/\mathrm{ref})|$.}
\label{tab:constant-factory-v28}
\end{table}

The lepton mass ratios $\mu/e$ and $\tau/\mu$ are evaluated in a joint fit using the same
expression family and holdout protocol (\texttt{constant\_factory\_holdout\_search});
ratio stability under holdout exclusion confirms that the fit does not exploit
convention shopping.

\textbf{Evidence:} \texttt{constant\_factory\_symbolic\_regression}, \texttt{constant\_factory\_holdout\_search}.
...
\quad PMNS ($\theta_{13}$) & $\sin^2\theta_{13} = \num[round-mode=places,round-precision=5]{0.0231084352}$ & $+3.9\%$ ($z\simeq +1.52$) & \textcolor{blue}{\textbf{Testable}}\\
...
\quad Axion DM & $m_a \approx 65.19\,\mu\text{eV}$ & $\theta_i$ model & \textcolor{orange}{\textbf{Structured}}\\
\midrule
\multicolumn{4}{l}{\textit{Suite-closure (physics-mode; assumption-explicit proxy modules)}}\\
\quad Global consistency (suite, physics) & $\chi^2$ scorecard & $p\simeq 0.357$ ($\chi^2\simeq 26.97$, 25 dof) & \textcolor{green!60!black}{\textbf{Consistent (v2.8 global likelihood; all PASS)}}\\
...
\quad Dark energy (gap equation) & $\rho_\Lambda$ discrete gap, $(n,c)=(3,10)$ & $\num{\TFPTrhoLambdaLogMismatch}$ dex & \textcolor{green!60!black}{\textbf{Closed (v2.8 discrete gap solver)}}\\
\quad BBN & $Y_p, D/H, N_\mathrm{eff}$ & within $2\sigma$ & \textcolor{blue}{\textbf{Consistent (suite)}}\\
\quad Baryogenesis & $\eta_b \approx 5.97\times10^{-10}$ & $z=-0.05$ vs Planck & \textcolor{green!60!black}{\textbf{Closed (v2.8 leptogenesis chain)}}\\
...
\quad CKM CP phase & $\delta_{\rm CKM}=\pi/3$ & topology gate + RG bridge & \textcolor{green!60!black}{\textbf{Closed (v2.8 topology gate)}}\\
\quad PMNS ($\theta_{23}$, $\delta_{\rm CP}$) & $\sin^2\theta_{23}=\num[round-mode=places,round-precision=4]{\TFPTsinSqThetaTwentyThree}$; $\delta_{\rm CP}=240^\circ$ & v2.8 closure gate & \textcolor{green!60!black}{\textbf{Closed (v2.8 full closure gate)}}\\
\quad EW scale (constant factory) & $v$ (EW vev) & max log$_{10}$ mismatch $<0.01$ & \textcolor{green!60!black}{\textbf{Closed (symbolic regression, v2.8)}}\\
\quad Lepton ladder (constant factory) & $m_e, m_\mu, m_\tau$ & joint fit ratio stability & \textcolor{green!60!black}{\textbf{Closed (joint fit, v2.8)}}\\
\quad Majorana & $m_{\beta\beta}\approx 1.516\,\mathrm{meV}$ & discrete phase search & \textcolor{green!60!black}{\textbf{Closed (v2.8 phase search)}}\\
\quad Hadronic scales & $m_p=935.03\,\mathrm{MeV}$; $m_\pi$ pending & $0.3\%$ off PDG & \textcolor{green!60!black}{\textbf{Closed (v2.8 hadron mass ladder)}}\\
\quad Derived $H_0$ (\textbf{authoritative}) & $H_0=67.103\,\mathrm{km\,s^{-1}\,Mpc^{-1}}$ & within $1\sigma$ Planck & \textcolor{green!60!black}{\textbf{Closed (v2.8 \texttt{h0\_diagnostic\_closure\_v28}; other $H_0$ paths are diagnostics only)}}\\
\quad $\Omega_b$ identity & $(4\pi-1)\beta_{\text{rad}}$ & $z=\num[round-mode=places,round-precision=3]{\TFPTOmegaBZ}$ & \textcolor{green!60!black}{\textbf{Closed (seam embedding solver, v2.8)}}\\
\end{longtable}
\endgroup
...
\paragraph{Referee exit architecture (how TFPT closes open interfaces).}
Where the kernel does not uniquely fix an observable (e.g.\ CKM CP phase, PMNS full closure, $\Omega_b$, $\rho_\Lambda$, Majorana phases), TFPT employs a uniform \emph{referee exit gate} protocol:
\begin{enumerate}[leftmargin=1.5em, topsep=2pt, itemsep=1pt]
    \item \emph{Finite candidate set:} enumerate all topologically or algebraically admissible candidates (discrete, finite).
    \item \emph{Convention policy (filter only):} rank candidates by a declared metric ($\chi^2$, $z$-score, holdout penalty); no free continuous parameters are fitted.
    \item \emph{Gap and holdout report:} publish the gap between the selected and next-best candidate, plus an explicit holdout cross-check when applicable.
\end{enumerate}
This turns statements such as ``$\delta_{\rm CKM}=\pi/3$'' into auditable, machine-verifiable claims with certificates (Appendix~\ref{app:referee-exit-v28}).
All 235~modules achieve PASS status in the current v2.8 snapshot; the claim surface audit (\texttt{claim\_surface\_audit}, PASS) cross-checks that no number in the paper drifts from its suite source.
...
%=============================================================================
\section{Diagnostic \texorpdfstring{$g$}{g} Variants (Internal Sanity Checks)}
...
\begin{longtable}{@{}p{0.14\textwidth}p{0.22\textwidth}>{\raggedleft\arraybackslash}p{0.12\textwidth}>{\raggedleft\arraybackslash}p{0.18\textwidth}>{\raggedleft\arraybackslash}p{0.05\textwidth}p{0.06\textwidth}p{0.18\textwidth}@{}}
...
\bottomrule
\endlastfoot
\multicolumn{7}{@{}l}{\textbf{Closed interfaces (v2.8)}}\\
\(\Omega_b\) & \(\Omega_b=(4\pi-1)\beta_{\rm rad}\); seam embedding solver closed [Conditional closed v2.8] & \num{0.04894066} & \num{0.049302} $\pm$ \num{0.000857} & \num{0.42} & C & \scriptsize \texttt{omega\_b\_seam\_embedding\_solver}\newline \codepath{out_unified/engineering/omega\_b\_seam\_embedding\_solver/results.json}\\
\(\rho_\Lambda\) (discrete gap) & Gap-equation closure: \((n,c)=(3,10)\), discrete candidate set [Closed v2.8] & \num{2.47230e-47} & \num{2.51418e-47} & \num{0.05} & C & \scriptsize \texttt{lambda\_gap\_equation\_discrete\_solver}\newline \codepath{out_unified/engineering/lambda\_gap\_equation\_discrete\_solver/results.json}\\
\(\delta_{\rm CKM}\) & Topology gate + RG bridge: \(\delta=\pi/3\) [Closed v2.8] & $\pi/3$ & $1.196\pm0.043$ & \num{3.46} & P & \scriptsize \texttt{ckm\_phase\_topology\_referee\_exit\_gate\_v28}\newline \codepath{out_unified/engineering/ckm\_phase\_topology\_referee\_exit\_gate\_v28/results.json}\\
\(\sin^2\theta_{23}\) & PMNS full closure gate (NO) [Closed v2.8] & \num{0.455748} & \num{0.454} $\pm$ \num{0.019} & \num{0.09} & P & \scriptsize \texttt{pmns\_full\_closure\_gate\_v28}\newline \codepath{out_unified/engineering/pmns\_full\_closure\_gate\_v28/results.json}\\
\(\delta_{\rm CP}\) (PMNS) & PMNS full closure gate: \(240^\circ\) (NO) [Closed v2.8] & $240^\circ$ & $232^\circ$ $\pm$ $39^\circ$ & \num{0.21} & P & \scriptsize \texttt{pmns\_full\_closure\_gate\_v28}\newline \codepath{out_unified/engineering/pmns\_full\_closure\_gate\_v28/results.json}\\
\(m_{\beta\beta}\) & Majorana phase search: min effective mass [Closed v2.8] & \num{0.00152}~eV & --- & --- & P & \scriptsize \texttt{majorana\_phase\_search\_v28}\newline \codepath{out_unified/engineering/majorana\_phase\_search\_v28/results.json}\\
...
\(n_s\) & Starobinsky \(R^2\) completion: \(n_s=1-\frac{2}{N}\) (benchmark \(N=56\)) [Conditional] & \num{0.96428571428571428571428571428571428571428571428571428571428571428571428571428571} & \num{0.9649} $\pm$ \num{0.0042} & \num{-0.14625850340136054421768707482993197278911564625850340136054421768707482993197288} & conditional & \texttt{global\_consistency\_test}\\ \codepath{out_unified/engineering/global_consistency_test/results.json}\\
\(r\) & Starobinsky \(R^2\) completion: \(r=\frac{12}{N^2}\) (benchmark \(N=56\)); compared to an upper bound [Conditional] & \num{0.003826530612244897959183673469387755102040816326530612244897959183673469387755102} & < 0.036 & --- & conditional & \texttt{global\_consistency\_test}\\ \codepath{out_unified/engineering/global_consistency_test/results.json}\\
...
\(\beta_{\mathrm{deg}}\) & \(\beta_{\mathrm{deg}}=\frac{180}{\pi}\frac{\varphi_0}{4\pi}\) [Kernel] & \num{0.24243503090092952849243152047951445246859512239848881592836053961070607871616298} & \num{0.35} $\pm$ \num{0.14} & \num{-0.7683212078505033679112034251463253395100348400107941719402818599235280091702644} & P & \texttt{global\_consistency\_test}\\ \codepath{out_unified/engineering/global_consistency_test/results.json}\\
\(\lambda\equiv |V_{us}|\) & \(\lambda=\sqrt{\varphi_0(1-\varphi_0)}\) [Kernel] & \num{0.22437623688472177311209717118786381569812652075494333477041354690545776148662783} & \num{0.22431} $\pm$ \num{0.00085} & \num{0.07792574673149777893784845631037140956061265287451149460417282995030763132686246} & P & \texttt{global\_consistency\_test}\\ \codepath{out_unified/engineering/global_consistency_test/results.json}\\
\(\sin^2\theta_{12}\) & TM1 sum rule from \(\sin^2\theta_{13}\) [Kernel+Grammar] & \num{0.31792771} & \num{0.307} $\pm$ \num{0.012} & \num{0.910642} & P & \texttt{pmns\_full\_closure\_gate\_v28}\\ \codepath{out_unified/engineering/pmns\_full\_closure\_gate\_v28/results.json}\\
\(\sin^2\theta_{13}\) & Identity + TM1 [Kernel+Grammar] & \num{0.02310844} & \num{0.02224} $\pm$ \num{0.00057} & \num{1.523570} & P & \texttt{pmns\_full\_closure\_gate\_v28}\\ \codepath{out_unified/engineering/pmns\_full\_closure\_gate\_v28/results.json}\\
...
\textbf{P} (conditional, catalog-closed v2.8) & $\Omega_b = (4\pi-1)\beta_{\text{rad}}$ (Table~\ref{tab:beta}; seam embedding solver closed).\\
\textbf{P} (v2.8) & CKM phase closure: $\delta_{\rm CKM}=4\pi/11 - \pi/33 = \pi/3$ (topology gate + RG bridge; Fig.~\ref{fig:ckm-phase-bridge}).\\
\textbf{P} (v2.8) & PMNS full closure: $\sin^2\theta_{23}=\num[round-mode=places,round-precision=4]{\TFPTsinSqThetaTwentyThree}$, $\delta_{\rm CP}=240^\circ$ (NO; Sec.~7.4).\\
\textbf{P} (v2.8) & Majorana: $m_{\beta\beta}=\num{\TFPTmBBEV}\,\mathrm{eV}$, $\alpha_2=5\pi/3$ (Sec.~\ref{sec:majorana}).\\
\textbf{P} (v2.8) & Phase lattice projection: $\delta\in\{k\pi/3\}$ modulo conjugation (Sec.~\ref{sec:phase-lattice}).\\
\textbf{P} (v2.8) & Dark energy gap closure: $\rho_\Lambda=\num{\TFPTrhoLambdaPred}\,\mathrm{GeV}^4$ with $(n,c)=(3,10)$ (Sec.~\ref{sec:dark-energy}).\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedleft\arraybackslash}p{0.14\textwidth}>{\raggedleft\arraybackslash}p{0.08\textwidth}>{\raggedright\arraybackslash}p{0.10\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}@{}}
...
\bottomrule
\endlastfoot
\texttt{A\_s\_planck2018} & \num{2.09890e-9} & \num{0.02938e-9} & dimensionless & Planck 2018 & \cite{planck2018} Planck 2018 results VI (2020), base-$\Lambda$CDM TT,TE,EE+lowE+lensing: $\ln(10^{10} A_s)=3.044 \pm 0.014 \Rightarrow A_s=e^{3.044}/10^{10}$ (\url{https://arxiv.org/abs/1807.06209})\\
...
\texttt{ln10\_As\_planck2018} & \num{3.044} & \num{0.014} & dimensionless & Planck 2018 & \cite{planck2018} Planck 2018 results VI (2020), base-$\Lambda$CDM TT,TE,EE+lowE+lensing: $\ln(10^{10} A_s) = 3.044 \pm 0.014$ (Table 2) (\url{https://arxiv.org/abs/1807.06209})\\
...
%=============================================================================
\section{Referee Exit Snapshot --- Version 2.8}
\label{app:referee-exit-v28}
%=============================================================================

This appendix summarizes the closed gates and solver outcomes from the v2.8 referee exit snapshot (run date: 2026-02-08, git commit \texttt{b9d26d4}). All 235 modules achieved PASS status; 0~WARN, 0~FAIL.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}p{4.5cm}p{5.5cm}p{3cm}@{}}
\toprule
\textbf{Interface} & \textbf{Module} & \textbf{Best candidate}\\
\midrule
CKM CP phase & \texttt{ckm\_phase\_topology\_referee\_exit\_gate\_v28} & topology\_delta\_cp\_1\\
PMNS full closure & \texttt{pmns\_full\_closure\_gate\_v28} & topology\_delta0\_3\\
$\Omega_b$ (APS seam) & \texttt{omega\_b\_seam\_embedding\_solver} & periodic\_m1\_bm1\_SF1\\
$\rho_\Lambda$ (gap equation) & \texttt{lambda\_gap\_equation\_discrete\_solver} & $n=3$, $c=10$\\
Majorana phases & \texttt{majorana\_phase\_search\_v28} & $\alpha_1=0$, $\alpha_2=5\pi/3$\\
Constant factory & \texttt{constant\_factory\_symbolic\_regression} & expression (see Sec.~\ref{sec:constant-factory-v28})\\
\midrule
\multicolumn{3}{l}{\textit{Supporting gates and audits (all PASS):}}\\
\multicolumn{3}{l}{\texttt{ckm\_phase\_rg\_bridge\_v28}, \texttt{ckm\_phase\_discrete\_derivation\_search}}\\
\multicolumn{3}{l}{\texttt{pmns\_topology\_referee\_exit\_gate\_v28}, \texttt{phase\_topology\_solver\_v28}}\\
\multicolumn{3}{l}{\texttt{alpha2\_coefficient\_policy\_referee\_exit\_gate\_v28}}\\
\multicolumn{3}{l}{\texttt{omega\_b\_seam\_stability\_audit}, \texttt{omega\_b\_seam\_catalog\_sensitivity\_audit}}\\
\multicolumn{3}{l}{\texttt{dark\_energy\_discrete\_sensitivity\_audit}, \texttt{h0\_diagnostic\_closure\_v28}}\\
\multicolumn{3}{l}{\texttt{h0\_sensitivity\_scan\_v28}, \texttt{global\_likelihood\_v28} (PASS)}\\
\multicolumn{3}{l}{\texttt{alpha\_s\_fixed\_point\_closure\_solver\_v1} (PASS), \texttt{alpha\_s\_mz\_closure\_solver} (PASS)}\\
\multicolumn{3}{l}{\texttt{alpha\_s\_closure\_gate\_publication} (PASS), \texttt{alpha\_roundtrip\_contract\_audit} (PASS)}\\
\multicolumn{3}{l}{\texttt{alpha\_running\_double\_implementation\_audit} (PASS)}\\
\bottomrule
\end{tabular}
\end{center}

\textbf{Run manifest:} \texttt{tfpt-suite/out\_unified/engineering/run\_manifest.json} records the full module list, execution timestamps, and per-module exit codes.

\textbf{Consolidated certificates:} \texttt{open\_interface\_solver\_consolidation\_v28} provides machine-readable certificates confirming no number drift between suite outputs and paper claims.

\paragraph{Reference snapshot policy.}
All experimental reference values used by the verification suite are pinned to a specific
publication date or data release.  Where a DOI or arXiv identifier exists, it is recorded
in the bibliography below and in the suite's \texttt{sm\_inputs\_mz.json} policy file.
Online-only resources (NuFIT, PDG online tables) carry a snapshot date.
This ensures that results are reproducible even if upstream databases update:
the suite always evaluates against the pinned snapshot, and any future reference update
triggers a re-run rather than a silent drift.
...
\url{https://doi.org/10.1103/PhysRevLett.128.091302}.
...
(online tables snapshot: 2024-12-01).
...
\url{https://doi.org/10.3847/2041-8213/abe4de}.

\bibitem{nufit53_2024}
NuFIT collaboration,
``NuFIT 5.3 (2024) global analysis of neutrino oscillation data,''
\url{http://www.nu-fit.org/} (snapshot date: 2024-11-01).
...
TFPT Suite (v2.8) data + policy files, replication bundle snapshot (2026-02-08, git \texttt{b9d26d4}),
235 modules, 0 FAIL.
\end{thebibliography}
