...
overhead is not cherry-picking --- and those five are not diffuse slack but the forced two-family ladder
$6\cdot\text{spine}\{2,3,4,5\}\sqcup\text{det-ladder}\{8,14,20\}$ (\vref{v431}), so even the overhead is
...
\end{keybox}
\begin{keybox}[The unconditional floor, and the $\alpha$-form lever \mC\ (\veri{v432\_overdet\_floor.py})]
Multiplying \emph{only} across the genuinely disjoint pieces --- the input over-determination (the ``$8$''
forced four ways), the foreign $\alpha^{-1}$ census ($\sim1/94500$, \vref{v100}), the $\varphi_0$-seed cluster
counted \emph{once}, and one downstream witness ($K^+\!\to\!\pi^+\nu\bar\nu$) --- yields an \emph{unconditional}
improbability floor: $\le 10^{-6}$ across a grid of deliberately generous chance assignments, nominal
$\sim 10^{-10}$. This floor is grammar-\emph{independent} (it survives even if a skeptic rejects the declared
formula grammar) and sits $\sim20$ orders \emph{above} the conditional null-model ceiling $10^{-30.7}$
(\vref{v100}). That gap is exactly the evidential payoff of \emph{deriving} the $\alpha$ form: the open
form-derivation \texttt{ALPHA.QUILLEN.EXACT.01} (\vref{v382}; its cubic level now the \emph{computed} bulk Chern
invariant $|C|{=}1$ and its normalisation the embedding index $k_Y{=}5/3$, \vref{v470}; the determinant-line
bridge lemma exhibited at the finite level over the $U(1)$-twist moduli, \vref{v472} --- the continuum
$\zeta$-det identification still \mO) and this
floor are the \emph{same lever} ---
closing it moves the $\alpha$ factor from the $1/94500$ census toward certainty. Honest scope: this is a
conservative \emph{floor} (an upper bound on chance-plausibility), not a posterior, and it bounds only the
``is it numerology?'' question --- not the physics bridge (the firewall, \vref{v187}).
\emph{Hardened (\veri{v436\_overdet\_floor2.py}).} The verdict does not even need the chance assignments
above: stripped to a \emph{pure counting} census --- of the $94500$ complexity-matched $F_{U(1)}$ variants
only the TFPT one hits the CODATA window (\vref{v100}) --- the floor is $1/94500\approx4.40\sigma$ with
\emph{no} subjective probability, prior or multi-observable grammar. A monotone concession ladder
($30.7\!\to\!25.8\!\to\!4.98$ dex) keeps the ``not chance'' verdict at the \emph{most adversarial} rung, so
it is independent of the contestable input-``$8$'' independence; the only honest gap to the conventional
$5\sigma$ line is exactly one further independent confirmation (a factor $\le0.054$), stated openly.
\end{keybox}
\begin{figure}[ht]\centering
...
foreign witness $\alpha^{-1}\!\approx\!137$. The disjoint pieces give an \emph{unconditional} floor
$\sim10^{-10}$ (\veri{v432\_overdet\_floor.py}), $\sim20$ orders above the conditional $10^{-30.7}$;
hardened to an assumption-minimal \emph{counting} floor $1/94\,500\!\approx\!4.40\sigma$
(\veri{v436\_overdet\_floor2.py}) that drops every subjective chance assignment and a monotone
concession ladder ($30.7\!\to\!25.8\!\to\!4.98$ dex) that keeps the ``not chance'' verdict at the most
adversarial rung.}\label{fig:witness}
\end{figure}
...
$116/116$ base checks plus a $450/450$ extension mirror every exact algebraic / identity / lattice result
...
\veri{v424\_seam\_lto\_rp\_reduction.py}/\veri{v426\_seam\_lto\_rp\_kms.py}, its extension leg since re-founded
on peer-reviewed crossed-product theorems with the realisation input at invariant level,
\veri{v469\_seam\_crossedproduct\_route.py}), the metrology unit
...
No free pattern $+$ reverse audit & cherry-picking, decorative identities & \vref{v305}/\vref{v100}/\vref{v431}/\texttt{E8.REVERSE.AUDIT.01} \\
Over-determination map & rhetorical ``everything multiplies'' (incl.\ \emph{our own} miscount) & \vref{v427}/\vref{v428}/\vref{v432}/\vref{v436} \\
