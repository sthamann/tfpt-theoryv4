"""WP5e-beta of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no verification claim).

"THE EQUIVARIANT ANOMALY LEDGER ON TWISTOR SPACE" -- the twistor-side milestone
of WP5e (the BCOV/Costello-Li twistor uplift; preregistered kill: the anomaly
inflow demands a level != 1).  WP5e-alpha (v502) anchored the CFT side (q^{-1/3}
prefactor; k = 1 forced three ways).  Here the TWISTOR side is pushed as far as
exact arithmetic reaches: the Atiyah-Bott/Lefschetz fixed-point skeleton of the
one-loop box anomaly (Costello arXiv:2111.08879: box anomaly ~ Tr_adj X^4, GS
axion coupling lambda_g/(8 pi sqrt 3); Bittleston-Sharma-Skinner
arXiv:2208.12701 / CMP 403: axion eta in Omega^{0,1}(Omega^2_cl), O(-2)-valued
Poisson structure, Poisson-CS tower (+)_s O(2s-2)) evaluated EXACTLY on C^2/Z4
with the E8 mu4-glue monodromy (v492: inner element h = (2,2,2,2,2;0^4),
graded dims (60,64,60,64)).

E1  ATIYAH-BOTT FIXED-POINT LEDGER, exact:
    (i)   denominators det(1 - g_j^{-1}|_{T_0 C^2}) = (1-i^{-j})(1-i^j) =
          (2, 4, 2) for j = 1,2,3; sum of reciprocals 1/2+1/4+1/2 = 5/4 =
          (|Z4|^2-1)/12 (the A-series Dedekind sum identity); 0 is the only
          fixed point.
    (ii)  equivariant characters tr_{g_j}(Ad E8) = sum_m i^{jm} dim g_m =
          (248, 0, -8, 0), two independent routes (dims route / root-sum
          route); invariant average 60 = the D5+A3 carrier; Frobenius sum
          61568 = 4(60^2+64^2+60^2+64^2); tr_{g_2} = -8 = 120 - 128 (so16
          adjoint-minus-spinor supertrace).
    (iii) twisted quadratic forms K^{(j)} = sum_alpha i^{j class}<alpha,x>^2:
          K^{(0)} = 60<x,x> (Killing), K^{(1)} = K^{(3)} = 4 s5 - 12 s3
          (REAL), K^{(2)} = -4<x,x> (= 28 - 32); twisted quartics Q^{(j)}
          decomposed EXACTLY in the invariant basis {P1 = s5^2, P2 = s5 s3,
          P3 = s3^2, T5 = sum x^4, T3 = sum y^4}: class table Q_0 =
          12P1+6P3+4T5+8T3, Q_1 = Q_3 = 12P1+24P2-8T5+16T3, Q_2 =
          24P2+30P3+12T5-40T3.
    (iv)  OKUBO PER SECTOR -- honest sharpening: the invariant sector obeys
          Okubo exactly (Q^{(0)} = 36<x,x>^2 = (1/100)(K^{(0)})^2, T5 = T3 =
          0; v495 re-derived through the glue grading), but the TWISTED
          sectors do NOT reduce to (tr_{g_j} X^2)^2: exact independent-
          quartic content T5: (-8, 32, -8), T3: (48, -64, 48) for j = 1,2,3.
          In the AB-weighted fixed-point sum Q^{(1)}/2 + Q^{(2)}/4 +
          Q^{(3)}/2 the D5-quartic CANCELS exactly (T5: -4+8-4 = 0) and the
          A3-quartic leaves the exact residual 32 T3; RIGIDITY: no nonzero
          weighting of the twisted sectors alone is quartic-free, and on the
          class blocks the only quartic-free weighting is the uniform Z4
          average (= the invariant projector); the graded GS-exchange ansatz
          (products K^{(a)}K^{(b)}, a+b = j mod 4) cancels sector 0 and is
          exactly obstructed in sectors 1, 2, 3.  Heterotic anchors drop out
          on the way: Tr_120 = 8 tr^4 + 3 (tr^2)^2, Tr_128 = -8 tr^4 +
          6 (tr^2)^2, Tr_E8 = 9 (tr^2)^2 on the D5(+)A3 Cartan.
E2  McKAY/KRONHEIMER LEVEL MATCHING on the resolved A3 ALE:
    intersection form = -Cartan(A3), det C = 4 = |Z4|; tautological c1
    arithmetic: int c1(T_m)^2 = -(C^{-1})_mm, so ch2(T_m) = -(C^{-1})_mm/2 =
    (-3/8, -1/2, -3/8) = MINUS the A3 discriminant weights (v502 Casimir
    energies, geometric face); McKay adjacency of Q = chi_1 + chi_3 = affine
    A3 cycle, kernel (1,1,1,1) = tautological ranks; THE INDEX BRIDGE (the
    centrepiece): the per-character Atiyah-Bott sum (1/4) sum_j (i^{jm}-1)/
    det_j equals ch2(T_m) EXACTLY for m = 1,2,3 -- fixed-point ledger =
    intersection ledger; hence the equivariant ch2 defect of the glue bundle
    Ad = (+)_m g_m (x) T_m: (1/4) sum_j (chi(g_j)-248)/det_j = -78 = 64*
    (-3/8) + 60*(-1/2) + 64*(-3/8), an INTEGER, both routes agreeing; bridge
    identity sum_m h_A3(m) = 5/4 = sum_j 1/det_j; sphere pairing <alpha,h>
    INTEGER for all 240 roots, per-class Killing sums K_m(h) = (320, 320,
    240, 320), total 1200 = 2 h_vee (h,h) = 60*20 (v495 Killing identity at
    x = h); LEVEL DIAL: h(J;k) = k(0,1,1,1) integer for ALL k (does not fix
    k -- honest), the lattice current count fixes k = 1 (norm-2/k vectors in
    the glue cosets: 240 at k = 1, ZERO at k = 2,3,4: no adjoint closure),
    embedding residuals 47k^2+219k-266 = (0, 360, 814, 1362); v493 lockstep
    periods Pi = t(i-1)(1,i,i^2) => ONE scale => ONE level (det(A-1) = -4:
    no invariant flux vector, k is a scalar).
E3  AXION SLOT (weight/degree arithmetic, exact):
    volume form <lambda dlambda> dmu^1 dmu^2 has weight 2+1+1 = 4 = -deg
    K_PT; A3-family fibre form omega = Res(dX dY dZ / F): 4+4+2-8 = 2 in
    O(2) (same as EH 2+2+2-4 = 2: the graviton condition is k-independent);
    moduli weights a_m in O(8-2m): a0 in O(8) (the Z2 -> Z4 doubling O(4) ->
    O(8), ratio 2 = |Z4|/|Z2|, v493 S3.8); Hamiltonian slots: {O(a), O(b)}
    -> O(a+b-2), so the a0-deformation Hamiltonian sits in O(2) (2+8-2 = 8):
    a0 fills the BSS GRAVITON slot (s = 2), NOT the axion slot; the
    untwisted axion eta in Omega^2_cl corresponds to V in T (x) O(-4), i.e.
    Hamiltonian weight O(-2) = the s = 0 tower slot: X = -2 != +2 = Y -- the
    identification "a0 = GS axion" FAILS at the weight level (mismatch = 4 =
    -deg K_PT = |mu4|, typed [C] as a coincidence); what DOES match: the
    three H^2(ALE) classes carry the Coxeter eigencharacters {i,-1,-i} =
    exactly the three twisted ledger sectors j = 1,2,3 (bijection), no
    invariant class (det(A-1) = -4): the three sphere 2-forms fill the three
    TWISTED axion slots; the bulk axion must come from the O(-2) tower field
    itself -- construction [O].
E4  NEGATIVE CONTROLS: (a) diag(i,i): AB denominators (2i, 4, -2i) COMPLEX,
    reciprocal sum 1/4 != 5/4, omega-weight -1 (not CY), invariant ring
    needs 5 generators (not a hypersurface) -- ledger killed four ways;
    (b) SO(16) glue (so16 = so10 (+) so6 (+) (10,6) under the same h):
    classes {0,2} only (a mere Z2 monodromy), characters (120, 0, 120, 0)
    with +120 != -8 at j = 2, odd McKay nodes EMPTY, ch2 defect -30 != -78,
    and bulk Okubo FAILS (T5 = 16, T3 = -32 nonzero: independent quartic --
    no single-axion GS, v495-sl4-style control); (c) k = 2: current count 0,
    embedding residual 360, c = 31/2 => prefactor -31/48 -- but n(k) stays
    integral: integrality does NOT kill k = 2, the closure/current dial does
    (honest, exactly as on the v502 CFT side).
E5  HONEST FENCES: this is the Lefschetz SKELETON of the inflow, not the
    BCOV quantisation; what stays [O] is listed at the end.

Throwaway probe: standalone (sympy + Fractions), prints tables + PASS/FAIL +
verdict, ends with a check count.  Nothing here is a claim; promotion (if any)
goes through the usual workflow.  verification/, ledger, papers, changelog,
...
def section7():
    ...
    check("S7.4 [O-FENCE] open: the global BCOV/Kodaira-Spencer theory on "
          ...
          "NO marker moves anywhere -- exploration only", True)


# ---------------------------------------------------------------------------
def main():
    print("WP5e-beta CELEST.SEAM.01 -- the equivariant anomaly ledger on "
          "twistor space (exploration only)")
    ...
    print("\n=== VERDICT (see report) ===")
    print("  E1: the Atiyah-Bott ledger is exact -- denominators (2,4,2),")
    print("  characters (248,0,-8,0), invariant average 60 = carrier; the")
    print("  invariant sector obeys Okubo exactly (36<x,x>^2 = (1/100)K^2),")
    print("  the TWISTED sectors do NOT (T5, T3 enter); the AB sum kills")
    print("  the D5 quartic and leaves 32 T3 exactly (rigid).")
    print("  E2: INDEX BRIDGE f(m) = ch2(T_m) = -h_A3(m) exactly; glue")
    print("  defect -78 by two routes (integer); level dials all say k = 1")
    print("  (current count 240/0/0/0, residual 0/360/814/1362, one scale).")
    print("  E3: a0 fills the GRAVITON slot O(2), NOT the axion slot O(-2)")
    print("  (mismatch 4); the three sphere classes fill the three twisted")
    print("  axion slots (Coxeter characters = sector characters).")
    print("  E4: diag(i,i), SO(16) glue and k = 2 each break the ledger in")
    print("  quantified independent ways.  E5: BCOV quantisation stays [O].")
