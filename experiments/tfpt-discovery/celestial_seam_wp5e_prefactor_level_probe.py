"""WP5e-alpha of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no verification claim).

"PREFACTOR AND LEVEL BOOKKEEPING" -- the first computable milestone of the
multi-year WP5e (the BCOV/Costello-Li twistor uplift: boundary partition
function = E4/eta^8 INCLUDING the q^{-1/3} prefactor; kill: anomaly inflow
demands a level != 1).  The full uplift stays out of reach; two of its
consistency anchors are exactly computable and are probed here.

PART A -- the q^{-1/3} prefactor from the mu4-sector vacuum energies.
  S1  Hurwitz-zeta vacuum energies, symbolic (sympy): for one real boson
      twisted by theta, E_b(theta) = (1/4)[zeta(-1,theta) + zeta(-1,1-theta)]
      = -1/24 + theta(1-theta)/4 (exact Bernoulli identity B2); fermionic
      counterpart E_f = -E_b (Ising h_sigma = 1/16 = E_R - E_NS); 8 untwisted
      bosons give -8/24 = -1/3 = -c/24 at c = 8 (= Sugawara 248/(1+30));
      eta^8 = q^{1/3} prod(1-q^n)^8, so E4/eta^8 = q^{-1/3}(1,248,4124,34752).
  S2  The mu4 twist DISTRIBUTION from the v492 machinery: the glue is INNER
      (h = (2,2,2,2,2;0^4) reads the class mod 4 on all 240 roots; the A3-side
      detector h' = (0^5;1,1,1,-3) reads the SAME class); clock eigenvalue
      multiplicities on the 248 = (60,64,60,64) with the 8 Cartan directions
      ALL at eigenvalue i^0 = 1 => the induced twist on the 8 torus bosons is
      theta = 0 (x8) in EVERY sector C_j: a SHIFT orbifold (sectors = lattice
      cosets, oscillators integer-moded), NOT a rotation orbifold; oscillator
      vacuum energy = -1/3 in all four sectors = the sector-independent
      q^{-1/3} prefactor.
  S3  DISCRIMINANT FORM = CASIMIR ENERGY: sector ground weights = coset minima
      h_D5 = (0,5/8,1/2,5/8), h_A3 = (0,3/8,1/2,3/8), glue diagonal (0,1,1,1)
      INTEGER; the identity holds mod 1 via SPECTRAL FLOW (h_flow(j) =
      j^2 (h,h)/32: (h,h) = 20 -> 5j^2/8, (h',h') = 12 -> 3j^2/8, sum 32 ->
      j^2 = 0 mod 1 = the isotropy/Lagrangian condition) and EXACTLY via the
      FREE-FERMION realisation (D5 = 10 Majorana, A3 = D3 = 6, glue diagonal
      = 16 = the WP5d seam carrier: R-NS Casimir shift n/16 = 5/8, 3/8, 1);
      the naive ROTATION reading fails (the geometric A3 deck gives 3/16 !=
      3/8): the v492 S5 lesson (clock != deck) at the Casimir level.
  S4  CHARACTER RECONSTRUCTION: Theta_{C_j} are even integer q-series; the
      four sector characters q^{-1/3}(Theta_j / prod(1-q^n)^8) have leading
      exponents -1/3 + h_j and leading coefficients (1,64,60,64), level-1
      currents (60,64,60,64), and sum EXACTLY to E4/eta^8 =
      q^{-1/3}(1 + 248q + 4124q^2 + 34752q^3).

PART B -- level k = 1 from three independent exact constraints.
  S5  (i) GLUE/EXTENSION: simple-current weights at level k are k times the
      discriminant form (h(k om_v) = k/2, h(k om_{s,c}) = 5k/8 for D5;
      h(k om_j) = k j(4-j)/8 for A3 -- affine formula, exact closed forms);
      glue-diagonal totals = k(0,1,1,1): INTEGER FOR ALL k = 1..8 --
      integrality alone does NOT fix k (honest sharpening of the naive
      expectation); what fixes k on this side: the extension fields must be
      CURRENTS (h(J) = k = 1) for the four sectors to close onto the E8
      adjoint 60+64+60+64 = 248, and the conformal-embedding equations
      (D5+A3 in E8: 47k^2 + 219k - 266 = 47(k-1)(k+266/47), unique positive
      root k = 1; D8 in E8: 128(k-1) = 0).
      (ii) CENTRAL CHARGE: 248k/(k+30) = 8 <=> 240(k-1) = 0 <=> k = 1
      (unique; 240 = #roots); prefactor -c/24 = -1/3 <=> c = 8 <=> k = 1.
      (iii) WP5B LEVEL DIAL: mini affine-sl2 PBW engine (theta-sl2 at level
      k): ||(E^theta_{-1})^m 0||^2 = m! k(k-1)...(k-m+1) (engine-verified),
      singular vector exactly at grade k+1; k = 1: level-2 vacuum dim =
      31124 - 27000 = 4124 (Weyl dims 248/3875/27000 recomputed from E8 root
      data; Sym^2 248 = 1 + 3875 + 27000); k = 2: <s|s> = +4 (the WP5b/WP5c
      value), nothing deleted at level 2, 31124 != 4124.
      (iv) [O] FENCE: the twistor-side anomaly inflow (Costello's axion
      lambda_g/(8 pi sqrt 3), the CPS level-from-flux mechanism) is NOT
      recomputed here -- v495 anchors the Lie-algebra arithmetic only; the
      BCOV/Costello-Li partition-function side of WP5e stays [O].
  S6  NEGATIVE CONTROLS: (a) D8/SO(16)_1 -- same c = 8, same q^{-1/3}
      prefactor, but sector h = (0,1/2,1,1): the discriminator is the
      h-INTEGRALITY of the glue diagonal, not the prefactor; Theta_D8 +
      Theta_s = E4 (the {1,s} Lagrangian extension IS E8). (b) wrong twist:
      the Z2 ROTATION (all 8 bosons theta = 1/2) gives E = +1/6, h = 1/2 --
      no mu4-shift sector sits there; a mu4 ROTATION makes E_j sector-
      DEPENDENT (-1/3, 1/24, 1/6, 1/24): the common prefactor breaks.
      (c) wrong k in {2,3,4}: five independent failures per k (current
      condition, embedding residual, central charge, prefactor exponent,
      singular-vector norm / level-2 count).
  S7  VERDICT + honest typing.
...
# ---------------------------------------------------------------------------
# exact integer power series helpers (v492/WP5c style)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# E8 root system in DOUBLED integer coordinates (WP5c crib, trimmed):
...
# ---------------------------------------------------------------------------
# mini affine sl2 PBW engine (WP5c Affine crib on the 3-dim theta-sl2):
...
D8_OM = {'v': (F(1),) + (F(0),) * 7,
         's': (HALF,) * 8,
         'c': (HALF,) * 7 + (-HALF,)}
D8_RHO = tuple(F(7 - i) for i in range(8))
D8_HVEE = 14
...
# ---------------------------------------------------------------------------
def main():
    print("WP5e-alpha CELEST.SEAM.01 -- prefactor and level bookkeeping "
          "(exploration only)")
    p8, e4, chi = section1()
    ...
    print("\n=== VERDICT (see report) ===")
    print("  WP5e-alpha: BOTH consistency anchors of the twistor uplift hold")
    print("  exactly.  (A) The q^{-1/3} prefactor of E4/eta^8 is the")
    print("  mu4-sector vacuum-energy bookkeeping: the clock is INNER, so the")
    print("  twist distribution on the 8 torus bosons is theta = 0^8 in all")
    print("  four sectors (shift orbifold) and every sector carries the same")
    print("  8 x (-1/24) = -1/3 = -c/24; the sector ground exponents are")
    print("  -1/3 + (0,1,1,1) with h_j = coset minima = discriminant form")
    print("  (5j^2/8, 3j^2/8) -- an identity that holds via spectral flow")
    print("  (j^2 (h,h)/32) and exactly via free fermions (10+6 = 16")
    print("  Majoranas, R-NS shift n/16: 5/8, 3/8, 1 -- the WP5d seam")
    print("  carrier); the four sector characters sum to E4/eta^8 =")
    print("  q^{-1/3}(1, 248, 4124, 34752).  (B) k = 1 is forced three")
    print("  independent ways: h(J) = k = 1 (current condition; integrality")
    print("  alone holds for all k -- honest sharpening), conformal embedding")
    print("  (47(k-1)(k+266/47) = 0, 128(k-1) = 0), c = 8 <=> 240(k-1) = 0")
    print("  (= the prefactor itself), and the WP5b singular-vector dial")
    print("  ((E^theta_{-1})^{k+1}: 31124 - 27000 = 4124 at k = 1 only).")
    print("  Controls: D8 has the SAME prefactor but h = (0,1/2,1,1) (the")
    print("  discriminator is glue-h integrality, not the prefactor); a Z2 or")
    print("  mu4 ROTATION twist breaks the common prefactor (E_j sector-")
    print("  dependent); k = 2,3,4 fail all five dials coherently.  OPEN:")
    print("  the BCOV/Costello-Li side (the actual twistor inflow) -- [O].")
