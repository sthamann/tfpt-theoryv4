"""WP5e-M1 probe of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no
verification claim).

"M1: THE A3 OMEGA_N" -- executes the first preregistered backreaction
milestone of v514 S8.1 (VERBATIM there): construct the backreacted
holomorphic 3-form Omega_N on the twistor uplift of the (deformed) A3
ALE and check its periods exactly.  Preregistered criterion:
SUCCESS = a closed-form Omega-deformation whose S^3/Z4 periods are
(2 pi i)^2 x (integer flux vector) reproducing the lockstep phase
structure (i-1)(1, i, i^2); KILL = unequal sphere fluxes or a
non-integer period.

References: Costello-Paquette-Sharma arXiv:2306.00940 (Omega_N brane
backreaction, eq. 3.5/3.33; large-gauge invariance of hCS quantises the
period in (2 pi i)^2 Z); Hitchin "Polygons and gravitons" (A-series
twistor family XY = prod_p (Z - q_p(lambda)) with q_p in O(2); the
twistor space is the small resolution of the nodal family); Gibbons-
Hawking Phys. Lett. 78B (centres); v492 (clock diag(i,1), omega-phase
det = i), v493 (clock family XY = Z^4 + a0, Coxeter clock, lockstep),
v505 E3 (weights Z in O(2), X,Y in O(4), F in O(8), Omega in O(4)),
v509 (CPS skeleton (2 pi i)^2 N, lockstep theorem, 0/24 negative probe),
v514 (eps1 ledger, GH/A3 dichotomy, M1-M3 preregistration).
Exact sympy arithmetic throughout, no floats.

E1  RESIDUE FORM (S1): omega2 = dX^dZ/X is the Poincare residue of the
    A3 family (three ambient representatives agree on the patch and
    against dF); orbifold-cover pullback omega2 = 4 dz1^dz2 = |mu4| x
    (flat form): the residue normalisation CARRIES the source charge 4;
    clock diag(i,1): omega2 -> i omega2 (v492 S5 det = i, exact);
    weight ledger 4+4+2-8 = 2 = O(2) fibre symplectic form, Omega in
    O(4) = -deg K_PT (v505 S5.1/S5.2); EH anchor factor 2 = |Z2|;
    wrong-weight control Z in O(4): omega2 weight 8+8+4-16 = 4 != 2 (an
    O(4)-valued form, not the symplectic slot), Omega weight 6 != 4,
    real-section count 5 != 3 = dim R^3 (GH base broken).
E2  TWISTOR FAMILY (S2): free-orbit centre sections q_p(la) = i^p t0 -
    i^-p t0 la^2 (honest O(2) real sections); the family is EXACTLY
    XY = Z^4 + 4 t0^2 la^2 Z^2 - t0^4 (1 - la^4)^2 (e1 = e3 = 0
    identically): the v493 clock family at the seam fibre la = 0, the
    a2-slot opens off-seam exactly as the O(4) weight household demands,
    and the la -> 1/la patch gives the SAME form (self-mirror); clock
    lift gamma: (Z, la) -> (iZ, -i la) preserves the family, gamma^4 =
    1, omega2 -> i omega2 and dla -> -i dla, so Omega -> +Omega (the
    CY-compatible lift); axis sections 2 h la are clock-invariant (the
    resolution branch, v514 S4.1 dichotomy at section level).
E3  FIBRE PERIODS + LAMBDA REDUCTION (S3): int_{S_j} omega2 = 2 pi i
    (q_{j+1} - q_j) EXACTLY for generic radial profile and generic path
    reparametrisation (both drop out identically) -- the A-series
    "period = root difference" normalisation of v493 is DERIVED from
    the residue form, not assumed; Pi_j(la) = 2 pi i t0 [(i-1) i^j +
    (1+i) i^-j la^2]; at la = 0: Pi = 2 pi i t0 (i-1)(1, i, i^2), all
    three moduli 2 sqrt(2) pi t0 (LOCKSTEP with the Coxeter phases);
    clock covariance Pi_{j+1}(-i la) = i Pi_j(la) for ALL la = the
    omega2-phase i (consistent bookkeeping, Pi . A = i Pi at la = 0);
    sphere x la-circle periods of Omega_0: contour integral of the
    polynomial Pi_j(la) = 0 (no residue): the UNDEFORMED Omega_0
    carries no quantised 3-flux -- all flux must be sourced;
    collision ledger: 12 nodes, adjacent pairs at la^2 = i^{2j+3},
    non-adjacent at la = +-1, +-i, la-support = ALL 8 eighth roots of
    unity (8 = 2|mu4|, the Z8 seam bridge), antipodal pairing q_{p+2} =
    -q_p forces DOUBLE nodes on the adjacent fibres, clock orbits
    4+4+4, consecutive node positions rotate by one clock step.
E4  OMEGA_N AND ITS QUANTISED PERIODS (S4): the CPS/Bochner-Martinelli
    kernel K = d^2v ^ (vb1 dvb2 - vb2 dvb1)/|v|^4: int_{S^3} K =
    (2 pi i)^2 exactly (density -sin 2 theta, v509 S1.1 replicated),
    dK = 0 away from the source (exact exterior derivative), scale
    degree 0; the (2 pi i)^2 factorises as (fibre phase circle) x
    (transverse phase circle) = the iterated-residue reading of
    "fibre period x lambda residue"; deck diag(i, i^-1) acts FREELY on
    S^3, leaves K and Omega_0 invariant: everything descends, lens
    period int_{S^3/Z4} = (2 pi i)^2 N/4, so quotient large-gauge
    quantisation FORCES N in 4Z: source charge 4 = |mu4| (v514 S6
    confirmed at Omega level); minimal invariant charge 4 <-> lens
    period (2 pi i)^2 x 1 = the minimal CPS quantum N = 1 <-> k = 1
    (v509 bridge); deformed phase: the four brane curves = the centre
    twistor lines L_p = {X = Y = 0, Z = q_p(la)} lie on the family,
    the clock permutes them cyclically AND they are pairwise linked
    into ONE connected curve (all 6 pairs meet: K4), so a single N is
    forced twice over (clock orbit + connectivity): flux vector
    N (1,1,1,1), boundary period 4 N (2 pi i)^2 by Stokes + the local
    anchors, orbifold-limit cover count 16 = |mu4|^2 (v509 S3.5 mu =
    16 resonance); the 12 collision points are honest 3-fold NODES
    (dF = 0, Hessian rank 4): the sphere classes are the exceptional
    curves of the small resolution, node = where Pi_j(la) = 0, the
    per-sphere flux vector is uniform on each clock orbit: M1 SUCCESS
    criterion met: (2 pi i)^2 x integer vector + lockstep phases.
E5  KILL PROBE (S5): the clock-forbidden family (centres {0, 1, w, w^2},
    P(Z, 0) = Z^4 - Z, the a1 != 0 type of v509 S4.4): section set NOT
    clock-covariant ({i z_p} != {z_p}); period moduli at the seam fibre
    2 pi {1, sqrt 3, sqrt 3}: 0/24 orderings lockstep (vs 8/24 for the
    mu4 orbit); node la-support = the 12 TWELFTH roots of unity (not
    Z8), no antipodal double-node pairing; HONEST: the (2 pi i)^2
    quantisation itself holds there too (it is local gauge invariance
    -- not the discriminator); what fails is the lockstep phase/modulus
    structure and the clock forcing -- the v509 dial SURVIVES, the
    KILL does not fire.
E6  NEGATIVE CONTROLS (S6): EH/Z2 from the same machinery (XY = Z^2 -
    t0^2 (1 - la^2)^2, 2 nodes at la = +-1, lens forces charge 2 =
    |Z2|); CPS original = k = 1 flat patch (full S^3, every integer N
    allowed: the unconstrained anchor); diag(i,i): Omega_0 -> -Omega_0
    (no invariant 3-form on the quotient: the deformation problem
    collapses at step zero) while K stays invariant (U(2)); fractional
    seam charges N = 1, 2, 3 all fail the lens quantisation (only
    0 mod 4 passes).
E7  HONEST FENCES (S7): what M1 does NOT show -- the twisted KS measure
    per sector (M2), the a0/Kahler-log uplift (M3), the quantised BCOV
    coefficient, the global smooth patching of the local kernels
    (Green function on the resolved 3-fold) and the exceptional-curve
    <-> glue-current dictionary stay [C]; SEAM.EQUIV.01 untouched; no
    marker moves anywhere.

Throwaway probe: standalone (sympy), prints tables + PASS/FAIL +
...
def section4():
    ...
    lens_per = sp.integrate(dens, (th, 0, PI / 2)) \
        * (2 * PI) * (PI / 2)
    Ns = sp.symbols('N', integer=True, positive=True)
...
def section7():
    ...
    check("S7.3 [M1 VERDICT, PREREGISTERED WORDING] SUCCESS criterion "
          "met at probe level: closed-form Omega-deformation, S^3/Z4 "
          "periods (2 pi i)^2 x (integer flux vector), lockstep phase "
          ...
          "the separator is the phase/modulus lockstep; promotion "
          "(if any) via the usual workflow, suggested id "
          "v515_celestial_wp5e_m1_a3_omega_n", True)


# ---------------------------------------------------------------------------
def main():
    print("WP5e-M1 probe: the A3 Omega_N -- backreacted 3-form and its "
          "exact periods (EXPLORATION ONLY)")
    section1()
    section2()
    section3()
    section4()
