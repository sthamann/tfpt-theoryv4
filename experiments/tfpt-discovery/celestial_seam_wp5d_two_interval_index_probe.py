"""WP5d-alpha of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no verification claim).

"THE TWO-INTERVAL INDEX FROM THE LATTICE" -- first / preparation milestone of WP5d
(the local net: interval algebras on the seam circle, Haag duality / local normality,
KLM criteria).  Roadmap discriminator: (E8)_1 => mu = 1 (Haag-dual two-interval net,
ONE sector); SO(16)_1 => mu = 4.  KILL is operationalised here as "the two-interval
mu-offset does NOT vanish after the condensation".

Setup: the v367 seam edge is 16 chiral Majorana layers (c_- = 8, v489).  The faithful
1d edge reduction is the critical Majorana ring (one Majorana per site, NS/antiperiodic
closure, c = 1/2 per layer -- the same reduction style as v450).  Everything Gaussian
is EXACT via the Peschel covariance Gamma = i sgn(iA); the two-interval objects are:

    fermionic (fixed spin structure)  : rho_AB          (the graded/CAR algebra state)
    orbifold  (sector-summed, SO(16)) : sigma = average over the parity group
                                        {1, P_A, P_B, P_AB}  = (rho + P_A rho P_A)/2
    dual (2-of-4 structures, "wrong") : average over {1, P_AB} = rho  (machine identity)

The entropic two-interval duality defect D(x) = S(sigma) - S(rho) is the mu-index
indicator (continuum dictionary, cited: KLM mu = index of the 2-interval subfactor;
Longo-Xu / Casini-Huerta-Magan-Pontello: D(x -> 1) = ln[index of the subnet] =
(1/2) ln mu when the parent is graded-dual).  Operational reading:  mu_op =
exp(2 * D(x->1)).

  S0  MACHINERY VALIDATED AGAINST ED (no formula trusted untested): covariance from
      exact-diagonalised Gaussian rho; Peschel entropy; Tr(rho1 rho2) =
      2^{-m} sqrt(det(1 - G1 G2)); <P_A> vs Pf(Gamma_A); the Gaussian two-interval
      Renyi-2 deficit == ED deficit to machine precision (disjoint construction).
  S1  CHAIN SANITY: exact purity; Calabrese-Cardy c = 1/2 per layer; 16 layers
      additive => c_- = 16/2 = 8 = g_car + N_fam; Ptolemy x + (1-x) = 1 exact.
  S2  FERMIONIC MI IS EXTENSIVE (the mu = 1 reference curve): I(x) = -(c/3)ln(1-x)
      with F(x) := I_lat - I_CFT -> 0 (numerical witness of the Casini-Huerta
      extensivity for the fixed-spin-structure prescription); c fit from MI.
  S3  THE MU-OFFSET (core): the sector-summed prescription costs entropy;
      Delta_2(x) = -ln[(1 + R)/2], R = Tr(rho P rho P)/Tr rho^2 (exact Gaussian),
      interpolates 0 -> ln 2; ED (N = 12..24) confirms the von Neumann and
      Renyi-3/4 deficits track it (plateau is n-independent); the 16-layer seam
      carrier (diagonal Z2) hits |Delta - ln 2| < 1e-14 at N = 512; per-layer
      gauging control gives 16 ln 2 (the offset measures ln[index], not a blob).
      Index reading: [F:O] = exp(Delta_inf) = 2 => mu_gauged = 4 = v490's mu_pre.
  S4  HAAG-DUALITY INDICATOR: fermionic complementarity S(E) = S(E') machine-exact
      (graded duality proxy); the orbifold breaks it (D(E) != D(E'), the direct
      duality-failure witness); the pair sum D(E) + D(E') is exchange-symmetric,
      bounded by ln 4 = ln mu(SO(16)_1) (max at the self-dual x = 1/2) and equals
      ln 2 per saturated side; the "dual algebra" prescription {1, P_AB} is
      machine-identically defect-free.
  S5  CONDENSATION ARITHMETIC 16 -> 4 -> 1 ANCHORED: det Cartan (D5)x(A3) = 16,
      mu/|L|^2 with |L| = |mu4| = 4 => 1 (E8), |L| = 2 => 4 (SO(16)_1), 4/2^2 = 1;
      Sigma d_i^2 (Ising 4, SO(16)_1 4, E8 1); theta_v = e^{2 pi i nu/16} = 1 at
      nu = 2 c_- = 16 exactly (condensable) vs nu = 1, 2, 8 rivals; the measured
      lattice chain mu: 1 (graded) -> 4 (gauged, measured AND v490-counted) -> 1
      (arithmetic, the interacting condensed net is not free-fermion computable).
  S6  NEGATIVE CONTROLS: (a) nu = 1 single Majorana: offset ln 2 present but NOT
      removable (theta_v = e^{i pi/8} != 1, Ising mu = 4 stays > 1); (b) trivial
      (dimerised) phase: MI = 0, offset = 0, c = 0 -- nothing conformal, nothing
      topological; (c) wrong sector sum ({1, P_AB} only, "2 of 4 structures"):
      offset exactly 0 instead of ln 2 -- measurably wrong bookkeeping.
  S7  HONEST TYPING + VERDICT: [E] the arithmetic (Cartan dets, KLM/Longo-Rehren
      quotients, theta_v, Sigma d^2); [N] the lattice curves/offsets (Gaussian-exact
      formulas, ED-validated); [C] the continuum dictionary "offset = ln index"
      (KLM, Longo-Xu, CHMP) and the condensation step itself (interacting); [O]
      the actual continuum Haag duality / local normality / split property of the
      quotient net (WP5d-beta) and Costello-Li (WP5e).  KILL ("mu-offset != 0
      after condensation") NOT triggered: post-condensation offset = 0 by the
      same arithmetic that the lattice anchors at both ends (1 and 4).

Throwaway probe: standalone (numpy + scipy + fractions), prints tables +
PASS/FAIL + verdict, ends with a check count.  Nothing here is a claim;
verification/, ledger, papers, changelog, website, scorecard untouched.
"""
import numpy as np
...
# ---------------------------------------------------------------------------
def main():
    print("WP5d-alpha CELEST.SEAM.01 -- the two-interval index from the "
    ...
    print("\n=== VERDICT (see report) ===")
    print("  WP5d-alpha: the mu = 1 indicator chain PASSES.  The fermionic")
    print("  (fixed-structure) two-interval MI is extensive (offset 0, the")
    print("  mu = 1 reference); the sector-summed (orbifold / SO(16)-class)")
    print("  prescription pays exactly one classical bit, Delta -> ln 2, i.e.")
    print("  mu_gauged = exp(2 ln 2) = 4 -- the entropic twin of v490's")
    print("  parity census; the complementary-pair budget is bounded by")
    print("  ln 4 = ln mu(SO(16)_1) and the orbifold breaks S(E) = S(E')")
    print("  (duality-failure witness).  The theta_v = 1 extension")
    print("  (|L| = 2, or the one-step mu4 glue |L| = 4 from the carrier)")
    print("  removes it: mu = 4/2^2 = 16/4^2 = 1 -- offset 0, one sector,")
    print("  trivial fusion: KILL not triggered.  The nu = 1 control shows")
    print("  a non-removable offset (theta_v != 1), the trivial phase shows")
    print("  no offset at all, and the wrong sector sum (2 of 4 structures)")
    print("  loses the full ln 2.  OPEN (WP5d-beta/WP5e): continuum Haag")
    print("  duality / local normality / split property of the QUOTIENT net,")
    print("  and the Costello-Li uplift.")
