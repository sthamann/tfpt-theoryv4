"""WP5e-gamma of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no verification claim).

"THE SPHERE-AXION PAIRING CHECK" -- can the three twisted sphere axions
eta_m (m = 1, 2, 3; Coxeter eigencharacters {i, -1, -i} on H^2(ALE), v505 S5.8)
cancel the rigid 32 T3 residual of the Atiyah-Bott anomaly ledger
(v505 S3.7: A_fix = Q^{(1)}/2 + Q^{(2)}/4 + Q^{(3)}/2 = 9P1 - 30P2 - 15P3
+ 32 T3) by Green-Schwarz-type EXCHANGE with sector-compatible quadratic
vertices?  Exact linear algebra throughout (sympy / Fraction, no floats).

E1  VERTEX-SPACE THEOREM (the structural backbone): the space of
    W(D5) x W(A3)-invariant polynomials on the 8-dim glue Cartan is computed
    EXACTLY by nullspace arithmetic over the Weyl generators:
    quadratics dim 2 = span{s5, s3} (bidegree dims 1/0/1), quartics dim 5 =
    span{P1, P2, P3, T5, T3} (bidegree dims 2/0/1/0/2).  COROLLARY (the
    master kill): the product of ANY two invariant quadratics is
    ac P1 + (ad+bc) P2 + bd P3 -- identically ZERO T5- and T3-content.
    Hence every scalar-exchange image is annihilated by the functionals
    Phi_T5 = [T5], Phi_T3 = [T3], and Phi_T3(A_fix) = 32 != 0: NO exchange
    mechanism with gauge-invariant quadratic vertices can cancel the
    residual -- independent of the selection rule, the number of axions,
    and the coupling values.
E2  SELECTION-RULE ENUMERATION (Z4 charge m + j + j' = 0 mod 4):
    (i)  class(-alpha) = -class(alpha) mod 4 for all 240 roots; the
         Killing-paired class bilinears C_{j,j'} = sum_{alpha+beta=0}
         <alpha,x><beta,x> are nonzero ONLY for j' = -j, with C_{j,-j} =
         -K_j: under the strict two-index reading the rule forces m = 0 --
         the sphere axions have NO invariant bilinear Cartan vertex at all
         (coupling-space collapse; 16 raw triples, 4 nonzero, all m = 0).
    (ii) twist-insertion reading (the generous route): vertices
         eta_m tr(g^a X^2) = eta_m K^{(a)} with m + a = 0 mod 4: allowed
         (m; a) = {(0;0), (1;3), (2;2), (3;1)}; exchange channels
         (m, 4-m): E13 = K^{(3)}K^{(1)} = (16, -96, 144, 0, 0), E22 =
         (K^{(2)})^2 = (16, 32, 16, 0, 0), bulk E00 = (K^{(0)})^2 =
         225 E22 (K^{(0)} = -15 K^{(2)}: parallel).
E3  SOLVABILITY (the core): exchange matrix M (5 x 2 sphere channels):
    rank 2, kernel 0 (a solution, if it existed, would be UNIQUE/rigid);
    rank([M | A_fix]) = 3 > 2: UNSOLVABLE.  Obstruction certificate: the
    annihilator of im(M) is spanned by Phi_T5, Phi_T3, Phi_P = 3[P1] -
    [P2] - [P3], with values (0, 32, 72) on A_fix: TWO independent
    obstructions -- 32 T3 (structural, rule-independent) and Phi_P = 72
    (selection-rule-driven: even the FACTORISED part 9P1 - 30P2 - 15P3 is
    unreachable through charge-0 channels, because K^{(0)} || K^{(2)}
    collapses the charge-0 product span to 2 dims); Phi_P(Q^{(0)}) = 0
    (the bulk Okubo square stays consistently inside the reachable slice);
    ker M = 0 forces the unique non-disturbing coupling c = 0: the sphere
    axions are RIGIDLY SILENT in the exchange ansatz.  Relaxed control:
    dropping charge conservation makes the P-block fully reachable (rank 3,
    det -64) but T5 = T3 = 0 stays -- the deep kill is T3.
E4  NATURALNESS ANCHORS: couplings prop to ch2 data f(m) = (0, -3/8, -1/2,
    -3/8) give image (25/4, -11/2, 97/4, 0, 0); AB-weight couplings
    (1/det) give (12, -40, 76, 0, 0); BOTH have certificate values (0, 0)
    vs required (32, 72) -- the mismatch is coupling- and scale-
    INDEPENDENT (the naturalness question dissolves: the ansatz space
    misses the target identically).  Numerology fences typed [C] only:
    32 = (h,h) + (h',h'), 72 = 2 * 36 = 2 lambda~^2_e8.
E5  NEGATIVE CONTROLS: (a) invariant axion only: v505 S3.9 replicated
    (sector 0 cancellable, sectors 1, 2 rank-obstructed; A_fix not in
    span{(K^{(0)})^2}); (b) flipped rule m + j - j': twist-insertion
    channel set unchanged {(1,3), (2,2)} (the kill is rule-flip invariant
    there), two-index reading now couples ONLY eta_2 (via -K_1) and gives
    the bulk axion the full span{K_0, K_2} -- the P-part becomes reachable,
    32 T3 stays dead (sharpening: Phi_P is rule-dependent, Phi_T3 is not);
    (c) SO(16) glue: odd classes EMPTY (no sphere partners at all) and the
    so16 AB sum (15, -18, -15, -4, 40) does not even cancel T5 -- the
    mechanism is structurally broken, not just obstructed; (d) D8 native
    quartic 16 T8 + 12 s8^2 (independent quartic, no Okubo, 2 h_vee(D8) =
    28 Killing) with no A3 block: no T3 structure and nothing to pair.
E6  VERDICT + FENCES: the exchange route for the pairing is KILLED with a
    three-functional certificate; the slot bijection (three sphere classes
    <-> three twisted sectors) is untouched; the 32 T3 burden moves to
    twisted closed-string CONTACT terms (the BCOV one-loop measure on
    C^2/Z4) and/or the full-tensor (non-Cartan-leg) ledger -- named
    milestones WP5e-delta/epsilon; the celestial inflow is NOT dead, the
    exchange sub-branch is.
...
def rank_of(vectors):
    return sp.Matrix([[v[i] for v in vectors] for i in range(5)]).rank()
...
# ---------------------------------------------------------------------------
# S0 -- baseline replication (v505 anchors this probe builds on)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# S1 -- E1: the vertex-space theorem (exact Weyl-invariant arithmetic)
# ---------------------------------------------------------------------------
def section1(Ktw):
    print("  -- S1 (E1): the invariant vertex space and the master kill")
...
# ---------------------------------------------------------------------------
# S2 -- E2: selection-rule enumeration (both readings, exact)
# ---------------------------------------------------------------------------
def section2(roots, K, Ktw):
    print("  -- S2 (E2): Z4 selection rule m + j + j' = 0 mod 4 -- "
          "enumeration")
...
# ---------------------------------------------------------------------------
# S3 -- E3: solvability of the pairing (the core result)
# ---------------------------------------------------------------------------
def section3(E13, E22, E00, dA, Qtw):
    print("  -- S3 (E3): exchange matrix, rank/kernel, solvability")
...
# ---------------------------------------------------------------------------
# S4 -- E4: naturalness anchors (ch2 data, Coxeter characters)
# ---------------------------------------------------------------------------
def section4(E13, E22, dA):
    print("  -- S4 (E4): naturalness anchors -- what natural couplings "
          "would produce")
...
# ---------------------------------------------------------------------------
# S6 -- E6: verdict and honest fences
# ---------------------------------------------------------------------------
def section6():
    print("  -- S6 (E6): verdict and honest fences")
    ...
    check("S6.2 [O-FENCE + ROADMAP HOOKS] open after gamma: (delta-1) the "
          ...
          "untouched; NO marker moves anywhere -- exploration only", True)


# ---------------------------------------------------------------------------
def main():
    print("WP5e-gamma CELEST.SEAM.01 -- the sphere-axion pairing check "
          "(exploration only)")
    ...
    print("\n=== VERDICT (see report) ===")
    print("  E1: the gauge-invariant quadratic vertex space is EXACTLY")
    print("  span{s5, s3} (dim 2, Weyl nullspace); products live in")
    print("  span{P1, P2, P3}: T5/T3 content of ANY exchange image is 0.")
    print("  E2: strict two-index rule COLLAPSES the sphere couplings")
    print("  (only m = 0 survives); the generous twist-insertion reading")
    print("  gives channels E13 = (16,-96,144), E22 = (16,32,16),")
    print("  E00 = 225 E22 (parallel).")
    print("  E3: M is 5x2 rank 2 kernel 0; rank([M|A_fix]) = 3:")
    print("  UNSOLVABLE with certificate (Phi_T5, Phi_T3, Phi_P) =")
    print("  (0, 32, 72) -- the sphere axions cannot cancel 32 T3 by")
    print("  exchange, and are forced silent (c = 0).")
    print("  E4: ch2-natural and AB-weight couplings both miss the target")
    print("  scale-independently -- naturalness dissolves.")
    print("  E5: invariant-only fails (v505 replication); rule flip")
    print("  changes the enumeration but not the T3 kill; SO(16) has no")
    print("  partners AND uncancelled T5; D8 has no T3 structure at all.")
    print("  E6: exchange sub-branch dead, inflow alive via contact terms")
    print("  (BCOV one-loop = WP5e-delta); slot bijection untouched.")
