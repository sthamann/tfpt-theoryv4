"""WP5d-beta of CELEST.SEAM.01 (EXPLORATION ONLY -- experiments/, no verification claim).

"SPLIT + STRONG ADDITIVITY FROM THE LATTICE" -- the two remaining KLM legs.
Complete rationality (Kawahigashi-Longo-Mueger) = split property + strong
additivity + finite two-interval mu-index.  WP5d-alpha witnessed mu (offset
ln 2 => [F:F_even] = 2, mu_gauged = 4, chain 16 -> 4 -> 1); this probe
witnesses the OTHER TWO legs for the orbifold/quotient prescription on the
same 1d critical Majorana ring (v367/v450 reduction, Peschel-exact), with the
U(1)/Dirac number-conserving subnet as the preregistered negative control
(the current-net analogue where strong additivity famously FAILS).

  S0  MACHINERY VALIDATED: analytic NS covariance == eigh route; the touching
      (adjacent-interval) parity deficit Gaussian formula == ED; the U(1)
      Fock-built density matrix (covariance round trip, dephased Tr sigma^2 ==
      exact finite quadrature -- R(alpha) is a trig polynomial of degree n_A,
      so the (2 n_A + 1)-node grid is EXACT); the second-compound identity
      (cross-correlations of even bilinears have singular values sigma_a
      sigma_b); parity flip C -> -C leaves sigma_k invariant; sigma_X = rho_X
      for every SINGLE region (Gaussian parity superselection).
  S1  STRONG ADDITIVITY, EXACT ALGEBRA [E-machine]: (a) CAR reference: full
      generation, trivially, disjoint or touching; (b) Majorana EVEN algebras
      with the shared boundary Majorana ("touching = share the point"):
      Even(A) v Even(B) = Even(A u B) EXACTLY (GF2 span 2^{n-1}/2^{n-1} on
      four sizes + matrix rank 32/32) -- the lattice strong-additivity
      witness; (c) disjoint: index EXACTLY 2 (span half, rank 16/32; the
      missing sector is odd(x)odd -- the ln 2 bit); (d) U(1)/neutral algebras:
      even WITH the shared site the span has a GROWING gap (18/20, 60/70,
      200/252): the charge-transfer tower is not generated -- the lattice
      shadow of the continuum strong-additivity failure of the current net.
  S2  STRONG ADDITIVITY, ENTROPIC SCALING [N]: fermionic touching MI diverges
      with the exact point-gap coefficient (c/3) ln: increments per doubling
      == (1/6) ln 2 to 3e-6 (the "Punkt-Luecke" is pure UV, present for EVERY
      net); the honest discriminator is BOUNDED vs DIVERGENT sector-sum
      deficit: Z2/orbifold D(touch) rises 0.4045 -> 0.5899 (N = 64 -> 4096),
      always < ln 2, with (ln 2 - D) ~ N^{-0.245} (Ising disorder dimension
      2 Delta_mu = 1/4) -- bounded <=> finite index <=> (Longo-Xu dichotomy)
      complete rationality; split-point dependence weak + exactly reflection
      symmetric; ED vN deficits track the direction; gap-continuity: the
      touching value is the m/N -> 0 limit of the disjoint family (x -> 1);
      U(1) CONTRAST: D_U1(touch) = 0.658 -> 0.861 (L = 64 -> 1024) BURSTS
      the finite-group ceiling (> ln 2 from L = 256 on) and keeps growing
      ~ (1/2) ln Var Q_A with Var Q_A = (1/pi^2) ln ell + const
      (Klich-Levitov slope check to 3e-4): infinite index, no saturation --
      strong additivity fails for the number-conserving subnet, as known in
      the continuum (Buchholz-Schulz-Mirbach current net).
  S3  SPLIT PROPERTY (nuclearity witness) [N + E-machine]: the singular
      values of the off-diagonal covariance block Gamma_AB for DISJOINT
      intervals come in near-degenerate pairs whose geometric means fall
      exponentially in k with rate pi K(1-x)/K(x) (the free-fermion
      two-interval elliptic nome; measured/predicted = 0.98..0.99 over
      d = 16..512 at N = 4096); sigma_1 ~ x^{1/2} (fitted 0.504; the
      dimension-1/2 fermion two-point law => sigma_1 ~ sqrt(l_A l_B)/d);
      trace norm finite and falling in d (Buchholz-Wichmann nuclearity
      proxy); ORBIFOLD INHERITANCE IS EXACT AT MACHINE LEVEL: the parity
      flip maps C -> -C (identical sigma_k), and the even-bilinear coupling
      Gram is the second compound Lambda^2 C with singular values sigma_a
      sigma_b -- the SAME ladder at doubled rate, so the quotient net's
      split scaling is the fermionic one (finite-index heredity, Longo);
      Renyi-2 MI of the orbifold prescription vanishes at least as fast as
      the fermionic one (ratio -> 0.014); dimerised control: sigma_1(d)
      falls EXPONENTIALLY per site (massive, split trivially).
  S4  PIMSNER-POPA / LONGO-REHREN, EXACT [E]: E(x) = (x + PxP)/2 is unital,
      trace-preserving, idempotent, an even-bimodule map (machine); the
      operator bound E(a) - a/2 = PaP/2 >= 0 holds IDENTICALLY (the PP
      inequality with lambda = 1/2 = 1/[F:F_even], one line, exact); the
      bound is ATTAINED: x = 1 + c_j (and every commuting even/odd monomial
      pair) gives E(x*x) - (1/2 + eps) x*x with a negative eigenvalue for
      any eps > 0, via exact INTEGER identities (M^2 = 4, Tr M = 0,
      PMP = -M) -- swept over ALL 16384 (even, odd) monomial pairs at N = 8
      and all 2048 hermitianised odd monomials at N = 12 (exact, zero
      deviations); random pencil sweep (balanced rank-ones + Wisharts,
      N = 12) min = 0.5000000000; the two-interval group {1, P_A, P_B,
      P_AB} gives lambda = 1/4 = 1/mu EXACTLY (bound identity + uniform
      4-sector attainment); INDEX CONSISTENCY: exp(Delta_inf) = 2.0000 =
      1/lambda_PP (entropic offset, WP5d-alpha route, |Delta - ln 2| <
      1e-9 at N = 512) and exp(2 Delta_inf) = 4 = 1/lambda_E4 = mu -- the
      Longo-Rehren index witnessed by TWO independent lattice routes;
      U(1) control: lambda = 1/(m+1) -> 0 with the mode number (dephasing
      over m+1 charge sectors; exact attainment): infinite index.
  S5  NEGATIVE CONTROLS: dimerised/trivial phase -- touching deficit 0.0051
      (N-independent to 1e-9, no drift toward ln 2), touching MI bounded
      (increment < 1e-9 per doubling vs ln2/6 critical), sigma_1
      exponential: all indicators trivial; U(1) small-L honesty: D_U1
      non-monotone below L = 16 (parity artifacts) -- small systems cannot
      distinguish the nets; Z2 finite-size honesty: at N = 4096 the
      touching deficit still sits 14.9% below ln 2 (the N^{-1/4} Ising
      approach is slow -- documented, not hidden); wrong prescription
      {1, P_AB} on the touching pair: deficit EXACTLY 0 (machine identity),
      measurably wrong bookkeeping, as in WP5d-alpha.
  S6  HONEST TYPING: [E] the exact algebra spans/indices, the PP identities
      and attainments, the quadrature/compound/machine identities; [N] all
      scaling witnesses (deficit tables, nome ladder, power laws, KL slope);
      [C] the lattice -> continuum KLM dictionary (bounded deficit = finite
      index = complete rationality via Longo-Xu; nome ladder = modular
      coupling; split heredity under finite index, Longo; CHMP entropic
      order parameters) AND the citation that the continuum theorem for
      finite-group orbifolds is Xu's (algebraic orbifold CFTs) -- cited,
      not claimed; [O] the continuum split/strong-additivity proof for THE
      seam quotient net itself and the interacting condensed (E8)_1 net
      (WP5e / Costello-Li territory).  SEAM.EQUIV.01 untouched, no marker
      moves, no ledger/paper/website edits.

Throwaway probe: standalone (numpy + scipy + itertools), prints tables +
...
# ---------------------------------------------------------------------------
# Gaussian machinery (identical conventions to WP5d-alpha)
# ---------------------------------------------------------------------------
...
def section0():
    ...
    check("S0.1: ANALYTIC COVARIANCE -- closed-form NS Gamma equals the eigh "
          "route at N = 256 to %.1e (used for N up to 4096)" % dev, dev < 1e-12)
    ...
    check("S0.2: TOUCHING DEFICIT vs ED -- adjacent intervals (region 12 Maj, "
          "split at 6): Gaussian R = %.10f == ED R = %.10f (dev %.1e); "
          "Delta_2 = %.6f, Delta_vN = %.6f (same direction)"
    ...
    check("S0.3: U(1) MACHINERY vs ED -- Fock rho from C (Tr = 1 dev %.1e, "
          "covariance round trip %.1e); charge-dephased Tr sigma^2 = %.10f "
          "== Tr rho^2 <R>_quad = %.10f (dev %.1e)"
...
def section1():
    ...
    check("S1.3: DISJOINT INDEX 2 [E] -- without the shared site the even "
          ...
          "localised at the split point" % r_d, ok_d and 2 * r_d == 32)
...
def section2():
    ...
    for N in [12, 16, 20]:
        ...
        # flip exactly the first la Majoranas = first ceil(la/2) qubits only
        # when la even; for odd la fall back to Gaussian formula check only
        rhoP = (P[:, None] * rho) * P[None, :]
    ...
    check("S2.6: GAP CONTINUITY [N] -- |D(touch) - D(one-site gap)| shrinks "
          ...
          "continuous x -> 1 endpoint of WP5d-alpha's disjoint family, not "
          "a separate lattice artefact"
          % (diffs[0][1], diffs[-1][1]),
...
def section4():
    ...
    d16, _ = deficit2_flip(sub(G512, ia + ib), 254, layers=16)
    check("S4.6: INDEX CONSISTENCY, TWO ROUTES [N/E] -- the WP5d-alpha "
          "entropic offset gives exp(Delta_inf) = %.9f (|Delta - ln 2| = "
          "%.1e at N = 512, 16 layers) and the PP pencil gives 1/lambda = "
          "2 exactly; squared: exp(2 Delta) = %.6f = 1/lambda_E4 = 4 = mu: "
          "[F : F_even] = 2 per step, witnessed ENTROPICALLY and "
          "OPERATOR-ALGEBRAICALLY, independently"
          % (np.exp(d16), abs(d16 - LN2), np.exp(2 * d16)),
...
def section5():
    ...
    for N in [256, 1024]:
        ...
        SAB = S_vN(GAB)
        vals.append((N, d2, SA + SB - SAB, -np.log((1 + R ** 16) / 2)))
    ...
    check("S5.4: WRONG PRESCRIPTION [E-machine] -- averaging the touching "
          ...
          "the bookkeeping is falsifiable, as in WP5d-alpha S6.3" % dev,
          dev < 1e-15)
...
# ===========================================================================
def main():
    print("WP5d-beta CELEST.SEAM.01 -- split + strong additivity from the "
          "lattice (exploration only)")
    ...
    print("\n=== VERDICT (see report) ===")
    print("  WP5d-beta: both remaining KLM legs are WITNESSED for the")
    print("  orbifold prescription.  STRONG ADDITIVITY: exact at the")
    print("  lattice level with the shared boundary point (Even(A) v")
    print("  Even(B) = Even(AuB), index 1), index exactly 2 without it")
    print("  (the ln 2 bit), and the entropic touching defect is BOUNDED")
    print("  by ln 2 with the Ising 1/4-exponent approach -- while the")
    print("  U(1)/number-conserving control DIVERGES ((1/2) ln Var Q,")
    print("  infinite index): bounded-vs-divergent is the lattice")
    print("  discriminator, and the quotient net sits on the bounded side.")
    print("  SPLIT: the coupling operator's singular values fall")
    print("  exponentially in k at the elliptic-nome rate pi K(1-x)/K(x)")
    print("  (2-5%), sigma_1 ~ x^{1/2}, trace norm summable; the orbifold")
    print("  inherits the SAME ladder exactly (machine identity + compound")
    print("  Lambda^2 C): split scaling is index-stable, as Longo heredity")
    print("  demands.  PIMSNER-POPA: E(a) >= a/2 identically with exact")
    print("  integer attainment (lambda = 1/2 = 1/[F:F_even]), lambda_E4 =")
    print("  1/4 = 1/mu, and exp(Delta_inf) = 2 = 1/lambda -- the")
    print("  Longo-Rehren index pinned by two independent routes.  The")
    print("  continuum statements remain [C] (dictionary) / [O] (Xu's")
    print("  orbifold theorem cited; WP5e for the seam net itself).")
