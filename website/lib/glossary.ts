...
export const GLOSSARY: GlossaryEntry[] = [
  ...
  },
  {
    ...
    short:
      "The metric-sector interface (historically labelled G_metric): R + R² is heat-kernel grounded (G2) and gap-decoupled (G5); the closing statement is the index-4 μ₄ seam-net inclusion ⇒ holomorphic (E₈)₁. Net existence and full-cone reflection positivity are discharged to [E] (v175). The whole emergent-QFT layer collapses onto one premise (the Modular Spectral Closure, v258–v261: the finite Dirac is a covariance induction of the seam KMS state, the spectral-action cutoff is that KMS weight with f₂/f₀ = 1, and the seam, carrier-16 and E₈ live on one Kummer/K3 surface), so the boundary QFT is one relative object closed modulo a single named theorem, the Seam Equivalence Theorem (SEAM.EQUIV.01): the raw RP seam IS the holomorphic (E₈)₁ net at τ=i. Its conformal-deck face QGEO.SYM.01 (the carrier μ₄ clock is the seam's deck) is downstream and Lean-pinned (FORM.SEAMEQUIV.01) to named cited steps plus the derived Recovery gap Δ = 6·ln(3/2) > 0. Full QG closure is a certification layer, not a prerequisite for testing the readouts [C]/[O].",
  },
  {
    term: "F_transfer",
