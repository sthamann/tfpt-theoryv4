  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "F_transfer = F_observable ∘ F_threshold ∘ F_RG — standard physics fed TFPT source data — with exactly four interfaces. F_pole (Koide): the 53/54 factor is an exact operator readout [E], the pole interpretation [C] (v183). F_Boltzmann (η_B): the washout anchors (m̃₁ = m_3/A_Λ) but M₁ = M_R·φ₀⁴ only relocates the free input (M_R is the seesaw scale, not a compiler power), so η_B stays a sharper scenario [C] (v184). F_relic (axion): a converged full finite-T solve finds the predicted θ_i ≈ 170° hilltop OVER-produces (Ω_a h² ≈ 0.66, ~5.5× Ω_DM), so axion-as-dominant-DM is a confirmed tension (Ω_DM needs θ_i ≈ 106°) [C] (v185). F_QCD (m_p/m_e): a QCD matching contract [O], not a compiler number. A machine guard (v187) enforces that all four stay [C]/[O] and are never promoted to a primitive [E] compiler prediction — exact algebraic sub-parts (53/54, b₃ = −7) may be [E], the physical prediction never is.",
        formulas: [
