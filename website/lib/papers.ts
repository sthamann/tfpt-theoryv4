...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    abstract:
      "The deliberately adversarial layer: instead of confirming TFPT, this document attacks the six load-bearing reductions (Targets A–F) at their weakest logical transitions. Each target runs through one fixed protocol — minimal statement, assumptions, logical chain, counterexample search, limiting cases, alternative structures, verdict. A red-team check asserts an adversarial fact (a counterexample really exists, a hidden assumption is really needed, a firewall really holds); the honest outcome lives in the status of each target, never in a green pass. Verdicts: A reduced (one residual), B/D/E/F survive narrowed, C survives; none broken. Target F audits the perturbative-QFT + scale round (v269–v275): two attacks land — the R²/Weyl² gravity S_pert is non-unitary (Stelle ghost, ⇒ the nonperturbative QG.AMB.01 frontier) and the anchor over-determination is conditional on the Λ-branch — both folded back into v269/v274. A conditional infinite-derivative narrowing (v304) reads the Stelle ghost as a truncation artefact if the seam KMS-cutoff form factor is entire (Tomboulis; Biswas–Mazumdar–Siegel) — a narrowing of the attack, not a QG.AMB.01 closure.",
    status: "redteam",
    ...
    contribution: [
      "Target A (seam–Calderón = (E8)₁ net): reduced to ONE residual — boundary-net holomorphy + c = 8 (⇔ the index-4 inclusion); E₈ and bulk uniqueness then follow (v83/v87/v89; Lie-level realisation v143). Since reduced to zero new open content: the free-bulk premise is a fixed-point theorem (quasi-free ⇒ κ₂ₙ=0), the infinite Schwinger cone is eliminated (cone gap = one-particle gap (2/3)⁶ by Bogoliubov second quantisation), and the residual factors into the already-open A2 net-existence + GATE.QGEO, irreducible core {π, v_geo} a theorem (v160–v165). The two structural residuals are then discharged to a theorem (v175): the CAR second-quantisation functor Γ(t)=⊕ₘΛᵐ(t) makes full-cone reflection positivity reduce for every m to the one-particle contraction (verified on the complete 2¹⁶-dim Fock space — contraction, fixed multiplicity 2⁸, sub-leading (2/3)⁶), and A2 is an assembled, verified (E₈)₁ certificate (character 1+248q+4124q²+…, embedding 248=120+128, E₈ Cartan even unimodular). Net existence and full-cone RP become [E]; the single irreducible open premise is the seam realisation QGEO.REALIZE.01. Subsequent reductions (v176–v180) formulate this as one central theorem, split it into the marks + kernel obligations, close their finite cores, unify them to one conformal-realisation premise, and finally reduce it to a single milder, non-circular seam-isometry premise QGEO.ISO.01 — that the carrier clock is an order-4 orientation-preserving isometry of the RP seam metric — via uniformisation + Kerékjártó + the order-4 Möbius classification, and one step further (v181) to the conformal-symmetry / deck bedrock QGEO.SYM.01 — the carrier μ4 clock is the conformal deck of the seam, a definitional premise not reducible further without relabeling. It is then driven to its sharpest non-circular form (v194–v198): the raw seam DtN is RP-canonical (Osterwalder–Schrader on the quasi-free state, v194), the four marks are forced by a Lefschetz/character argument rather than posited (v195), and the bedrock is cracked to a clean state-invariance — the DtN principal symbol |k|=diag(|n|) commutes exactly with the clock diag(iⁿ) on all of L², and by Tomita–Takesaki [ρ,Λ_Σ]=0 follows from ω∘ρ=ω with no conformal-covariance assumption (removing the Bisognano–Wichmann circularity, v198) — so the entire residual is now the single checkable statement “the raw quasi-free seam state is μ4-invariant”, the maximally-operational form of QGEO.SYM.01, still [O].",
    ...
    sections: [
      ...
      },
      {
        ...
        body: "Level-1 primary counting (det Cartan: D₈ has 4, E₈ has 1) makes holomorphy necessary AND sufficient — a holomorphic c = 8 chiral CFT is the lattice theory of the unique even unimodular rank-8 lattice. Bulk uniqueness is not independent: for a holomorphic net Rep(A) = Vect, so the bulk pairing is unique (machine contrast: SO(16)₁ admits six modular invariants). Target A therefore collapses to one residual statement — and that statement carries no new open content: the free-bulk premise is a fixed-point theorem (v160), the infinite Schwinger cone is eliminated since the cone gap equals the one-particle gap (2/3)⁶ by Bogoliubov second quantisation (v161, value forced by μ₄-equivariance, v162), and the residual factors into the already-open A2 (net existence, an assembly of established net constructions) + R1 = GATE.QGEO (seam boundary = ℙ¹∖μ₄), with the irreducible core {π, v_geo} a theorem (v165). The heavy artillery then discharges the two structural residuals to a theorem (v175): the CAR second-quantisation functor Γ(t)=⊕ₘΛᵐ(t) makes full-cone reflection positivity reduce for every m (not only m≤6) to the one-particle contraction — verified on the complete 2¹⁶=65536-dim Fock space (a contraction, eigenvalue-1 multiplicity 2⁸=256, sub-leading = the one-particle gap (2/3)⁶) — and A2 is an assembled and verified (E₈)₁ certificate (character 1+248q+4124q²+34752q³+213126q⁴+1057504q⁵, embedding 248=120+128, E₈ Cartan even unimodular with det 1). Both then need the same single input — the seam realisation QGEO.REALIZE.01 (finite half proven, v168) — so net existence and full-cone RP become [E] and the one irreducible structural premise is that geometric realisation, left honestly open. Later reductions then sharpen it (v176–v180): the realisation is formulated as one central theorem, split into the marks + kernel obligations, their finite cores closed, unified to one conformal-realisation premise, and finally reduced to a single milder seam-isometry premise QGEO.ISO.01 (the carrier clock is an order-4 orientation-preserving isometry of the RP seam metric) via uniformisation + Kerékjártó + the order-4 Möbius classification, and one step further (v181) to the conformal-symmetry / deck bedrock QGEO.SYM.01 — the carrier μ4 clock is the conformal deck of the seam, a definitional premise not reducible further without relabeling. It is then driven to its sharpest non-circular form (v194–v198): the raw seam DtN is RP-canonical (Osterwalder–Schrader on the quasi-free state, v194), the four marks are forced by a Lefschetz/character argument rather than posited (v195), and the bedrock is cracked to a clean state-invariance — the DtN principal symbol |k|=diag(|n|) commutes exactly with the clock diag(iⁿ) on all of L², and by Tomita–Takesaki [ρ,Λ_Σ]=0 follows from ω∘ρ=ω with no conformal-covariance assumption (removing the Bisognano–Wichmann circularity, v198) — so the entire residual is now the single checkable statement “the raw quasi-free seam state is μ4-invariant”, the maximally-operational form of QGEO.SYM.01, still [O].",
        formulas: [
  ...
  },
  {
    ...
    contribution: [
      ...
      "G_net: IR tier closed under RP + gap (Decoupling Theorem, Δ_eff = 1.648 > 0); the ambient measure is holographically reduced to a finite seam-boundary measure that is the rigorously-constructed (E₈)₁ lattice net (c = 8 = 5 + 3, conformal embedding (D₅)₁×(A₃)₁, coset c = 0). The closing statement is the index-4 seam-net inclusion — and the free-bulk premise (A) behind it is now a fixed-point theorem with the cone eliminated (cone gap = one-particle gap (2/3)⁶), factoring into the already-open A2 net-existence + GATE.QGEO with zero new open content (v160–v165).",
      "The quark ratio c_u/c_d = 55/117 is closed (Readout Rigidity); the '11' is the Pascal sum 16 − g_car.",
      "Modular Spectral Closure (v258–v261): the boundary QFT is one relative object reduced to one premise. The finite Dirac is the modular/covariance induction of the seam KMS state ([D_F] = [D_Σ]⊗[K_car], v258); the spectral-action cutoff is that same KMS weight, fixing f₂/f₀ = 1 and turning κ into a finite-triple trace ratio (v259); the seam (pillowcase), carrier-16 (Kummer nodes) and E₈ (H²(K3) = U³⊕E₈(−1)²) are facets of one Kummer/K3 surface (v260). The assembly certificate (v261) pins the cross-consistency (4 = index = marks = fixed points = glue order; one carrier-16; one gap 6log(3/2)) — so the whole layer is QFT-complete modulo QGEO.SYM.01, ambient QG separate. Latest round (v269–v278): the perturbative 4D layer is now built — the spectral-action S-matrix S_pert is Epstein–Glaser-constructible (v269) with a one-loop quartic (v271) and the gauge β-coefficients (41/10, −19/6, −7) from the carrier content (v273), LSZ-bridged to the physical S_phys with one-loop unitarity (the optical theorem: bubble cut = two-body phase space, v278) for matter+gauge; the R²/Weyl² gravity sector is renormalizable but non-unitary (the Stelle ghost, rt_F), which IS the nonperturbative frontier. The two remaining obligations are sharply located: (1) QGEO.SYM.01 collapses to one geometric postulate — the raw seam is the flat τ=i pillowcase (v276: granting it, the modular commutator [ρ,H]=0 closes to all orders); (2) QG.AMB.01 Tier-B reduces to one holomorphy bit — is the seam-Calderón boundary net holomorphic, det K=1? (v277: every other (E8)₁ invariant matches; SO(16) has det Cartan 4). The single mass anchor is over-determined (v274: gravity = dark energy to 0.11%) and the absolute neutrino scale is one seesaw ratio (v272). Self-investigation round (v280–v283): a numerical experiment on the actual flat τ=i pillowcase realises the chain ω∘ρ=ω (route-(i) evidence, v280); the Tier-B bit is the anyon count (#anyons=|det K|, tower 16→4→1, Gauss–Milgram c=8, v281); and — the candidate simplification — χ_E8=j^{1/3} gives χ_E8(i)=1728^{1/3}=12, so τ=i is BOTH the order-4 CM point AND the (E8)₁ modulus: the two obligations are two faces of ONE object, 'the raw seam is (E8)₁ at τ=i', dropping the open count from two to one (v282). That single open lemma is attacked from two decomposed routes — RP-state uniqueness (6-lemma chain, 5/6 discharged, v284) and seam-net condensation (4-lemma chain, 3/4 discharged, v285) — whose open lemmas coincide, so the one closing statement is a canonical equivalence between the raw seam KMS/DtN state and the holomorphic (E8)₁ net at τ=i. Closing sprint (v286–v288): that statement is now the single named theorem SEAM.EQUIV.01 — the contract types four objects and six arrows and enforces an import firewall so the two routes never feed each other (no 'E8 smuggled into the geometry and pulled back out'); Route A (AQFT, v287) reduces the theorem to ONE standard import 'invertible Gaussian bulk ⇒ single-sector boundary' (4/5 lemmas discharged); and Route B (DtN, v288) PROVES the full-L² lift of the subprincipal ℤ₄ block-diagonality, shrinking the residual to the single sharper question 'why is the raw seam subprincipal term mark-local?'. Flat-Away round (v289–v297): that residual is decomposed into a 5-lemma chain whose only open link is Flat-Away — RP+gap+4 marks ⇒ the curvature vanishes away from the marks (v289); an honest red-team shows ℤ₄-invariance is NOT mark-locality (a smooth ε·cos(4θ) passes the commutator but shifts the spectrum, v290), so Flat-Away is named as its own mini-theorem (v291) with three independent routes — heat-kernel, spectral-rigidity and RP-energy/Troyanov — all carried to a precise reduction: the heat-trace deviation is a positive-definite quadratic form (v292), proven analytically via convexity with the exact a₂ divided-difference kernel (v295) now in closed form W_k(t)=t(1−e^{−4kt})/(8k(1−e^{−t}))+finite middle (v296) and Lean-formalised (FORM.FLATAWAY.01); the spectral-mismatch Hessian over the full smooth-ℤ₄ space is positive-definite (v293); Troyanov gives the unique flat cone metric (v294); and Route A's import is a citable theorem stack (Kitaev/Freed–Hopkins → Müger/Kawahigashi–Longo–Müger → Conway–Sloane/Dong–Xu, v297) modulo the SAME one geometric input as Route B. So both routes reduce SEAM.EQUIV.01 to one shared external fact. Closing arc (v300–v302): that fact is then pinned. Flat-Away is hardened from a soft minimum to a discrete degeneracy obstruction (a smooth off-mark mode splits a doubly-degenerate Steklov level, changing the spectral multiset) and its pin is derived from the (E8)₁ integer-weight character via 2d Steklov rigidity, so Route B's residual coincides with Route A's rationality (v300); Route A's invertibility hypothesis is discharged by the free-fermion classification — a gapped 16-Majorana (c=8) Gaussian bulk is automatically invertible (#anyons=|det K_E8|=1, the SO(16) contrast has det 4) (v301); and the last spectral input, 'the quasi-free bulk is gapped', is the derived Recovery gap Δ=6·ln(3/2)≈2.43>0 of the transfer spectrum {1,(2/3)⁶,(1/3)⁶}, which by Osterwalder–Schrader/quasi-free clustering IS a bulk mass gap (v302). Net: SEAM.EQUIV.01 stays [O] (not machine-proved end-to-end) but its entire residual is a composition of standard cited theorems over established TFPT facts — no undischarged TFPT-internal assumption remains. The recent predictions are all consistent < 2σ with two sharp near-term kill tests (v283).",
    ],
    notClaimed: [
    ...
    highlights: [
      ...
      { label: "Seam Equivalence Theorem", value: "SEAM.EQUIV.01", description: "The one open core is now a single named theorem: the raw RP seam IS the holomorphic (E8)₁ net at τ=i — the theory's one irreducible structural postulate, the role the constancy of c plays in relativity (v267). An import firewall keeps its two routes non-circular (v286); Route A reduces it to one standard import (v287); Route B proves the full-L² ℤ₄ lift, leaving 'why is the raw subprincipal mark-local?' (v288)" },
      ...
      { label: "Closing arc (v300–v302)", value: "no TFPT-internal assumption left", description: "Flat-Away hardened to a discrete degeneracy obstruction + its pin derived from the (E8)₁ integer-weight character via 2d Steklov rigidity (v300); Route A's invertibility discharged by the free-fermion classification (gapped 16-Majorana c=8 bulk is invertible, #anyons=|det K_E8|=1; v301); the last input is the derived Recovery gap Δ=6·ln(3/2)≈2.43>0 = a bulk mass gap via OS/quasi-free clustering (v302). SEAM.EQUIV.01 stays [O] (not machine-proved end-to-end), but its residual is a composition of cited theorems over established facts" },
    ],
  },
];
