  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "The candidate is the determinant-line axion of the strong-CP sector; WIMPs are ruled out (no spare E₈ singlet). The misalignment angle is closed; the decay constant is a conjecture. A misalignment estimate (v185) and a fuller relic solve (experiments/ftransfer/axion_relic) make the abundance honest: the harmonic estimate under-produces at θ ~ O(1), but at the predicted θ_i ≈ 170° hilltop the anharmonic factor F ≈ 4 times θ_i² ≈ 8.8 OVER-shoots — Ω_a h² ~ 0.6–2.3, robustly above Ω_DM h² = 0.12. So as the dominant dark matter the determinant-line axion at (f_a = M_scal/128 ≈ 2.39×10¹¹ GeV, θ_i ≈ 170°) is in mild TENSION (it would prefer θ_i ~ 120–130°, or extra dilution); the near-hilltop regime is exactly where the θ²F estimate is least reliable (O(1)-uncertain, exponentially θ_i-sensitive), so a full finite-T solve decides. Either way a scenario, not a sharp prediction; over-production kills only the axion-as-dominant-DM branch, not the theory.",
        formulas: [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "F_transfer = F_observable ∘ F_threshold ∘ F_RG — standard physics fed TFPT source data — with exactly four interfaces. F_pole (Koide): the 53/54 factor is an exact operator readout [E], the pole interpretation [C] (v183). F_Boltzmann (η_B): the washout anchors (m̃₁ = m_3/A_Λ) but M₁ = M_R·φ₀⁴ only relocates the free input (M_R is the seesaw scale, not a compiler power), so η_B stays a sharper scenario [C] (v184). F_relic (axion): the relic solve finds the predicted θ_i ≈ 170° hilltop OVER-produces (Ω_a h² ~ 0.6–2.3 > 0.12), so axion-as-dominant-DM is in mild tension, O(1)-uncertain near the hilltop [C] (v185). F_QCD (m_p/m_e): a QCD matching contract [O], not a compiler number. A machine guard (v187) enforces that all four stay [C]/[O] and are never promoted to a primitive [E] compiler prediction — exact algebraic sub-parts (53/54, b₃ = −7) may be [E], the physical prediction never is.",
        formulas: [
