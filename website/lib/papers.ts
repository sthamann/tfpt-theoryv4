...
export const papers: Paper[] = [
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: `TFPT ${SITE_VERSION} closes the discrete compiler, the algebraic SM readout, the EM fixed point, the admissible gapped IR sector and the R + R² spectral-action shadow. It does not yet certify a strict physical TOE. The live residual is three named interfaces: one dimensionful scale anchor v_geo, the metric-sector inclusion G_net (open only via the single named keystone SEAM.EQUIV.01), and the typed transfer functor F_transfer. The historical labels (U_wall)/(G_metric)/(F_frontier) are kept only for ledger continuity.`,
        formulas: [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "c₃ = 1/(8π) is the gravitational seam constant; the spectral action gives R + R² structurally (G2), and the closed admissible sector is gap-decoupled from the un-built ambient (G5, Decoupling Theorem). NEW (v358): beyond the R + R² action, the classical field equation is now supplied directly by the entanglement first law δS = δ⟨K⟩ (Jacobson; Faulkner et al.), run with TFPT's atoms — it gives the linearised Einstein equation G_ab = c₃⁻¹ T_ab with c₃⁻¹ = 8π FIXED (no free dimensionless Newton dial). The new exact result is a coincidence: the thermodynamic origin of c₃ (the first-law coefficient 2π/η) equals the geometric one (the one-sided Gauss–Bonnet |Z₂|·2π·χ) iff |μ₄| = |Z₂|·χ(S²) = 4 — so c₃ is triply over-determined (anchor v23, geometry v58, thermodynamics v358). The matter flux is assembled (the Casini–Huerta–Myers ball modular Hamiltonian, boost via Bisognano–Wichmann v323) and the entropy density is atom-fixed (1/4 = 1/|μ₄|, central charge c = g_car + N_fam = 8); only the linear→nonlinear extension and the absolute scale v_geo remain. The ambient measure (G6) is holographically reduced to a finite seam-boundary measure — now fluctuations around a parameter-free saddle. Plus three archive readouts: (i) an INDEPENDENT gravitational 3/4 — ξ = c₃/φ_tree = 3/4 = q(A₃) = ln(m/μ) (v152); (ii) a Hubble value from the Λ branch ⇒ H₀ = 66.5–67.1 km/s/Mpc (the Hubble tension is NOT relieved), a [C] readout; (iii) a [P] FRG cross-check with a non-Gaussian UV fixed point.",
        formulas: [
    ...
    keyFormulas: [
      {
        ...
        description: "One statement; E₈ and the unique 2D bulk follow. Reduced to no new gate: (A) factors into the already-open A2 net-existence + the keystone SEAM.EQUIV.01 (v160–v165). [C/O]",
      },
      {
  ...
  },
  {
    ...
    contribution: [
      ...
      "G_net: IR tier closed under RP + gap (Decoupling Theorem, Δ_eff = 1.648 > 0); the ambient measure is holographically reduced to a finite seam-boundary measure that is the rigorously-constructed (E₈)₁ lattice net (c = 8 = 5 + 3, conformal embedding (D₅)₁×(A₃)₁, coset c = 0). The closing statement is the index-4 seam-net inclusion — and the free-bulk premise (A) behind it is now a fixed-point theorem with the cone eliminated (cone gap = one-particle gap (2/3)⁶), factoring into the already-open A2 net-existence + the keystone SEAM.EQUIV.01 with zero new open content (v160–v165).",
      "The quark ratio c_u/c_d = 55/117 is closed (Readout Rigidity); the '11' is the Pascal sum 16 − g_car.",
      "Modular Spectral Closure (v258–v261): the boundary QFT is one relative object reduced to one premise. The finite Dirac is the modular/covariance induction of the seam KMS state ([D_F] = [D_Σ]⊗[K_car]); the spectral-action cutoff is that same KMS weight (f₂/f₀ = 1); and the seam (pillowcase), carrier-16 (Kummer nodes) and E₈ (H²(K3) = U³⊕E₈(−1)²) are facets of one Kummer/K3 surface. So the whole layer is QFT-complete modulo the single named keystone SEAM.EQUIV.01 (the raw RP seam IS the holomorphic (E₈)₁ net at τ=i; its conformal-deck face QGEO.SYM.01 is downstream), with the ambient measure QG.AMB.01 kept separate (and QGEO.SYM.01 now a corollary of the keystone, v335). The perturbative 4D layer is built: the spectral-action S-matrix is Epstein–Glaser-constructible with the SM one-loop β-coefficients (41/10, −19/6, −7) from the carrier content, LSZ-bridged with one-loop unitarity for matter+gauge; the R²/Weyl² gravity sector is the non-unitary Stelle ghost, which IS the decoupled general QG problem QG.AMB.01 (the Euclidean-QG conformal-factor problem; gap-decoupled, not a TFPT structural item). The single mass anchor is over-determined (gravity = dark energy to 0.11%, v274). SEAM.EQUIV.01 stays [O] (not machine-proved end-to-end) but its entire residual is now a composition of standard cited theorems (OS/clustering, the Kitaev free-fermion classification, the AQFT stack) over established TFPT facts — Lean-pinned (FORM.SEAMEQUIV.01) to exactly those named steps plus the derived Recovery gap Δ = 6·ln(3/2) > 0 — with no undischarged TFPT-internal assumption left. The full sprint-by-sprint reduction (v269 → v302) lives on the /changelog page, not here.",
    ],
    notClaimed: [
