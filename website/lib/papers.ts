...
export const papers: Paper[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "THIS IS THE ACTUAL BRIDGE FROM COMPILER TO PHYSICS — everything in CELEST.SEAM.01 is preparation for it. A research contract [O] (ledger row WOIT.OS.TWISTOR.01: Open, research contract), not a claim. The external programme it engages — Woit's Euclidean Twistor Unification (arXiv:2104.05099) — is a NAMED reference frame for the shape of the target, never a confirmation in either direction. INPUT: X₊ = (PT/Γ)₊ (Γ = Γ_ALE ≅ ℤ₄, normalised by the clock); A_hol, the INTERACTING open+closed twistorial algebra (SDYM(E₈) + BCOV — the WP5e-proper object whose free/equivariant skeleton is pinned by v492–v515); ρ, the order-4 clock diag(i,1); Θ, the anti-linear real structure induced by the seam reflection (the free covering deck, v510); μ_BCOV+SDYM, the interacting functional. TARGET THEOREM: Θ² = 1 and ΘρΘ = ρ⁻¹; the gauge-invariant algebra is reflection-positive; and the OS quotient produces (H, Ω, U(P↑₊), A_Mink) with: a positive Hilbert metric, positive energy, local causality, ONE chiral fermion generation WITHOUT mirror doubling, the TFPT charge lattice, an internal SU(3)×SU(2)×U(1) action, and the (E₈)₁ seam net as an ACTUAL boundary net (not only as a character). KILL TESTS (all seven live): (1) Θ incompatible with the clock; (2) RP fails after gauge fixing; (3) the reconstruction produces a vector-like mirror generation; (4) the Penrose transform reaches only free/self-dual states; (5) the reconstructed net has the right character but the wrong OPE / fails net equivalence; (6) the internal SU(2) remains a spacetime factor; (7) the four μ₄ marks are not incidence-compatibly extendable over spacetime. SCOPE FENCE (explicit non-claims of the celestial/twistor branch until this closes): both helicities, generic amplitudes, local matter, full Einstein dynamics, EWSB, confinement. SEAM.EQUIV.01 and its route split (MMST/TWISTOR) are stated in their own rows and are not moved by anything here.",
        formulas: [
  ...
  },
  {
    ...
    contribution: [
      ...
      "Two independent paths + red team: an independent Wolfram engine (116/116 + 498/498) and a Lean 4 kernel proof (hypercharge, anomaly, Pascal ladder, seam chain) re-derive the exact core, and the red-team companion attacks the theory and publishes what survives each attack.",
    ],
    notClaimed: [
