"""v528 -- SEAM.BIT.TWISTCLASS.01: the alignment bit as an exact,
measurable TWIST-CLASS choice (the physical definition).  Verdict
SUCCESS.  NO marker moves -- the bit remains formal input; the
definition replaces no derivation, it makes the input measurable.

[E] 1. FACET #14 (eta-flip <=> delta=pi/2): both directions exact
    (sigma o r = -sigma on all four fixing axes with eta jump +i->-i;
    off pi/2 on EVERY axis sigma o r = +sigma; CARDINALITY LEMMA:
    sigma o f = -sigma forces |A+| = |A-| => 4m = 32-4m <=> m=4;
    g-class census over all 224 axes: flip class exactly at {15,31}
    for m=4).
[E] 2. FACET #15 (twist-frustration <=> delta=pi/2): theta exchanges
    D_short <-> D_long exactly, minimal theta-closed insertion unique,
    Gram indefinite (4,4,0)/(8,8,0) exactly at pi/2; off reducible and
    PD; same cardinality lemma locks; side finding omega(D_s)=omega(D_l)
    (complement-string identity, 40-digit).
[E] 3. ORDER PARAMETER: O = flip-axis fraction = 1/2 at m=4, exactly 0
    else (whole ladder + counterwitness + 2-mark anchor); class
    alternation (-,+,-,+) around the torsor = nontrivial Z2 character
    of the Z8 clock-tower shadow; gauge-robust under sigma global flip,
    admissible site redress, mu4 overlay in all variants, string
    orientation; rejected candidates with teeth (sign omega(D),
    spectral shift, v510 index).
[E] 4. UNIFICATION THEOREM: eta-flip = frustration = flag-transitivity
    are ONE Z2 invariant (same arc system, A+ = supp(D_short)); the
    v510 Kramers class is ANOTHER (constant on the family, exact
    Cl(16)) -- Kramers = POSITION half (derived), twist class =
    MODULUS half (the input).  v512 web grows 13 -> 15 facets.
[C] 5. MEASUREMENT SKETCH (principle only): defect transport around the
    axis torsor accumulates the eta holonomy = relative pi phase iff
    bit set -- the bit is in principle interferometrically readable.
    Honest fence: the bit remains formal input.

Status: [E] exact integer/symbolic + 40-digit certificates; [C]
measurement sketch.  Python; Wolfram-mirrored (cardinality lemma /
224-axis census / O-values -- Pfaffian spectra Python-only), counted
per GATE.WOLFRAM.02.  Discovery provenance:
experiments/tfpt-discovery/seam_bit_twist_class_definition_probe.py
(18/18, 2026-07-23).
...
def run():
    reset()
    print("v528 SEAM.BIT.TWISTCLASS.01 -- the alignment bit as an exact measurable TWIST-CLASS choice"
    ...
    print("SUMMARY: %d/%d checks passed%s   [runtime %.1f s]"
          % (npass, len(RESULTS),
             ...
             time.time() - T0))
    return summary("v528 SEAM.BIT.TWISTCLASS.01: the alignment bit is PHYSICALLY DEFINED as the twist-class choice -- facets #14 (eta flip <=> delta=pi/2) and #15 (twist frustration <=> delta=pi/2) both directions exact (cardinality lemma |A+|=|A-| => m=4; g-class census 224 axes: flip class exactly at {15,31}); order parameter O=1/2 at m=4 else 0; unification #14=#15=flag-transitivity = ONE Z2 invariant (v512 web 13->15); Kramers = POSITION half (derived), twist class = MODULUS half (the input); [C] measurement sketch (interferometric in principle); bit remains formal input; verdict SUCCESS; no marker moves")
