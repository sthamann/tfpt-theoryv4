"""v512 -- SEAM.TAU.FLAG.01: the flag-transitivity equivalence web -- the
tau = i attack executed on the free RP seam circle.  After v510 the last
identification input of the theory is the square-modulus datum tau = i
(the position half of the alignment bit is topology; "harmonic + free =>
mu4" is proven, but the counterwitness {+-1, +-(3+4i)/5} is free, central,
nonsplit and NOT harmonic: j = 148176/25 != 1728).  QUESTION: which
ESTABLISHED seam datum excludes the free-but-non-harmonic configurations?
Test family: the deck-invariant 4-mark configurations on the free circle
are, up to rotation, M(delta) = {+-1, +-e^{i delta}}, delta in (0, pi/2];
delta = pi/2 <=> harmonic <=> mu4 <=> tau = i.  Exact throughout (sympy;
mpmath 30 digits for the modular/heat-trace numerics; one numpy lattice
commutator, v503 style), deterministic.

[E] FALSIFICATION OF BARE TRANSITIVITY (the finding IS the failure): on
      the free RP circle M(delta) carries the symmetry group G(delta) =
      {z, -z, b/z, -b/z} ~= V4 for EVERY delta, and the two inversions
      are pair-EXCHANGING: V4 acts SIMPLY TRANSITIVELY on the four marks
      for every delta (complete census: any Moebius map commuting with
      the deck z -> -z is diagonal or antidiagonal, so the two families
      z -> lam z / z -> lam/z with |lam| = 1 exhaust the symmetries) --
      the expectation "transitivity <= v216 local equality selects
      tau = i" is REFUTED; hardening: ALL undirected per-mark data
      (arc-pair multiset {delta, pi-delta}, chordal distance multiset,
      cone germ) are identical at the four marks for EVERY delta -- no
      local jet sees the bit.
[E] THE SHARP SELECTOR: FLAG transitivity (marks AND their two sides
      indistinguishable) <=> delta = pi/2 <=> tau = i <=> the clock --
      stabiliser jump 1 -> 2, flag/arc orbit collapse 2 -> 1, order-4
      mark rotation (b^2 = -1 solveset = {pi/2} exactly), mark-fixing
      mirror z -> 1/z <=> cos delta = 0.
[E] 12-FOLD EQUIVALENCE WEB (every link exact): clock rotation, D4 (vs
      V4), mark mirror, flag transitivity, three spectral degeneracies
      (odd-doublet split of the seam covariance = (2/pi) ln cot(delta/2),
      zero only at pi/2 -- counterwitness exactly (2/pi) ln 2; arc-
      weighted seam Laplacian degeneracy 4/pi; the v215-K3/v503 mod-4
      indicator in CLOSED FORM (2 sqrt 2/pi)|ln tan(delta/2)| -- the
      v503 lattice curve analytically), rho-twist existence (mode
      identity (m^2 - n^2)(t - 1/t) = 0 <=> t = 1; Atiyah-Bott
      rationality is delta-BLIND -- Tr(sigma e^{t Delta}) = 1 for ALL
      delta, j_cw = 148176/25 rational non-CM: rationality does NOT
      select), cusp-4-cycle implementability (H4 rigidity: V4 with
      element orders {1,2,2,2} cannot induce the cusp 4-cycle;
      centraliser scalar, U^2 nonscalar => no projective involution --
      the implementer has projective order exactly 4, the Z8-shadow
      consistent v510), harmonic, j = 1728.
[E] NEGATIVE CONTROLS: hexagonal (marks not concyclic, fails the
      PRECONDITIONS of the battery -- consistent v503/v510); silver
      (harmonic but topologically dead -- position and modulus halves
      independent, consistent v510); 3 marks (impossible on the free
      circle: 0 invariant 3-subsets vs 28 invariant 4-subsets on Z16;
      on a plain circle bare transitivity ALREADY forces equal spacing
      at n = 3 -- "4 counts": the deck-pairing is exactly what makes
      bare transitivity free of charge at n = 4).
[C] THE REST (verbatim, no overclaim): the last input reduces from a
      continuous modulus to ONE discrete symmetry-lift bit (V4 -> D4:
      side-indistinguishability of the four equally-decorated marks);
      v216 local equality does NOT deliver it (no local jet can see the
      side bit); its negation is now concretely measurable (odd-split
      (2/pi) ln cot(delta/2) != 0).  Scoreboard: the counterwitness
      passes all 7 established unsided tests and fails all 9 clock-
      equivalent ones.

Status: [E] every equivalence link, census, closed forms, controls
(sympy exact + mpmath 30 digits); [C] the bit itself -- it stays a
postulate, the gain is the FORM (discrete, seam-intrinsic, v216-adjacent
language), not a derivation.  QGEO.REALIZE.01 and P2.TYPING.01/AX.P2.01
text-precised only, SEAM.BIT.FREEDOM.01 cross-referenced -- NO marker
moves.  Python; Wolfram-mirrored (V4/D4 census, solveset equivalences,
odd-split closed form, twist mode identity, H4 centraliser rigidity),
counted per GATE.WOLFRAM.02.  Discovery provenance: experiments/
tfpt-discovery/seam_tau_i_established_selector_probe.py (2026-07-22,
29/29)."""
...
I = sp.I
N_SEAM = 2 ** (g_car - 1)      # 16 seam sites = the Z16 carrier circle
...
def h1_symmetry():
    ...
    check("H1.2 THE FALSIFICATION [exact, symbolic delta]: the generic "
          ...
          "select delta = pi/2; the expected orbit split {+-1} | {+-b} is "
    ...
    print("        delta        |G|  #rot  transitive  #flag-orbits  "
          "#arc-orbits")
    table_ok = True
...
def nc_controls():
    ...
    n3 = sum(1 for s in combinations(range(N_SEAM), 3)
             if set((x + N_SEAM // 2) % N_SEAM for x in s) == set(s))
    n4 = sum(1 for s in combinations(range(N_SEAM), 4)
             if set((x + N_SEAM // 2) % N_SEAM for x in s) == set(s))
...
def syn_synthesis():
    ...
    check("SYN.3 THE SENTENCE + TYPING [recorded, no overclaim]: 'On the "
          ...
          "marks).'  [E]: every equivalence link (exact, this module + "
          ...
          "a derivation.  Bare-transitivity-as-selector is FALSIFIED "
          "(transitivity delta-blind); H2/H3/H4 are exact reformulations, "
          "none an independent established selector.  NO marker moves",
          True)
...
def run():
    reset()
    print("v512  SEAM.TAU.FLAG.01: the flag-transitivity equivalence web "
          "-- which ESTABLISHED seam datum excludes the free-but-non-"
          "harmonic family? (H1 transitivity / H2 spectra / H3 heat "
          "traces / H4 monodromy + controls)")
    s0_frame()
    h1_symmetry()
    h2_spectral()
    h3_heat()
    ...
    return summary("v512 SEAM.TAU.FLAG.01: FALSIFICATION of bare "
                   "transitivity -- on the free RP circle M(delta) = "
                   "{+-1, +-e^(i delta)} carries the pair-exchanging V4 "
                   "for EVERY delta (complete deck-commuting Moebius "
                   "census: diagonal/antidiagonal), so bare mark-"
                   "transitivity is delta-blind and all undirected "
                   "per-mark data are identical -- no local jet sees the "
                   "bit.  THE SHARP SELECTOR: FLAG transitivity (marks "
                   "AND sides) <=> delta = pi/2 <=> tau = i <=> the "
                   "clock (stabiliser jump 1 -> 2, flag/arc orbits "
                   "2 -> 1, b^2 = -1 solveset {pi/2}, mark mirror <=> "
                   "cos delta = 0).  12-FOLD EQUIVALENCE WEB, each link "
                   "exact: clock rotation, D4, mark mirror, flag "
                   "transitivity, odd-doublet split (2/pi) log "
                   "cot(delta/2) (counterwitness (2/pi) log 2), arc-"
                   "Laplacian degeneracy 4/pi, K3 indicator (2 sqrt 2/pi)"
                   "|log tan(delta/2)| in closed form, rho-twist "
                   "existence (t = 1; AB rationality delta-blind, j_cw = "
                   "148176/25 rational non-CM), cusp-4-cycle "
                   "implementability (V4 orders {1,2,2,2}; centraliser "
                   "scalar, U^2 nonscalar, projective order exactly 4), "
                   "harmonic, j = 1728.  CONTROLS: hexagonal fails "
                   "preconditions, silver harmonic but topologically "
                   "dead, 3 marks impossible on the free circle ('4 "
                   "counts').  THE [C] REST (verbatim): the last input "
                   "reduces from a continuous modulus to ONE discrete "
                   "symmetry-lift bit (V4 -> D4: side-indistinguishability "
                   "of the four equally-decorated marks); v216 local "
                   "equality does NOT deliver it; its negation is now "
                   "concretely measurable (odd-split != 0).  Scoreboard: "
                   "counterwitness passes all 7 established unsided "
                   "tests, fails all 9 clock-equivalent ones.  No marker "
                   "moves")
