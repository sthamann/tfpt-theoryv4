"""v509 -- CELEST.WP5E.EPS2.01: WP5e-epsilon-2 of the research contract
CELEST.SEAM.01 -- "the CPS level-from-flux dial", executed (verdict B).
Question: derive the boundary level k = 1 from flux quantisation on the
lockstep spheres, following the Costello-Paquette-Sharma (CPS)
Burns-holography mechanism (arXiv:2208.14233 / PRL 130, 061602 and
arXiv:2306.00940 "Burns space and holography").  Exact sympy/Fraction
arithmetic throughout, no floats.

[E] S1. CPS SKELETON REPLICATED EXACTLY (arXiv:2306.00940): the S^3
      period of the brane backreaction 3-form, int_{S^3} Omega_N =
      (2 pi i)^2 N exactly (pullback density -sin(2 theta), phases
      cancel; their eq. 3.33 -- large-gauge invariance of hCS quantises
      N in Z); the exceptional-sphere Kahler flux int_{P^1} omega =
      2 pi N exactly (K = |u|^2 + N log|u|^2, the flat part dies at
      t -> 0; their eq. 3.37); the boundary level magnitude = 2N via
      the symplectic-boson double contraction (exact for N = 1, 2, 3;
      their eq. 9.4), normalisation Dynkin index T(so8 vector) = 1,
      T(adj) = 6 = h_vee(so8) -- the CPS formula "level = flux quantum
      x Dynkin index of the boundary matter", level LINEAR in the flux.
[E] S2. PAIRING MATRIX PINNED, NOT POSTULATED: on the resolved A3 ALE
      the v505 ch2 ledger (index bridge f(m) = -(C^-1)_mm/2 =
      (-3/8, -1/2, -3/8)) + integrality + unimodularity + effectivity
      pin P_mi = c1(T_m).[Sigma_i] = delta_mi by COMPLETE enumeration
      (row candidates 8/6/8; 48 unimodular solutions of the full form
      condition P C^-1 P^T = C^-1; with P >= 0 exactly 2 = identity +
      A3 diagram flip); full c1 matrix int c1(T_m) c1(T_n) =
      -(C^-1)_mn; glue FLUXES F_i = sum_m dim(g_m) P_mi = (64, 60, 64)
      = dim g_i, total 188 = 248 - 60, flip-invariant; flux per charged
      root current uniformly ONE quantum through exactly one sphere.
[E] S3. THE LEVEL FORMULA, honest: "level = total open-string flux" is
      KILLED by the lockstep test itself (k_i = (64, 60, 64) unequal --
      F_i is the FLAVOUR multiplicity, in CPS language the brane matter
      content); what works is k = flux per adjoint current = F_i/dim
      g_i = (1, 1, 1), anchored non-circularly by the lattice current
      count (240, 0, 0, 0) for k = 1..4 (v505 S4.8) and the embedding
      index j(D5 in E8) = j(A3 in E8) = 1 (Killing coefficient 60 =
      2 h_vee(E8), both factors).  ORD-4-VS-LEVEL-1 RESOLVED: the clock
      order counts FRACTIONAL flux sectors, not the level -- mu =
      det(D5) det(A3) = 16 = ord(monodromy)^2, the mu4 glue diagonal is
      isotropic and Lagrangian (order 4 = sqrt 16), KLM condensation
      16 -> 16/4^2 = 1 = zero residual fractional flux; NEW DIAL:
      #primaries((E8)_k) = (1, 3, 5, 10, 15, 27) for k = 1..6 from the
      affine marks -- exactly ONE sector iff k = 1 (level from sector
      count, independent of the CPS transplantation).
[E] S4. THE LOCKSTEP THEOREM (the lifting of v505 S4.10 "one scale =>
      one level" to a theorem): clock invariance P(iZ) = P(Z) forces
      a3 = a2 = a1 = 0 sharply, so the four branch points are ONE mu4
      orbit i^k t; the periods are Pi_j = t(i-1) i^{j-1} with |Pi_j| =
      sqrt(2) t EQUAL on all three spheres; the monodromy acts as the
      pure phase i on the period vector and det(A - 1) = -4 != 0: no
      invariant flux vector exists, so no sphere-dependent level
      assignment is possible -- with the CPS flux-level dictionary,
      k1 = k2 = k3 IS A THEOREM of clock invariance.  NEGATIVE PROBE
      (new falsifier): the clock-forbidden family Z^4 - Z (smooth, disc
      = -27) has branch points {0, 1, w, w^2}: NOT a mu4 orbit, and
      over ALL 24 orderings ZERO chains give three equal period moduli
      (1 vs sqrt 3) -- non-lockstep => unequal k_i => no single (E8)_k
      boundary.  HONEST LIMIT: uniform flux doubling keeps the lockstep
      -- the theorem proves EQUALITY of the three k, not the value; the
      value 1 comes from the current/sector counters (S3) and, CPS-
      faithfully, from the minimal quantum N = 1.
[E] S5. NEGATIVE CONTROLS: (a) k = 2 / doubled flux: current count 0,
      embedding residual 360, c = 31/2 => prefactor -31/48 != -1/3,
      #primaries 3 != 1 -- while the lockstep dial does NOT fire
      (honest: the kill is closure/condensation, not geometry);
      (b) SO(16) glue: classes (60, 0, 60, 0) => fluxes (0, 60, 0),
      spheres 1 and 3 FLUX-DARK, ch2 defect -30 != -78, condensation
      stops at 16/2^2 = 4 = det Cartan(D8) (four sectors survive, no
      holomorphic net); (c) wrong intersection form A2: det 3 != 4 =
      |Z4| (McKay bijection broken), glue diagonal 23/24 not integer
      (non-isotropic), mu = 12 not a perfect square (NO Lagrangian
      subgroup exists), Dedekind 2/3 != 5/4, Coxeter order 3 != 4.
[C] The CPS dictionary itself on PT/Z4: CPS work on the blow-up of C^2
      (ONE (-1)-sphere, boundary so(8) at negative level) --
      transplanting "k = flux quantum" to the resolved C^2/Z4 seam
      (three (-2)-spheres, (E8)_1) is typed as an analogy until the
      type-I B-model backreaction on PT/Z4 is computed.
[O] O-FENCE (unchanged): the actual PT/Z4 backreaction (twisted-sector
      Kodaira-Spencer measure, the A3 analogue of CPS eq. 3.5) -- the
      same fence as v502 S5.7 / v505; the delta-1/delta-2 contact-term
      targets and epsilon-1 (O(-2) bulk axion) stay open;
      SEAM.EQUIV.01 untouched; NO marker moves anywhere (the v505
      "one scale => one level" SENTENCE is lifted to theorem status --
      a text move, not a gate move).

Status: [E] exact CPS-skeleton/pairing/lockstep/sector arithmetic
(sympy + Fraction); [C] the CPS dictionary on PT/Z4; [O] the type-I
B-model backreaction on PT/Z4.  Python; Wolfram-mirrored (CPS period
integrals, pairing enumeration 48 -> 2, fluxes (64, 60, 64), #primaries
table (1, 3, 5, 10, 15, 27), lockstep periods + 0/24 negative probe,
A2 controls), counted per GATE.WOLFRAM.02.  Discovery provenance:
experiments/tfpt-discovery/
celestial_seam_wp5e_eps2_level_from_flux_probe.py (2026-07-21, 28/28).
"""
...
# ---------------------------------------------------------------------------
def section1():
    print("  -- S1: the CPS level-from-flux mechanism, exact skeleton")
    I = sp.I
...
# ---------------------------------------------------------------------------
# S2 -- the pairing matrix and the three fluxes (Rechenziel 1)
# ---------------------------------------------------------------------------
def section2(roots):
    print("  -- S2: pairing matrix c1(T_m).[Sigma_i] and the fluxes F_i")
    I = sp.I
...
# ---------------------------------------------------------------------------
# S3 -- level identifications (Rechenziel 2: CPS-faithful vs analogy)
# ---------------------------------------------------------------------------
def section3(roots, C, Cinv, dims):
    print("  -- S3: the level formula -- which identification works")
    ...
    # ord(monodromy) = 4 vs level 1: the KLM condensation as a flux
    # statement.
...
# ---------------------------------------------------------------------------
# S4 -- the lockstep theorem (Rechenziel 3)
# ---------------------------------------------------------------------------
def section4():
    print("  -- S4: the lockstep test -- 'one level' as a theorem")
    I = sp.I
...
# ---------------------------------------------------------------------------
# S5 -- negative controls (Rechenziel 4)
# ---------------------------------------------------------------------------
def section5(C, Cinv):
...
# ---------------------------------------------------------------------------
def section6():
    print("  -- S6: verdict against the WP5e-epsilon-2 scope")
    ...
    check("S6.3 [O-FENCE] open: the actual PT/Z4 backreaction (twisted-"
          "sector Kodaira-Spencer measure, the A3 Omega_N -- the same "
          "fence as v502 S5.7 / v505), the identification of the seam "
          "boundary net level with the WZW level (SEAM.EQUIV.01 "
          "adjacent), the delta-1/delta-2 contact-term targets and the "
          "epsilon-1 O(-2) bulk-axion construction; WP5e stays [O]; the "
          "v505 'one scale => one level' sentence is lifted to THEOREM "
          "status (text move, no gate move); NO marker moves anywhere",
          True)
...
def run():
    reset()
    print("v509  CELEST.WP5E.EPS2.01: the CPS level-from-flux dial "
          "(WP5e-epsilon-2 of CELEST.SEAM.01 -- verdict B)")
    section1()
    ...
    return summary("v509 CELEST.WP5E.EPS2.01: k = 1 from flux quantisation "
                   "-- partially derived (verdict B): the CPS skeleton is "
                   "exact (S^3 period (2 pi i)^2 N, exceptional flux "
                   "2 pi N, level magnitude 2N); the pairing matrix "
                   "P_mi = delta_mi is pinned by complete enumeration "
                   "(48 unimodular, 2 effective) giving fluxes "
                   "(64, 60, 64) with uniformly one quantum per current; "
                   "'level = total flux' is killed by the lockstep test "
                   "itself, 'level = flux per adjoint current' = "
                   "(1, 1, 1) anchored by the lattice current count "
                   "(240, 0, 0, 0) and embedding index 1; ord 4 vs level "
                   "1 resolved (16 = ord^2 fractional sectors condense "
                   "to 1; #primaries((E8)_k) = 1 iff k = 1); the "
                   "LOCKSTEP THEOREM lifts v505 'one scale => one level' "
                   "to a theorem of clock invariance (det(A-1) = -4, "
                   "no invariant flux vector; falsifier Z^4 - Z: 0/24 "
                   "orderings lockstep); k = 2 / SO(16) / A2 controls "
                   "separate honestly; the CPS dictionary on PT/Z4 "
                   "stays [C], the type-I B-model backreaction [O]; "
                   "no marker moves")
