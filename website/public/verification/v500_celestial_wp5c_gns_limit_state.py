"""v500 -- CELEST.WP5C.01: WP5c of the research contract CELEST.SEAM.01 -- "the
GNS limit state", the third constructive milestone of WP5.
Exact integer/Fraction arithmetic throughout, no floats, deterministic (seeded).

Question: WP5a (v497) made the boundary limit an exact coefficientwise limit
of the character family chi_w (generator energy E_w = m + w r, stabilization
threshold w = n+1) and derived the null-ideal size 27000 = h_vee^3; WP5b
(v498) constructed the deleting operator |s> = (E^theta_{-1})^2|0> and left
the STATE open (its S4.10 twisted-slot tension is exactly the WP5c seam).
WP5c now CONSTRUCTS the state: a family omega_w of quasi-free states on the
jet algebra whose limit omega_inf has the null ideal in its GNS kernel.
Preregistered criteria (contract): SUCCESS = the family omega_w is explicit,
positive, omega_w -> omega_inf, and the GNS(omega_inf) level-2 space has
dimension EXACTLY 4124; KILL = no w-uniform state damps the radial modes
without breaking the 248 layer.

  [E] S1. CONSTRUCTION + THE 248 LAYER.  omega_w is the quasi-free state
        with loop sector (r = 0) = the affine k=1 vacuum n-point functions
        (exact Shapovalov/contravariant form via the v498 PBW engine) and
        radial sector (r >= 1) = oscillator pairings x^{w r} (x = e^{-beta/4}
        kept exact: rational sample value 1/2 AND formal x-adic grading --
        the exact Gibbs regulator).  The compact anti-involution is MACHINE-
        DETERMINED, not chosen: theta(e_a) = -e_{-a}, theta(h) = h is the
        UNIQUE sign that is an anti-automorphism on ALL 61504 basis pairs;
        contravariance <J^x_{-1} u, v> = <u, J^{theta x}_1 v> exact.  The
        248 layer: level-1 Gram rank 248, POSITIVE DEFINITE (240 root modes
        at norm k, Cartan block = the E8 Cartan matrix, det 1).
  [E] S2. CONVERGENCE, SHARP.  Loop entries are w-independent; radial
        entries x^{w r} have x-adic order w r >= w: omega_w = omega_inf
        mod x^{N+1} for ALL w >= N+1 (N = 8), and NOT yet at w = N (the
        r = 1 modes sit at order N exactly) -- the WP5a threshold w = n+1
        holds IDENTICALLY at the state level; positivity of every omega_w
        checked at the one-particle level (all 36 radial modes, x = 1/2);
        GNS graded dims of omega_w = the chi_w coefficients (w = 2 member
        = the chiral jet grading).
  [E] S3. THE KERNEL (core result, fully machine).  Level-2 Fock = 31124
        monomials in 9361 weight blocks (block-dim profile 164:1, 37:240,
        7:2160, 1:6960); the FULL exact Gram is computed block by block:
        level-2 rank 4124 EXACTLY (the preregistered target check), kernel
        27000, every block PSD; rank per Weyl orbit (norm 8/6/4/2/0) =
        (0, 0, 1, 8, 44); kernel = V(2 theta) WEIGHT BY WEIGHT (Freudenthal
        cross-check on all 9361 weights); component route: unique highest-
        weight vectors at (0, omega, theta, 2theta) with norms
        (3844, 49, 2, 0) -- the radical is exactly the multiplicity-free
        V(2 theta); direct ideal checks: F_i|s> and F_j F_i|s> have norm 0
        and are orthogonal to their full weight blocks.  GNS graded dims
        (level 0, 1, 2) = (1, 248, 4124) = E4/eta^8 through q^2.
  [E] S4. MU4 EQUIVARIANCE + THE WP5b RESOLUTION.  Frame map glue ->
        standard via the UNIQUE E8 diagram isomorphism (class counts
        (52,64,60,64), j(theta) = 1); omega_w o clock = omega_w for ALL w
        (nonzero vacuum values only at total weight 0 = clock class 0);
        the clock unitary DESCENDS to GNS(omega_inf); TWO ROUTES AT THE
        STATE LEVEL: GNS level-2 rank split by clock class =
        (1036, 1024, 1040, 1024) = Theta_{C_j}/eta^8 at q^2, level-1 split
        (60,64,60,64).  RESOLUTION of WP5b S4.10: |s> IS the zero vector
        of GNS(omega_inf) -- nothing needs to "carry" it to q^2; the
        twisted two-C1-mode floor (6 quarters = q^{3/2}) concerns only
        pre-limit objects (GNS has NO state at 6 quarters: 6 mod 4 != 0).
  [E] S5. NEGATIVE CONTROLS (the construction has teeth).  (a) k = 2:
        <s|s> = +4, dominant blocks FULL rank -- the GNS kernel contains
        NO level-2 ideal (the deletion is a k = 1 state-level fact);
        (b) k = 0: level-1 Gram identically 0 -- no 248 layer without the
        central extension; (c) D8/SO(16)_1 same pipeline: level-2 rank
        2076 = 7380 - 5304, kernel = V(2 theta_D8) weight by weight, PSD,
        but h = (0, 1/2, 1, 1): the vacuum GNS is 1 of 4 blocks -- ONE-
        BLOCK completeness of the limit state stays E8/mu4-specific;
        (d) wrong family E'_w = m w + r: level-1 limit rank 0 (the 248
        layer is ERASED), u^4 survivor content 710955 != 248 -- the kill
        has teeth; (e) no damping: 897266 != 248; (f) CCR OBSTRUCTION: on
        a FIXED radial pairing c > 0 positivity forces omega(b b*) >= c --
        there is NO w-uniform damping state: the family formulation is
        NECESSARY, and the family EXISTS (S2) => the preregistered KILL
        is probed and NOT triggered.
  [C] Named identifications: the quasi-free/Gaussian extension of the
        positive one-particle data to the full *-algebra (standard); Kac
        positivity beyond level 2 (levels <= 2 machine-checked here);
        complete reducibility / multiplicity-freeness in the component
        route (the rank itself is [E], the module identification is [C]);
        k = 2 non-dominant blocks by Weyl-orbit rank constancy.
  [O] HONEST LIMIT: local normality of omega_inf, the net structure /
        Haag duality (WP5d), Costello-Li (WP5e), and the operator-algebraic
        closure of the radial contraction beyond the graded/x-adic topology
        stay OPEN; SEAM.EQUIV.01 stays [O], untouched; no gate closes, no
        marker moves.

Verdict: SUCCESS on the preregistered criteria, at the graded-state level
(the target check dim GNS-level-2 = 4124 is met exactly).

Status: [E] exact construction/Gram/control arithmetic (integer/Fraction,
seeded deterministic); [C] the named identifications above; [O] the net /
normality questions (WP5d/e).  Python; Wolfram-mirrored (exact rank/kernel
counts 4124/27000, rank table (0,0,1,8,44), clock-class split
(1036,1024,1040,1024), h.w. norms (3844,49,2,0), k-dial and D8 counts),
...
def run():
    reset()
    print("v500  CELEST.WP5C.01: the GNS limit state (WP5c of "
          "CELEST.SEAM.01 -- third constructive milestone of WP5)")
    ...
    return summary("v500 CELEST.WP5C.01: the family omega_w (affine k=1 vacuum on the "
                   "currents + x^{wr}-contracted radial oscillators; compact adjoint "
                   "theta(e_a) = -e_{-a} machine-determined on all 61504 pairs) is "
                   "positive for every finite w and stabilizes EXACTLY at the WP5a "
                   "threshold (omega_w = omega_inf mod x^{N+1} for w >= N+1, sharp at "
                   "w = N); GNS(omega_inf) levels (1, 248, 4124): full 9361-block exact "
                   "Gram, level-2 rank 4124 = chi_2, kernel 27000 = V(2 theta) weight "
                   "by weight, every block PSD, rank table (0,0,1,8,44). The 248 layer "
                   "survives (level-1 rank 248 PD); the CCR obstruction shows NO "
                   "w-uniform state exists (the family is necessary AND exists: KILL "
                   "not triggered). mu4: the clock descends to GNS; level-2 rank split "
                   "(1036,1024,1040,1024) = Theta_Cj/eta^8 at q^2 (two routes, state "
                   "level); |s> IS the zero vector of GNS(omega_inf) -- the WP5b S4.10 "
                   "twisted-slot tension is resolved at the state level. Controls: "
                   "k = 2 keeps everything (<s|s> = +4), k = 0 has no current layer, "
                   "D8 gives 2076 = one block of four (h = 1/2), wrong family erases "
                   "the 248 layer (710955 != 248), no damping keeps 897266 != 248. "
                   "Open (WP5d/e): local normality, net/Haag duality, Costello-Li. "
